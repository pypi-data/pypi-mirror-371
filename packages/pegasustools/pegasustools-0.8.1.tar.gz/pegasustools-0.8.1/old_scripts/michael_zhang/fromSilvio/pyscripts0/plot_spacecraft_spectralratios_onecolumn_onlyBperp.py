...
p1a, = plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sBperpf_avg[1:m],'b',linewidth=line_thick,label=r"$\mathcal{E}_{B_\perp}$")
...
plt.tight_layout()
if (fit_method == 'local_slope_manual'):
  flnm = prob+".StationarySpacecraftsAvg.FreqSpectrumBperp-Slope.npt"+"%d"%n_pt+".t-interval."+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))
if (fit_method == 'logspaced_gradient'):
  flnm = prob+".StationarySpacecraftsAvg.FreqSpectrumBperp-Slope.nlogbins"+"%d"%n_logbins+".t-interval."+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))
