...
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sQtotf_avg_norm[1:m],'k',linewidth=1,label=r"$\mathcal{E}_{\widetilde{Q}_{\mathrm{tot}}}$")#_{\mathbf{w}\cdot\mathbf{E}}$")
plt.scatter(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sQprpf_avg_norm[1:m],color='b',s=1.5)
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sQprpf_avg_norm[1:m],'b',linewidth=1,label=r"$\mathcal{E}_{\widetilde{Q}_\perp}$")#_{\mathbf{w}_\perp\cdot\mathbf{E}_\perp}$")
plt.scatter(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sQparf_avg_norm[1:m],color='r',s=1.5)
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),sQparf_avg_norm[1:m],'r',linewidth=1,label=r"$\mathcal{E}_{\widetilde{Q}_\parallel}$")#_{w_\parallel E_\parallel}$")
...
#plt.show()
plt.tight_layout()
flnm = prob+".TrackedParticles.EW_HeatingPowerSpectrum-vs-Freq.Nparticle"+"%d"%int(Nparticle)+".t-interval."+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))
