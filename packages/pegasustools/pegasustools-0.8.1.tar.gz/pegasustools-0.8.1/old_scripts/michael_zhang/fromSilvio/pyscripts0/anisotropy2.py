...
######################
  ...
  xplot_min = np.log10(betai0)-1. #0.1*betai0 in logscale
  xplot_max = np.log10(betai0)+1. #10.*betai0 in logscale
  yplot_min = -1 
  yplot_max = 1
  ybins = 10**np.linspace(-1,1,400)
  xbins = 10**np.linspace(-2, 2, 400)
  ...
  if (hist_scale == 'linear'):
    ax0.hist2d(log10b,log10a,cmap=cmap,range=((xplot_min,xplot_max),(yplot_min,yplot_max)),bins=(400, 400),normed=True)
  if (hist_scale == 'log'):
    ax0.hist2d(log10b,log10a,cmap=cmap,range=((xplot_min,xplot_max),(yplot_min,yplot_max)),bins=(400, 400),normed=True,norm=mpl.colors.LogNorm())
  x = np.linspace(xplot_min,xplot_max,num=100)
  aa = [0.43,0.77,-0.47,-1.4]
  bb = [0.42,0.76,0.53,1.0]
  bi0 = [-0.0004,-0.016,0.59,-0.11]
  y1 = 1. + aa[0] / ( (10.**x-bi0[0])**bb[0] )
  y2 = 1. + aa[1] / ( (10.**x-bi0[1])**bb[1] )
  y3 = 1. + aa[2] / ( (10.**x-bi0[2])**bb[2] )
  y4 = 1. + aa[3] / ( (10.**x-bi0[3])**bb[3] )
  adiab = betai0/10.**x - 1.0
  ax0.plot((x),np.log10(y1),'--',c='k',linewidth=0.5)
  ax0.plot((x),np.log10(y2),'--',c='k',linewidth=0.5)
  ax0.plot((x),np.log10(y3),'k:',linewidth=0.5)
  ax0.plot((x),np.log10(y4),'k-.',linewidth=0.5)
  ax0.plot((x),np.log10(adiab),'m:',linewidth=0.5)
  ax0.axhline(y=0.,ls='-.',linewidth=0.5)
  ax0.axvline(x=np.log10(betai0),ls='-.',linewidth=0.5)
  plt.xlabel(r"$\beta_\parallel$",fontsize=9)
  plt.ylabel(r"$P_\perp/P_\parallel$",fontsize=9)
...
######################################################################
  ...
  if (hist_scale == 'linear'):
    fig_name = path_save+"anisotropy-vs-beta_par."+"%05d"%ii+"."+fig_frmt
  if (hist_scale == 'log'):
    fig_name = path_save+"anisotropy-vs-beta_par."+"%05d"%ii+"_log."+fig_frmt
