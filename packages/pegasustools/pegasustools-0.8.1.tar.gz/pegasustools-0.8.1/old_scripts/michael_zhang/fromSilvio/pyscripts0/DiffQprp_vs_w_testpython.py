...
for ind in range(it0,it1+1):
  ...
  #
  #first normalization by number of processors
  #vdf_ = vdf_ / np.float(N_perp*N_perp*N_para*N_ppc) #np.float(n_proc)
  edotv_prl_ = edotv_prl_ / np.float(N_perp*N_perp*N_para*N_ppc) #np.float(n_proc)
  edotv_prp_ = edotv_prp_ / np.float(N_perp*N_perp*N_para*N_ppc) #np.float(n_proc)
  ...
  dfdwprp_avg += np.gradient(fvprp_t[:,ind-it0],vprp,edge_order=edge_grad_order) / np.float(it1-it0+1)
  dfdwprp_t[:,ind-it0] = np.gradient(fvprp_t[:,ind-it0],vprp,edge_order=edge_grad_order)
  #for iit in range(it0,it1+1):
  #  dfdwprp_t[:,iit-it0] = np.gradient(np.sum(fff_t[:,:,iit-it0]*dvprl,axis=1),vprp,edge_order=2)
