...
for jj in range(len(id_particle)):
  for ii in range(len(id_proc)):
    data = pegr.tracked_read(path_read,prob,id_particle[jj],proc_offset+id_proc[ii])
...
if ( imax_dmu < n_procs ):
  imax_dmu_part = 0
  imax_dmu_proc = proc_offset + imax_dmu
else:
  imax_dmu_part = 1
  imax_dmu_proc = proc_offset + imax_dmu - n_procs
...
if ( imax_denergy < n_procs ):
  imax_denergy_part = 0
  imax_denergy_proc = proc_offset + imax_denergy
else:
  imax_denergy_part = 1
  imax_denergy_proc = proc_offset + imax_denergy - n_procs
