...
same_norm_all = False #True

#exponential correction
exp_corr = True #False #True
c0exp = 0.2 #0.001 #1.0
c0exp03 = 0.2 #0.001
...
kappaU01 = 1.27 #1.27
kappaB01 = 1.27 #1.27 
kappaPHI01 = 1.27 #1.27 #np.sqrt(kappaU01*kappaB01) #geometric mean..?
#
kappaU03 = 1.0 
kappaB03 = 1.0
kappaPHI03 = 1.0 #np.sqrt(kappaU03*kappaB03) #geometric mean..?
...
#
dUprp_k = np.sqrt(kprp_u_rho*Ukprp) #from Uperp power spectrum to dUperp fluctuations (in k_perp*rho_th)
...
#
if dBprp_complement:
  dPhi_of_dUprp_v += kparoverkprp_v*dBprp_v #apply finite-anisotropy correction  
  if TWOPIcoeff:
...
#
dBz_k = np.sqrt(kprp_bz_rho*Bzkprp) #from Bpara power spectrum to dBpara fluctuations (in k_perp*rho_th)
...
#
if dBprp_complement:
  dPhi_of_dBz_v += kparoverkprp_v*dBprp_v #apply finite-anisotropy correction 
#
if TWOPIcoeff:
...
if apply_smoothing:
  ...
  if exp_corr:
    normD_phi01 = cst01*Dprpprp_smooth[i01] / Dprpprp_Phik[np.where(Dprpprp_Phik == np.max(Dprpprp_Phik))[0][0]]
    normD_phicomb01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Phik_comb[np.where(Dprpprp_Phik_comb == np.max(Dprpprp_Phik_comb))[0][0]]
    normD_bz01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Bzk[np.where(Dprpprp_Bzk == np.max(Dprpprp_Bzk))[0][0]]
    normD_u01 = cst01*Dprpprp_smooth[k01] / Dprpprp_Uk[np.where(Dprpprp_Uk == np.max(Dprpprp_Uk))[0][0]]
  else:
...
lbl_sim = r'simulation'
lbl_phi = r'theory (full $\delta\Phi$)'
lbl_phicomb = r'theory ($\delta \Phi_{\mathrm{tot}}$)' 
lbl_dU = r'theory (only $\delta \Phi_{\mathrm{mhd}}$)' #r'theory (only $\delta u_\perp$)'
lbl_dB = r'theory (only $\delta \Phi_{\mathrm{kin}}$)' #r'theory (only $\delta B_\parallel$)'
...
ax.axvspan(0.35,0.35*7., alpha=0.33, color='firebrick')
plt.text(1.0,1.01*yr_max_Q_k,r'$\mathrm{stochastic}$',va='bottom',ha='center',color='firebrick',rotation=0,fontsize=font_size-0.5)
ax.axvspan(2.,2.*4., alpha=0.33, color='royalblue')
plt.text(3.8,1.01*yr_max_Q_k,r'$\mathrm{cyclotron}$',va='bottom',ha='center',color='royalblue',rotation=0,fontsize=font_size-0.5)
...
plt.text(0.97*kdi01,0.475*yr_max_Q_k,r'$k_\perp d_{\mathrm{i}}=1$',va='top',ha='right',color='k',rotation=90,fontsize=font_size-1)
...
#
ax = fig2.add_subplot(grid[0:3,0:3])
ax.axvspan(0.3,0.3*5., alpha=0.33, color='firebrick')
plt.text(0.8,1.01*yr_max_Q_k03,r'$\mathrm{stochastic}$',va='bottom',ha='center',color='firebrick',rotation=0,fontsize=font_size-0.5)
ax.axvspan(1.5,1.5*4., alpha=0.33, color='royalblue')
plt.text(3.3,1.01*yr_max_Q_k03,r'$\mathrm{cyclotron}$',va='bottom',ha='center',color='royalblue',rotation=0,fontsize=font_size-0.5)
