...
for ind in range(it0,it1+1):
  ...
  #normalize f
  vdf /= np.sum(vdf*dvprp*dvprl)
  vdf0 /= np.sum(vdf0*dvprp*dvprl)
  #
  #computing <df/dwprp>
  ...
  kperp_u = dataUkprp[:,0]
  kperp_bz = dataBzkprp[:,0]
  kperp_phi = dataPhikprp[:,0]
  Ukperp = dataUkprp[:,1]
  Bzkperp = dataBzkprp[:,1]
  Phikperp = dataPhikprp[:,1]
  ...
  sum_prl = ew_prl_Bloc[ind,:]
  sum_prp = ew_prp_Bloc[ind,:]
  if cooling_corr:
    #cooling corrections
    print "\n [ apply cooling correction (vs k_perp) ]"
    #
    it1corr_loc = np.min([ind,it1corr])
    #
    sum_prl -= np.sum(ew_prl_Bloc[it0corr:it1corr_loc+1,:],axis=0) / np.float(it1corr_loc+1-it0corr)
    sum_prp -= np.sum(ew_prp_Bloc[it0corr:it1corr_loc+1,:],axis=0) / np.float(it1corr_loc+1-it0corr)
  #normalization
  if verb_diag:
    print "\n [ normalization ]"
  norm = np.abs( np.sum(ew_prl_Bloc[ind,np.where(kprp_rho > 0.1)[0][0]:len(kprp_rho)]*dlogkprp) + np.sum(ew_prp_Bloc[ind,np.where(kprp_rho > 0.1)[0][0]:len(kprp_rho)]*dlogkprp) ) 
  sum_prl /= norm
  sum_prp /= norm
  ...
  #
  # normalization of Qprp vs kprp
  #
  normQk = np.max(Qprp_k_sim) / np.max([np.max(Qprp_k_phik),np.max(Qprp_k_bzk),np.max(Qprp_k_uk)])
  Qprp_k_phik *= normQk
  Qprp_k_bzk *= normQk
  Qprp_k_uk *= normQk
  #
  ...
  yr_max_Q_k = 1.2*np.max(Qprp_k_sim)
  ...
  plt.legend(loc='upper left',markerscale=0.5,frameon=False,fontsize=font_size-2,ncol=1,handlelength=3)#,weight=fontweight_legend)
  #
  #--show and/or save
  if output_figure:
    plt.tight_layout()
    flnm = "Qperp_kperp_sim-vs-theory"
    if cooling_corr: 
      flnm += "_coolingcorr"
    flnm += '.'+'%05d'%ind
...
if (not jumptopaperplots): 
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "diffusion_beta03_varyingk0_oldnorm"
    else:
      flnm = "diffusion_beta03_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "diffusion_beta01_varyingk0_oldnorm"
    else:
      flnm = "diffusion_beta01_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
    path_output = path_read_lev+flnm+fig_frmt
    plt.savefig(path_output,bbox_to_inches='tight')#,pad_inches=-1)
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "Qprp_vs_wprp_beta03_varyingk0_oldnorm"
    else:
      flnm = "Qprp_vs_wprp_beta03_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
    path_output = path_read_lev+flnm+fig_frmt
    plt.savefig(path_output,bbox_to_inches='tight')#,pad_inches=-1)
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "Qprp_vs_wprp_beta01_varyingk0_oldnorm"
    else:
      flnm = "Qprp_vs_wprp_beta01_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
    path_output = path_read_lev+flnm+fig_frmt
    plt.savefig(path_output,bbox_to_inches='tight')#,pad_inches=-1)
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "Qprp_vs_kprp_beta01_varyingk0_oldnorm"
    else:
      flnm = "Qprp_vs_kprp_beta01_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
    path_output = path_read_lev+flnm+fig_frmt
    plt.savefig(path_output,bbox_to_inches='tight')#,pad_inches=-1)
  ...
  if output_figure:
    plt.tight_layout()
    if old_norm_version:
      flnm = "Qprp_vs_kprp_beta03_varyingk0_oldnorm"
    else:
      flnm = "Qprp_vs_kprp_beta03_varyingk0_newnorm"
    if apply_smoothing:
      flnm = flnm+'_smooth'
    path_output = path_read_lev+flnm+fig_frmt
    plt.savefig(path_output,bbox_to_inches='tight')#,pad_inches=-1)
