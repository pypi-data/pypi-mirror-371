...
plt.axhline(y=1.0,c='k',ls='--',linewidth=1.)
ax1.plot(time,Tprl/Tprl0,'orange',linewidth=1.5, label=r"$\langle T_\parallel\rangle$")
#ax1.scatter(time,Tprl/Tprl0,s=0.1,color='orange')
ax1.plot(time,Tprp/Tprp0,'magenta',linewidth=1.5, label=r"$\langle T_\perp\rangle$")
...
plt.axvline(x=280,c='b',ls=':',linewidth=1)
plt.text(290,1.03,r'first reconnection events',va='bottom',ha='left',color='b',rotation=90,fontsize=16)
plt.text(900,1.05,r'quasi-steady state',va='center',ha='center',color='k',rotation=0,fontsize=16)
...
ax1.set_ylabel(r'$T/T_0$',fontsize=16)
...
ax2.set_xlabel(r"$\Omega_{\mathrm{c,i}}\,t$",fontsize=16)
#ax2.axvspan(700, t_real[len(t_real)-1], alpha=0.25, color='grey')
ax2.axvspan(650,np.max(t_real), alpha=0.25, color='grey')
...
#plt.show()
plt.tight_layout()
flnm = "turb_hst_TprpTprl_BprpEprp" 
