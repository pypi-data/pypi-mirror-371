...
for ind in range(it0,it1+1):
  ...
  fvprp_t = np.zeros([Nvperp,it1-it0+1])
  for ivv in range(Nvperp):
    fff[ivv,:] = vdf_[ivv,:] / vprp[ivv]
    fff_t[ivv,:,:] = vdf_t[ivv,:,:] / vprp[ivv]
  fvprp_t[:,ind-it0] = np.sum(fff*dvprl,axis=1) / np.sum(fff*dvprl*dvprl)
  dfdwprp_avg = dfdwprp_avg + np.gradient(fvprp_t[:,ind-it0],vprp,edge_order=edge_grad_order) / np.float(it1-it0+1)
  dfdwprp_t[:,ind-it0] = np.gradient(fvprp_t[:,ind-it0],vprp,edge_order=edge_grad_order)
  if (ind == it0):
...
#    Ukperp_t = np.zeros([len(dataUkprp),it1-it0+1])
#    Bzkperp_t = np.zeros([len(dataBzkprp),it1-it0+1])
#    Phikperp_t = np.zeros([len(dataPhikprp),it1-it0+1])
    kprp_u = dataUkprp[1:,0]
    kprp_bz = dataBzkprp[1:,0]
    kprp_phi = dataPhikprp[1:,0]
    Ukprp = np.zeros(len(kprp_u))
    Bzkprp = np.zeros(len(kprp_bz))
    Phikprp = np.zeros(len(kprp_phi))
    if dBprp_complement:
      Bprpkprp = np.zeros(len(kprp_bz))
    Bkprp = np.zeros(len(kprp_bz))
    Ukprp_t = np.zeros([len(kprp_u),it1-it0+1])
    Bzkprp_t = np.zeros([len(kprp_bz),it1-it0+1])
    Phikprp_t = np.zeros([len(kprp_phi),it1-it0+1])

  #1D specra vs k_perp
#  for jj in range(len(dataUkprp)):
#    Ukperp[jj] += dataUkprp[jj,1]
#    Ukperp_t[jj,ind-it0] = dataUkprp[jj,1]
#  for jj in range(len(dataBzkprp)):
#    Bzkperp[jj] += dataBzkprp[jj,1]
#    Bzkperp_t[jj,ind-it0] = dataBzkprp[jj,1]
#  for jj in range(len(dataPhikprp)):
#    Phikperp[jj] += dataPhikprp[jj,1]
#    Phikperp_t[jj,ind-it0] = dataPhikprp[jj,1]
#  if dBprp_complement:
#    for jj in range(len(dataBprpkprp)):
#      Bprpkperp[jj] += dataBprpkprp[jj,1]
#  for jj in range(len(dataBkprp)):
#    Bkperp[jj] += dataBkprp[jj,1]
  Ukprp[:] = Ukprp[:] + dataUkprp[1:,1]/np.float(it1-it0+1) 
  Bzkprp[:] = Bzkprp[:] + dataBzkprp[1:,1]/np.float(it1-it0+1)
  Phikprp[:] = Phikprp[:] + dataPhikprp[1:,1]/np.float(it1-it0+1)
  if dBprp_complement:
    Bprpkprp[:] = Bprpkprp[:] + dataBprpkprp[1:,1]/np.float(it1-it0+1)
  Bkprp[:] = Bkprp[:] + dataBkprp[1:,1]/np.float(it1-it0+1)
  #
  Ukprp_t[:,ind-it0] = dataUkprp[1:,1]
  Bzkprp_t[:,ind-it0] = dataBzkprp[1:,1]
...
#
#print("shapes? ",Ukprp.shape,Ukperp.shape)
#print("shapes? ",Bzkprp.shape,Bzkperp.shape)
#print("shapes? ",Phikprp.shape,Phikperp.shape)
...
if cooling_corr:
  ...
  corr_save_temp = edotv_prp_corr/(np.abs( np.abs(np.sum(edotv_prl_corr*dvprp*dvprl)) + np.abs(np.sum(edotv_prp_corr*dvprp*dvprl)) ))
  np.save('./correction2V_prp_beta01.npy',corr_save_temp)
...
dlogkprp = np.mean(np.log(kprp_rho[2:-1])-np.log(kprp_rho[1:-2]))
...
data2 = np.loadtxt(path_read_lev+'heat_prl_beta03.dat')
print(data2)
#
ik0_Q03 = 1
print(data.shape)
...
dlogkprp03 = np.mean(np.log(kprp_rho_sim03[2:-1])-np.log(kprp_rho_sim03[1:-2]))
#print(np.log10(kprp_rho_sim03[2:-1])-np.log10(kprp_rho_sim03[1:-2]))
print(np.log(kprp_rho_sim03[2:-1])-np.log(kprp_rho_sim03[1:-2]))
print(dlogkprp03)
print(np.log10(kprp_rho_sim03))
...
print(Qprp_k_sim03)
#norm = Qnormalization03/(np.sum(Qprp_k_sim03*dlogkprp03) + np.sum(Qprl_k_sim03*dlogkprp03))
print(np.sum(Qprp_k_sim03*dlogkprp03))
print(np.sum(Qprl_k_sim03*dlogkprp03))
...
#
dUprp_k_t = np.zeros([len(kprp_u_rho),it1-it0+1])
dUprp_v_t = np.zeros([len(vprp),it1-it0+1])
dPhi_of_dUprp_v_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  dUprp_k_t[:,ind-it0] = np.sqrt(kprp_u_rho[:]*Ukprp_t[:,ind-it0]/np.sqrt(betai0)) # attention! spectra were originally computed in kprp*di units!
  dUprp_v_t[:,ind-it0] = np.interp(vprp, alphaU/kprp_u_rho[::-1], dUprp_k_t[::-1,ind-it0])
  dPhi_of_dUprp_v_t[:,ind-it0] = (np.sqrt(betai0)/alphaU) * vprp * dUprp_v_t[:,ind-it0] 
  dPhi_of_dUprp_v_t[:,ind-it0] = dPhi_of_dUprp_v_t[:,ind-it0] / betai0
#
#Phi_kin
#
...
#
dBz_k_t = np.zeros([len(kprp_bz_rho),it1-it0+1])
dBz_v_t = np.zeros([len(vprp),it1-it0+1])
dPhi_of_dBz_v_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  dBz_k_t[:,ind-it0] = np.sqrt(kprp_bz_rho[:]*Bzkprp_t[:,ind-it0]/np.sqrt(betai0)) # attention! spectra were originally computed in kprp*di units!
  dBz_v_t[:,ind-it0] = np.interp(vprp, alphaB/kprp_bz_rho[::-1], dBz_k_t[::-1,ind-it0])
  dPhi_of_dBz_v_t[:,ind-it0] = (1./(1.+tau0))*dBz_v_t[:,ind-it0] 
  dPhi_of_dBz_v_t[:,ind-it0] = dPhi_of_dBz_v_t[:,ind-it0] / betai0
#
#Phi_comb
#
...
dPhi_v_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  ...
  dPhi_v_t[:,ind-it0] = np.interp(vprp, alphaPHI/kprp_phi_rho[::-1], dPhi_k_t[::-1,ind-it0])
  dPhi_v_t[:,ind-it0] = dPhi_v_t[:,ind-it0] / betai0
#
...
#
Dprpprp_Uk_t = np.zeros([len(vprp),it1-it0+1])
Dprpprp_Bzk_t = np.zeros([len(vprp),it1-it0+1])
Dprpprp_Phik_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  Dprpprp_Uk_t[:,ind-it0] = (dPhi_of_dUprp_v_t[:,ind-it0]**3.) / (vprp[:]**2.) 
  Dprpprp_Bzk_t[:,ind-it0] = (dPhi_of_dBz_v_t[:,ind-it0]**3.) / (vprp[:]**2.)
  Dprpprp_Phik_t[:,ind-it0] = (dPhi_v_t[:,ind-it0]**3.) / (vprp[:]**2.)
  if exp_corr:
    Dprpprp_Uk_t[:,ind-it0] = Dprpprp_Uk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))#*np.exp(-c0exp*(vprp[:]**2./dPhi_of_dUprp_v_t[:,ind-it0]))
    Dprpprp_Bzk_t[:,ind-it0] = Dprpprp_Bzk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))#*np.exp(-c0exp*(vprp[:]**2./dPhi_of_dBz_v_t[:,ind-it0]))
    Dprpprp_Phik_t[:,ind-it0] = Dprpprp_Phik_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))
Dprpprp_Uk_t_avg = np.sum(Dprpprp_Uk_t,axis=1)/np.float(it1-it0+1)
Dprpprp_Bzk_t_avg = np.sum(Dprpprp_Bzk_t,axis=1)/np.float(it1-it0+1)
Dprpprp_Phik_t_avg = np.sum(Dprpprp_Phik_t,axis=1)/np.float(it1-it0+1)
#
if exp_corr:
...
#
Qprp_Uk_t = np.zeros([len(vprp),it1-it0+1])
Qprp_Bzk_t = np.zeros([len(vprp),it1-it0+1])
Qprp_Phik_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  Qprp_Uk_t[:,ind-it0] = - Dprpprp_Uk_t[:,ind-it0] * dfdwprp_t[:,ind-it0]
  Qprp_Bzk_t[:,ind-it0] = - Dprpprp_Bzk_t[:,ind-it0] * dfdwprp_t[:,ind-it0]
  Qprp_Phik_t[:,ind-it0] = - Dprpprp_Phik_t[:,ind-it0] * dfdwprp_t[:,ind-it0]
Qprp_Uk_t_avg = np.sum(Qprp_Uk_t,axis=1)/np.float(it1-it0+1)
Qprp_Bzk_t_avg = np.sum(Qprp_Bzk_t,axis=1)/np.float(it1-it0+1)
Qprp_Phik_t_avg = np.sum(Qprp_Phik_t,axis=1)/np.float(it1-it0+1)
...
if apply_smoothing:
  ...
  normD_phi01_t_avg = cst01*Dprpprp[i01] / Dprpprp_Phik_t_avg[i01]
  ...
  normD_bz01_t_avg = normD_phi01_t_avg*Dprpprp_Phik_t_avg[ind_BtoPHI01] / Dprpprp_Bzk_t_avg[ind_BtoPHI01]
  normD_u01 = normD_phi01*Dprpprp_Phik[ind_UtoPHI01] / Dprpprp_Uk[ind_UtoPHI01]
  normD_u01_t_avg = normD_phi01_t_avg*Dprpprp_Phik_t_avg[ind_UtoPHI01] / Dprpprp_Uk_t_avg[ind_UtoPHI01]
else:
  #if exp_corr:
  ...
  normD_phi01 = cst01*Dprpprp_smooth[i01] / Dprpprp_Phik[i01]
  normD_phi01_t_avg = cst01*Dprpprp_smooth[i01] / Dprpprp_Phik_t_avg[i01]
  normD_phicomb01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Phik_comb[j01]
  normD_bz01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Bzk[j01]
  normD_bz01_t_avg = cst01*Dprpprp_smooth[j01] / Dprpprp_Bzk_t_avg[j01]
  normD_u01 = cst01*Dprpprp_smooth[k01] / Dprpprp_Uk[k01]
  normD_u01_t_avg = cst01*Dprpprp_smooth[k01] / Dprpprp_Uk_t_avg[k01]
#
if same_norm_all:
  #if exp_corr:
...
Dprpprp_Uk_t_avg *= normD_u01_t_avg
Dprpprp_Bzk_t_avg *= normD_bz01_t_avg
Dprpprp_Phik_t_avg *= normD_phi01_t_avg
#
print('\n')
print('### betai0 = 1/9 ###')
print('* kappa0 values: ')
...
print('  normD_Phi_t_avg =',normD_phi01_t_avg)
...
Qprp_Bzk *= normD_bz01
Qprp_Uk *= normD_u01
Qprp_Uk_t_avg *= normD_u01_t_avg
Qprp_Bzk_t_avg *= normD_bz01_t_avg
Qprp_Phik_t_avg *= normD_phi01_t_avg
#
#####
#
# << vs k_perp >>
...
#
Qprp_k_uk_t_avg = np.interp( kprp_u_rho, alphaU/vprp[::-1], Qprp_Uk_t_avg[::-1] )
Qprp_k_uk_t_avg *= (alphaU/kprp_u_rho)
Qprp_k_uk_t_avg /= np.log(10.)
Qprp_k_bzk_t_avg = np.interp( kprp_bz_rho, alphaB/vprp[::-1], Qprp_Bzk_t_avg[::-1] )
Qprp_k_bzk_t_avg *= (alphaB/kprp_bz_rho)
Qprp_k_bzk_t_avg /= np.log(10.)
Qprp_k_phik_t_avg = np.interp( kprp_phi_rho, alphaPHI/vprp[::-1], Qprp_Phik_t_avg[::-1] )
Qprp_k_phik_t_avg *= (alphaPHI/kprp_phi_rho)
Qprp_k_phik_t_avg /= np.log(10.)
# 
k_vprp0_uk = 1. / vprp0
k_vprp0_uk *= alphaU
...
if exp_corr:
  ...
  Dprpprp_Uk03 *= np.exp(-c0exp03*(vprp**2./dPhi_v03))#np.exp(-c0exp03*(vprp**2./dPhi_of_dUprp_v03))
  Dprpprp_Bzk03 *= np.exp(-c0exp03*(vprp**2./dPhi_v03))#np.exp(-c0exp03*(vprp**2./dPhi_of_dBz_v03))
...
yr_max_Q_k = 1.75*np.max(Qprp_k_sim+0.5*stdQk01)
yr_min_Q_k03 = np.max([1.025*np.min(Qprp_k_sim03),-0.25])
yr_max_Q_k03 = 1.2 #1.25*np.max(Qprp_k_sim03+errQk03)
...
plt.text(1.167*vA03,0.9*yr_max_f_w,r'$w_\perp = v_{\mathrm{A}0}$',va='top',ha='right',color='k',rotation=90,fontsize=font_size)
...
plt.text(1.067*xr_min_w,1.0,r'$\mathrm{(d)}$',va='center',ha='left',color='k',rotation=0,fontsize=font_size+1)#,weight='bold')
#
...
ax1b.fill_between(vprp,(1.-1./3.)*Dprpprp03,(1./(1.-1./3.))*Dprpprp03,facecolor='grey',alpha=0.33)
...
plt.plot(np.ma.masked_where( vprp < 0.1, vprp),Dprpprp_Uk03,c=clr_dU,ls=lnstyl[ils_dU],linewidth=line_thick,label=lbl_dU)
...
plt.text(1.067*xr_min_w,0.9*yr_max_D_w,r'$\mathrm{(e)}$',va='top',ha='left',color='k',rotation=0,fontsize=font_size+1)#,weight='bold')
#
# beta = 0.1
print(">> beta = 1/9 ")
...
#np.save('./D_sim_t-avg_beta01.npy',Dprpprp_t_avg)
#np.save('./D_sim_t-avg_beta01_StandardDeviation.npy',stdDprpprp_vprp)
#np.save('./D_theory_PHI_t-avg_beta01.npy',Dprpprp_Phik_t_avg)
#np.save('./D_theory_PHImhd_t-avg_beta01.npy',Dprpprp_Uk_t_avg)
#np.save('./D_theory_PHIkin_t-avg_beta01.npy',Dprpprp_Bzk_t_avg)
#
...
plt.plot(vprp,Dprpprp_t_avg,c=clr_sim,ls=lnstyl[ils_sim],linewidth=line_thick,label=lbl_sim)
ax2b.fill_between(vprp,Dprpprp_t_avg-c_pm_std*stdDprpprp_vprp,Dprpprp_t_avg+c_pm_std*stdDprpprp_vprp,facecolor='grey',alpha=0.33)
...
plt.plot(vprp,Dprpprp_Phik_t_avg,c=clr_phi,ls=lnstyl[ils_phi],linewidth=line_thick,label=lbl_phi)
...
ax1c.fill_between(vprp,0.9*(1.-(1./3.*np.abs(dfdwprp03)/np.max(np.abs(dfdwprp03))))*Qprp_vprp03,1.1*(1./(1.-(1./3.*np.abs(dfdwprp03)/np.max(np.abs(dfdwprp03)))))*Qprp_vprp03,facecolor='grey',alpha=0.33)
...
plt.plot(np.ma.masked_where( vprp < 0.1, vprp),Qprp_Uk03,c=clr_dU,ls=lnstyl[ils_dU],linewidth=line_thick)#,label=r'theory (only $\delta u_\perp$)')
...
plt.text(1.067*xr_min_w,0.875*yr_max_Q_w,r'$\mathrm{(f)}$',va='bottom',ha='left',color='k',rotation=0,fontsize=font_size+1)#,weight='bold')
#
# beta = 0.1
#
#np.save('./Q_sim_beta01.npy',Qprp_vprp)
#np.save('./Q_sim_beta01_StandardDeviation.npy',stdQprp_vprp)
#np.save('./Q_theory_PHI_t-avg_beta01.npy',Qprp_Phik_t_avg)
#np.save('./Q_theory_PHImhd_t-avg_beta01.npy',Qprp_Uk_t_avg)
#np.save('./Q_theory_PHIkin_t-avg_beta01.npy',Qprp_Bzk_t_avg)
# 
...
plt.plot(vprp,Qprp_Phik_t_avg,c=clr_phi,ls=lnstyl[ils_phi],linewidth=line_thick)#,label=r'theory (full $\delta\Phi$)')
plt.plot(vprp,Qprp_Uk_t_avg,c=clr_dU,ls=lnstyl[ils_dU],linewidth=line_thick)#,label=r'theory (only $\delta u_\perp$)')
plt.plot(vprp,Qprp_Bzk_t_avg,c=clr_dB,ls=lnstyl[ils_dB],linewidth=line_thick)#,label=r'theory (only $\delta B_\parallel$)')
plt.axvline(x=1.0,c='k',linestyle=':',linewidth=line_thick_aux,alpha=0.66)
plt.axvline(x=vA01,c='k',linestyle='--',linewidth=line_thick_aux,alpha=0.66)
plt.xlim(xr_min_w,xr_max_w)
plt.ylim(yr_min_Q_w,yr_max_Q_w)
...
plt.text(1.067*xr_min_w,0.875*yr_max_Q_w,r'$\mathrm{(c)}$',va='bottom',ha='left',color='k',rotation=0,fontsize=font_size+1)#,weight='bold')
#--show and/or save
if output_figure:
  plt.tight_layout()
  flnm = "diffusion_it%03d"%it0+"-%03d"%it1+"_LAST"
...
plt.text(k0_st,1.05*yr_max_Q_k,r'$\mathrm{stochastic}$',va='bottom',ha='center',color=clr_shd_st,rotation=0,fontsize=font_size)
plt.text(k0_ic,1.05*yr_max_Q_k,r'$\mathrm{cyclotron}$',va='bottom',ha='center',color=clr_shd_ic,rotation=0,fontsize=font_size)
plt.text(0.1,1.05*yr_max_Q_k,r'$\beta_{\mathrm{i}0}=1/9$',va='bottom',ha='left',color='k',rotation=0,fontsize=font_size)
plt.axhline(y=0.,c='k',linestyle=':',linewidth=line_thick_aux,alpha=0.66)
...
plt.plot(kprp_phi_rho,Qprp_k_phik_t_avg,c=clr_phi,ls=lnstyl[ils_phi],linewidth=line_thick,label=lbl_phi)
plt.plot(kprp_bz_rho,Qprp_k_uk_t_avg,c=clr_dU,ls=lnstyl[ils_dU],linewidth=line_thick,label=lbl_dU)
plt.plot(kprp_bz_rho,Qprp_k_bzk_t_avg,c=clr_dB,ls=lnstyl[ils_dB],linewidth=line_thick,label=lbl_dB)
...
plt.legend(loc='upper left',markerscale=0.5,frameon=True,fontsize=font_size-2,ncol=1,handlelength=1.67)#,weight=fontweight_legend)
#--show and/or save
if output_figure:
  plt.tight_layout()
  flnm = "heating_k_it%03d"%it0+"-%03d"%it1+"_LAST"
...
ax.axvspan(kmin_shd_st,kmax_shd_st, alpha=0.33, color=clr_shd_st)
ax.axvspan(kmin_shd_ic,kmax_shd_ic, alpha=0.33, color=clr_shd_ic)
plt.text(k0_st,1.05*yr_max_Q_k03,r'$\mathrm{stochastic}$',va='bottom',ha='center',color=clr_shd_st,rotation=0,fontsize=font_size)
plt.text(k0_ic,1.05*yr_max_Q_k03,r'$\mathrm{cyclotron}$',va='bottom',ha='center',color=clr_shd_ic,rotation=0,fontsize=font_size)
plt.text(0.1,1.05*yr_max_Q_k03,r'$\beta_{\mathrm{i}0}=0.3$',va='bottom',ha='left',color='k',rotation=0,fontsize=font_size)
plt.axhline(y=0.,c='k',linestyle=':',linewidth=line_thick_aux,alpha=0.66)
...
plt.text(0.97*kdi03,0.125*yr_max_Q_k03,r'$k_\perp d_{\mathrm{i}0}=1$',va='bottom',ha='right',color='k',rotation=90,fontsize=font_size-1)
plt.xlim(xr_min_k,xr_max_k)
