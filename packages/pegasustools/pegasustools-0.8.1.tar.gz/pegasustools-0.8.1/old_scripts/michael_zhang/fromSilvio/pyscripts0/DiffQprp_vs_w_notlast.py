...
exp_corr = True #False 
c0exp = 0.025 #0.05 #0.055 #0.05 
c0exp03 = 0.025 #0.09 #0.11 #0.08 

#shaded area: +/- c_pm_std*sigma (sigma = standard dev.)
c_pm_std = 1.0 #0.5

#--parameters for final plot
kappaU01 = 1.0 #1.25 #1.2 
kappaB01 = 1.5 #1.25 #1.2 
kappaPHI01 = 1.0 #1.25 #1.2 
#
kappaU03 = 1.1 #1.1 
kappaB03 = 1.5 #1.1 
kappaPHI03 = 1.1 #1.1 
...
dPhi_comb_v_t = np.sqrt(dPhi_of_dUprp_v_t**2. + dPhi_of_dBz_v_t**2.)
#
#Phi_tot
...
Dprpprp_Phik_comb_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  ...
  Dprpprp_Phik_comb_t[:,ind-it0] = (dPhi_comb_v_t[:,ind-it0]**3.) / (vprp[:]**2.)
  if exp_corr:
    #Dprpprp_Uk_t[:,ind-it0] = Dprpprp_Uk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))
    #Dprpprp_Uk_t[:,ind-it0] = Dprpprp_Uk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_of_dUprp_v_t[:,ind-it0]))
    Dprpprp_Uk_t[:,ind-it0] = Dprpprp_Uk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_comb_v_t[:,ind-it0]))
    #Dprpprp_Bzk_t[:,ind-it0] = Dprpprp_Bzk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))
    #Dprpprp_Bzk_t[:,ind-it0] = Dprpprp_Bzk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_of_dBz_v_t[:,ind-it0]))
    Dprpprp_Bzk_t[:,ind-it0] = Dprpprp_Bzk_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_comb_v_t[:,ind-it0]))
    Dprpprp_Phik_t[:,ind-it0] = Dprpprp_Phik_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_v_t[:,ind-it0]))
    Dprpprp_Phik_comb_t[:,ind-it0] = Dprpprp_Phik_comb_t[:,ind-it0]*np.exp(-c0exp*(vprp[:]**2./dPhi_comb_v_t[:,ind-it0]))
...
Dprpprp_Phik_comb_t_avg = np.sum(Dprpprp_Phik_comb_t,axis=1)/np.float(it1-it0+1)
#
if exp_corr:
...
Qprp_Phik_comb_t = np.zeros([len(vprp),it1-it0+1])
for ind in range(it0,it1+1):
  ...
  Qprp_Phik_comb_t[:,ind-it0] = - Dprpprp_Phik_comb_t[:,ind-it0] * dfdwprp_t[:,ind-it0]
...
Qprp_Phik_comb_t_avg = np.sum(Qprp_Phik_comb_t,axis=1)/np.float(it1-it0+1)
...
if apply_smoothing:
  ...
  normD_phicomb01 = cst01*Dprpprp[j01] / Dprpprp_Phik_comb[i01]
  normD_phicomb01_t_avg = cst01*Dprpprp[i01] / Dprpprp_Phik_comb_t_avg[i01]
  normD_bz01 = cst01*Dprpprp[j01] / Dprpprp_Bzk[j01]
  normD_bz01_t_avg = cst01*Dprpprp[j01] / Dprpprp_Bzk_t_avg[j01]
  normD_u01 = cst01*Dprpprp[k01] / Dprpprp_Uk[k01]
  normD_u01_t_avg = Dprpprp[k01] / Dprpprp_Uk_t_avg[k01]
  ...
  normD_phicomb01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Phik_comb[j01]
  normD_phicomb01_t_avg = cst01*Dprpprp_smooth[i01] / Dprpprp_Phik_comb_t_avg[i01]
  normD_bz01 = cst01*Dprpprp_smooth[j01] / Dprpprp_Bzk[j01]
...
Dprpprp_Phik_comb_t_avg *= normD_phicomb01_t_avg
#
print('\n')
print('### betai0 = 1/9 ###')
print('* kappa0 values: ')
...
Qprp_Phik_comb_t_avg *= normD_phicomb01_t_avg
#
#####
#
# << vs k_perp >>
...
Qprp_k_phik_comb_t_avg = np.interp( kprp_phi_rho, alphaPHI/vprp[::-1], Qprp_Phik_comb_t_avg[::-1] )
Qprp_k_phik_comb_t_avg *= (alphaPHI/kprp_phi_rho)
#
#Qprp_k_uk_t_avg /= np.log(10.)
#Qprp_k_bzk_t_avg /= np.log(10.)
#Qprp_k_phik_t_avg /= np.log(10.)
#Qprp_k_phik_comb_t_avg /= np.log(10.)
# 
k_vprp0_uk = 1. / vprp0
k_vprp0_uk *= alphaU
...
plt.plot(vprp,Dprpprp_Phik_comb_t_avg,c=clr_phicomb,ls=lnstyl[ils_phi],linewidth=line_thick,label=lbl_phi)
...
plt.plot(vprp,Qprp_Phik_comb_t_avg,c=clr_phicomb,ls=lnstyl[ils_phi],linewidth=line_thick)#,label=r'theory (full $\delta\Phi$)')
...
plt.plot(kprp_phi_rho,Qprp_k_phik_comb_t_avg,c=clr_phicomb,ls=lnstyl[ils_phi],linewidth=line_thick,label=lbl_phi)
