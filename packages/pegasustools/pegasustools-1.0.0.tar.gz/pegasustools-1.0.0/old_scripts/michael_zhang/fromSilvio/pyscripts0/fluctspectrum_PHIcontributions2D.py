...
def specEB2D(ii,tot,kxmin,kxmax,kzmin,kzmax,Nperp,asp,Nx):
  ...
  print "\n  [ Computing spectrum... ] "
  spectrumPHI1_3d = np.abs(locPHI1)**3. 
  spectrumPHI2_3d = np.abs(locPHI2)**3. 
  spectrumPHI3_3d = np.abs(locPHI3)**3. 
  ...
  #
  #PHI_mhd
  flnm = "".join([path_out,"turb.","%05d"%ii,".spectrum2d.nkperp","%d"%tot,".nkpara","%d"%Nx,".linear.PHI.UxBcontribution.3rdPower.dat"])
  out = open(flnm,'w+')
  for i in range(tot):
    for j in range(Nx):
  ...
  #
  #PHI_hall
  flnm = "".join([path_out,"turb.","%05d"%ii,".spectrum2d.nkperp","%d"%tot,".nkpara","%d"%Nx,".linear.PHI.JxBcontribution.3rdPower.dat"])
  out = open(flnm,'w+')
  for i in range(tot):
    for j in range(Nx):
  ...
  #
  #PHI_kin 
  flnm = "".join([path_out,"turb.","%05d"%ii,".spectrum2d.nkperp","%d"%tot,".nkpara","%d"%Nx,".linear.PHI.KINcontribution.3rdPower.dat"])
  out = open(flnm,'w+')
  for i in range(tot):
    for j in range(Nx):
