...
#wperp = np.zeros(Nparticle)
#wpara = np.zeros(Nparticle)
#vperp = np.zeros(Nparticle)
#vpara = np.zeros(Nparticle)
  ...
  # [1]=time     [2]=x1       [3]=x2       [4]=x3       [5]=v1       [6]=v2       [7]=v3       [8]=B1       [9]=B2       [10]=B3       [11]=E1       [12]=E2       [13]=E3       [14]=U1       [15]=U2       [16]=U3       [17]=dens     [18]=F1       [19]=F2       [20]=F3
    ...
    if ( (ii == 0) and (jj == 0) ):
      ...
      for iit in range(len(t)):
        ...
        if (t[iit] <= t1turb):
          it1turb = iit
      nt_turb = it1turb - it0turb + 1
      wperp = np.zeros(Nparticle*nt_turb)
      wpara = np.zeros(Nparticle*nt_turb)
      vperp = np.zeros(Nparticle*nt_turb)
      vpara = np.zeros(Nparticle*nt_turb)

    vpara[(ii + jj*len(id_proc))*nt_turb:(ii + jj*len(id_proc) + 1)*nt_turb] = v_para[it0turb:it1turb+1]
    vperp[(ii + jj*len(id_proc))*nt_turb:(ii + jj*len(id_proc) + 1)*nt_turb] = v_perp[it0turb:it1turb+1]
    wpara[(ii + jj*len(id_proc))*nt_turb:(ii + jj*len(id_proc) + 1)*nt_turb] = w_para[it0turb:it1turb+1]
    wperp[(ii + jj*len(id_proc))*nt_turb:(ii + jj*len(id_proc) + 1)*nt_turb] = w_perp[it0turb:it1turb+1]
...
n_bins=400
axs[0].hist2d(vpara, vperp, bins=(2*n_bins,n_bins))
axs[1].hist2d(wpara, wperp, bins=(2*n_bins,n_bins))
plt.show()
