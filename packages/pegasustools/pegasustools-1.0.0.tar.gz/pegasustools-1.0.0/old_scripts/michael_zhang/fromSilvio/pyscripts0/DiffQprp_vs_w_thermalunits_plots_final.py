...
kappaU01 = 1.3 #1.33 #1.25 #1.3 #1.2 
kappaB01 = 1.3 #1.33 #1.25 #1.3 #1.2 
kappaPHI01 = 1.3 #1.33 #1.25 #1.3 #1.2 
#
kappaU03 = 1.15 #1.0 
kappaB03 = 1.15 #1.0
kappaPHI03 = 1.15 #1.0 
...
for ind in range(it0,it1+1):
  ...
  if use_3rd_order:
    filename1b = base+"."+"%05d"%ind+".spectrum1d.nkperp"+"%d"%nkshells+"."+bin_type+".PHI.UxBcontribution.3rdPower.dat"
    filename2b = base+"."+"%05d"%ind+".spectrum1d.nkperp"+"%d"%nkshells+"."+bin_type+".PHI.KINcontribution.3rdPower.dat"
    filename3b = base+"."+"%05d"%ind+".spectrum1d.nkperp"+"%d"%nkshells+"."+bin_type+".PHI.3rdPower.dat"
  if dBprp_complement:
  ...
  if use_3rd_order:
    dataPhimhdkprp_b = np.loadtxt(filename1b)
    dataPhikinkprp_b = np.loadtxt(filename2b)
    dataPhitotkprp_b = np.loadtxt(filename3b)
  if dBprp_complement:
  ...
  if (ind == it0):
    ...
    if use_3rd_order:
      Phimhdkperp_b = np.zeros(len(dataPhimhdkprp_b))
      Phikinkperp_b = np.zeros(len(dataPhikinkprp_b))
      Phitotkperp_b = np.zeros(len(dataPhitotkprp_b))
    if dBprp_complement:
  ...
  if use_3rd_order: 
    for jj in range(len(dataPhimhdkprp_b)):
      Phimhdkperp_b[jj] += dataPhimhdkprp_b[jj,1]
    for jj in range(len(dataPhikinkprp_b)):
      Phikinkperp_b[jj] += dataPhikinkprp_b[jj,1]
    for jj in range(len(dataPhitotkprp_b)):
      Phitotkperp_b[jj] += dataPhitotkprp_b[jj,1]
  if dBprp_complement:
...
if use_3rd_order:
  Phimhdkperp_b /= np.float(it1-it0+1)
  Phikinkperp_b /= np.float(it1-it0+1)
  Phitotkperp_b /= np.float(it1-it0+1)
if dBprp_complement:
...
Phikprp = np.array([])
if use_3rd_order:
  Phimhdkprp_b = np.array([])
  Phikinkprp_b = np.array([])
  Phitotkprp_b = np.array([])
if dBprp_complement:
  Bprpkprp = np.array([])
Bkprp = np.array([])
...
if use_3rd_order:
  for jj in range(len(kperp_phi)):
    if ( Phimhdkperp_b[jj] > 1e-30 ):
      Phimhdkprp_b = np.append(Phimhdkprp_b,Phimhdkperp_b[jj])
    if ( Phikinkperp_b[jj] > 1e-30 ):
      Phikinkprp_b = np.append(Phikinkprp_b,Phikinkperp_b[jj])
    if ( Phitotkperp_b[jj] > 1e-30 ):
      Phitotkprp_b = np.append(Phitotkprp_b,Phitotkperp_b[jj])
if dBprp_complement:
  for jj in range(len(kperp_bz)):
...
Phikprp = Phikprp[1:]
if use_3rd_order:
  Phimhdkprp_b = Phimhdkprp_b[1:]
  Phikinkprp_b = Phikinkprp_b[1:]
  Phitotkprp_b = Phitotkprp_b[1:]
if dBprp_complement:
  Bprpkprp = Bprpkprp[1:]
Bkprp = Bkprp[1:]
print "dB/B0 vs kprp at beta = 1/9 "
for iik in range(len(kprp_bz_rho)):
  print iik, kprp_bz_rho[iik],np.sqrt(kprp_bz_rho[iik]*Bkprp[iik])
...
#
#if apply_smoothing:
#  Qprp_vprp03_original = np.load(path_read_lev+"edotv_beta03.npy")
#  Qprp_vprp03_original *= normQ_sim03
#  Qprp_vprp03 = gaussian_filter(Qprp_vprp03_original,sigma=sigma_smoothing)
...
if output_figure:
  ...
  if apply_smoothing:
    flnm += '_smooth-sigma%.3f'%sigma_smoothing
  if (not dBprp_complement):
    flnm += '_NOdBprpCorrections'
  if cooling_corr:
    flnm += '_CoolCorr_%03d'%it0corr+'-%03d'%it1corr
  else:
    flnm += '_NOcoolingcorr'
  if use_3rd_order:
    flnm += '_3rdPower'
