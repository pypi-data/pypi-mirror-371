...
sQprpf = Qprpf[0:m]
for ii in range(m-1):
  sQtotf[ii+1] = sQtotf[ii+1] + Qtotf[len(Qtotf)-1-m]
  sQparf[ii+1] = sQparf[ii+1] + Qparf[len(Qparf)-1-m]
  sQprpf[ii+1] = sQprpf[ii+1] + Qprpf[len(Qprpf)-1-m]

freq = freq*2.*np.pi
print freq[m]

plt.plot(freq[1:m],sQtotf[1:m],'k')
plt.plot(freq[1:m],sQprpf[1:m],'b')
plt.plot(freq[1:m],sQparf[1:m],'r')
plt.xscale("log")
plt.yscale("log")
