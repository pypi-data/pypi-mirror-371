...
def get_Diff_Q(path_in,ii0,ii1,Nv_prp,Nv_prl,v_prp_min,v_prp_max,v_prl_min,v_prl_max,num_procs,cool_corr=False,ii0corr,ii1corr,smoot_curves=False,sigma_=1):
  #
  #reading initial condition
  print("\n [ reading initial condition ]")
  vdf0, vprp0, vprl0 = pegr.readnpy_vdf(path_in,0,Nv_prp,Nv_prl,v_prp_min,v_prp_max,v_prl_min,v_prl_max,verbose=verb_read)
  #
  #first normalization by number of processors
  vdf0 = vdf0 / np.float(num_procs)
  #
  #
  print("\n ### HEATING VS W_PERP ###")
  #
  for iii in range(ii0,ii1+1):
    ...
    vdf_, vprp, vprl = pegr.readnpy_vdf(path_in,iii,Nv_prp,Nv_prl,v_prp_min,v_prp_max,v_prl_min,v_prl_max,verbose=False)
    edotv_prl_ = pegr.readnpy_vspaceheat_prl(path_in,iii,Nv_prp,Nv_prl,v_prp_min,v_prp_max,v_prl_min,v_prl_max,grid=False,verbose=False)
    edotv_prp_ = pegr.readnpy_vspaceheat_prp(path_in,iii,Nv_prp,Nv_prl,v_prp_min,v_prp_max,v_prl_min,v_prl_max,grid=False,verbose=False)
    #
    dvprp = vprp[2]-vprp[1]
    dvprl = vprl[2]-vprl[1]
    #
    #first normalization by number of processors
    vdf_ /= np.float(num_procs)
    edotv_prl_ /= np.float(num_procs)
    edotv_prp_ /= np.float(num_procs)
  
    if smooth_curves:
      ggsmooth = gaussian_filter(edotv_prp_,sigma=sigma_)
      edotv_prp_ = ggsmooth
      ggsmooth = gaussian_filter(edotv_prl_,sigma=sigma_)
      edotv_prl_ = ggsmooth
    else:
      ggsmooth = gaussian_filter(edotv_prp_,sigma=sigma_)
    ...
    if (iii == ii0):
      vdf_avg = np.zeros([Nv_prp,Nvpara])
      dfdwprp_avg = np.zeros(Nv_prp)
      edotv_prl_avg = np.zeros([Nv_prp,Nv_prl])
      edotv_prp_avg = np.zeros([Nv_prp,Nv_prl])
      vdf_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])
      dfdwprp_t = np.zeros([Nv_prp,ii1-ii0+1])
      edotv_prp_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])   
      edotv_prl_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])
      if (not smooth_curves):
        edotv_prl_smooth_avg = np.zeros([Nv_prp,Nv_prl])
        edotv_prp_smooth_avg = np.zeros([Nv_prp,Nv_prl])
        edotv_prp_smooth_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])
        edotv_prl_smooth_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])


    vdf_avg += vdf_ / np.float(ii1-ii0+1)
    edotv_prl_avg += edotv_prl_ / np.float(ii1-ii0+1)
    edotv_prp_avg += edotv_prp_ / np.float(ii1-ii0+1)
    vdf_t[:,:,iii-ii0] = vdf_
    edotv_prp_t[:,:,iii-ii0] = edotv_prp_ 
    edotv_prl_t[:,:,iii-ii0] = edotv_prl_ 
    if (not smooth_curves):
      edotv_prl_smooth_avg += edotv_prl_smooth_ / np.float(ii1-ii0+1) 
      edotv_prp_smooth_avg += edotv_prp_smooth_ / np.float(ii1-ii0+1) 
      edotv_prp_smooth_t[:,:,iii-ii0] = edotv_prp_smooth_
      edotv_prl_smooth_t[:,:,iii-ii0] = edotv_prl_smooth_

  
    #computing <df/dwprp>
    fff = np.zeros([Nv_prp,Nv_prl])
    fff_t = np.zeros([Nv_prp,Nv_prl,ii1-ii0+1])
    for ivv in range(Nv_prp):
      fff[ivv,:] = vdf_[ivv,:] / vprp[ivv]
      fff_t[ivv,:,:] = vdf_t[ivv,:,:] / vprp[ivv]
    dfdwprp_avg += np.gradient(np.sum(fff*dvprl,axis=1),vprp) / np.float(ii1-ii0+1)
    for itt in range(ii0,ii1+1):
      dfdwprp_t[:,itt-ii0] = np.gradient(np.sum(fff_t[:,:,itt-ii0]*dvprl,axis=1),vprp)
      dfdwprp_t[:,itt-ii0] /= np.sum(fff_t[:,:,itt-ii0]*dvprl*dvprp)
    if (iii == ii0):
...
print("  -> integral of Dprpprp: ",np.sum(Dprpprp*dvprp))
print(" ############################################################\n")

#plt.plot(vprp,dfdwprp)
#plt.plot(vprp,dfdwprp_finitediff,ls='--')
#plt.xscale("log")
#plt.show()
...
yr_min_D_w = 8e-3#8e-1 #np.min(np.abs(Dprpprp_corrected))
yr_max_D_w = 8e+1#8e+3 #np.max(np.abs(Dprpprp_corrected))
#
# vs k_perp
xr_min_k = 1./12.
xr_max_k = 12.
