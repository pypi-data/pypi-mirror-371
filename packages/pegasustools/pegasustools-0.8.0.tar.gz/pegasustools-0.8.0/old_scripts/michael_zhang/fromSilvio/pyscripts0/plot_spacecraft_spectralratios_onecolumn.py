...
# [1]=time     [2]=x1       [3]=x2       [4]=x3       [5]=v1       [6]=v2       [7]=v3       [8]=B1       [9]=B2       [10]=B3       [11]=E1       [12]=E2       [13]=E3       [14]=U1       [15]=U2       [16]=U3       [17]=dens     [18]=F1       [19]=F2       [20]=F3
  ...
  sBf_ = np.abs(B1f)*np.abs(B1f) + np.abs(B2f)*np.abs(B2f) + np.abs(B3f)*np.abs(B3f)
  sEf_ = np.abs(E1f)*np.abs(E1f) + np.abs(E2f)*np.abs(E2f) + np.abs(E3f)*np.abs(E3f)
  ...
  if (ii == 0): 
    ...
    #
    sBf_avg_ = np.zeros(len(sBf_))
    sEf_avg_ = np.zeros(len(sEf_))
  ...
  for kk in range(len(sBparaf_)):
    sBf_avg_[kk] += sBf_[kk]/Nspcrft
    sEf_avg_[kk] += sEf_[kk]/Nspcrft
...
sBf_avg = sBf_avg_[0:m]
sEf_avg = sEf_avg_[0:m]
...
for ii in range(m-1):
  sBf_avg[ii+1] += sBf_avg_[len(sBf_)-1-m]
  sEf_avg[ii+1] += sEf_avg_[len(sEf_)-1-m]
...
sBf_avg_smooth = gaussian_filter1d(sBf_avg, sigma=sigma_smoothing)
sEf_avg_smooth = gaussian_filter1d(sEf_avg, sigma=sigma_smoothing)
...
aBfreq = np.zeros(len(sBf_avg_smooth))
aEfreq = np.zeros(len(sEf_avg_smooth))
...
f_fit = freq[0:m]
Bf_fit = sBf_avg_smooth
Ef_fit = sEf_avg_smooth
...
if (fit_method == 'local_slope_manual'):
  for jj in range(3,len(f_fit)-2*n_pt):
    if (jj < n_pt):
      #--odd indices
      i_npt = jj-1 
      aBfreq[jj+i_npt],c = np.polyfit(np.log10(f_fit[jj:jj+2*i_npt+1]),np.log10(Bf_fit[jj:jj+2*i_npt+1]),1)
      aEfreq[jj+i_npt],c = np.polyfit(np.log10(f_fit[jj:jj+2*i_npt+1]),np.log10(Ef_fit[jj:jj+2*i_npt+1]),1)
      ...
      aBfreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Bf_fit[jj:jj+2*n_pt+1]),1)
      aEfreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Ef_fit[jj:jj+2*n_pt+1]),1)
      aBperpfreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Bperpf_fit[jj:jj+2*n_pt+1]),1)
      aBparafreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Bparaf_fit[jj:jj+2*n_pt+1]),1)
      aEperpfreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Eperpf_fit[jj:jj+2*n_pt+1]),1)
      aEparafreq[jj+n_pt],c = np.polyfit(np.log10(f_fit[jj:jj+2*n_pt+1]),np.log10(Eparaf_fit[jj:jj+2*n_pt+1]),1)
  ...
  for jj in range(len(aBperpfreq)):
    if ( (np.abs(aBfreq[jj])>1e-20) and (np.abs(aEfreq[jj])>1e-20) and (np.abs(aBperpfreq[jj])>1e-20) and (np.abs(aBparafreq[jj])>1e-20) and (np.abs(aEperpfreq[jj])>1e-20) and (np.abs(aEparafreq[jj])>1e-20) and (np.abs(aDnfreq[jj])>1e-20) ):
      freq_fit = np.append(freq_fit,f_fit[jj])
      aBf = np.append(aBf,aBfreq[jj])
      aEf = np.append(aEf,aEfreq[jj])
...
if (fit_method == 'logspaced_gradient'):
  ...
  print df_logspaced
  Bf_logspaced = np.interp(f_logspaced,f_fit,Bf_fit)
  Ef_logspaced = np.interp(f_logspaced,f_fit,Ef_fit)
  ...
  #
  aBfreq = np.gradient(np.log10(Bf_logspaced),df_logspaced)
  aEfreq = np.gradient(np.log10(Ef_logspaced),df_logspaced)
...
##yr_min = 8.e-11 #0.5*np.min([np.min(sBparaf_avg),np.min(sBperpf_avg),np.min(sEperpf_avg),np.min(normKAW*sDnf_avg)])
yr_min = 2.e-7
##yr_max = 8.e-3 #2.0*np.max([np.max(sBparaf_avg),np.max(sBperpf_avg),np.max(sEperpf_avg),np.max(normKAW*sDnf_avg)])
yr_max = 2.e-6
yr_min_s = -6
yr_max_s = +0.75
yr_min_r5 = 7e-1
...
p1a, = plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),(freq[1:m]**2.)*sBperpf_avg[1:m],'b',linewidth=line_thick,label=r"$\mathcal{E}_{B_\perp}$")
...
plt.text(1.1*omega_drift,2.*yr_min,r'$\omega = \omega_{\mathrm{D}}$',va='bottom',ha='left',color='m',rotation=90,fontsize=font_size)
##p6a, = plt.plot(freq[1:m],2e-7*np.power(freq[1:m],-2.0),'k--',linewidth=slope_thick,label=r"$\omega^{\,-2}$")
##p7a, = plt.plot(freq[1:m],2e-7*np.power(freq[1:m],-4.0),'k-.',linewidth=slope_thick,label=r"$\omega^{\,-4}$")
#p6a, = plt.plot(freq[1:m],sBperpf_avg[8]*np.power(freq[1:m]/freq[8],-2.0),'k--',linewidth=slope_thick,label=r"$\omega^{\,-2}$")
#p7a, = plt.plot(freq[1:m],0.6*sBperpf_avg[8]*np.power(freq[1:m]/freq[8],-2.0),'k--',linewidth=slope_thick,label=r"$\omega^{\,-2}$")
...
#plt.scatter(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r5[1:m],color='k',s=2.5)
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r5[1:m],'k',linewidth=line_thick)#,label=r"$|\delta E_\perp|^2/|\delta B_\perp|^2$")
plt.plot(freq[1:m],7.0*np.power(freq[1:m],1.),'g--',linewidth=line_thick,label=r"$\propto\omega$")
plt.plot(freq[1:m],6.0*np.power(freq[1:m],6./5.),'b--',linewidth=line_thick,label=r"$\propto\omega^{6/5}$")
plt.plot(freq[1:m],5.0*np.power(freq[1:m],3./2.),'r--',linewidth=line_thick,label=r"$\propto\omega^{3/2}$")
...
#plt.scatter(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r3[1:m],color='k',s=2.5)
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r3[1:m],'k',linewidth=line_thick,label=r"$C_1|\delta B_z|^2/|\delta B_\perp|^2$")
plt.axhline(y=1.0,c='k',ls='--',linewidth=slope_thick)
plt.axvline(x=1.0,c='k',ls=':',linewidth=slope_thick)
plt.axvline(x=2.0,c='k',ls=':',linewidth=slope_thick)
plt.axvline(x=3.0,c='k',ls=':',linewidth=slope_thick)
...
#plt.scatter(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r2[1:m],color='k',s=2.5)
plt.plot(np.ma.masked_where(freq[1:m] > f_mask, freq[1:m]),r2[1:m],'k',linewidth=line_thick,label=r"$C_2\delta n^2/\delta B_z^2$")
plt.axhline(y=1.0,c='k',ls='--',linewidth=slope_thick)
plt.axhline(y=1.0,c='k',ls='--',linewidth=slope_thick)
plt.axvline(x=1.0,c='k',ls=':',linewidth=slope_thick)
plt.axvline(x=2.0,c='k',ls=':',linewidth=slope_thick)
...
#plt.axvline(x=5.0,c='k',ls=':',linewidth=1.5)
plt.axvline(x=omega_drift,c='m',ls=':',linewidth=slope_thick)
#plt.text(1.1*omega_drift,1.25*yr_min_r2,r'$\omega/\Omega_{\mathrm{c,i}}\sim\langle\beta_{\mathrm{i}}\rangle_{\mathrm{turb}}$',va='bottom',ha='left',color='m',rotation=90,fontsize=16)
plt.text(1.1*omega_drift,1.25*yr_min_r2,r'$\omega = \omega_{\mathrm{D}}$',va='bottom',ha='left',color='m',rotation=90,fontsize=font_size)
plt.xlim(xr_min,xr_max)
plt.ylim(yr_min_r2,yr_max_r2)
plt.xscale("log")
plt.yscale("log")
plt.xlabel(r'$\omega / \Omega_{\mathrm{c,i}}$',fontsize=font_size)
...
if output_figure:
  plt.tight_layout()
  if (fit_method == 'local_slope_manual'):
    flnm = prob+".StationarySpacecraftsAvg.FreqSpectrumEBDen-Slopes-Ratios.one-column.npt"+"%d"%n_pt+".t-interval."+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))
  if (fit_method == 'logspaced_gradient'):
    flnm = prob+".StationarySpacecraftsAvg.FreqSpectrumEBDen-Slopes-Ratios.one-column.nlogbins"+"%d"%n_logbins+".t-interval."+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))
...
if save_npy_plots:
  #--spectra
  flnm_save = path_save_npy+prob+".spectra-vs-freq.omega."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,freq[1:m])
  print " * freq saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.B."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sBf_avg[1:m])
  print " * Bf_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.E."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sEf_avg[1:m])
  print " * Ef_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.Bprp."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sBperpf_avg[1:m])
  print " * Bperpf_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.Bprl."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sBparaf_avg[1:m])
  print " * Bparaf_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.Eprp."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sEperpf_avg[1:m])
  print " * Eperpf_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.Eprl."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sEparaf_avg[1:m])
  print " * Eparaf_avg saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".spectra-vs-freq.Den."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,sDnf_avg[1:m])
  print " * Dnf_avg saved in -> ",flnm_save
  #--slopes
  flnm_save = path_save_npy+prob+".slope-vs-freq.omega."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,freq_fit)
  print " * freq_fit saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.B."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aBf)
  print " * aBf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.E."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aEf)
  print " * aEf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.Bprp."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aBperpf)
  print " * aBperpf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.Bprl."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aBparaf)
  print " * aBparaf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.Eprp."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aEperpf)
  print " * aEperpf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.Eprl."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aEparaf)
  print " * aEparaf saved in -> ",flnm_save
  flnm_save = path_save_npy+prob+".slope-vs-freq.Den."+"t-avg.it"+"%d"%int(round(t_real[it0turb]))+"-"+"%d"%int(round(t_real[it1turb]))+".npy"
  np.save(flnm_save,aDnf)
  print " * aDnf saved in -> ",flnm_save
