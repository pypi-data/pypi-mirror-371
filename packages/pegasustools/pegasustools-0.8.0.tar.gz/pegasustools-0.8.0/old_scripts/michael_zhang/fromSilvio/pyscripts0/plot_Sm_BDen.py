...
if ( (compare_avg) and (it1 != it0) ):
  for ii in range(it0,it1+1):
    ...
    if (ii == it0):
      ...
      temp = np.loadtxt(flnmS4_dn)
      lprpS4_dn_avg = np.zeros(len(temp[:,0]))
      lprlS4_dn_avg = np.zeros(len(temp[:,0]))
      temp = np.loadtxt(flnm_bprp)
    ...
    print " -> ",flnm_dn
    temp = np.loadtxt(flnm_dn)
    lprp_dn_avg += temp[:,0] / norm2
    lprl_dn_avg += temp[:,1] / norm2
    print " -> ",flnmS4_dn
    temp = np.loadtxt(flnmS4_dn)
    lprpS4_dn_avg += temp[:,0] / norm2
    lprlS4_dn_avg += temp[:,1] / norm2
    #--dBperp
    print " -> ",flnm_bprp
    temp = np.loadtxt(flnm_bprp)
...
#--vs l_perp
Kdn_prp = s4_dn_prp / (s2_dn_prp*s2_dn_prp) - 3.0
