...
Phi_beta01 = np.load(root_path+"turb.spectra-vs-kprp.PHI.t-avg.it65-144.npy")
...
fp1 = interp1d(x_bperp1, x_bperp1**2*(np.sqrt(x_bperp1*y_phi1))**3)
fp03 = interp1d(x_bperp03, x_bperp03**2*(np.sqrt(x_bperp03*y_phi03))**3)
fp01 = interp1d(kprp_beta01, kprp_beta01**2*(np.sqrt(kprp_beta01*Phi_beta01))**3)
...
DppP03 = fp03(kk03)
DppP01 = fp01(kk01)
DppP01 = (kk01 >= 2*np.min(kprp_beta01))*fp01(kk01)
...
LambdaP_beta1 = 1./DppP1[30] * np.absolute(diffusion_beta1_2[30])
LambdaP_beta03 = 1./DppP03[20]* np.absolute(diffusion_beta03_2[20])
LambdaP_beta01 = 1./DppP01[20]* np.absolute(diffusion_beta01_2[20])
...
ax1.plot(u[1:-1],LambdaP_beta1 * np.absolute(DppP1[1:-1]),'m-')
...
ax2.plot(u[1:-1],LambdaP_beta03 * np.absolute(DppP03[1:-1]),'m-')
plt.xlim((0.1,5.0))
plt.ylim((0.011,1100))
plt.yscale('log')
plt.xscale('log')
...
ax5.plot(u[1:-1],LambdaP_beta1 * np.absolute(DppP1[1:-1] * (spec_beta1[2:]-spec_beta1[:-2])/(u[2:]-u[:-2])),'m-')
...
ax6.plot(u[1:-1],LambdaP_beta03 * np.absolute(DppP03[1:-1] * (spec_beta03[2:]-spec_beta03[:-2])/(u[2:]-u[:-2])),'m-')
...
ax7.plot(u[1:-1],LambdaP_beta01 * np.absolute(DppP01[1:-1]),'m-')
...
ax8.plot(u[1:-1],LambdaP_beta01 * np.absolute(DppP01[1:-1] * (spec_beta01[2:]-spec_beta01[:-2])/(u[2:]-u[:-2])),'m-')
