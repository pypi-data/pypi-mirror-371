...
alphaU = np.pi/2. #1.5 #np.pi*np.sqrt(betai0) #np.pi
alphaB = np.pi/2. #1.5 #np.pi
alphaPHI = np.pi/2. #1.5 #np.pi
...
c0 = 1.#np.abs(Qprp_vprp[np.where( vprp > 1./alphaPHI)[0][0]])/Qprp_Phik_norm[np.where( vprp > 1./alphaPHI)[0][0]]
plt.plot(vprp,c0*Qprp_Phik_norm,'r',linewidth=line_thick)
...
plt.plot(kprp_u_rho,dfdwprp_k_uk,'g--')
plt.plot(kprp_bz_rho,dfdwprp_k_bzk,'m--')
