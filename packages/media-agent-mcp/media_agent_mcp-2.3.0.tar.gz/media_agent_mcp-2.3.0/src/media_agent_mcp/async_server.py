...
@mcp.tool()
@async_retry()
async def combine_audio_video_tool(video_url: str, audio_url: str, audio_start_time_ms: int = 0) -> dict:
    """
    Asynchronously combine an audio and a video file from URLs, with an optional start time for the audio.
    The combined video will be uploaded to TOS.
    If the audio is longer than the video, it will be truncated to match the video's duration.
    
    Args:
        video_url: URL of the video file.
        audio_url: URL of the audio file.
        audio_start_time_ms: Start time for the audio insertion in milliseconds. Defaults to 0.
    
    Returns:
        Dictionary with status, data, and message.
    """
    result = await async_combine_audio_video_tool(video_url, audio_url, audio_start_time_ms)
    return result
