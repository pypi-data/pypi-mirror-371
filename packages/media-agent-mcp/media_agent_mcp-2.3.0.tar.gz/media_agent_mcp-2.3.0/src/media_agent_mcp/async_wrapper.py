...
from media_agent_mcp.audio.audio_video_combiner import combine_audio_video
...
@async_wrapper
@json_response_wrapper
def _sync_combine_audio_video(video_url: str, audio_url: str, audio_start_time_ms: int = 0) -> str:
    """Synchronous wrapper for combining audio and video."""
    return combine_audio_video(video_url, audio_url, audio_start_time_ms)
...
async def async_combine_audio_video_tool(video_url: str, audio_url: str, audio_start_time_ms: int = 0) -> str:
    """Async tool for combining audio and video."""
    result = await _sync_combine_audio_video(video_url, audio_url, audio_start_time_ms)
    return json.loads(result)


@async_wrapper
def _sync_add_subtitles_to_video(video_url: str, subtitles_input: str, font_name: Optional[str] = None, font_color: Optional[str] = None) -> str:
    ...
    """
    try:
        result = add_subtitles_to_video(video_url, subtitles_input, font_name=font_name, font_color=font_color)
        return json.dumps(result)
    except Exception as e:
        return json.dumps({
            "status": "error",
...
async def async_add_subtitles_to_video_tool(video_url: str, subtitles_input: str, font_name: Optional[str] = None, font_color: Optional[str] = None) -> str:
    """Async tool for adding subtitles to video. If font_name or font_color is None, auto-select using Seed1.6."""
    return await _sync_add_subtitles_to_video(video_url, subtitles_input, font_name, font_color)
