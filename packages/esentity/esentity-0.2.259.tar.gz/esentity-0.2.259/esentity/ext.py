...
from jinja2.nodes import List, CallBlock, Const
from esentity.models import Page, Activity
from esentity.models import OddsWidget, OddsMarket, OddsEvent
...
class CollectionsExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        items, _ = Page.get(
            ...
            _source=['title', 'path', 'alt_title', 'logo', 'theme_color'],
            _count=count,
            _exclude=exclude,
            _sort=[
                {'alt_title.keyword': {'order': 'asc'}}
        ...
        if aggs_key:
            ...
            if current_app.config.get('ENTITY_MULTIPLE_LOCALE', False):
                filter_list.append({"term": {"locale_available.keyword": g.language}})
            else:
                filter_list.append({"term": {"locale.keyword": g.language}})
            ...
            exclude_list = []
            exclude_list.append({"terms" if isinstance(v, list) else "term": {pre_field(k, v): v} for k, v in dict(provider_tags=['closed']).items()})
...
class CasinosExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        default = [10, 'casinos_top', True, False, None] 
        # cnt, template, has_geo, has_append, sort, [tags...]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]
        ...
        try:
            ...
            _total = current_app.redis.get('gs:cnt_casinos_total')
            _total = _total.decode('utf-8') if _total else 0
            ...
            s = t.render(
                ...
                cnt_casinos_total=_total,
                caller=caller,
            )
        except TemplateNotFound:
            logger.warning(f'Template _ext-{template}.html not found')
    ...
    def parse(self, parser):
        ...
        body = ''
        is_caller = str(tag) in ['casinoscustom']
        if is_caller:
            body = parser.parse_statements(['name:endcasinoscustom'], drop_needle=True)

        return CallBlock(self.call_method("_render", args=[List(args)]), [], [], body).set_lineno(tag.lineno)          
...
class TestExtension(Extension):
    ...
    def parse(self, parser):
        ...
        body = ''
        is_caller = str(tag) in ['testcall']
        if is_caller:
            body = parser.parse_statements(['name:endtestcall'], drop_needle=True)

        return CallBlock(self.call_method('_render', args=[List(args)]), [], [], body).set_lineno(tag.lineno)

    def _render(self, args, caller=None):
        return f'args: {args}, caller: {caller()}'
...
class SlotsV2Extension(Extension):
    tags = set(['slots'])

    def __init__(self, environment):
        super(SlotsV2Extension, self).__init__(environment)

    def _render(self, args, caller=None):
        logger.info(f'slots_v2 Args IN: {args}')

        default = [24, False, ['software', 'slot_features', 'volatility', 'themes']]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]

        logger.info('slots_v2 Args OUT: {0}'.format(args))

        count = int(args[0])
        parse_kwargs = bool(args[1])
        aggs_fields = args[2]

        # tags = args[len(default):]
        # logger.info('slots_v2 Tags: {0}'.format(tags))
        ...
        # args: service
        items, total, aggs, _ = Page.slots_by_context(
            is_searchable=True,
            is_redirect=False,
            _source=True,
            _count=count,
        ...
        if parse_kwargs:
            ...
            try:
                ...
                logger.warning(f'slots_v2 kwargs parse error: {rv}')

        t = current_app.jinja_env.get_template('_ext-rating_slots.html')
        s = t.render(
            items=items, 
            total=total, 
            count=len(items),
            cpp=count,
            percent=int(((count if total > count else total) / total) * 100) if total else 0,
            aggs=aggs,
            **kw
        )
        return s
    ...
    def parse(self, parser):
        ...
        body = parser.parse_statements(['name:endslots'], drop_needle=True)
...
class BlogExtension(Extension):
    ...
    def _render(self, args, caller=None):
        logger.info(f'blog Args in: {args}')

        default = [12, 3, True, 'blog']
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]

        logger.info(f'blog Args out: {args}')
        ...
        t = current_app.jinja_env.get_template(f'_ext-{_template}.html')
...
class RelatedPagesExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        default = ['RelatedBlog', None, 10, None]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]
        ...
        if query:
            pages = [item for item, _ in Page.query(
                query, False, 
                count=count + 1, 
                locale_list=current_app.config.get('AVAILABLE_LOCALE'),
        ...
        s = t.render(pages=pages[:count], id=eid)
        return s
...
class PagesListExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        default = [20, 4, 'pages_list', [], {'tags': 'Promo'}, None, [{'publishedon': {'order': 'desc'}}]]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]
...
class UIExtension(Extension):
    ...
    def _render(self, args, caller):
        ...
        if is_parse:
            def parse_item(s):
                if dlmtr_kv:
                    ...
                    if len(_d) >= 2:
                        _res.update({'key': _d[0], 'value': dlmtr_kv.join(_d[1:])})
                        if len(_d) > 2:
                            _res.update({f'args_{idx}': item for idx, item in enumerate(_d[1:])})
                else:
                    _res = s

                return _res
            try:
                items = [parse_item(item) for item in rv.split(dlmtr_items)]
            except:
        ...
        try:
            ...
            logger.error(f'UI extension exception: {e}')
...
class RatingV2Extension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        default = ['casino', 10, False, ['software', 'provider_tags', 'deposits', 'licences', 'languages'], 1000, []]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]
...
class SitemapExtension(Extension):
    ...
    def __init__(self, environment):
        super(SitemapExtension, self).__init__(environment)

    def _render(self, args, caller=None):
        logger.info('sitemap Args: {0}'.format(args))
        ...
        pages, total = Page.get(
            ...
            is_redirect=False, 
            locale=g.language, 
            _source=['category', 'path', 'title', 'publishedon', 'tags'],
            _all=True,
            _sort=[
        ...
        logger.info(f'Sitemap found pages: {total}')

        t = current_app.jinja_env.get_template('_ext-sitemap.html')
        return t.render(pages=pages, casinos=[])
...
class OddsExtension(Extension):
    tags = set(['odds'])

    def __init__(self, environment):
        super(OddsExtension, self).__init__(environment)

    def _render(self, args, caller=None):
        logger.info('odds Args: {0}'.format(args))
        ...
        widget = OddsWidget.get_one(is_active=True, alias=args[0])
        if widget:
            for item in widget.markets:
                for attr in item.keys():
                    if 'mid_' in attr and item[attr]:
                        mids.append(item[attr])

        if mids:
            markets, _ = OddsMarket.get(_include=mids, is_active=True)
            markets = {item._id: item.to_dict() for item in markets}

        t = current_app.jinja_env.get_template('_ext-odds.html')
        return t.render(widget=widget, markets=markets, odds_format=odds_format)
...
class AuthorsListExtension(Extension):
    tags = set(['author_list'])

    def __init__(self, environment):
        super(AuthorsListExtension, self).__init__(environment)

    def _render(self, args, caller=None):
        logger.info(f'author_list Args IN: {args}')

        default = [20, 4, 'pages_list', None]
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]

        logger.info('author_list Args OUT: {0}'.format(args))
        ...
        pages, found = Page.get(
            ...
            is_searchable=True,
            is_redirect=False,
            category=['page', 'collection', 'provider'],
            authors=author,
            _sort=[{'publishedon': {'order': 'desc'}}],
            _source = [
                "category",
