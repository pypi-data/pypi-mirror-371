...
def lora_finetune(
    ...
    optimizer: Union[str, torch.optim.Optimizer, Callable[[iter], torch.optim.Optimizer], None] = None,
    ...
    if optimizer is None:
        ...
        key = optimizer.lower()
        if key not in OPTZ:
            raise ValueError(f"Unknown optimizer '{optimizer}'. Try one of {list(OPTZ.keys())} or pass a callable.")
