...
class Track():
    ...
    **penalty** : ``int``, default ``0``
        Note: thil feature is experimental
        0: use distance, intensity, size, background gradient for similarity criterion (similar to Lupo et al. 2023)
    ...
    **QS_min_radius** : ``int`` or ``float``, default ``800``
        Minimum size for a significant blocking [km] (~S in Schwierz et al. 2004).  
        800 km corresponds to about 2,000,000 km^2.
    ...
    **only_count** : ``bool``, default ``False``
    ...
    def __init__(self, 
                 times,  # tracking timestops as DatetimeIndex
                 lev,  # level name
                 ty='both',  # "L","H","both"
