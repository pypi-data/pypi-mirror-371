...
def interpolate_HL_vals(df, vws, alphas, value='Omega_tilde_int_extrap',
    ...
    df2 = df[columns]
    val_alphas = jnp.unique(df2['alpha'])
    val_vws = jnp.unique(df2['v_wall'])

    # Omegas = jnp.zeros((len(val_vws), len(val_alphas))) - 1e30
    Omegas = jnp.full((len(val_vws), len(val_alphas)), -1e30)
    ...
    for i in range(0, len(val_alphas)):
        for j in range(0, len(val_vws)):
            ...
            if Om.size > 0:
                Omegas = Omegas.at[j, i].set(Om[0])
                # for curly_K0 we interpolate
                # kappa0 = curly_K0/alpha*(1 + alpha)
                if value == 'curly_K_0_512':
                    val = Omegas[j, i] * (1 + val_alphas[i]) / val_alphas[i]
                    Omegas = Omegas.at[j, i].set(val)

    # interpolate for all values of vws for the 3 values of alpha
    Omss = jnp.zeros((len(vws), len(val_alphas)))
    for i in range(0, len(val_alphas)):
        inds = jnp.where(Omegas[:, i] > -1e30)[0]
        Omss[:, i] = jnp.interp(vws, val_vws[inds], Omegas[inds, i])
        inds2 = jnp.where(Omegas[:, i] == -1e30)[0]
        Omegas = Omegas.at[inds2, i].set(
            jnp.interp(val_vws[inds2], val_vws[inds], Omegas[inds, i])
        )

    # interpolate for all values of alpha
    if mult_alpha:
        Omsss = jnp.zeros((len(vws), len(alphas)))
        for i in range(0, len(vws)):
            Omsss = Omsss.at[i, :].set(
            jnp.interp(jnp.log10(alphas), jnp.log10(val_alphas), Omss[i, :])
            )

        # for curly_K0 we interpolate kappa0 = curly_K0/alpha*(1 + alpha)
        if value == 'curly_K_0_512':
            Omsss = Omsss.at[i, :].set(
                Omsss[i, :] * alphas / (1 + alphas)
            )
            Omegas = Omegas * val_alphas / (1 + val_alphas)
    else:
        Omsss = jnp.zeros(len(vws))
        for i in range(0, len(vws)):
            Omsss = Omsss.at[i].set(
                jnp.interp(
                    jnp.log10(alphas), jnp.log10(val_alphas), Omss[i, :]
                )
            )
        # for curly_K0 we interpolate kappa0 = curly_K0/alpha*(1 + alpha)
        if value == 'curly_K_0_512':
...
def ampl_GWB_sw(model='fixed_value', OmGW_sw=OmGW_sw_ref, vws=None,
    ...
    vws = jnp.array(vws) if vws is not None else jnp.array([])
    alphas = jnp.array(alphas) if alphas is not None else jnp.array([])

    if model == 'fixed_value':
        Omegas = jnp.full((len(vws), len(alphas)), OmGW_sw)
    elif model == 'higgsless':
        val_str = 'Omega_tilde_int_extrap'
        if vws.size == 0 or alphas.size == 0:
