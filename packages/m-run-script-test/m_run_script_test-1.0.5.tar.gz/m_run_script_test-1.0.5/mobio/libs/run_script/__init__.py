...
class MobioRunScript:
    ...
    def call_script(cls, command):
        try:
            command_run = "source ~/.bashrc; " + command
            print("call_script begin command: {}".format(command_run))
            result = subprocess.run(["/bin/bash", "-c", command_run])
            print("call_script end command: {}".format(command_run))
