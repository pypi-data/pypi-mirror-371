...
class MethodVisitor(PTNodeVisitor):
    ...
    def visit_signature(cls, node, children):
        """
        icaps_name input_parameters output_type? NL
        """
        name, iparams = children[:2]
        otype = None if len(children) < 3 else children[2]
        m = None if otype and len(otype) == 1 else '1'
        type_name = None if not otype else otype['type_name']
        mult_out = None if not otype else otype['mult']
        return {'method_name': name, 'flows_in': iparams, 'flow_out': type_name, 'mult_out': mult_out}
    ...
    def visit_output_type(cls, node, children):
        ...
        """
        result = {'type_name': children[0], 'mult': '1' if len(children) == 2 else None}
        return result
