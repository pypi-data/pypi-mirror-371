...
class Session:
    ...
    def __init__(
        ...
        self._query_endpoint_v2 = endpoint + "/cypherv2"
    ...
    def execute_v2(self, query: str):
        """
        Execute a query on the NeuG server using the v2 endpoint.
        This will return the query result in a format compatible with Neo4j.
        ...
        logger.info(
            f"Executing query: {query} on endpoint: {self._query_endpoint_v2} with timeout: {self.timeout}"
        )
        try:
            response = self._http_session.post(
                self._query_endpoint_v2, data=query, timeout=self.timeout
            )
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to execute query: {query}. Error: {e}")
            raise ConnectionError(
