...
DEFAULT_MODEL = os.getenv("AKSCMD_GEMINI_MODEL", "gemini-1.5-flash-latest")
API_KEY = os.getenv("AKSCMD_GEMINI_API_KEY") or os.getenv("GEMINI_API_KEY")
...
def query_gemini(prompt: str, temperature: float = 0.0) -> str:
    if not API_KEY:
        print("[akscmd] API key not found. Set AKSCMD_GEMINI_API_KEY or GEMINI_API_KEY environment variable.")
        sys.exit(1)
    payload = {
        "contents": [
    ...
    try:
        ...
        _fatal(f"Gemini request failed: {e}")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in known_subcommands:
        ...
        parser = argparse.ArgumentParser(
            prog="akscmd",
            description="Natural language → shell command via Gemini",
        )
