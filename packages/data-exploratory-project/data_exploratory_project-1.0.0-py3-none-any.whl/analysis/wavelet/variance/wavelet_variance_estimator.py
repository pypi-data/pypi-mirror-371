...
class WaveletVarianceEstimator(BaseEstimator):
    ...
    def get_confidence_interval(self, confidence: Optional[float] = None) -> Tuple[float, float]:
        ...
        residuals = np.array(self.variance_logs) - (
            ...
            np.mean(self.variance_logs) - np.mean(self.scale_logs) * (2 * self.estimated_hurst - 1)
        )
    ...
    def plot_analysis(self, figsize: Tuple[int, int] = (12, 8)) -> None:
        ...
        ax1.loglog(self.scales, list(self.wavelet_variances.values()), 'bo-', 
        ...
        ax2.scatter(self.scale_logs, self.variance_logs, color='blue', 
                   label='Data Points')
        
        # Plot regression line
        x_line = np.array([min(self.scale_logs), max(self.scale_logs)])
        y_line = (2 * self.estimated_hurst - 1) * x_line + np.mean(self.variance_logs) - \
