...
import yaml
from tagmapper.connector_api import get_api_url, get_json, get_file
...
class Schema:
    ...
    def get_yaml(self):
        ...
        """
        url = get_api_url() + f"/download-schema?filename={self.file_name}"
        return yaml.safe_load(get_file(url, "", stream=True))
    ...
    def download_schema(file_name: str):
        ...
        url = get_api_url() + f"/download-schema?filename={file_name}"
        get_file(url, file_name, stream=True)
