...
def _global_job_function(scheduler_id: str):
    ...
    try:
        # 获取待处理任务
        pending_taskIds = scheduler.task_repository.get_pending_taskIds(
        ...
        scheduler.backoff_worker.execute_batch_tasks(pending_taskIds)
...
class TaskBackoffScheduler:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"backoff_worker 核心执行器初始化: task_timeout={self.config.task.task_exec_timeout}"
            )
    ...
    def default_biz_scheduler(self):
        ...
        logger.info(f"backoff_scheduler 核心调度器初始化成功:  interval: [{self.config.scheduler.interval}s] ")
    ...
    def shutdown(self):
        ...
        try:
            ...
            self._initialized = False
            logger.debug(
                f"任务退避管理工具已关闭，启动时间戳: {self.startup_timestamp}"
            )
