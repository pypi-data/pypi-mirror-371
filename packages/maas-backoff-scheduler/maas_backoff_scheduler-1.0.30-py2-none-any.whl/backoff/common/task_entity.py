...
class TaskEntity:
    ...
    backoff_type: str = BackoffType.EXPONENTIAL.value
    backoff_interval: int = 60  # 基础间隔时间(秒)
    ...
    def from_redis_dict(cls, data: Dict[str, str]) -> "TaskEntity":
        ...
        return cls(
            ...
            service_instance_id=data.get("service_instance_id", ""),  # 添加服务实例ID
            max_retry_count=int(data.get("max_retry_count", "3")),
            backoff_type=data.get("backoff_type", BackoffType.EXPONENTIAL.value),
            backoff_interval=int(data.get("backoff_interval", "60")),
    ...
    def calculate_next_execution_time(self) -> int:
        ...
        if self.backoff_type == BackoffType.FIXED.value:
            next_time = current_time + self.backoff_interval
        elif self.backoff_type == BackoffType.LINEAR.value:
            next_time = current_time + (self.backoff_interval * (self.retry_count + 1))
        elif self.backoff_type == BackoffType.EXPONENTIAL.value:
