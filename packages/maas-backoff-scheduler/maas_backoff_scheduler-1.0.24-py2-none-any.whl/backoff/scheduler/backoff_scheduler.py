...
class TaskBackoffScheduler:
    ...
    def __init__(self, config: Optional[TaskBackoffConfig] = None):
        ...
        # 生成唯一的服务实例ID，用于任务隔离
        self.service_instance_id = self._generate_service_instance_id()
        
        # 初始化调度器
        scheduler_name = f"{self.config.task.biz_prefix}_{self.service_instance_id}_scheduler"
        ...
        logger.info(f"任务退避管理工具初始化完成，服务实例ID: {self.service_instance_id}")

    def _generate_service_instance_id(self) -> str:
        """生成唯一的服务实例ID"""
        # 使用进程ID + 随机UUID确保唯一性
        import random
        pid = os.getpid()
        random_suffix = str(random.randint(1000, 9999))
        return f"pid{pid}_{random_suffix}"
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.backoff_threadpool = BackoffThreadPool(
                ...
                thread_name_prefix=f"{self.config.task.biz_prefix}_{self.service_instance_id}_worker",
    ...
    def default_biz_scheduler(self):
        ...
        job_stores = {
            "default": RedisJobStore(
                ...
                jobs_key=f"{self.config.task.biz_prefix}_{self.service_instance_id}_jobs",
                run_times_key=f"{self.config.task.biz_prefix}_{self.service_instance_id}_run_times"
            )
        }
        ...
        scheduler.add_job(
            ...
            id=f"{self.config.task.biz_prefix}_{self.service_instance_id}_job",
            name=f"退避工具内置调度任务_{self.service_instance_id}",
            max_instances=1,
    ...
    def default_job_function(self):
        ...
        try:
            # 获取待处理任务，只处理属于当前服务实例的任务
            pending_taskIds = task_repository.get_pending_taskIds(
                batch_size=self.config.task.batch_size,
                service_instance_id=self.service_instance_id  # 添加服务实例ID过滤
            )
            if not pending_taskIds:
                logger.info(f"{self.config.task.biz_prefix}_{self.service_instance_id},没有待处理任务")
                return
    ...
    def create_task(
        ...
        try:
            ...
            task_entity = TaskEntity.create(
                ...
                service_instance_id=self.service_instance_id,  # 添加服务实例ID
            )
    ...
    def shutdown(self):
        ...
        try:
            ...
            self._initialized = False
            logger.info(f"任务退避管理工具已关闭，服务实例ID: {self.service_instance_id}")
