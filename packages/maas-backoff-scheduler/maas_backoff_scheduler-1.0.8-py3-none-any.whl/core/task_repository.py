...
class TaskRepository:
    ...
    def create_task(self, task_entity: TaskEntity) -> bool:
        """
        创建任务
        ...
        try:
            ...
            self.adapter.save_task(task_id, task_data)

            logger.info(f"任务创建成功: {task_id}")
            return True

        except Exception as e:
            logger.error(f"创建任务失败: {task_entity.task_id}, 错误: {str(e)}")
            return False
    ...
    def update_task(self, task_entity: TaskEntity) -> bool:
        ...
        try:
            ...
            logger.error(f"更新任务失败: {task_entity.task_id}, 错误: {str(e)}")
            return False
    ...
    def mark_task_processing(self, task_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"标记任务处理中失败: {task_id}, 错误: {str(e)}")
            return False
    ...
    def mark_task_completed(self, task_id: str, result: str = "") -> bool:
        ...
        try:
            ...
            logger.error(f"标记任务已完成失败: {task_id}, 错误: {str(e)}")
            return False
    ...
    def mark_task_failed(self, task_id: str, result: str = "") -> bool:
        ...
        try:
            ...
            if task.can_retry():
                ...
                logger.info(
                    f"任务重试: {task_id}, 下次执行时间: {task.next_execution_time}"
                )
            else:
                # 达到最大重试次数，标记为失败
                ...
                logger.warning(f"任务达到最大重试次数: {task_id}")

            return True

        except Exception as e:
            logger.error(f"标记任务失败: {task_id}, 错误: {str(e)}")
            return False
    ...
    def get_queue_stats(self) -> Dict[str, int]:
        ...
        Returns:
            Dict[str, int]: 各队列的任务数量
        """
        try:
            return {
                "pending": self.adapter.queue_length(self.pending_queue_key),
                "processing": self.adapter.queue_length(self.processing_queue_key),
                "completed": self.adapter.queue_length(self.completed_queue_key),
                "failed": self.adapter.queue_length(self.failed_queue_key),
            }
        except Exception as e:
