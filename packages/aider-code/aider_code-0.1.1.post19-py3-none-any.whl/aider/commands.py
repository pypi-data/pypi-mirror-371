...
from aider.editor import pipe_editor, subprocess_editor
...
class Commands:
    ...
    def __init__(
        ...
        self.help = None
        self.editor = editor
        self.i18n = i18n or I18n(args.chat_language or locale.getlocale()[0])
    ...
    def run(self, inp):
        ...
        if inp.startswith("@"):
            self.coder.event("command_add")
            return self.do_run("add", inp[1:])
    ...
    def cmd_lint(self, args="", fnames=None):
        ...
        for fname in fnames:
            ...
            if not self.io.confirm_ask(
                self.i18n.t("ask.001", fname=fname), default="y"
            ):
                continue
    ...
    def cmd_tokens(self, args):
        ...
        # read-only files
        fnames = self.coder.get_abs_rule_files()
        for fname in fnames:
            relative_fname = self.coder.get_rel_fname(fname)
            content = self.io.read_text(fname)
            if content is not None and not is_image_file(relative_fname):
                ...
                content = f"{relative_fname}\n{fence}\n" + content + "{fence}\n"
                tokens = self.coder.main_model.token_count(content)
                file_res.append((tokens, f"{relative_fname} (rule)", "/drop to remove"))
    ...
    def cmd_add(self, args):
        "Add files to the chat so aider can edit them or review them in detail(alias: @)"
        ...
        for word in filenames:
            ...
            if self.io.confirm_ask(self.i18n.t("ask.002", word=word, fname=fname)):
                try:
        ...
        # rule 처리
        self._add_rule_file(matched_file)

    def _add_rule_file(self, matched_file):
        file_ext = os.path.splitext(matched_file)[1][1:]  # .을 제외한 확장자 추출
        # file_ext와 일치하는 파일 추가로 필터링
        if file_ext:
            # 파일명을 .으로 분리하고 뒤에서 두 번째 항목이 file_ext와 일치하는지 확인
            add_files = [
                fname
                for fname in self.coder.abs_read_only_fnames
                if (
                    "rules" in fname.lower()
                    and fname.split(".")[-2].lower() == file_ext.lower()
                )
                or ("rules" not in fname.lower())
            ]

            # 중복 제거를 위해 set으로 변환 후 다시 리스트로 변환
            self.coder.abs_rule_fnames = list(
                set(self.coder.abs_rule_fnames + add_files)
            )
        ...
        for word in filenames:
            ...
            for matched_file in matched_files:
                ...
                if abs_fname in self.coder.abs_fnames:
                    ...
                    ext = os.path.splitext(abs_fname)[1][1:]
                    for rule_file in self.coder.abs_rule_fnames:
                        parts = abs_fname.split(".")
                        if parts[-2] == ext:
                            self.coder.abs_rule_fnames.remove(rule_file)
                    self._drop_rule_file(matched_file)

    def _drop_rule_file(self, matched_file):
        file_ext = os.path.splitext(matched_file)[1][1:]  # .을 제외한 확장자 추출
        if file_ext:
            # 제거할 파일 목록 생성
            files_to_remove = [
                fname
                for fname in self.coder.abs_rule_fnames
                if (
                    (
                        ".rules." in os.path.basename(fname).lower()
                        and fname.split(".")[-2].lower() == file_ext.lower()
                    )
                )
            ]
            ...
            for fname in files_to_remove:
                if fname in self.coder.abs_rule_fnames:
                    self.coder.abs_rule_fnames.remove(fname)
    ...
    def cmd_run(self, args, add_on_nonzero_exit=False):
        ...
        if add_on_nonzero_exit:
            ...
            add = self.io.confirm_ask(self.i18n.t("ask.003", k_tokens=k_tokens))
...
"""
    ...
    def _add_read_only_file(self, abs_path, original_name):
        ...
        if abs_path in self.coder.abs_read_only_fnames:
            ...
            self.coder.abs_read_only_fnames.add(abs_path)
            self.io.tool_output(f"Added {original_name} to rule files.")
    ...
    def _add_read_only_directory(self, abs_path, original_name):
        ...
        if added_files > 0:
            self.io.tool_output(
                f"Added {added_files} files from directory {original_name} to rule files."
            )
        else:
...
"""
    ...
    def cmd_auth(self, args):
        """API key를 주기적으로 변경, API key change"""
        try:
            args = int(args)
        except ValueError:
            self.io.tool_error(f"Argument '{args}' must be a number.")
            return

        cmd_auth_action(self.coder.main_model, args)

    def cmd_vi(self, filename):
        "Open an editor"
        subprocess_editor(filename, editor=self.editor)

    def cmd_rule(self, params):
        "Add/Remove a rule"
        # params가 포함된 파일 찾기
        # Check if the rule already exists in abs_rule_fnames
        rule_files = [
            fname
            for fname in self.coder.abs_rule_fnames
            if params.lower() in os.path.basename(fname).lower()
        ]

        if rule_files:
            self.io.tool_output(f"Rule already exists: {rule_files}")
            if not self.io.confirm_ask(
                self.i18n.t("ask.013", fname=rule_files), default="y"
            ):
                return
            ...
            read_only_files = [
                fname
                for fname in self.coder.abs_read_only_fnames
                if params.lower() in fname.lower()
            ]

            # Add new rule if it doesn't exist in read-only files
            # If the rule already exists in read-only files, add it to the list of added rules
            if read_only_files:
                # 중복 제거를 위해 set으로 변환 후 다시 리스트로 변환
                existing_files = set(self.coder.abs_rule_fnames)
                existing_files.update(read_only_files)
                self.coder.abs_rule_fnames = list(existing_files)
                for fname in read_only_files:
                    self.io.tool_output(f"Added new rule: {fname}")

    def cmd_mcp_server(self, args):
        """List all configured MCP servers"""
        servers = list_mcp_servers()
        server_name = args.strip()
        if server_name:
            # 대소문자 구분 없이 서버 이름 검색 (문자열 포함 여부로 검색)
            server_name_lower = server_name.lower()
            servers = [
                server for server in servers if server_name_lower in server.name.lower()
            ]
        ...
        if servers:
            ...
            console = Console()
            table = Table(
                title="Configured MCP(Model Context Protocol) Servers",
                box=box.ROUNDED,
                show_header=True,
            ...
            for server in servers:
                ...
                env_vars = (
                    "\n".join(
                        [
                            f"[bold]{var}[/]={'*' * len(value) if value else '(empty)'}"
                            for var, value in (
                                server.env.items()
                ...
                # 테이블에 행 추가
                table.add_row(server.name, status, command, env_vars)
            ...
            self.io.tool_output("No MCP(Model Context Protocol) servers configured.")

    def cmd_mcp_server_tool(self, args):
        """List all available MCP tools, optionally filtered by server name"""
        from rich.console import Console
        from rich.table import Table, box

        tool_server_name = args.strip()
        tools_by_server = list_mcp_tools()

        if not tools_by_server:
            self.io.tool_output("No MCP(Model Context Protocol) tools available.")
            return
        ...
        for server_name, tools in tools_by_server.items():
            if server_name:
                # 대소문자 구분 없이 서버 이름 검색 (문자열 포함 여부로 검색)
                tool_server_name = tool_server_name.lower()
                if tool_server_name not in server_name.lower():
                    continue

            table = Table(
                title=f"[bold]Server: [cyan]{server_name}[/][/]",
                box=box.ROUNDED,
                show_header=True,
                header_style="bold magenta",
                expand=True,
            ...
            table.add_column("Permission", style="green", justify="center")
            table.add_column("Description", style="yellow")

            # 도구가 없는 경우
            if not tools:
                table.add_row("-", "-", "No tools available")
            else:
                # 도구 목록 추가
                for tool in tools:
                    description = (
            ...
            # 테이블 출력
            console.print(table)
            console.print()  # 서버 간 구분을 위한 빈 줄

    def cmd_mcp_server_tool_execute(self, args):
        """Execute an MCP tool with the given arguments"""
        # Parse the arguments
        parts = args.strip().split(maxsplit=2)
        if len(parts) < 2:
            self.io.tool_error(
                "Usage: /mcp-execute <server_name> <tool_name> [arguments_json]"
            )
            return
        ...
        # Execute the tool
        self.io.tool_output(
            f"Executing MCP tool '{tool_name}' from server '{server_name}'..."
        )
        try:
    ...
    def cmd_watcher(self, params):
        """Start a watcher process"""

        if self.coder.file_watcher:
            return
        ...
        if repo:
            ignores.append(str(Path(repo.working_tree_dir) / ".gitignore"))
        ...
        file_watcher = FileWatcher(
            ...
            analytics=self.coder.analytics,
...
def cmd_auth_action(model, args):
    global stop_event
    global auth_thread

    oauth_keys_file = Path.home() / ".devflux" / ".aider" / "oauth-keys.json"
    provider = model.info.get("litellm_provider", "").lower()
    if not provider:
        return

    def auth_worker(auth_stop_event, auth_interval_minutes):
        while not auth_stop_event.is_set():
            try:
                with open(oauth_keys_file) as json_file:
                    json_config = json.load(json_file)
                    json_env_var = f"{provider.strip().upper()}_API_KEY"
                    if provider and provider in json_config:
                        json_keys = json_config[provider].get(
                            "keys", [os.environ.get(json_env_var, "")]
                        )
                        if (
                            json_keys and json_keys[0]
                        ):  # 키가 있고, 빈 문자열이 아닌 경우에만 설정
                            os.environ[json_env_var] = (
                                random.choice(json_keys)
                                if len(json_keys) > 1
                                else json_keys[0]
                            )
                            # print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] API 키가 업데이트되었습니다.")

            except Exception as e:
                print(f"[에러] 인증 처리 중 오류 발생: {e}")

            # 다음 실행까지 대기 (초 단위로 변환)
            auth_stop_event.wait(timeout=auth_interval_minutes * 60)
    ...
    try:
        ...
        interval_minutes = int(args) if args and str(args).isdigit() else 0
        if interval_minutes <= 0:
            ...
            with open(oauth_keys_file) as f:
                ...
                if provider and provider in config:
                    keys = config[provider].get("keys", [os.environ.get(env_var, "")])
                    if keys and keys[0]:  # 키가 있고, 빈 문자열이 아닌 경우에만 설정
                        os.environ[env_var] = (
                            random.choice(keys) if len(keys) > 1 else keys[0]
                        )
                        print(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] API 키가 설정되었습니다."
                        )
        else:
            # 새로운 스레드 시작
            stop_event = Event()
            print(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {interval_minutes}분마다 API 키 업데이트를 시작합니다."
            )
            auth_thread = Thread(
                target=auth_worker, args=(stop_event, interval_minutes), daemon=True
            )
            auth_thread.start()
