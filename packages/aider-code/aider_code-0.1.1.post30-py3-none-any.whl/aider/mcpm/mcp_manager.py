...
class McpManager:
    ...
    def configure_from_args(self, args) -> None:
        ...
        conf_fnames = [Path(".aider.mcp.conf.yml"), Path(".aider.mcp.conf.yaml")]
    ...
    async def _async_server_loop(self) -> None:
        ...
        try:
            async with AsyncExitStack() as exit_stack:
                if McpManager.event_loop.is_closed() or self.is_running == False:
                    self.message_queue.task_done()
                    return  # 이미 루프 종료됨 → 작업 스킵
                # 서버 세션 초기화
                for server in self.get_enabled_servers():
                ...
                while self.is_running:
                    try:
                        ...
                        try:
                            if (
                                McpManager.event_loop.is_closed()
                                or self.is_running == False
                            ):
                                self.message_queue.task_done()
                                return  # 이미 루프 종료됨 → 작업 스킵
