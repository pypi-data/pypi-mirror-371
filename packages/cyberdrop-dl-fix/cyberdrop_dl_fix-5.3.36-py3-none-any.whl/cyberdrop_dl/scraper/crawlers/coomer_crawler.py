...
class CoomerCrawler(Crawler):
    ...
    async def profile(self, scrape_item: ScrapeItem) -> None:
        ...
        profile_url = self.api_url / service / "user" / user / 'profile'
        user_str = await self.get_user_str_from_profile_api(profile_url)
    ...
    @error_handling_wrapper
    async def get_user_str_from_profile_api(self, url: str) -> str:
        """Gets the user string from a user id"""
        async with self.request_limiter:
            name = await self.client.get_name_from_url(self.domain, url)
            return name
