...
class BaseMultimodalService(BaseService):
    """
    Base class for multimodal services that support both text and vision.
    Defines the common interface that all multimodal services must implement.
    """

    def __init__(self, api_url: str, model_name: str, api_token: str = None, temperature: float = 0.6,
                 max_tokens: int = 2500, ):
        """
        Initialize a new BaseMultimodalService.
    ...
    def chat(self, message: str, session_id: str, 
        ...
        Args:
            ...
            format: Optional JSON object defining how to format the response.
