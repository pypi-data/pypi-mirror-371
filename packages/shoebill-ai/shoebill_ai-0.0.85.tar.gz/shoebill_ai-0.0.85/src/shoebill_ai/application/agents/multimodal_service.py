...
from .base_multimodal_service import BaseMultimodalService
from ...domain.agents.interfaces.multimodal_service_interface import MultimodalServiceInterface
...
class MultimodalService(BaseMultimodalService, MultimodalServiceInterface):
    """
    Implementation of a multimodal service that can work with any multimodal model.
    ...
    def __init__(self, factory: OllamaFactory, temperature: float = 0.6,
        ...
        """
        Initialize a new MultimodalService.
        ...
        #         )
        #     else:
        #         raise ValueError(f"Unsupported multimodal model: {model_name}. Must be an Ollama or OpenAI vision model.")
    ...
    def chat(self, message: str, session_id: str, 
                    ...
                    stream: bool = False) -> Optional[Dict[str, Any]]:
        """
        Unified chat method that can handle regular chat, documents, images, and tools.
        ...
        if documents and image_path:
            # Handle both documents and image
            # Format documents for the message
            docs_text = "\n\n".join([f"Document: {doc.get('title', 'Untitled')}\n{doc.get('content', '')}" 
                                    for doc in documents])

            # Combine documents with the user's message
            combined_message = f"Context:\n{docs_text}\n\nUser question about the image and documents: {message}"

            # Encode the image to base64
            image_base64 = encode_image(image_path)

            # Create custom messages with both document context and image
            messages = [
                {"role": "system", "content": system_prompt or self.system_prompt or "You are a helpful assistant."},
                {"role": "user", "content": combined_message, "images": [image_base64]}
            ]
            ...
            # Use chat with messages and tools
            return self.chat_repository.chat(
                messages=messages,
        ...
        elif documents and current_tools:
            # Handle both documents and tools
            # Format documents for the message
            docs_text = "\n\n".join([f"Document: {doc.get('title', 'Untitled')}\n{doc.get('content', '')}" 
                                    for doc in documents])

            # Combine documents with the user's message
            combined_message = f"Context:\n{docs_text}\n\nUser question about the documents: {message}"
        ...
        elif image_path and current_tools:
            # Handle both image and tools
            ...
            return self._chat_with_image(message, image_path, session_id, chat_history, system_prompt, format, stream)
        elif current_tools:
