...
class Authentication(BaseModel):
    ...
    def assert_mandatory(self):
        if (
            self.user.is_authenticated is False
            or self.user.display_name == ""
            or len(self.user.display_name) == 0
            or self.user.identity == ""
            or len(self.user.identity) == 0
            or self.credentials.token is None
            or self.credentials.scopes is None
        ):
            raise ValueError(
                "Failed asserting mandatory authentication. User is not authenticated"
            )
