...
class IPAddressMiddleware:
    ...
    def __call__(self, request):
        ...
        if log:
            log.warning(
                f"{request.META.get('HTTP_X_FORWARDED_FOR')} HTTP_X_FORWARDED_FOR"
            )
...
class MediaDomainSubstitutionJSONMiddleware:
    ...
    def __init__(self, get_response):
        ...
        self.log = (
            logging.getLogger(ADJANGO_BASE_LOGGER) if ADJANGO_BASE_LOGGER else None
        )
    ...
    def __call__(self, request):
        ...
        if not self.media_domain:
            raise ValueError(
                'settings.MEDIA_SUBSTITUTION_URL = "https://XXX.com" not specified'
            )
        if "application/json" in content_type and response.content:
            try:
                ...
                if self.log:
                    self.log.exception(
                        "Error in MediaDomainSubstitutionJSONMiddleware: %s", e
                    )
        return response
