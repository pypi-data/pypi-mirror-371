...
from .myfuncs import generate_report, generate_report2, import_file, count_pupils, dialog_stats, count_teacher_impulses, llm_analysis_groq, llm_analysis_openai
            ...
            ui.card(
                ...
                ui.layout_columns(
                    ...
                    ui.card(
                        ...
                        ui.row(
                            ui.markdown(
                            '''**Anmerkung:** EI = Invitation to expand ideas; IR = Invitation for reasoning; CH = Challenge; 
...
#            generate_report2(tmp_file.name, input.name_group(), input.num_pupils(), participation_rate.get(), {"num": 1, "words": 2, "mean_sd": 3}, {"num": 4, "words": 5, "mean_sd": 6}, sim_plot.get(), teacher_impulses_count.get(), qual_plot.get(), qual_stats_df.get())
            generate_report2(tmp_file.name, input.name_group(), input.num_pupils(), num_participants.get(), participation_rate.get(), {"num": t_turns.get(), "words": t_turns_length.get(), "mean_sd": t_turns_length_mean_sd.get()}, {"num": p_turns.get(), "words": p_turns_length.get(), "mean_sd": p_turns_length_mean_sd.get()}, sim_plot.get(), teacher_impulses_count.get(), qual_plot.get(), qual_stats_df.get())

        else:
            generate_report(tmp_file.name, input.name_group(), input.num_pupils(), participation_rate.get(), sim_plot.get(), teacher_impulses_count.get(), placeholder_plot.get())
...
### KAnn das weg?
    @render.data_frame
    def sim_stats_pupils():
        req(input.button_analysis())
        return render.DataGrid(stats.get())
