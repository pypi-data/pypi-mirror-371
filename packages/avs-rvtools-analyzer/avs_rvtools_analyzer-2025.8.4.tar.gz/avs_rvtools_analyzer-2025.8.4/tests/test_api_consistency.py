...
class TestAPIDataStructureConsistency:
    ...
    def test_all_risk_functions_return_consistent_structure(self, comprehensive_excel_data):
        ...
        for func in risk_functions:
            ...
            assert isinstance(result['count'], int), f"{func.__name__} count should be integer"
            assert isinstance(result['data'], (list, dict)), f"{func.__name__} data should be list or dict"
            ...
            if isinstance(result['data'], list) and func.__name__ != 'detect_non_dvs_switches':
    ...
    def test_gather_all_risks_api_structure(self, comprehensive_excel_data):
        ...
        for level in expected_levels:
            ...
            assert isinstance(risk_levels[level], int), f"{level} count should be integer"
    ...
    def test_error_handling_consistency(self):
        ...
        for func in risk_functions:
            try:
                ...
                assert 'count' in result, f"{func.__name__} should return count even on error"
                assert 'data' in result, f"{func.__name__} should return data even on error"
                assert result['count'] == 0, f"{func.__name__} should return 0 count for empty data"
