...
from polaris.utils.env_utils import is_not_windows, is_windows, inside_qgis
...
def ensure_spatialite_binaries(directory: Optional[AnyPath] = None) -> None:
    if is_windows():
        ...
        return ensure_spatialite_binaries_ubuntu()


def ensure_spatialite_binaries_ubuntu() -> None:
    try:
    ...
    so_files = [Path(d) / "mod_spatialite.so" for d in dirs]
    ...
    os.environ["MOD_SPATIALITE_PATH"] = str(so_files[0])
...
def _dll_already_exists(d: Path) -> bool:
    ext = "so" if is_not_windows() else "dll"
