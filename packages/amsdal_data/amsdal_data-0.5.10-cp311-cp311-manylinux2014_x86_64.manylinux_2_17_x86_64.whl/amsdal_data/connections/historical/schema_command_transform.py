...
class SchemaCommandExecutor(_BaseSchemaCommandExecutor):
    ...
    def _fetch_and_generate_m2m_inserts(
        ...
        prev_metadata = Metadata(
            ...
            next_version=current_metadata.object_version,
        ...
        table_schemas = {
            ...
            for table_schema in self.connection.query_schema(
                filters=glue.Conditions(*_conditions),
            )
        }
        ...
        for (table_name, class_version), references in grouped_references.items():
            ...
            for _ref in references:
                ...
                _found = False
                for _, val in _data.items():
                    if not isinstance(val, dict):
                        continue
                    if 'ref' not in val:
                        continue

                    if val['ref']['class_name'] != new_schema_ref.name:
                        continue
                    ...
                    if val['ref']['object_id'] != _target_id:
                        continue

                    if val['ref']['object_version'] != current_metadata.prior_version:
                        continue

                    # Found! Update object_version to the latest one.
                    val['ref']['object_version'] = current_metadata.object_version
                    # And update class version to actual one
                    val['ref']['class_version'] = new_schema_ref.version
                    _found = True
                    break
