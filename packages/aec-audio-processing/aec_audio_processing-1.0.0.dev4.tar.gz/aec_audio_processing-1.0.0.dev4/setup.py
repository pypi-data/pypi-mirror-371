...
# Create files directory in src for package distribution
package_files_dir = Path('src/files')
package_files_dir.mkdir(exist_ok=True)
if lib_path.exists():
    dylib_dest = package_files_dir / lib_path.name
    shutil.copy2(lib_path, dylib_dest)
    print(f"Copied library to package: {dylib_dest}")
...
if platform.system() == 'Darwin':
    extra_link_args = [
        str(lib_path),
        '-Wl,-rpath,@loader_path/files',  # Look in the files directory
        '-Wl,-install_name,@rpath/files/libwebrtc-audio-processing-2.dylib'  # Use rpath for dylib name
    ]
else:
    extra_link_args = [
        str(lib_path),
        '-Wl,-rpath,$ORIGIN/files',  # For Linux
...
setup(
    ...
    packages=['aec_audio_processing', 'aec_audio_processing.files'],
    ext_modules=[
        Extension(
    ...
    package_data={
        'aec_audio_processing': ['*.py', '*.cpp', '*.h', '*.i', 'files/*']
    }
)