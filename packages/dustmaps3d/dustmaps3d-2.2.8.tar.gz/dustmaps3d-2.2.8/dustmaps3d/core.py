...
def read_map(df):
    ...
    df['distance'] = df['distance'].fillna(df['max_distance'])

    EBV = component4(df['distance'], df['b_lim'], df['bubble'], df['diffuse_dust_rho'], df['h'], 
    ...
    dust = derivative_of_component4(df['distance'], df['b_lim'], df['bubble'], df['diffuse_dust_rho'], df['h'], 
                    df['distance_1'], df['span_1'], df['Cum_EBV_1'], 
                    df['distance_2'], df['span_2'], df['Cum_EBV_2'],
                    df['distance_3'], df['span_3'], df['Cum_EBV_3'],
                    df['distance_4'], df['span_4'], df['Cum_EBV_4']) 
    ...
    distance_col = df['distance'].values

    mask = distance_col < 1
    if np.any(mask):
        sigma_finally[mask] = np.nanmin(np.array([df['sigma'].values[mask], df['sigma_0_2'].values[mask]]), axis=0)
        
    mask = (distance_col >= 1) & (distance_col < 2)
    if np.any(mask):
        sigma_finally[mask] = np.nanmin(np.array([df['sigma'].values[mask], df['sigma_0_2'].values[mask], df['sigma_1_4'].values[mask]]), axis=0)
        
    mask = (distance_col >= 2) & (distance_col < 4)
    if np.any(mask):
        primary_sigma = np.nanmin(np.array([df['sigma_1_4'].values[mask], df['sigma_2_max'].values[mask]]), axis=0)
        fallback_sigma = df['sigma'].values[mask]
        sigma_finally[mask] = np.where(np.isnan(primary_sigma), fallback_sigma, primary_sigma)
        
    mask = distance_col >= 4
    if np.any(mask):
        primary_sigma = df['sigma_2_max'].values[mask]
        fallback_sigma = df['sigma'].values[mask]
        sigma_finally[mask] = np.where(np.isnan(primary_sigma), fallback_sigma, primary_sigma)
