...
class KModel(torch.nn.Module):
    ...
    def _download_voices_folder(self):
        """Tải và giải nén folder voices từ Google Drive - PHẦN MỚI THÊM"""
        ...
        # Tải file zip
        logger.info(f"Downloading voices folder from Google Drive to {zip_path}")
        gdown.download(self.VOICES_ZIP["url"], zip_path, quiet=False)
        
        # Giải nén
        logger.info(f"Extracting voices folder to {self.voices_dir}")
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall(self.voices_dir)
        
        # Xóa file zip sau khi giải nén
        os.remove(zip_path)
        logger.info(f"Voices folder downloaded and extracted successfully to {self.voices_dir}")
