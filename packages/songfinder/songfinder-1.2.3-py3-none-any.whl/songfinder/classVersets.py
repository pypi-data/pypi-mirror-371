...
from songfinder import classPaths, gestchant
from songfinder import classSettings as settings
from songfinder import fonctions as fonc


class class_versets(tk.Frame):
    ...
    def affiche_livres(self, _event=None):
    ...
    def affiche_chap1(self, _event=None):
    ...
    def affiche_chap2(self, _event=None):
    ...
    def affiche_vers1(self, _event=None):
    ...
    def affiche_vers2(self, _event=None):
    ...
    def affiche_text(self, _event=None):
        if self.liste_vers2.curselection():
            ...
            self.vers2 = 0
        self.chap2 = max(self.chap2, self.chap1)
        ...
        if self.chap1 == self.chap2:
            for i, text in enumerate(
                ...
                self.text.insert(tk.INSERT, f"  {self.vers1 + i + 1!s}  {text.text}")
        else:
            self.text.insert(tk.INSERT, f"  Chapitre {self.chap1 + 1!s}\n")
            for i, text in enumerate(self.bible[self.livre][self.chap1][self.vers1 :]):
                self.text.insert(tk.INSERT, f"  {self.vers1 + i + 1!s}  {text.text}")
            self.text.insert(tk.INSERT, f"\n  Chapitre {self.chap2 + 1!s}\n")
            for i, text in enumerate(
                self.bible[self.livre][self.chap2][: self.vers2 + 1],
            ):
                self.text.insert(tk.INSERT, f"  {i + 1!s}  {text.text}")
