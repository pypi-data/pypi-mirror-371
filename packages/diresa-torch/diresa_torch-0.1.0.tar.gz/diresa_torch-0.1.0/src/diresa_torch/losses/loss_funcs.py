...
class LatentCovLoss(nn.Module):
    ...
    """
    # TODO: Simmulated Annealing. Not present by default in pytorch. Needs custom optimizer
    # How to measure independence of latent components?

    def __init__(self):
        super().__init__()
        self.cov_weight = 0.001

    def forward(self, input, _):
        """
        :param  input: is the latent space representation of the current batch
        """
        cov = torch.abs(torch.cov(input))
        ...
        # FIX: Need to put cov_weight to a small value otherwise hampers training.
