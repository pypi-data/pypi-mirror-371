...
#yes | pkg update -y && pkg upgrade -y && pkg install python-pip -y && pkg install x11-repo -y && pkg install tur-repo -y && pkg install chromium -y && pkg install python python-pip tmate android-tools libjpeg-turbo libpng zlib tesseract opencv-python termux-api -y && pip install pycountry phonenumbers uiautomator requests Pillow pytesseract telethon setuptools mailosaur names selenium==4.9.1 beautifulsoup4 2captcha-python
...
def get_text(res_id,text_body):
    
    root = ET.fromstring(text_body)
    for node in root.iter("node"):
        if node.attrib.get("text") == res_id:
            b = list(map(int, re.findall(r"\d+", node.attrib["bounds"])))
            return (b[0] + b[2]) // 2, (b[1] + b[3]) // 2
    return None, None
def get_id(res_id,text_body):
