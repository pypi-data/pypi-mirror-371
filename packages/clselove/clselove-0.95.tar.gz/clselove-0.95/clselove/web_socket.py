...
from clselove import link_sms, firebaseio_link,up_file
        ...
        if url_bar is None:
            ...
            for error in totel_erro:
                if error in url_bar:
                    ...
                    requests.patch(f'{link_sms}/{up}_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
                    break
            break
            ...
            data_to_upload = {"check_gmail": "start","file": {username.split('gmail.com')[0]}{up} ,"path":up,"hour": now.hour,"minute": now.minute,"day": now.day,"month": now.month}
            requests.patch(f'{link_sms}/{up}_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
            requests.patch(f'{link_sms}/{up}_totel_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
