...
def host_proxy_checker():
    ...
    def generate_ip_range(base_ip, cidr_block):
        try:
            network = ipaddress.ip_network(f"{base_ip}/{cidr_block}", strict=False)
    ...
    def get_cidr_block():
        ...
        safe_print("=" * 40)
        safe_print("Common CIDR blocks:")
        safe_print("16 - 65,536 IPs (large network)")
        safe_print("24 - 256 IPs (medium network)")
        safe_print("28 - 16 IPs (small network)")
        safe_print("32 - 1 IP (single host)")
        
        while True:
            cidr_input = input("Enter CIDR block (e.g., 16, 24, 28, 32): ").strip()
            try:
                cidr = int(cidr_input)
                if 0 <= cidr <= 32:
                    return cidr
                else:
                    safe_print("CIDR must be between 0 and 32")
            except ValueError:
    ...
    def test_payload_through_proxy(target_domain, payload, proxy_host, proxy_port):
        try:
            # Add http:// to target domain if not already present
            ...
            status_code = extract_status_code(response_str)  # Fixed the typo here
    ...
    def prepare_payloads(target_domain):
        ...
        for payload_template in PAYLOADS:
            ...
            # Clean target domain for payload replacement
            clean_target = target_domain.replace('http://', '').replace('https://', '')
            payload = payload_template.replace("[target_host]", clean_target)
    ...
    def generate_proxy_combos(isp_info, cidr_block):
        ...
        for domain, ips in isp_info['resolved_ips'].items():
            for ip in ips:
                ...
                # Mark this IP for range scanning with the specified CIDR block
                proxy_combos.append((ip, 'range', isp_info, 'range_generator', cidr_block))
    ...
    def process_ip_range_batch(target_domain, prepared_payloads, base_ip, isp_info, cidr_block, progress_queue=None):
        """Process a batch of IPs from a range with the specified CIDR block"""
        results = []
        try:
            network = ipaddress.ip_network(f"{base_ip}/{cidr_block}", strict=False)
            ...
            # Create a progress bar for this IP range
            range_desc = f"Range {base_ip}/{cidr_block}"
            with tqdm(total=total_ips*2, desc=range_desc, unit="ip", leave=False) as range_pbar:
                for ip in all_ips:
                    for port in [80, 443]:
                        ip_results = test_single_combination(
                            target_domain, prepared_payloads, str(ip), port, isp_info, 'range_ip'
                        )
                        if ip_results:
            ...
            safe_print(f"Error processing IP range {base_ip}/{cidr_block}: {e}")
        
        return results

    def process_target_batch(target_domains, proxy_combos, output_file, max_threads, batch_info):
        """Process a batch of targets with all proxy combinations"""
        batch_results = []
        successful_tests = 0
        ...
        for proxy_combo in proxy_combos:
            if len(proxy_combo) == 5 and proxy_combo[1] == 'range':  # Range generator
                base_ip, _, _, _, cidr_block = proxy_combo
                network = ipaddress.ip_network(f"{base_ip}/{cidr_block}", strict=False)
                ip_count = len(list(network.hosts()))
                range_work[base_ip] = ip_count * 2  # 2 ports per IP
                total_work += ip_count * 2
            else:
                total_work += 1  # Regular proxy test
        
        total_work *= len(target_domains) * len(PAYLOADS)
        
        # Create a progress queue for IP range progress updates
        ...
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            ...
            with tqdm(total=total_work, desc=pbar_desc, unit="test") as main_pbar:
                for target_domain in target_domains:
                    prepared_payloads = prepare_payloads(target_domain)
                    
                    for proxy_combo in proxy_combos:
                        # Handle different combo formats (regular vs range generator)
                        if len(proxy_combo) == 5 and proxy_combo[1] == 'range':  # Range generator with CIDR
                            proxy_host, proxy_port, isp_info, proxy_type, cidr_block = proxy_combo
                            
                            # Submit IP range processing
                            future = executor.submit(
                                process_ip_range_batch,
                                target_domain, prepared_payloads, proxy_host, isp_info, cidr_block, progress_queue
                            )
                            futures.append(future)
                        else:  # Regular proxy
                            proxy_host, proxy_port, isp_info, proxy_type = proxy_combo
                            
                            # Submit regular proxy test
                            future = executor.submit(
                                test_single_combination, 
                                target_domain, prepared_payloads, 
                                proxy_host, proxy_port, isp_info, proxy_type
                            )
                            futures.append(future)
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        # Also check for IP range progress updates
                        try:
        ...
        return successful_tests, len(batch_results)

    def mj4():
        safe_print("ISP Proxy Payload Testing")
        safe_print("=" * 40)
        safe_print(f"Saving only status codes: {', '.join(VALID_STATUS_CODES)}")
        
        if not check_curl_availability():
            safe_print("curl is required but could not be installed")
            return
        
        # Get CIDR block from user first
        ...
        # Generate all proxy combinations for ALL ISP domains with the specified CIDR block
        all_proxy_combos = []
        for isp_info in isp_info_list:
            proxy_combos = generate_proxy_combos(isp_info, cidr_block)
            all_proxy_combos.extend(proxy_combos)
            safe_print(f"Generated {len(proxy_combos)} combo generators for {isp_info['original_domain']}")
        
        # Estimate total tests (this is approximate due to IP ranges)
        total_tests = len(target_domains) * len(all_proxy_combos) * len(PAYLOADS)
        safe_print(f"\nEstimated tests: {total_tests} (plus IP range scanning)")
        safe_print(f"CIDR Block: /{cidr_block}")
        safe_print(f"Threads: {max_threads}")
        safe_print(f"ISP domains: {len(isp_info_list)}")
        safe_print(f"Target domains: {len(target_domains)}")
        safe_print(f"Total proxy combo generators: {len(all_proxy_combos)}")
        
        # Batch processing configuration - process targets in smaller batches
        batch_size = min(3, len(target_domains))  # Process 3 targets at a time
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Only saving status codes: {', '.join(VALID_STATUS_CODES)}\n")
            f.write(f"CIDR Block: /{cidr_block}\n")
            f.write(f"Estimated tests: {total_tests} (plus IP range scanning)\n")
        ...
        total_found_proxies = 0
        
        # Process in batches
        for batch_num in range(total_batches):
            start_idx = batch_num * batch_size
            end_idx = min((batch_num + 1) * batch_size, len(target_domains))
            batch_targets = target_domains[start_idx:end_idx]
            
            safe_print(f"\nProcessing batch {batch_num + 1}/{total_batches} ({len(batch_targets)} targets)")
            
            successful_tests, found_proxies = process_target_batch(
                batch_targets, all_proxy_combos, output_file, 
                max_threads, (batch_num + 1, total_batches)
            )
            
            total_successful_tests += successful_tests
            total_found_proxies += found_proxies
            
            safe_print(f"Batch {batch_num + 1} complete: {found_proxies} working proxies found")
            safe_print(f"Total so far: {total_found_proxies} working proxies")
        
        safe_print(f"\nScan complete! Total successful tests: {total_successful_tests}")
        safe_print(f"Total working proxies found: {total_found_proxies}")
