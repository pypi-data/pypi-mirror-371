...
def host_proxy_checker():
    ...
    def process_ip_range_batch_fast(target_domain, prepared_payloads, base_ip, isp_info, cidr_block, output_file, progress_queue=None):
        results = []
        try:
    ...
    def process_target_batch(target_domains, proxy_combos, output_file, max_threads, batch_info):
        ...
        for combo in proxy_combos:
            if len(combo) == 5:  # IP range
                base_ip, _, _, _, cidr_block = combo
                network = ipaddress.ip_network(f"{base_ip}/{cidr_block}", strict=False)
                    ...
                    for combo in proxy_combos:
                        if len(combo) == 5:  # IP range
                            future = executor.submit(
                                ...
                                target_domain, prepared_payloads, combo[0], combo[2], combo[4], output_file, progress_queue
                            )
                        else:  # Regular proxy
                            future = executor.submit(
                                test_single_combination_fast,
    ...
    def mj4():
        ...
        try:
            max_threads = min(int(input("Threads (max 20): ") or "20"), 20)
        except:
            max_threads = 20
