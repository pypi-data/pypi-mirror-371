...
class OrganizationAPI:
    ...
    def _validate_organization_data(self, organization: Dict[str, Any], record_index: int = 1) -> None:
        ...
        if organization['organize_punique_id'].strip() == '':
            raise SARMValidationError(f"第{record_index}条记录的 organize_unique_id 不能为空字符串")
    ...
    def create_batch(
        ...
        for i, org in enumerate(organizations):
            if isinstance(org, OrganizeInsert):
                org_dict = org.dict()
                self._validate_organization_data(org_dict, i + 1)
    ...
    def create(self, organization: OrganizeInsert, execute_release: bool = False) -> BatchOperationResult:
        ...
        Returns:
            批量操作结果
        """
        if not isinstance(organization, (OrganizeInsert, dict)):
            raise SARMValidationError("组织数据必须是 OrganizeInsert 对象或字典类型")
