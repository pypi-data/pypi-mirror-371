...
class Liquid(DataContainer):
    ...
    def __init__(
        ...
        "Exceptions collected during call"

        self._id = internals.c_ulonglong(id(self._medium_wrapper))
        "Identifier for logger"

        self._id_p = internals.c_voidp(internals.addressof(self._id))
        "Pointer to identifier for logger"
        internals._logger_storage[self._id.value] = (self._exceptions, self._medium_wrapper, logger)
        ...
        with _liq_lock as locked:
            if locked:
                self._status, self._nc_internal = Liquid_isValid_getInfo_errorInterface(
                    liquid_name + "", _xi_autodetect, True, internals.c_message_function, self._id_p.value
                )
        if self._status == 0:
    ...
    def __del__(self):
        if self._status != 0:
            self._medium = 0
            internals._logger_storage.pop(self._id.value)
