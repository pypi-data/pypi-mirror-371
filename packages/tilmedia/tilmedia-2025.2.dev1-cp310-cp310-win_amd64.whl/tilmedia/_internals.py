...
from ctypes import CFUNCTYPE, c_char_p, c_int, c_ulonglong, addressof, c_voidp, POINTER
...
_logger_storage: Dict[int, Tuple[list, list, logging.Logger]] = {}
"Enable mapping from message user data to python instance"
...
CPRINTFUNCTION = CFUNCTYPE(c_int, c_char_p, c_int, POINTER(c_ulonglong))
""" The following statements define a callback c-function for interaction with the python callback compatibility dll"""


def tilmedia_message_logger_function(message, level, user_data):
    key = user_data.contents.value
    interface = _logger_storage.get(key)
    if interface is None:
        _logger.fatal("Output interface is not available!")
        return 0
    exceptions, medium_wrapper, logger = interface
    ...
    if exceptions is not None and _logger_exceptions["info"]:
        exceptions.append(TILMediaError(message))
        if medium_wrapper and medium_wrapper[0] != 0:
            BatchFunctionCaller_abort(medium_wrapper[0])
    return 0


c_message_function = CPRINTFUNCTION(tilmedia_message_logger_function)
