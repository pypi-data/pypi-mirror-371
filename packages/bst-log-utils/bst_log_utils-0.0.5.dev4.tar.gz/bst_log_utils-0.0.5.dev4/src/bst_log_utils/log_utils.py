...
def handle_ias(ax,fname):
    ...
    ax.plot(com['t_SPEED']/k_correct,com['SPEED'],'.-',color='xkcd:red')
...
def handle_altitude(ax,fname):
    ...
    ax.plot(tstate[state['altitude'] > 0],state['altitude'][state['altitude'] > 0],'.-')
    ax.plot(tc,hc,'.-',color='xkcd:red')
    ax.plot(tgps,gps_alt,color='xkcd:green')

    if gps is not None:
        gps_h = gps['altitude'][gps['pdop'] < 2]
        if len(gps_h > 0):
            ax.set_ylim([np.min(gps_h),np.max(gps_h)])
    else:
        ax.set_ylim([np.min(state['altitude']),np.max(state['altitude'])])
...
def plot_surfaces(ax,fname,surf_list=None, firstlast=-1,fname2=None,xmlname=None):
    fname2 = '/home/stachura/tmp/fw/s2_problems/bad.nc'
...
def plot_library(fname=None, ax=None, ptype=None,displayOnly=False):
    case_handlers = {
        'roll': handle_roll,
        'pitch': handle_pitch,
        'yaw': handle_yaw,
    ...
    if displayOnly:
        print('Plot Options in Library:')
        for k in case_handlers.keys():
            print('\t%s'%k)
    else:
