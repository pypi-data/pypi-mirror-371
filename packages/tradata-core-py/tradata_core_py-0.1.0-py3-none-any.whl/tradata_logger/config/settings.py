...
class HandlerConfig(BaseModel):
    ...
    type: str = Field(default="console", description="Handler type: console, file, timed_rotating, null")
    level: str = Field(default="INFO", description="Log level for this handler")
    filename: Optional[str] = Field(default=None, description="File path for file handlers")
    max_bytes: Optional[int] = Field(default=10 * 1024 * 1024, description="Max file size for rotating handlers")
    backup_count: Optional[int] = Field(default=5, description="Number of backup files")
    when: Optional[str] = Field(default="midnight", description="When to rotate for timed handlers")
    interval: Optional[int] = Field(default=1, description="Rotation interval for timed handlers")
    formatter: Optional[str] = Field(default="json", description="Formatter type: json, console")
...
class LogConfig(BaseModel):
    ...
    include_client: bool = Field(default=True, description="Include client information in logs")
    
    # Performance settings
    log_performance: bool = Field(default=True, description="Enable performance logging")
    log_cache_operations: bool = Field(default=True, description="Enable cache operation logging")
    log_client_operations: bool = Field(default=True, description="Enable client operation logging")
    ...
    def from_env(cls) -> "LogConfig":
        ...
        return cls(
            ...
            log_client_operations=os.getenv("LOG_CLIENT_OPERATIONS", "true").lower() == "true",
