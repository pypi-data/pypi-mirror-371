...
class VirtualEnvDetector:
    ...
    def _is_valid_venv(self, path: Path) -> bool:
        ...
        # Check for site-packages directory (additional validation)
        [
            path / "lib" / "python*" / "site-packages",  # Unix-like
        ...
        has_site_packages = (
            any((path / "lib").glob("python*/site-packages"))
            or (path / "Lib" / "site-packages").exists()
            or (path / "lib").exists()  # At least lib directory should exist
            or (path / "Lib").exists()  # Windows variant
        )
