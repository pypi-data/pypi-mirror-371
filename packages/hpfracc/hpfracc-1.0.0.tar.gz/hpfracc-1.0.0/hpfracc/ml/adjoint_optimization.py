...
class AdjointFractionalDerivative(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor, None, None]:
        ...
        if method == "RL":
            grad_input = _adjoint_riemann_liouville_backward(grad_output, x, alpha)
        elif method == "Caputo":
            grad_input = _adjoint_caputo_backward(grad_output, x, alpha)
        elif method == "GL":
            grad_input = _adjoint_grunwald_letnikov_backward(grad_output, x, alpha)
        else:
...
def _adjoint_riemann_liouville_forward(x: torch.Tensor, alpha: float) -> torch.Tensor:
    """
    ...
    if alpha > 0 and alpha < 1:
        ...
        if fractional_part > 0:
            ...
            result = (1 - fractional_part) * current + fractional_part * gradient
...
def _adjoint_riemann_liouville_backward(grad_output: torch.Tensor, x: torch.Tensor, alpha: float) -> torch.Tensor:
    """
    ...
    if alpha > 0 and alpha < 1:
        ...
        if fractional_part > 0:
            ...
            result = (1 - fractional_part) * current_grad - fractional_part * gradient_grad
...
def _adjoint_caputo_backward(grad_output: torch.Tensor, x: torch.Tensor, alpha: float) -> torch.Tensor:
    """
...
def _adjoint_grunwald_letnikov_forward(x: torch.Tensor, alpha: float) -> torch.Tensor:
    """
...
def _adjoint_grunwald_letnikov_backward(grad_output: torch.Tensor, x: torch.Tensor, alpha: float) -> torch.Tensor:
    """
...
class AdjointFractionalLayer(nn.Module):
    ...
    def __init__(self, alpha: float, method: str = "RL", config: AdjointConfig = None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.use_adjoint:
            return AdjointFractionalDerivative.apply(x, self.alpha.alpha, self.method)
        else:
...
class MemoryEfficientFractionalNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor, use_fractional: bool = True, method: str = "RL") -> torch.Tensor:
        ...
        for i, layer in enumerate(self.layers[:-1]):
            print(f"      Debug: Before layer {i} ({type(layer).__name__}): {x.shape}")
            x = layer(x)
            ...
            if (self.adjoint_config.memory_efficient and 
                ...
                checkpoint_end = min(i + 1 + self.adjoint_config.checkpoint_frequency, len(self.layers) - 1)
                x = torch.utils.checkpoint.checkpoint(
                    lambda x: self._checkpoint_forward(x, i + 1, checkpoint_end),
                    x,
    ...
    def _checkpoint_forward(self, x: torch.Tensor, start_layer: int, end_layer: int) -> torch.Tensor:
