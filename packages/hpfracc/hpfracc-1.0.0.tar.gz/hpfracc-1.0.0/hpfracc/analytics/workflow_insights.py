...
class WorkflowInsights:
    ...
    def __init__(self, db_path: str = "workflow_analytics.db", enable_insights: bool = True):
    ...
    def _setup_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_session_id ON workflow_events(session_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_timestamp ON workflow_events(timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_method_name ON workflow_events(method_name)')
            
            conn.commit()
            conn.close()
            logger.info(f"Workflow insights database initialized at {self.db_path}")
            
        except Exception as e:
            logger.error(f"Failed to initialize workflow insights database: {e}")
            self.enable_insights = False
    
    def track_workflow_event(self, session_id: str, method_name: str, estimator_type: str,
                           parameters: Dict[str, Any], array_size: int, fractional_order: float,
                           execution_success: bool, execution_time: Optional[float] = None,
    ...
    def get_workflow_patterns(self, min_frequency: int = 2, max_pattern_length: int = 5) -> List[WorkflowPattern]:
        ...
        try:
            ...
            for length in range(2, max_pattern_length + 1):
                length_patterns = self._find_patterns_of_length(session_sequences, length, min_frequency)
    ...
    def _find_patterns_of_length(self, session_sequences: Dict[str, List], 
                                pattern_length: int, min_frequency: int) -> List[WorkflowPattern]:
        ...
        for session_id, sequence in session_sequences.items():
            ...
            for i in range(len(sequence) - pattern_length + 1):
                ...
                method_sequence = tuple(event['method'] for event in subsequence)
                ...
                # Track success rate
                success_count = sum(1 for event in subsequence if event['success'])
                pattern_counts[pattern_key]['successes'] += success_count
                
                # Track execution times
                exec_times = [event['exec_time'] for event in subsequence if event['exec_time']]
                pattern_counts[pattern_key]['execution_times'].extend(exec_times)
                
                # Track timestamps
                first_timestamp = min(event['timestamp'] for event in subsequence)
                last_timestamp = max(event['timestamp'] for event in subsequence)
        ...
        for pattern_key, data in pattern_counts.items():
            if data['count'] >= min_frequency:
                ...
                avg_success_rate = data['successes'] / (data['count'] * pattern_length)
                avg_execution_time = (sum(data['execution_times']) / len(data['execution_times']) 
                                   if data['execution_times'] else 0.0)
    ...
    def get_session_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if insights['event_counts']:
                insights['avg_events_per_session'] = sum(insights['event_counts']) / len(insights['event_counts'])
                insights['max_events_per_session'] = max(insights['event_counts'])
                insights['min_events_per_session'] = min(insights['event_counts'])
            
            if insights['success_rates']:
                insights['avg_success_rate'] = sum(insights['success_rates']) / len(insights['success_rates'])
            
            if insights['avg_execution_times']:
                insights['avg_execution_time'] = sum(insights['avg_execution_times']) / len(insights['avg_execution_times'])
    ...
    def get_user_behavior_clusters(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for session_id, duration in session_insights['session_durations'].items():
                ...
                session_index = None
                for i, (sid, _, _, event_count, _, _) in enumerate(session_stats):
                    if sid == session_id:
                        session_index = i
                        break
    ...
    def get_workflow_recommendations(self, current_method: str, 
                                   user_history: List[str]) -> List[Tuple[str, float]]:
    ...
    def export_workflow_data(self, output_path: str = "workflow_analytics.json"):
        ...
        try:
            ...
            for pattern in patterns:
                ...
                pattern_dict['user_sessions'] = list(pattern.user_sessions)  # Convert set to list
    ...
    def clear_old_data(self, days_to_keep: int = 30):
        ...
        try:
            ...
            cursor.execute('DELETE FROM workflow_events WHERE timestamp < ?', (cutoff_time,))
