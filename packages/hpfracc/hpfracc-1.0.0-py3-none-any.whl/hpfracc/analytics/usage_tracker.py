...
class UsageTracker:
    ...
    def __init__(self, db_path: str = "usage_analytics.db", enable_tracking: bool = True):
    ...
    def _setup_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_method_name ON usage_events(method_name)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_timestamp ON usage_events(timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_session_id ON usage_events(user_session_id)')
    ...
    def track_usage(self, method_name: str, estimator_type: str, parameters: Dict[str, Any],
                   array_size: int, fractional_order: float, execution_success: bool,
                   user_session_id: Optional[str] = None, ip_address: Optional[str] = None):
        """Track a usage event."""
        if not self.enable_tracking:
            return
    ...
    def get_usage_stats(self, time_window_hours: Optional[int] = None) -> Dict[str, UsageStats]:
    ...
    def _process_events_to_stats(self, events: List[Tuple]) -> Dict[str, UsageStats]:
        ...
        for event in events:
            ...
            method_stats[method_name]['array_sizes'].append(event[5])  # array_size
            method_stats[method_name]['fractional_orders'].append(event[6])  # fractional_order
        ...
        for method_name, data in method_stats.items():
            ...
            success_rate = data['successes'] / total_calls if total_calls > 0 else 0.0
            avg_array_size = sum(data['array_sizes']) / len(data['array_sizes']) if data['array_sizes'] else 0.0
    ...
    def get_method_trends(self, method_name: str, days: int = 7) -> List[Tuple[str, int]]:
    ...
    def clear_old_data(self, days_to_keep: int = 30):
        ...
        try:
            ...
            cursor.execute('DELETE FROM usage_events WHERE timestamp < ?', (cutoff_time,))
