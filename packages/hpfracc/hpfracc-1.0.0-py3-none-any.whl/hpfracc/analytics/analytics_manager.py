...
class AnalyticsManager:
    ...
    def __init__(self, config: AnalyticsConfig = None):
        ...
        self.usage_tracker = UsageTracker(enable_tracking=self.config.enable_usage_tracking)
        self.performance_monitor = PerformanceMonitor(enable_monitoring=self.config.enable_performance_monitoring)
        self.error_analyzer = ErrorAnalyzer(enable_analysis=self.config.enable_error_analysis)
        self.workflow_insights = WorkflowInsights(enable_insights=self.config.enable_workflow_insights)
    ...
    def track_method_call(self, method_name: str, estimator_type: str, parameters: Dict[str, Any],
                         array_size: int, fractional_order: float, execution_success: bool,
    ...
    def get_comprehensive_analytics(self, time_window_hours: Optional[int] = None) -> Dict[str, Any]:
            ...
            if self.config.enable_performance_monitoring:
                ...
                for method_name in list(perf_stats.keys())[:5]:
                    analytics['performance']['performance_trends'][method_name] = \
                        self.performance_monitor.get_performance_trends(method_name)
    ...
    def generate_analytics_report(self, time_window_hours: Optional[int] = None) -> str:
    ...
    def _create_analytics_plots(self, analytics: Dict[str, Any]):
        ...
        try:
            ...
            fig.suptitle('HPFRACC Analytics Dashboard', fontsize=16, fontweight='bold')
            
            # 1. Usage patterns
            if 'usage' in analytics and analytics['usage']['stats']:
                ax1 = axes[0, 0]
                methods = list(analytics['usage']['stats'].keys())
                call_counts = [stats.total_calls for stats in analytics['usage']['stats'].values()]
                
                bars = ax1.bar(methods, call_counts, color='skyblue', alpha=0.7)
            ...
            if 'performance' in analytics and analytics['performance']['stats']:
                ...
                exec_times = [stats.avg_execution_time for stats in analytics['performance']['stats'].values()]
                
                bars = ax2.bar(methods, exec_times, color='lightcoral', alpha=0.7)
            ...
            if 'errors' in analytics and analytics['errors']['stats']:
                ...
                error_rates = [stats.error_rate for stats in analytics['errors']['stats'].values()]
            ...
            if 'errors' in analytics and analytics['errors']['stats']:
                ax4 = axes[1, 1]
                methods = list(analytics['errors']['stats'].keys())
                reliability = [stats.reliability_score for stats in analytics['errors']['stats'].values()]
                
                bars = ax4.bar(methods, reliability, color='lightgreen', alpha=0.7)
    ...
    def cleanup_old_data(self):
        ...
        try:
            ...
            # Cleanup usage data
            if self.config.enable_usage_tracking:
                deleted_count = self.usage_tracker.clear_old_data(self.config.data_retention_days)
                cleanup_results['usage'] = deleted_count
            
            # Cleanup performance data
            if self.config.enable_performance_monitoring:
                deleted_count = self.performance_monitor.clear_old_data(self.config.data_retention_days)
                cleanup_results['performance'] = deleted_count
            
            # Cleanup error data
            if self.config.enable_error_analysis:
                deleted_count = self.error_analyzer.clear_old_data(self.config.data_retention_days)
                cleanup_results['errors'] = deleted_count
            
            # Cleanup workflow data
            if self.config.enable_workflow_insights:
                deleted_count = self.workflow_insights.clear_old_data(self.config.data_retention_days)
