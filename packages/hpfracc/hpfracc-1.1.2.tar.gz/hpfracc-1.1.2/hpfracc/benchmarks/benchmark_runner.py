...
class BenchmarkConfig:
    ...
    def __post_init__(self):
        ...
        if self.methods is None:
            self.methods = ["RL", "GL", "Caputo", "Weyl", "Marchaud", "Hadamard"]
        if self.fractional_orders is None:
            self.fractional_orders = [0.25, 0.5, 0.75]
        if self.test_functions is None:
            self.test_functions = ["polynomial", "exponential", "trigonometric"]
...
class BenchmarkRunner:
        ...
        for array_size in array_sizes:
            ...
            for alpha in [0.5]:  # Focus on alpha = 0.5 for accuracy
                ...
                test_cases = [
                    ...
                    ("exponential", lambda t: np.exp(t), lambda t, a: np.exp(t) * t**(-a) / gamma(1-a))
                ]
    ...
    def _benchmark_method(self, method: str, test_func: Callable, 
        ...
        try:
            ...
            for _ in range(self.config.iterations):
                ...
                result = method_func(test_func, t, alpha, h)
                end_time = time.time()
            ...
            return BenchmarkResult(
                ...
                memory_usage=0.0,
                parameters={"alpha": alpha, "function": func_name, "error": str(e)}
            )
    
    def _benchmark_accuracy(self, method: str, test_func: Callable, 
                           analytical: Callable, t: np.ndarray, 
                           alpha: float, h: float, func_name: str) -> BenchmarkResult:
        ...
        try:
            ...
            relative_error = np.mean(np.abs((numerical_result - analytical_result) / analytical_result))
            ...
            return BenchmarkResult(
                ...
                parameters={"alpha": alpha, "function": func_name, "test_type": "accuracy"}
            )
    ...
    def _benchmark_scaling(self, method: str, test_func: Callable,
                          t: np.ndarray, alpha: float, h: float) -> BenchmarkResult:
        """Benchmark scaling performance."""
        return self._benchmark_method(method, test_func, t, alpha, h, "scaling")
    
    def _benchmark_memory(self, method: str, test_func: Callable,
                         t: np.ndarray, alpha: float, h: float) -> BenchmarkResult:
    ...
    def _results_to_dict(self, results: Dict[str, List[BenchmarkResult]]) -> Dict:
    ...
    def _save_results_csv(self, results: Dict[str, List[BenchmarkResult]], csv_file: Path):
    ...
    def _plot_performance_results(self, results: List[BenchmarkResult]):
        ...
        for i, method in enumerate(methods):
            ...
            if sizes:
                ax.plot(sizes, times, 'o-', label=method, color=colors[i], linewidth=2, markersize=8)
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'performance_benchmark.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_accuracy_results(self, results: List[BenchmarkResult]):
        ...
        bars = ax.bar(methods, accuracies, color=plt.cm.viridis(np.linspace(0, 1, len(methods))))
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'accuracy_benchmark.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_memory_results(self, results: List[BenchmarkResult]):
        ...
        for i, method in enumerate(methods):
            ...
            if sizes:
                ax.plot(sizes, memory, 'o-', label=method, color=colors[i], linewidth=2, markersize=8)
    ...
    def _generate_summary_stats(self, results: Dict[str, List[BenchmarkResult]]):
        """Generate summary statistics."""
        ...
        with open(summary_file, 'w') as f:
            ...
            for benchmark_type, result_list in results.items():
                ...
                for method, method_results in method_stats.items():
                    ...
                    if times:
                        f.write(f"  Execution Time: {np.mean(times):.6f}s ± {np.std(times):.6f}s\n")
                    
                    # Memory stats
                    memory = [r.memory_usage for r in method_results if r.memory_usage > 0]
                    if memory:
                        f.write(f"  Memory Usage: {np.mean(memory):.2f}MB ± {np.std(memory):.2f}MB\n")
                    
                    # Accuracy stats
                    accuracy = [r.accuracy for r in method_results if r.accuracy is not None and r.accuracy != float('inf')]
                    if accuracy:
                        f.write(f"  Relative Error: {np.mean(accuracy):.2e} ± {np.std(accuracy):.2e}\n")
