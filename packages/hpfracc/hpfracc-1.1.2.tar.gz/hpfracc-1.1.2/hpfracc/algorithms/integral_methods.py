...
class RiemannLiouvilleIntegral:
    ...
    def __init__(
        ...
        if alpha <= 0:
            raise ValueError("Fractional order α must be positive for integrals")
...
class CaputoIntegral:
    ...
    def __init__(
        ...
        if alpha <= 0:
            raise ValueError("Fractional order α must be positive for Caputo integrals")
...
def optimized_riemann_liouville_integral(
    ...
    Returns:
        Integral values
    """
    calculator = RiemannLiouvilleIntegral(alpha, method="auto", optimize_memory=True)
    return calculator.compute(f, t, h)
