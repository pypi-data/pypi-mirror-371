...
class BaseFractionalGNNLayer(ABC):
    ...
    def _torch_fractional_derivative(self, x: Any, alpha: float) -> Any:
        ...
        if alpha == 0:
            ...
            if x.dim() > 1:
                # For multi-dimensional tensors, compute gradient along the last dimension
                ...
                if gradients.shape != x.shape:
                    ...
                    if gradients.shape[-1] < x.shape[-1]:
                        ...
                        gradients = torch.cat([gradients, torch.zeros_like(gradients[..., :padding])], dim=-1)
                    else:
    ...
    def _jax_fractional_derivative(self, x: Any, alpha: float) -> Any:
        ...
        if alpha == 0:
            ...
            if x.ndim > 1:
                # For multi-dimensional tensors, compute diff along the last dimension
    ...
    def _torch_forward(self, x: Any, edge_index: Any, edge_weight: Optional[Any] = None) -> Any:
        ...
        if edge_index is not None and edge_index.shape[1] > 0:
            ...
            if edge_weight is not None:
                ...
                out = torch.scatter_add(out, 0, self.tensor_ops.unsqueeze(row, -1).expand(-1, out.shape[-1]), weighted_features)
            else:
                # Simple aggregation without weights
                out = torch.scatter_add(out, 0, self.tensor_ops.unsqueeze(row, -1).expand(-1, out.shape[-1]), out[col])
...
class FractionalGraphAttention(BaseFractionalGNNLayer):
    ...
    def _initialize_layer(self):
        ...
        if self.backend == BackendType.TORCH:
            ...
            key = random.PRNGKey(0)
            # Initialize weights with proper dimensions
            self.query_weight = random.normal(key, (self.in_channels, self.out_channels))
            self.key_weight = random.normal(key, (self.in_channels, self.out_channels))
            self.value_weight = random.normal(key, (self.in_channels, self.out_channels))
            self.output_weight = random.normal(key, (self.out_channels, self.out_channels))
            ...
            # Initialize weights with proper dimensions
            self.query_weight = np.random.randn(self.in_channels, self.out_channels)
            self.key_weight = np.random.randn(self.in_channels, self.out_channels)
            self.value_weight = np.random.randn(self.in_channels, self.out_channels)
            self.output_weight = np.random.randn(self.out_channels, self.out_channels)
    ...
    def forward(self, x: Any, edge_index: Any, edge_weight: Optional[Any] = None) -> Any:
        ...
        # For graph attention, we only compute attention between connected nodes
        if edge_index is not None and edge_index.shape[1] > 0:
            # Ensure edge_index has correct shape [2, num_edges]
            if edge_index.ndim == 1:
                edge_index = self.tensor_ops.reshape(edge_index, (1, -1))
            ...
            # This is a simplified implementation - in practice, you'd want more sophisticated attention
            if hasattr(query, 'gather'):
                # PyTorch-like
                query_src = self.tensor_ops.gather(query, 0, self.tensor_ops.unsqueeze(row, -1).expand(-1, query.shape[-1]))
                key_tgt = self.tensor_ops.gather(key, 0, self.tensor_ops.unsqueeze(col, -1).expand(-1, key.shape[-1]))
                value_tgt = self.tensor_ops.gather(value, 0, self.tensor_ops.unsqueeze(col, -1).expand(-1, value.shape[-1]))
            else:
                # JAX/NUMBA-like
                query_src = query[row]
                key_tgt = key[col]
            ...
            # Use element-wise multiplication and sum instead of matrix multiplication
            attention_scores = self.tensor_ops.sum(query_src * key_tgt, dim=-1, keepdims=True)
            attention_scores = attention_scores / (min_dim ** 0.5)  # Use actual dimension
    ...
    def _aggregate_attention(self, query: Any, attended_values: Any, row: Any, col: Any) -> Any:
...
class FractionalGraphPooling(BaseFractionalGNNLayer):
    ...
    def _initialize_layer(self):
        ...
        if self.backend == BackendType.TORCH:
            ...
            self.score_network = torch.randn(self.in_channels, 1, requires_grad=True)
    ...
    def forward(self, x: Any, edge_index: Any, batch: Optional[Any] = None) -> Tuple[Any, Any, Any]:
        ...
        if x.shape[-1] != self.score_network.shape[0]:
            ...
            if x.shape[-1] > self.score_network.shape[0]:
                ...
                padded_score = self.tensor_ops.cat([self.score_network, zeros], dim=0)
            else:
        ...
        num_nodes = x.shape[0]
        num_pooled = max(1, int(num_nodes * self.pooling_ratio))  # Ensure at least 1 node
        ...
        # In practice, you'd want to filter edges to only include connections between pooled nodes
