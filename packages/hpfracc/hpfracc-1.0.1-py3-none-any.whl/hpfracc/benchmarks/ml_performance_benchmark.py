...
class BenchmarkConfig:
    ...
    def __post_init__(self):
        ...
        if self.hidden_sizes is None:
            self.hidden_sizes = [[32], [64, 32], [128, 64, 32], [256, 128, 64, 32]]
...
class MLPerformanceBenchmark:
    ...
    def measure_performance(self, component: str, config: Dict[str, Any]):
        ...
        execution_time = (time.time() - start_time) / self.config.measurement_runs
                        ...
                        for method in self.config.methods:
                            ...
                            with self.measure_performance("FractionalNeuralNetwork", config) as result:
                                if result.error is None:
                                    ...
                                    print(f"   ✅ {batch_size}x{input_size} → {hidden_sizes} (α={alpha}, {method})")
                                else:
                                    print(f"   ❌ {batch_size}x{input_size} → {hidden_sizes} (α={alpha}, {method}): {result.error}")
                    ...
                    for alpha in self.config.fractional_orders:
                        for method in self.config.methods:
                            ...
                            with self.measure_performance("FractionalAttention", config) as result:
                                if result.error is None:
                                    self.results.append(result)
                                    print(f"   ✅ {seq_len}x{batch_size}x{d_model} (α={alpha}, {method})")
                                else:
                                    print(f"   ❌ {seq_len}x{batch_size}x{d_model} (α={alpha}, {method}): {result.error}")
    ...
    def benchmark_fractional_layers(self):
        ...
        layer_configs = [
            ("FractionalConv1D", {"in_channels": 3, "out_channels": 16, "kernel_size": 3}),
            ("FractionalConv2D", {"in_channels": 3, "out_channels": 16, "kernel_size": 3}),
                ...
                for alpha in self.config.fractional_orders:
                    for method in self.config.methods:
                        ...
                        with self.measure_performance("FractionalLayers", config) as result:
                            if result.error is None:
                                self.results.append(result)
                                print(f"   ✅ {layer_name} {batch_size} (α={alpha}, {method})")
                            else:
                                print(f"   ❌ {layer_name} {batch_size} (α={alpha}, {method}): {result.error}")
    ...
    def _benchmark_fractional_network_warmup(self, config: Dict[str, Any]):
        ...
        x = torch.randn(config["batch_size"], config["input_size"], device=self.device)
        _ = net(x, use_fractional=True, method=config["method"])
    
    def _benchmark_fractional_network_measurement(self, config: Dict[str, Any]):
    ...
    def _benchmark_fractional_attention_warmup(self, config: Dict[str, Any]):
        ...
        x = torch.randn(config["sequence_length"], config["batch_size"], config["d_model"], device=self.device)
        _ = attention(x, method=config["method"])
    
    def _benchmark_fractional_attention_measurement(self, config: Dict[str, Any]):
        ...
        # Calculate throughput
        throughput = config["batch_size"] * config["sequence_length"] / config.get("execution_time", 1.0)
    ...
    def _benchmark_fractional_layers_warmup(self, config: Dict[str, Any]):
        ...
        if config["layer_type"] == "FractionalConv1D":
            layer = FractionalConv1D(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(config["batch_size"], config["layer_params"]["in_channels"], 32, device=self.device)
        elif config["layer_type"] == "FractionalConv2D":
            layer = FractionalConv2D(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(config["batch_size"], config["layer_params"]["in_channels"], 16, 16, device=self.device)
        elif config["layer_type"] == "FractionalLSTM":
            layer = FractionalLSTM(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(32, config["batch_size"], config["layer_params"]["input_size"], device=self.device)
        elif config["layer_type"] == "FractionalTransformer":
            layer = FractionalTransformer(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(config["batch_size"], 32, config["layer_params"]["d_model"], device=self.device)
        elif config["layer_type"] == "FractionalPooling":
            layer = FractionalPooling(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(config["batch_size"], 16, 16, 16, device=self.device)
        elif config["layer_type"] == "FractionalBatchNorm1d":
            layer = FractionalBatchNorm1d(config=layer_config, **config["layer_params"]).to(self.device)
            x = torch.randn(config["batch_size"], config["layer_params"]["num_features"], 32, device=self.device)
    ...
    def generate_report(self, output_dir: str = "benchmark_results"):
        ...
        with open(results_file, 'w') as f:
            json.dump([result.__dict__ for result in self.results], f, indent=2, default=str)
    ...
    def _generate_summary_statistics(self, output_path: Path):
        ...
        for component, stat in stats.items():
            ...
            print(f"  Memory: {stat['avg_memory']:.2f}MB ± {stat['max_memory'] - stat['min_memory']:.2f}MB")
            print()
    ...
    def _generate_visualizations(self, output_path: Path):
        ...
        if valid_results:
            ...
            component_avg_times = {comp: np.mean(times) for comp, times in component_times.items()}
            axes[1, 1].bar(component_avg_times.keys(), component_avg_times.values())
        ...
        plt.tight_layout()
        plt.savefig(output_path / "ml_benchmark_visualizations.png", dpi=300, bbox_inches='tight')
        plt.close()
