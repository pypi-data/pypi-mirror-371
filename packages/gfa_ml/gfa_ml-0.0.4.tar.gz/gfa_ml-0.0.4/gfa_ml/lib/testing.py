...
def train_and_evaluate(
    ...
    try:
        ...
        if model_type == ModelType.LSTM:
            ...
            logging.info("LSTM model built successfully.")
            if mlflow_enable:
                mlflow.log_param("hidden_neurons", ml_model_config.get("hidden_neurons", HIDDEN_NEURONS))
            ...
            if mlflow_enable:
                ...
                mlflow.log_param("dim_feedforward", ml_model_config.get("dim_feedforward", DIM_FEEDFORWARD))
        else:
        ...
        if explainability:
            ...
            if plot_path:
                ...
                file_path = f"{plot_path}/{run_name}_feature_importance.svg"
            ...
            if plot_path:
                if not os.path.exists(plot_path):
                    os.makedirs(plot_path)
                file_path = f"{plot_path}/{run_name}_time_step_importance.svg"
                plt.savefig(file_path, bbox_inches="tight")
                if mlflow_enable:
                    mlflow.log_artifact(file_path)
                logging.info(f"Plot saved to {file_path}")
        ...
        with torch.no_grad():
            ...
            if plot:
                ...
                if plot_path:
                    if not os.path.exists(plot_path):
                        os.makedirs(plot_path)
                    file_path = f"{plot_path}/{run_name}_test_prediction.svg"
                    plt.savefig(file_path, bbox_inches="tight")
                    if mlflow_enable:
                        mlflow.log_artifact(file_path)
                    logging.info(f"Plot saved to {file_path}")
        ...
        if plot:
            ...
            if plot_path:
                if not os.path.exists(plot_path):
                    os.makedirs(plot_path)
                file_path = f"{plot_path}/{run_name}_train_validation_loss.svg"
                plt.savefig(file_path, bbox_inches="tight")
                if mlflow_enable:
                    mlflow.log_artifact(file_path)
                logging.info(f"Plot saved to {file_path}")
            plt.show()
        input_example = X_test[:1].astype(
            np.float32
        )  # Get first 1 samples for MLflow logging
