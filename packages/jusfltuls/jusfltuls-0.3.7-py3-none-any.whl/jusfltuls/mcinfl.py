    ...
    if stdscr:
        ...
        if confirm:
            if (name is None) or (dbase is None) or (other_dbase is None):
                ...
                show_message_box(stdscr, "... performing copy ...", wait_key=False)
                res = maninf.copy_measurement(name, dbase, other_dbase, silent=True)
                show_message_box(stdscr, "... copy done ...", wait_key=False)
                logging.debug( f"{res}")
...
def on_delete_measurement(name, database, stdscr, asking=True):
    ...
    if stdscr:
        ...
        if confirm:
            show_message_box("... preforming delete ...", wait_key=False)
...
@click.option('--skip_test', "-s", is_flag=True, help='Skip all testing')
def main(logfile, admin, test_only, skip_test):
