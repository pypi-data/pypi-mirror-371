...
def apply_ground_truth_prior(instance):
    ...
    instance.settings['prior'] = {
        'x': {'name': 'constant', 'args': {'value': x0}},
        'y': {'name': 'constant', 'args': {'value': y0}},
        'alpha': {'name': 'constant', 'args': {'value': alpha}},
        'beta': {'name': 'constant', 'args': {'value': beta}},
        'gamma': {'name': 'constant', 'args': {'value': gamma}},
        'delta': {'name': 'constant', 'args': {'value': delta}},
    }
...
def save_scalar_observations(obs_tables, x_obs_file, y_obs_file):
    ...
    np.savetxt(
        x_obs_file, x_tbl, fmt='%d %f', header='time value', comments=''
    )
    np.savetxt(
        y_obs_file, y_tbl, fmt='%d %f', header='time value', comments=''
    )
...
def save_datetime_observations(obs_tables, x_obs_file, y_obs_file):
    ...
    np.savetxt(
        x_obs_file, x_tbl, fmt='%s %f', header='time value', comments=''
    )
    np.savetxt(
        y_obs_file, y_tbl, fmt='%s %f', header='time value', comments=''
    )
        ...
        for ax, df in plot.subplots():
            ...
            hs.extend(
                pypfilt.plot.observations(
                    ax, past_obs, label='Past observations'
                )
            )
            hs.extend(
                pypfilt.plot.observations(
                    ax, future_obs, future=True, label='Future observations'
                )
            )
...
def plot(data_file, png=True, pdf=True):
    ...
    with h5py.File(data_file, 'r') as f:
        ...
        forecasts = pypfilt.io.load_dataset(
            time_scale, f['/tables/forecasts']
        )
    ...
    var_mask = np.logical_or(cints['name'] == 'x', cints['name'] == 'y')
