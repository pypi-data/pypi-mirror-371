...
class Result:
    ...
    :param history: The simulation history component, if the :ref:`setting
        <settings>` ``filter.results.save_history`` is ``True`` (the default
        value is ``False``).
    :type history: Optional[~pypfilt.state.History]
    ...
    history: Optional[state_mod.History]
...
def reweight_ensemble(ctx, snapshot, logs):
    ...
    do_reweight = ctx.get_setting(['filter', 'reweight', 'enabled'], True)
...
def __log_step(ctx, when, do_resample, num_eff=None):
    ...
    if num_eff is not None:
        logger.debug(
            '{} RS: {}, #px: {:7.1f}'.format(
                ctx.component['time'].to_unicode(when),
                resp[do_resample],
                num_eff,
            )
        )
...
def resample_ensemble(ctx, snapshot, threshold_fraction=None):
    ...
    do_resample = ctx.get_setting(['filter', 'resample', 'enabled'], True)
    if not do_resample:
    ...
    for p_ix, partition in enumerate(partitions):
        ...
        ctx.call_event_handlers(
            event.BeforeResample(ctx, when, p_ix, curr[ixs])
        )
        ...
        ctx.call_event_handlers(
            event.AfterResample(ctx, when, p_ix, curr[ixs])
        )
        resample_mask[ixs] = True
...
def step(ctx, snapshot, time_step, step_obs, is_fs):
    ...
    if step_obs:
        ...
        # Notify that particles weights have been updated.
        ctx.call_event_handlers(event.AfterReweight(ctx, snapshot))
    ...
    ctx.call_event_handlers(
        event.AfterTimeStep(ctx, snapshot, step_obs, resample_mask)
    )
...
def run(
    ctx, start, end, obs_tables, history=None, save_when=None, save_to=None
):
    ...
    sim_time.set_period(start, end, ctx.settings['time']['steps_per_unit'])
    ...
    for step_num, time_step, obs in steps:
        ...
        if history.reached_window_end():
            ...
            shift = (
                ctx.settings['filter']['history_window']
    ...
    # Check whether to return the history matrix.
    if not ctx.get_setting(['filter', 'results', 'save_history'], True):
        history = None

    # Return the complete simulation state.
    return Result(
        settings=ctx.settings, history=history, tables=summary.get_stats()
    )
