...
def generate_smart_package_name(source_path: Path, files_list: List[Tuple[Path, Path]], manager, console) -> str:
    """Generate an intelligent package name using LLM based on directory contents.
    ...
    Returns:
        A suggested package name in kebab-case
    """
    try:
        # Import here to avoid circular dependency
        import sys
        ...
        # Get directory name (handle "." case properly)
        if source_path == Path("."):
            dir_name = Path.cwd().name
        else:
            dir_name = source_path.name if source_path.name else Path.cwd().name
        
        # Get file list
        file_names = [str(rel_path) for _, rel_path in files_list[:30]]  # First 30 files
        
        # Get 15 most recently edited files for content sampling
        files_by_mtime = sorted(files_list, key=lambda x: x[0].stat().st_mtime, reverse=True)[:15]
        
        # Read first 10 lines of each file
        file_contents = []
        for file_path, rel_path in files_by_mtime:
            try:
                # Skip binary files and large files
                if file_path.stat().st_size > 100000:  # Skip files > 100KB for this analysis
                    continue
                    
                suffix = file_path.suffix.lower()
                # Only read text files
                text_suffixes = {'.html', '.htm', '.css', '.js', '.ts', '.jsx', '.tsx', '.py', 
                                '.java', '.go', '.rs', '.rb', '.php', '.md', '.txt', '.json',
                                '.yaml', '.yml', '.xml', '.toml', '.ini', '.cfg', '.conf',
                                '.sh', '.bash', '.zsh', '.fish', '.sql', '.r', '.R', '.vue',
                                '.svelte', '.astro', '.swift', '.kt', '.scala', '.clj'}
        ...
        context_parts = [
            ...
            f"Number of files: {len(files_list)}",
            f"\nFile list (first 30):\n" + "\n".join(file_names),
        ]
        
        if file_contents:
            context_parts.append(f"\nContent samples from recent files:{chr(10).join(file_contents[:5])}")  # Limit to 5 files
        
        context = "\n".join(context_parts)
        
        # Create prompt
        prompt = f"""Based on this project structure and content, suggest a short, descriptive package name.

{context}

Requirements:
- Use 1-4 words, kebab-case (e.g., "style-guide", "data-analysis-tool")
- Be specific to what this project appears to be
- Don't use generic names like "project" or "files"
- Don't include version numbers or dates
- Make it memorable and searchable

Return ONLY the package name, nothing else. No explanation, just the name."""
        ...
        if hasattr(result, 'content'):
            suggested_name = result.content.strip()
        else:
            suggested_name = str(result).strip()
        
        # Clean up the suggestion (remove quotes, extra spaces, etc.)
        suggested_name = suggested_name.strip('"\'`').strip()
        suggested_name = suggested_name.lower().replace(' ', '-').replace('_', '-')
        # Remove any non-alphanumeric characters except hyphens
        import re
        suggested_name = re.sub(r'[^a-z0-9-]', '', suggested_name)
        suggested_name = re.sub(r'-+', '-', suggested_name)  # Remove multiple hyphens
        suggested_name = suggested_name.strip('-')  # Remove leading/trailing hyphens
        ...
        if suggested_name and len(suggested_name) <= 50:
            ...
            return dir_name.lower().replace(' ', '-').replace('_', '-')
            
    except Exception as e:
        console.print(f"[yellow]Could not generate smart name: {e}[/yellow]")
        # Fallback to directory name or timestamp
        if source_path == Path("."):
            fallback = Path.cwd().name
        else:
            fallback = source_path.name if source_path.name else "project"
        
        # Clean it up
        import re
        fallback = fallback.lower().replace(' ', '-').replace('_', '-')
        fallback = re.sub(r'[^a-z0-9-]', '', fallback)
        
        if not fallback:
            # Last resort: use timestamp
            from datetime import datetime
            fallback = f"project-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
...
def beamup_directory(source_path: Path, max_size_mb: int, package_name: Optional[str], 
    ...
    # Read .gitignore first to know what files we'll be working with
    ...
    # Now determine package name with all the context we have
    if not package_name:
        # Try to get from package.json or pyproject.toml first
        if (source_path / "package.json").exists():
            ...
            try:
                ...
                package_name = pkg_data.get("project", {}).get("name", "")
            except:
                package_name = ""
        
        # If still no package name or it's empty, use LLM to generate one
        if not package_name or package_name == "":
            console.print("\n[dim]🤖 Analyzing project structure and content...[/dim]")
            suggested_name = generate_smart_package_name(source_path, files_to_copy, manager, console)
            
            # Interactive prompt with suggestion
            from rich.prompt import Prompt
            console.print(f"[cyan]📦 Suggested package name: {suggested_name}[/cyan]")
        ...
        # User provided a name, clean it up
        package_name = package_name.replace("/", "-").replace("@", "").replace(" ", "-")
    
    # Final validation - never allow empty names
    if not package_name or package_name == "":
        from datetime import datetime
        package_name = f"beamup-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        console.print(f"[yellow]⚠️  Using fallback name: {package_name}[/yellow]")
