...
class T(Template):
    ...
    def _use(s):                                                                    return f"local {s.n[1]} = require(\"{s.n[1]}\")"
    def _use_as(s):                                                                 return f"local {s.n[2]} = require(\"{s.n[1]}\")"
    def _use_from(s):                                                               return f"local {s.n[1]} = require(\"{s.n[2]}\").{s.n[1]}"
    ...
    def _if_elif(s):                                                                return f"{s._if()}{s.body(list((" "*(s.indent-4))+f"elseif {s.wt(cond)} then{s.body(s.wt(prog))}" for cond, prog in s.n[3]))}\nend"
    def _if_elif_else(s):                                                           return f"{s._if()}{s.body(list((" "*(s.indent-4))+f"elseif {s.wt(cond)} then{s.body(s.wt(prog))}" for cond, prog in s.n[3]))}\n{" "*(s.indent-4)}else{s.body(s.wt(s.n[4]))}\nend"
    def _func_def_untyped(s):               s.indent += 4;                          return f"function {s.n[1]}({s.jwt(s.n[2], ", ")}){s.body(s.wt(s.n[3]))}\nend"
    def _func_def_local_untyped(s):         s.indent += 4;                          return f"local function {s.n[1]}({s.jwt(s.n[2], ", ")}){s.body(s.wt(s.n[3]))}\nend"
