...
def _serialize(store: Store) -> str:
    """
    ...
    def default(o: Any):
        if isinstance(o, datetime):
            return o.strftime(_ISO)
        if is_dataclass(o):
            return asdict(o)
        raise TypeError(f"Unrealisable: {o!r}")

    return json.dumps(store, default=default, option=json.OPT_INDENT_2).decode()
...
class JsonStore:
    ...
    def save(self):
        ...
        self.path.write_text(_serialize(self._data))
        self._modified_commits.clear()
        self._modified_files.clear()
