...
class MemoService:
    ...
    def delete_last(self, commit: str, is_file_memo: bool = False) -> models.Memo:
        ...
        For commit memos: last element in the list.
        For file memos: last by created timestamp among that commit's file memos.
        """
        if not is_file_memo:
        ...
        for blob_sha, memos in (self.store.data.files or {}).items():
            for m in memos:
                if getattr(m, "commit", None) == commit:
                    pool.append((blob_sha, m))
        ...
        def _key(pair):
            m = pair[1]
            return getattr(m, "created", None)

        blob_sha, target_memo = sorted(pool, key=_key)[-1]
        if getattr(target_memo, "visibility", "private") == "shared":
            raise ValueError("Refusing to delete a shared memo.")
