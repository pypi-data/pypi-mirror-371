...
class StreamingExecutor(DanaExecutor):
    ...
    def __init__(
        ...
        if log_streamer:
            self._output_buffer = StreamingOutputBuffer(
                original_buffer=self._original_output_buffer,
...
class StreamingOutputBuffer:
    ...
    def __init__(
        self, 
        original_buffer: list, 
        log_streamer: Union[Callable[[str, str], None], Callable[[str, str], Awaitable[None]]]
    ):
        """
