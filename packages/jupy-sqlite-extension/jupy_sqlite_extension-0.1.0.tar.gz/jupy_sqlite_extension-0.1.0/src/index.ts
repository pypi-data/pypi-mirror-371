      ...
      notebook.model!.cells.changed.connect(() => {
        notebook.widgets.forEach((cell) => {
          if (cell instanceof CodeCell) {
            ...
            codeCell.model.stateChanged.connect((sender: ICellModel) => {
              const codeCellModel = sender as any;
              if (codeCellModel.executionCount !== null && codeCellModel.executionCount > 0) {
                executeSQLCell(codeCell);
              }
            });
          }
        });
