...
from fourlabs_auth.services.token_manager import TokenManagerService
from django.contrib.auth import logout
from django.urls import reverse
from django.contrib.auth import get_user_model
from fourlabs_auth.services.send_mail import GraphEmailClient
...
class RequestEmailView(View):
    ...
    def post(self, request):
        ...
        # Converte o código em lista de dígitos
        code_digits = list(str(code))

        # Envia o email pelo Microsoft Graph
        try:
            client = GraphEmailClient()
            client.send_email(
                subject="Seu código de acesso - Foursys",
                to=[email],
                template_name="emails/send_code.html",
                context={"code": code_digits, "email": email}
            )
            messages.success(request, "✅ O código foi enviado para seu email Foursys.")
        except Exception as e:
            print(f"❌ Erro ao enviar email: {e}")
            return render(request, self.template_name, {
                "error": "Não foi possível enviar o código. Tente novamente mais tarde."
            })
