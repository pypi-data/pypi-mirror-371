...
def download_chadwick(install_dir="chadwick"):
    ...
    if os.name == "nt":
        ...
        print(f"Downloading Chadwick tools to {install_dir}")
    else:
        url = "https://github.com/chadwickbureau/chadwick/releases/download/v0.10.0/chadwick-0.10.0.tar.gz"
        filename = "chadwick-0.10.0.tar.gz"
        print(f"This function currently only supports Windows.")
        return None
    with tempfile.TemporaryDirectory() as temp_dir:
...
def add_states(df):
    ...
    df["BASES"] = (
        ((df["BASE1_RUN_ID"] != "") & (df["BASE1_RUN_ID"].notna())).astype(int).astype(str)
        + ((df["BASE2_RUN_ID"] != "") & (df["BASE2_RUN_ID"].notna())).astype(int).astype(str)
        + ((df["BASE3_RUN_ID"] != "") & (df["BASE3_RUN_ID"].notna())).astype(int).astype(str)
    )
    ...
    df["IS_RUNNER1"] = ((df["RUN1_DEST_ID"] == 1) | (df["BAT_DEST_ID"] == 1)).astype(int)

    df["IS_RUNNER2"] = (
        (df["RUN1_DEST_ID"] == 2) | (df["RUN2_DEST_ID"] == 2) | (df["BAT_DEST_ID"] == 2)
    ).astype(int)
    ...
    # calculate new outs and new state
    df["NEW_OUTS"] = df["OUTS_CT"] + df["EVENT_OUTS_CT"]
    df["NEW_BASES"] = (
        df["IS_RUNNER1"].astype(str) + df["IS_RUNNER2"].astype(str) + df["IS_RUNNER3"].astype(str)
    )
