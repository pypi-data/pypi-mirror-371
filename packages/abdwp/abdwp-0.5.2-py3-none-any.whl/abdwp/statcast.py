...
def _check_file_age(file_path):
    """Check if file is older than a year and warn user if so."""
    if os.path.exists(file_path):
        mod_time = datetime.fromtimestamp(os.path.getmtime(file_path))
        one_year_ago = datetime.now() - timedelta(days=365)
        if mod_time < one_year_ago:
            warnings.warn(
                f"File to be loaded is over a year old (last modified: {mod_time.strftime('%Y-%m-%d')}). "
