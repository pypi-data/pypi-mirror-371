...
import pytest
from hamcrest import assert_that, equal_to, greater_than, is_not, empty
...
@pytest.mark.parametrize(
    "file_name,mime_type,expected_response",
    assistant_chat_test_data,
    ids=[f"{row[0]}" for row in assistant_chat_test_data],
)
def test_create_assistant_and_prompt_with_file(
    ...
    system_prompt = """Your task is to show the content of the file. You MUST NOT:
        - analyze the content
        - use any scripts
    ...
    assistant = assistant(system_prompt=system_prompt)
    ...
    response = assistant_utils.ask_assistant(
        assistant, prompt, file_url=file_url, conversation_id=conversation_id
    )
...
def test_chat_with_pydantic_output_schema(assistant_utils, assistant):
    ...
    response = assistant_utils.ask_assistant(
        assistant(), "1+1?", output_schema=OutputSchema
    )
...
def test_chat_with_json_output_schema(assistant_utils, assistant):
    response = assistant_utils.ask_assistant(
        assistant(), "1+1?", output_schema=output_schema
    )
