...
from dataset_up.utils.file import sha256_parallel,get_file_type_by_magic,sha256
from dataset_up.utils.system_utils import get_logicalcpu_count
...
class UploadingFile(object):
    def __init__(self,client: Client,abs_path: str,rel_path: str,target_path: str,dataset_id: str,version: str):
        self.client = client
        ...
        self.tqdm = None
        self.tqdm_sha256 = tqdm(total=self.size, unit="B", unit_scale=True, desc=f'计算sha256 {self.rel_path}')
        #self.sha256 :str = sha256_parallel(abs_path,self.tqdm_sha256,buf_size=1024*1024*10,num_threads=get_logicalcpu_count())
        self.sha256 :str = None if self.size > 2*1024*1024*1024 else sha256(abs_path,self.tqdm_sha256,buf_size=1024*1024*10)
