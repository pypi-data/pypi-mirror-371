...
class HTTPConnection(object):
    ...
    supported by _hyper. Most optional parameters apply to *either* HTTP/1.1 or
    HTTP/2.
    ...
    :param window_manager: (optional) The class to use to manage flow control
        ...
        <_hyper.http20.window.BaseFlowControlManager>`. If not provided,
        :class:`FlowControlManager <_hyper.http20.window.FlowControlManager>`
        will be used.
    :param enable_push: (optional) Whether the server is allowed to push
        resources to the client (see
        :meth:`get_pushes() <_hyper.HTTP20Connection.get_pushes>`).
    :param ssl_context: (optional) A class with custom certificate settings.
        If not provided then _hyper's default ``SSLContext`` is used instead.
