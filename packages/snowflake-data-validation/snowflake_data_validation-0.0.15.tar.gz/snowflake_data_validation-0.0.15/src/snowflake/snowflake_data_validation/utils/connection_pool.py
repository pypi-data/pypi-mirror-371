...
class ConnectionPoolManager:
    ...
    def _get_or_create_connection_pair(self) -> ExecutionConnectors:
        ...
        retry_count = 3
        retry_delay = self.connection_timeout / 3

        for attempt in range(retry_count):
            try:
                if attempt > 0:
                    LOGGER.info(
                        "Retry attempt %d/%d for connection from pool",
                        attempt,
                        retry_count,
                    )
                return self._pool.get(timeout=retry_delay)
            except Empty:
                if attempt < retry_count - 1:
                    LOGGER.warning(
                        "Connection attempt %d timed out after %.1f seconds, retrying...",
                        attempt + 1,
                        retry_delay,
                    )
                    continue
                ...
                LOGGER.error(
                    "Pool at max capacity (%d) and no connection pair available "
                    "after %d attempts (total %.1f seconds)",
                    self.pool_size,
                    retry_count,
                ...
                try:
                    LOGGER.warning(
                        "Creating fallback connection pair outside of pool limits"
                    )
                    # This might exceed pool size but prevents failure (it should never get to this point)
                    connection_pair = self._create_connection_pair()
                    return connection_pair
                except Exception as e:
                    LOGGER.error("Failed to create emergency connection: %s", e)
                    # If emergency creation fails, raise a more informative exception
                    raise ConnectionError(
                        f"Unable to obtain connection after {retry_count} attempts. "
                        f"Pool at max capacity ({self.pool_size}). "
                    ) from e
