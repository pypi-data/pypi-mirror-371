...
class TestScriptWriterRedshift:
    ...
    def _create_mock_context(self):
        ...
        mock_sql_generator.extract_table_column_metadata.return_value = "SELECT * FROM metadata"
    ...
    def test_extract_table_column_metadata_success(self):
        ...
        result = self.script_writer.extract_table_column_metadata(mock_config, mock_context)
    ...
    @patch('snowflake.snowflake_data_validation.redshift.script_writer.script_writer_redshift.ProgressMetadata')
    def test_extract_table_column_metadata_query_failure_with_progress_metadata(self, mock_progress_metadata):
        mock_config = self._create_mock_table_configuration()
        mock_context = self._create_mock_context()
        mock_context.output_handler.console_output_enabled = False
        
        self.mock_connector.execute_query.side_effect = Exception("Database connection failed")

        with pytest.raises(Exception, match="Database connection failed"):
            self.script_writer.extract_table_column_metadata(mock_config, mock_context)

        mock_progress_metadata.assert_called_once()
        args = mock_progress_metadata.call_args[1]
        assert args['table'] == "test_db.test_schema.test_table"
        assert args['columns'] == []
        assert args['run_id'] == "test_run_123"
        assert args['run_start_time'] == "2025-01-01T00:00:00"

    def test_extract_table_column_metadata_logs_critical_error_on_failure(self):
        mock_config = self._create_mock_table_configuration()
        mock_context = self._create_mock_context()
        
        with patch('snowflake.snowflake_data_validation.redshift.script_writer.script_writer_redshift.LOGGER') as mock_logger:
            self.mock_connector.execute_query.side_effect = Exception("Connection timeout")
