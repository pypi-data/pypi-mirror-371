...
@router.post("/compounds/check-duplicate")
def check_duplicate(smiles: str, db: Session = Depends(get_db)):
    if not smiles:
        raise HTTPException(status_code=400, detail="SMILES value is required.")

    compound_data = {"smiles": smiles}
    registrar = CompoundRegistrar(db=db, mapping=None, error_handling=enums.ErrorHandlingOptions.reject_all)
    standardized_compound = registrar._build_compound_record(compound_data)
    
    exists = "deleted_at" in standardized_compound
