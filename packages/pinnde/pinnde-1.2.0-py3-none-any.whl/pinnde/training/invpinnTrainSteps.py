...
@tf.function
def trainStep(eqns, clps, bcs, data, network, boundary, constraint, constants):
    """
    ...
    Args:
        ...
        data (tensor): Sampled data points which is provided to learn constants.
        ...
        constants (list): List of string constants which will be trained.
    ...
    with tf.GradientTape() as tape:
        ...
        with tf.GradientTape(persistent=True) as tape1:
            ...
            if len(eqns) == 1:
                u = network(clps_group)[0]
                consts = network(clps_group)[-len(constants):]
                ...
                for e in range(len(eqns)):
                    globals()[f"u{e+1}"] = network(clps_group)[e]
                    consts = network(clps_group)[-len(constants):]
                    for i in range(dim):
                        globals()[f"x{i+1}"] = tf.cast(clps_group[i], tf.float32)
        ...
        for var in range(len(constants)):
            globals()[constants[var]] = consts[var]
        ...
        # ode
            ...
            if boundary.get_flag() == "ic":
                ...
                with tf.GradientTape(persistent=True) as tape2:
                    ...
                    for e in range(len(eqns)):
                        if (len(eqns) == 1):
                            u_init_pred = network(ics_group[:dim])[0]
                        else:
        ...
        with tf.GradientTape(persistent=True) as tape3:
            ...
            for e in range(len(eqns)):
                if (len(eqns) == 1):
                    u_data_pred = network(data_group[:dim])[0]
                else:
                    u_data_pred = network(data_group[:dim])[e]

                dataLoss += tf.reduce_mean(tf.square(u_data_pred - tf.cast(data_group[dim_iter], tf.float32)))
                dim_iter += 1

        loss = CLPloss + BCloss + dataLoss
    
    grads = tape.gradient(loss, network.trainable_variables)
    return CLPloss, BCloss, dataLoss, grads

@tf.function
def trainStepTime(eqns, clps, bcs, ics, data, network, boundary, t_orders, constraint, constants):
    """
    ...
    data_group = [t_data]
    for i in range(np.shape(data)[1]):
        data_group.append([data[:,i+1:i+2]])
    ...
    with tf.GradientTape() as tape:
        ...
        with tf.GradientTape(persistent=True) as tape1:
            ...
            if (len(eqns)) == 1:
                ...
                for e in range(len(eqns)):
                    globals()[f"u{e+1}"] = network(clps_group)[e]
                    consts = network(clps_group)[-len(constants):]
                    globals()[f"u{e+1}"] = tf.cast(globals()[f"u{e+1}"], tf.float32)
        ...
        if constraint == "hard":
            ...
            with tf.GradientTape(persistent=True) as tape2:
                ...
                for e in range(len(eqns)):
                    if (len(eqns) == 1):
                        u_init_pred = network(ics_group[:dim+1])[0]
                    else:
        ...
        with tf.GradientTape(persistent=True) as tape3:
            ...
            for e in range(len(eqns)):
                if (len(eqns) == 1):
                    u_data_pred = network(data_group[:dim+1])[0]
                else:
                    u_data_pred = network(data_group[:dim+1])[e]

                dataLoss += tf.reduce_mean(tf.square(u_data_pred - tf.cast(data_group[dim_iter+1], tf.float32)))
                dim_iter += 1
        
    
        loss = CLPloss + BCloss + ICloss + dataLoss
    
    grads = tape.gradient(loss, network.trainable_variables)
    return CLPloss, BCloss, ICloss, dataLoss, grads