...
class pde_tx_solution:
    ...
    def train_model(self, epochs, RAR_adaptivity=["Frequency", "PointsToAdd"]):
        """
        ...
        """
        if type(RAR_adaptivity[0]) == int:
            RAR_adaptivity.insert(0, "RAR")
            self._adapt = RAR_adaptivity
        else:
            self._adapt = [False]

        self._epochs = epochs
        self._epoch_loss, self._iv_loss, self._bc_loss, self._pde_loss, self._model = PINNtrainSelect_tx(self._pde_points, 
        self._init_points, self._epochs, self._eqn, self._t_order, self._N_pde, self._N_iv, self._setup_boundaries, 
        self._model, self._constraint, self._extra_ders, self._adapt, self._flag)
