            ...
            if args:
                ...
                if files:
                    ...
                    for file_path in files:
                        result = file_ops.rm(file_path, recursive=recursive, force=force)
                        results.append(result)
                    
                    # Возвращаем сводный результат
                    success_count = sum(1 for r in results if r.get("success"))
                    if success_count == len(results):
                        return {"success": True, "message": f"Удалено файлов: {success_count}"}
                    else:
                        errors = [r.get("error") for r in results if not r.get("success")]
                        return {"error": f"Частично выполнено. Ошибки: {'; '.join(errors)}"}
                else:
