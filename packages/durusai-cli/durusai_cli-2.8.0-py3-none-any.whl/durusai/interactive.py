...
Создай полный код проекта:"""
                ...
                self.console.print("🤖 [dim]Анализ через 3 AI модели (Claude/GPT/Gemini)...[/dim]")
                ...
                if project_structure['has_project']:
                    self.console.print("\n🔧 [yellow]Создаю файлы локально в текущей директории...[/yellow]")
                    
                    # Создаем директории проекта
                    for dir_name in project_structure['directories']:
                        result_dir = self.file_ops.mkdir(dir_name, parents=True)
                        if result_dir.get("success"):
                            self.console.print(f"✅ Создана директория: [cyan]{current_dir}/{dir_name}[/cyan]")
                    ...
                    for file_info in project_structure['files']:
                        ...
                        # Определяем путь - если есть директория проекта, создаем в ней
                        if project_structure['directories']:
                            project_dir = project_structure['directories'][0]
                            file_path = f"{project_dir}/{filename}"
                        else:
                        ...
                        if result_file.get("success"):
                            ...
                            self.console.print(f"✅ Создан файл: [green]{full_path}[/green]")
                            created_files.append(full_path)
                    
                    if created_files:
                        self.console.print(f"\n🎉 [green]Проект создан локально в: {current_dir}[/green]")
                        self.console.print(f"📁 Создано файлов: {len(created_files)}")
