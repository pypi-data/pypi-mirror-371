...
class DurusAIClient:
    ...
    def _make_request_no_auth(self, method: str, endpoint: str, **kwargs) -> httpx.Response:
        """Выполнить HTTP запрос без авторизации (для guest endpoints)"""
        url = endpoint if endpoint.startswith("/api/v1/") else f"/cli{endpoint}"
        
        # Создаем временный клиент без заголовков авторизации
    ...
    def query_sync(self, prompt: str, model: Optional[str] = None, stream: bool = False, 
        ...
        try:
            ...
            # Если не авторизованы, пробуем guest endpoint
            try:
                response = self._make_request_no_auth("POST", "/api/v1/cli/guest/query", json=data)
                result = response.json()
                # Добавляем информацию о demo режиме
                result["demo_mode"] = True
                return result
            except Exception as guest_e:
                raise AuthenticationError("Authentication required. Use 'durusai login' to authenticate or try demo mode.")
