...
def detect_file_command(query: str) -> Optional[Dict[str, Any]]:
    """Определить, является ли запрос файловой командой - ШИРОКАЯ детекция"""
    query_lower = query.lower().strip()
    
    # УНИВЕРСАЛЬНАЯ детекция файловых операций - любое упоминание файлов/кода
    file_keywords = [
        'код', 'code', 'файл', 'file', 'создай', 'create', 'сделай', 'make',
        'напиши', 'write', 'программа', 'program', 'скрипт', 'script',
        '.py', '.js', '.html', '.css', '.json', '.md', '.txt',
        'проект', 'project', 'игра', 'game', 'приложение', 'app',
        'модуль', 'module', 'класс', 'class', 'функция', 'function'
    ]
    
    # Если есть ЛЮБОЕ упоминание файловых операций - обрабатываем локально
