...
def detect_file_command(query: str) -> Optional[Dict[str, Any]]:
    """Определить, является ли запрос файловой командой - ТОЧНАЯ детекция"""
    query_lower = query.lower().strip()
    
    # СНАЧАЛА проверяем ТОЧНЫЕ команды навигации и просмотра
    if query_lower in ['pwd', 'где я', 'текущая директория', 'current directory']:
        return {"command": "pwd", "args": []}
    
    # Команды списка файлов - ПРИОРИТЕТ над созданием
    if query_lower in ['ls', 'dir', 'список файлов', 'файлы', 'покажи файлы', 
                       'покажи файлы в директории', 'show files', 'list files']:
    ...
    # ИСКЛЮЧЕНИЯ - команды которые НЕ должны создавать файлы
    view_exclusions = [
        'покажи', 'show', 'список', 'list', 'удали', 'delete', 'remove',
        'очисти', 'clean', 'where', 'где', 'какие', 'what', 'how',
        'что ты', 'who are', 'умеешь', 'можешь', 'help me', 'explain'
    ]
    
    if any(exclusion in query_lower for exclusion in view_exclusions):
        return None  # НЕ файловая операция создания
    
    # ТОЛЬКО ПОСЛЕ исключений - широкая детекция создания файлов
    creation_keywords = [
        'создай', 'create', 'сделай', 'make', 'напиши', 'write',
        'код на', 'code for', 'программа', 'program', 'скрипт', 'script',
        'игра в', 'game', 'приложение', 'app', 'проект', 'project'
    ]
    
    if any(keyword in query_lower for keyword in creation_keywords):
        return {"command": "smart_file_operation", "args": [query]}
