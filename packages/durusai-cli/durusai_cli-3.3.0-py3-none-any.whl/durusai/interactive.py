...
Создай полный код проекта:"""
    ...
    def handle_query(self, query: str):
        ...
        try:
            ...
            # Упрощенный промпт для краткости
            contextual_prompt = f"""Директория: {current_dir}
Файлы: {project_info}

{query}

ВАЖНО: Если создаешь код - используй формат:
```python filename.py
код
```
...
Отвечай кратко."""
            ...
            # Автоматически создаем файлы из блоков кода
            project_structure = extract_project_structure(response)
            if project_structure['has_project']:
                colored_print("\n🔧 Создаю файлы из кода...", "yellow")
                
                for file_info in project_structure['files']:
                    filename = file_info['filename']
                    content = file_info['content']
                    
                    result_file = self.file_ops.write_file(filename, content)
                    if result_file.get("success"):
                        colored_print(f"✅ Создан: {filename}", "green")
                    else:
                        colored_print(f"❌ Ошибка: {result_file.get('error')}", "red")
            ...
            if commands:
                ...
                for cmd_info in commands:
                    ...
                    if self.auto_execute:
                        # Автоматическое выполнение с проверкой подтверждения
                        result = execute_bash_command(command, self.file_ops)
                        
                        if result.get("requires_confirmation"):
                            # Команда требует подтверждения (rm)
                            files_to_delete = result.get("args", [])
                            colored_print(f"⚠️ Будут удалены файлы: {', '.join(files_to_delete)}", "yellow")
                            
                            try:
                                confirm = input("Подтвердить удаление? (y/n): ")
                                if confirm.lower() in ['y', 'yes', 'да']:
                                    # Выполняем удаление
                                    for file_path in files_to_delete:
                                        rm_result = self.file_ops.rm(file_path, 
                                                                    recursive=result.get("recursive", False),
                                                                    force=result.get("force", False))
                                        if rm_result.get("success"):
                                            colored_print(f"✅ {rm_result.get('message')}", "green")
                                        else:
                                            colored_print(f"❌ {rm_result.get('error')}", "red")
                                else:
                                    colored_print("❌ Удаление отменено", "yellow")
