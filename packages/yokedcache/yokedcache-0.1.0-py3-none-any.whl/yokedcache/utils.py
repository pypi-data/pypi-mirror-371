...
def serialize_data(
    ...
    try:
        if method == SerializationMethod.JSON:
            return json.dumps(data, default=_json_serializer, ensure_ascii=False).encode('utf-8')
            ...
            try:
                import msgpack
                return msgpack.packb(data, default=_msgpack_serializer, use_bin_type=True)
            except ImportError:
