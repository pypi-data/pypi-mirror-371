...
from .models import (
    CacheEntry, CacheStats, InvalidationType, SerializationMethod,
    TableCacheConfig, InvalidationRule, FuzzySearchResult
)
from .utils import (
    generate_cache_key, serialize_data, deserialize_data, 
    extract_table_from_query, get_operation_type_from_query,
    calculate_ttl_with_jitter, sanitize_key, normalize_tags,
    get_current_timestamp
)
from .exceptions import (
    YokedCacheError, CacheConnectionError, CacheKeyError,
    CacheSerializationError, CacheInvalidationError, CacheTimeoutError
)
            ...
            async with self._get_redis() as r:
                ...
                async with r.pipeline() as pipe:
                    ...
                    if tags:
                        ...
                        await self._add_tags_to_key(pipe, sanitized_key, normalized_tags, actual_ttl)
    ...
    async def invalidate_tags(self, tags: Union[str, List[str], Set[str]]) -> int:
        ...
        try:
            async with self._get_redis() as r:
                ...
                logger.info(f"Invalidated {total_invalidated} keys for tags: {list(normalized_tags)}")
                
                return total_invalidated
                
        except Exception as e:
            logger.error(f"Error invalidating tags {list(normalized_tags)}: {e}")
            raise CacheInvalidationError(str(normalized_tags), "tags", {"error": str(e)})
    ...
    async def fuzzy_search(
        ...
        try:
            async with self._get_redis() as r:
                ...
                key_strings = [key.decode() if isinstance(key, bytes) else str(key) 
                        ...
                        try:
                            ...
                            if value is not None:
                                ...
                                result = FuzzySearchResult(
                                    ...
                                    score=score,
                                    matched_term=query,
                                    cache_entry=CacheEntry(key=matched_key, value=value, created_at=datetime.utcnow())
                                )
                                results.append(result)
                        except Exception as e:
                            logger.debug(f"Error getting fuzzy match value for {matched_key}: {e}")
