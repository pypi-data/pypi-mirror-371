...
class Detect(nn.Module):
    ...
    def forward_with_rpn_op(self, x, w, h, conf_thres=0.20, iou_thres=0.45):
        z = []  # inference output
        # num_anchors 19200=80*80*3 4800=40*40*3 1200=20*20*3 3:anchor_num
        # w, h = 1280, 704
        num = (w // 32) * (h //32) * 3
        print(f'input rpn with h={h}, w={w}')
        self.box = [DecBBox(y=self.anchors[0],num_anchors=16*num,num_coords=4,num_classes=self.nc,gridW=w//8,gridH=h//8,imgW=w,imgH=h,useClassId=1,calc_mode=5,clip_bbox=1,share_loc =1),
                DecBBox(y=self.anchors[1],num_anchors=4*num,num_coords=4,num_classes=self.nc,gridW=w//16,gridH=h//16,imgW=w,imgH=h,useClassId=1,calc_mode=5,clip_bbox=1,share_loc=1),
                DecBBox(y=self.anchors[2],num_anchors=num,num_coords=4,num_classes=self.nc,gridW=w//32,gridH=h//32,imgW=w,imgH=h,useClassId=1,calc_mode=5,clip_bbox=1,share_loc=1)]

        self.filter = Filter(300, conf_thres)
        self.sort1 = Sort(300, 0, self.nc)
        self.nms = NMS(30, iou_thres)
    ...
    def forward(self, x):
        ...
        if global_config.HISI3403 or global_config.HISI3519:
            print(f'export rpn_op............')
            return self.forward_with_rpn_op(x, global_config.w, global_config.h)
        z = []  # inference output
        output =[]
        for i in range(self.nl):
            x[i] = self.m[i](x[i])  # conv
        ...
        if global_config.HISI3559 or global_config.ATLAS:
            return output
