...
class FlowRegLive:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.reference_raw = None  # Original reference (for warping unfiltered frames if needed)
        self.reference_proc = None  # Filtered reference (main reference for OF and normalization)
    ...
    def set_reference(self, frames: Optional[np.ndarray] = None):
        ...
        if frames is not None:
            ...
            if frames.ndim == 2:
                ...
                ref_options = self.options.model_copy(update={
            ...
            if len(self.reference_buffer) == 0:
                raise ValueError("Reference buffer is empty, cannot initialize reference")
    ...
    def __call__(
        ...
        if self.frame_count % self.reference_update_interval == 0:
            ...
            self.reference_proc = (
                (1 - self.reference_update_weight) * self.reference_proc +
                self.reference_update_weight * registered_proc
            )
            
            # Also update raw reference for consistency
            self.reference_raw = (
                (1 - self.reference_update_weight) * self.reference_raw +
                self.reference_update_weight * registered
            )
