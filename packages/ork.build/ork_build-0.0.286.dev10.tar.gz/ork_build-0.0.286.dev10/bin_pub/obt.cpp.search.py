...
def display_entities(entities: List, args):
    ...
    # Simple output for now - can be enhanced with the full formatting later
    for filepath in sorted(by_file.keys()):
        ...
        for entity in sorted(file_entities, key=lambda x: x.line):
            ...
            if entity.entity_type == "function" and hasattr(entity, 'return_type'):
                # Use the formatter for functions
                formatter = CppFormatter()
                sig = formatter.format_function_signature(entity)
                print(f"{entity.line:6d} {match_type:8s} {entity.entity_type:10s} {namespace_text:20s} {sig.plain_text()}")
            else:
                # Simple entities
                print(f"{entity.line:6d} {match_type:8s} {entity.entity_type:10s} {namespace_text:20s} {entity.name}")
