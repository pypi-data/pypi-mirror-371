...
def test_list_command(runner):
    ...
    with patch("pof.cli.PofManager") as mock_manager:
        mock_instance = Mock()
...
def test_add_command_with_ports(runner):
    ...
    with patch("pof.cli.PofManager") as mock_manager:
        ...
        assert result.exit_code == 0
        mock_instance.add_manual_pof.assert_called_once_with(8080, 80, name=None)
...
def test_add_command_with_name(runner):
    ...
    with patch("pof.cli.PofManager") as mock_manager:
        ...
        assert result.exit_code == 0
        mock_instance.add_manual_pof.assert_called_once_with(
...
def test_k8s_setup_command(runner, mock_questionary, mock_kubernetes_config):
    ...
    with patch("pof.cli.KubernetesPortForward") as mock_k8s:
        mock_instance = Mock()
