...
from pof.models import PofDbEntry, Status
...
def test_pof_initialization():
    ...
    from pof.models import Pof

    pof = Pof()
    assert pof._hap is None
    assert pof._db_entry is None
...
def test_pof_requires_initialization():
    ...
    with pytest.raises(PofError, match="Pof not initialized"):
        _ = pof.hid
...
def test_pof_with_hap(mock_hap_class):
    ...
    # Test properties
    assert pof.hid == "test-hap"
    assert pof.name == "test-forward"
    assert pof.uptime == "5m 30s"
...
def test_check_initialized_decorator():
    ...
    from pof.models import PofError, check_initialized
...
def test_pof_error_inheritance():
    """Test that PofError inherits from ValueError."""
    from pof.models import PofError

    error = PofError("Test error")
