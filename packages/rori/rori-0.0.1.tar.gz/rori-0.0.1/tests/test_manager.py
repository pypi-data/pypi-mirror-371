...
def test_pof_manager_initialization():
    ...
    with patch("pof.manager.PofDb") as mock_db:
        manager = PofManager()
...
def test_show_logs_without_follow(mock_hapless):
    ...
    with patch("pof.manager.PofDb"):
        ...
        with patch("pof.manager.console") as mock_console:
            manager.show_logs("test-forward", follow=False)
            mock_console.print.assert_called()


@patch("pof.manager.get_executable_or_exit")
