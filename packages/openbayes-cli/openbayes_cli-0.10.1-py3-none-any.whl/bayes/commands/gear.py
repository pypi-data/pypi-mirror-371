...
def init(ctx: typer.Context, 
    ...
    if open:
        print("正在跳转到浏览器...")
        open_usecase.open_browser(project.get_link_value("frontend"))
...
def stop(ctx: typer.Context,
    ...
    用法：
        ...
        bayes gear stop [选项]        # 在已初始化的项目目录下使用
    ...
    if id is None:
        ...
        if gear_settings.configuration is None or not gear_settings.configuration.id or not gear_settings.configuration.jid:
            print('项目编号不存在，请指定要停止的[任务编码]或者先使用 "bayes gear run [task 或 workspace 或 hypertuning] [选项]" 完成项目创建')
            raise typer.Exit(code=1)
        else:
...
def open(ctx: typer.Context, id_or_name: Optional[str] = typer.Argument(None, help="容器名称 或 容器ID 或 任务ID")):
    ...
    project = gear_usecase.get_project_by_id_or_name(party_name, id_or_name, ["BUSINESS_CHANNEL_ML"], 1, 200)
    if project:
...
def resource(ctx: typer.Context):
    ...
    resource_usecase.list_resources_display_table("--resource", party_name, ["JOB"], ["MACHINE_LEARNING"])
