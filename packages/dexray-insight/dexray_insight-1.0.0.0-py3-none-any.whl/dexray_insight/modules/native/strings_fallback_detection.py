...
class StringsFallbackDetectionModule(BaseNativeModule):
    ...
    def analyze_binary(self, binary_info: NativeBinaryInfo, r2: Any = None) -> NativeAnalysisResult:
        ...
        try:
            ...
            self.logger.debug(f"Strings fallback found {len(final_detections)} library versions in {binary_info.file_name}")
    ...
    def _extract_strings_with_command(self, binary_info: NativeBinaryInfo) -> List[str]:
        ...
        try:
            ...
            cmd = [
                'strings',
                '-n', '4',  # Minimum string length of 4
                '-a',       # Scan entire file
    ...
    def _analyze_strings_for_libraries(self, strings: List[str], binary_info: NativeBinaryInfo) -> List[NativeLibraryDetection]:
    ...
    def can_analyze(self, binary_info: NativeBinaryInfo) -> bool:
        ...
        return (self.enabled and 
                self.strings_available and
                binary_info.file_name.endswith('.so') and 
                binary_info.file_size > 1024)  # Skip very small files