...
class CVEAssessment(BaseSecurityAssessment):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.scan_config = {
            ...
            'native_library_patterns': cve_config.get('native_library_patterns', [
        ...
        try:
            ...
            if not scannable_libraries:
                ...
                if all_libs:
                    ...
                    high_confidence = sum(1 for lib in all_libs if isinstance(lib, dict) and lib.get('confidence', 0) >= self.scan_config['min_confidence'])
                    self.logger.warning(f"CVE Scanner: {len(all_libs)} total libraries detected, {with_versions} with versions, {high_confidence} with sufficient confidence")
                else:
            ...
            if not enabled_clients:
                ...
                self.logger.error("CVE Scanner: Make sure to set 'enabled: true' for at least one source in dexray.yaml under security.cve_scanning.sources")
                return findings
            ...
            if raw_vulnerabilities:
                ...
                self.logger.info(f"After relevance filtering: {len(filtered_vulnerabilities)} relevant CVE vulnerabilities")
    ...
    def _extract_scannable_libraries(self, analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            if hasattr(native_results, 'module_results'):
                for module_name, module_results in native_results.module_results.items():
                    for result in module_results:
                        if (hasattr(result, 'additional_data') and result.additional_data and 
                            ...
                            self.logger.debug(f"CVE Scanner: Extracted {len(extracted_libs)} libraries from {module_name}:")
                            for lib in extracted_libs:
                                ...
                                self.logger.debug(f"  - {lib_name} v{lib_version} (confidence: {confidence:.2f}) at {lib_path}")
        ...
        for library in detected_native_libs:
            if isinstance(library, dict):
                ...
                if not library_name or not library_version:
                    self.logger.debug(f"CVE Scanner: Skipping library - missing name or version (name: {library_name}, version: {library_version})")
                    continue
        ...
        for library_name, library_versions in library_groups.items():
            ...
            if len(library_versions) > 1:
                ...
                for lib in library_versions:
                    ...
                    self.logger.debug(f"  - {lib['name']} v{lib['version']} (confidence: {lib['confidence']:.2f}) {marker}")
            ...
            if best_version['confidence'] >= self.scan_config.get('min_confidence', 0.7):
                ...
                self.logger.info(f"CVE Scanner: Added native library: {best_version['name']} v{best_version['version']} (confidence: {best_version['confidence']:.2f}) with {len(all_so_files)} .so files")
            else:
                self.logger.debug(f"CVE Scanner: Skipped {library_name} - low confidence: {best_version['confidence']:.2f} < {self.scan_config.get('min_confidence', 0.7)}")
        ...
        if self.scan_config.get('scan_native_only', True):
            ...
            self.logger.info(f"CVE Scanner: Java/Android libraries will be {'included' if self.scan_config.get('include_java_libraries', False) else 'excluded'}")
        else:
            self.logger.info("CVE Scanner: Native library filtering DISABLED - scanning all library types")
        
        self.logger.info(f"CVE Scanner: {len(scannable_libraries)} libraries passed confidence and filtering thresholds")
        self.logger.info(f"CVE Scanner: Confidence threshold is set to {self.scan_config['min_confidence']} (libraries need >= this confidence to be scanned)")
        
        # Debug: Show confidence distribution for native libraries
        if detected_native_libs:
            confidences = [lib.get('confidence', 0) for lib in detected_native_libs if isinstance(lib, dict) and lib.get('confidence') is not None]
            if confidences:
                min_conf = min(confidences)
                max_conf = max(confidences)
                avg_conf = sum(confidences) / len(confidences)
                above_threshold = sum(1 for c in confidences if c >= self.scan_config['min_confidence'])
                self.logger.info(f"CVE Scanner: Library confidence range: {min_conf:.2f} - {max_conf:.2f} (avg: {avg_conf:.2f})")
                self.logger.info(f"CVE Scanner: {above_threshold}/{len(confidences)} libraries meet confidence threshold")
        ...
        if ffmpeg_scannable:
            ...
            for ffmpeg_lib in ffmpeg_scannable:
                self.logger.info(f"CVE Scanner: FFmpeg scannable: {ffmpeg_lib['name']} {ffmpeg_lib['version']} (confidence: {ffmpeg_lib['confidence']:.2f})")
        
        # Show example scannable libraries
        for i, lib in enumerate(scannable_libraries[:5]):
            self.logger.debug(f"CVE Scanner: Scannable lib {i+1}: {lib['name']} {lib['version']} (confidence: {lib['confidence']:.2f}, method: {lib['detection_method']})")
        ...
        if len(scannable_libraries) > max_libs:
            ...
            self.logger.info(f"CVE Scanner: Prioritized {native_count} native libraries (including {ffmpeg_count} FFmpeg) out of {len(scannable_libraries)} selected")
    ...
    def _scan_libraries_for_cves(self, libraries: List[Dict[str, Any]]) -> List[CVEVulnerability]:
        ...
        self.logger.info(f"CVE Scanner: Starting health checks for {len(clients_to_check)} enabled clients: {list(clients_to_check.keys())}")
        ...
        self.logger.info(f"CVE Scanner: Health check results: {len(healthy_clients)}/{len(clients_to_check)} enabled clients healthy")
        ...
        self.logger.info(f"CVE Scanner: Scanning {len(libraries)} libraries with {len(healthy_clients)} clients = {total_futures} total scans")
            ...
            for library in libraries:
                for source, client in healthy_clients.items():
                    future = executor.submit(
                        self._scan_single_library_with_retry,
            ...
            try:
                for future in as_completed(future_to_library, timeout=overall_timeout):
                    ...
                    if (completed_count + failed_count) % 10 == 0:
                        ...
                        self.logger.info(f"CVE Scanner: Progress {progress:.1f}% ({completed_count + failed_count}/{total_futures}) - {completed_count} successful, {failed_count} failed")
        ...
        self.logger.info(f"CVE Scanner: Completed {completed_count}/{total_futures} scans successfully, {failed_count} failed")
    ...
    def _scan_single_library_with_retry(self, client, source: str, library_name: str, version: str) -> List[CVEVulnerability]:
        ...
        for attempt in range(self.scan_config['retry_attempts'] + 1):
            try:
                self.logger.debug(f"CVE Scanner: Scanning {library_name}:{version} via {source} (attempt {attempt + 1})")
                ...
                if attempt > 0:
                    self.logger.debug(f"CVE Scanner: {library_name}:{version} via {source} succeeded on retry {attempt}")
                ...
                if attempt < self.scan_config['retry_attempts']:
                    if is_rate_limit:
                        ...
                        self.logger.debug(f"CVE Scanner: Rate limit detected for {library_name} via {source}, retrying in {delay}s")
                        time.sleep(delay)
                    elif is_timeout:
                        ...
                        time.sleep(1)
                        self.logger.debug(f"CVE Scanner: Error scanning {library_name} via {source} (attempt {attempt + 1}): {e}")
                else:
                    self.logger.warning(f"CVE Scanner: Failed to scan {library_name}:{version} via {source} after {self.scan_config['retry_attempts'] + 1} attempts: {last_exception}")
    ...
    def _create_security_findings(self, vulnerabilities: List[CVEVulnerability], 
        ...
        if critical_vulns:
            findings.append(self._create_enhanced_severity_finding(
                ...
                library_lookup, cve_library_mapping
            ))
    ...
    def _filter_relevant_cves(self, vulnerabilities: List[CVEVulnerability], 
                             scannable_libraries: List[Dict[str, Any]]) -> List[CVEVulnerability]:
        ...
        irrelevant_keywords = [
            # Network hardware
            '3com', 'cisco', 'router', 'switch', 'gateway', 'firewall',
            'officeconnect', 'linksys', 'netgear', 'tp-link',
            # Desktop/Server software  
            'microsoft data access', 'mdac', 'vmware', 'esxi', 'vcenter',
            'windows server', 'internet information services', 'iis',
            'oracle database', 'sql server', 'postgresql server',
            # Other non-mobile platforms
            'mainframe', 'as/400', 'z/os', 'solaris', 'aix',
            'cisco ios', 'juniper junos', 'palo alto',
        ]
            ...
            if hasattr(vuln, 'source_library') and vuln.source_library:
                ...
                for detected_lib_name in library_names:
                    ...
                    if source_lib_normalized == detected_normalized:
                        library_match = True
                        self.logger.debug(f"CVE Scanner: Including CVE {vuln.cve_id} - source library match: {vuln.source_library}")
                        break
                ...
                for affected_lib in vuln.affected_libraries:
                    ...
                    for detected_lib_name in library_names:
                        ...
                        if lib_name_normalized == detected_normalized:
                            library_match = True
                            self.logger.debug(f"CVE Scanner: Including CVE {vuln.cve_id} - affected library match: {affected_lib.name}")
                            break
            ...
            if not library_match:
                for detected_lib_name in library_names:
                    if detected_lib_name in summary_lower or detected_lib_name in description_lower:
                        library_match = True
                        self.logger.debug(f"CVE Scanner: Including CVE {vuln.cve_id} - library mentioned in text: {detected_lib_name}")
                        break
    ...
    def _create_cve_library_mapping(self, vulnerabilities: List[CVEVulnerability], 
                                   library_lookup: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Create detailed CVE-to-library mapping for JSON export"""
        cve_mapping = {}
        library_cve_counts = {}
    ...
    def _create_enhanced_severity_finding(self, vulnerabilities: List[CVEVulnerability], 
                                         severity: AnalysisSeverity, title: str, description: str,
                                         library_lookup: Dict[str, Dict[str, Any]],
                                         cve_mapping: Dict[str, Any]) -> SecurityFinding:
        ...
        additional_data = {
            'cve_library_mapping': {
                lib: [vuln.cve_id for vuln in vulns] 
                for lib, vulns in library_cves.items()
            },
            ...
            'severity_counts': {
                ...
                'by_library': {
                    lib: len(vulns) for lib, vulns in library_cves.items()
                }
            }
        }
    ...
    def _is_native_library(self, library_name: str, detection_method: str, source: str) -> bool:
        ...
        if self.scan_config.get('scan_native_only', True):
            ...
            for pattern in substantial_library_patterns:
                if pattern in library_name_lower:
                    self.logger.debug(f"CVE Scanner: {library_name} included as substantial library (pattern: {pattern})")
                    return True
