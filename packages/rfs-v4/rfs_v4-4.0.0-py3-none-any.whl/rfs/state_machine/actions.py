...
class Guard:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_duration_ms": self.total_duration_ms / max(self.evaluation_count, 1)
        }
...
def context_value_equals_guard(key: str, value: Any) -> Guard:
    ...
    return guard().name(f"{key}_equals_{value}").condition(lambda ctx: ctx.get(key) == value).build()