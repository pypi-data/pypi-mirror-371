...
if PYDANTIC_AVAILABLE:
    class RFSConfig(BaseSettings):
        ...
        # Cloud Run 전용 설정 (v4 신규)
        enable_cold_start_optimization: bool = Field(
            default=True,
            description="Cloud Run 콜드 스타트 최적화 활성화"
        )
        ...
        def validate_config_consistency(self) -> 'RFSConfig':
            ...
            # 성능 모니터링 활성화 시 메트릭 간격 조정
            if self.enable_performance_monitoring and self.metrics_export_interval > 300:
                print("Warning: 성능 모니터링 활성화 시 메트릭 간격을 300초 이하로 설정하는 것을 권장합니다.")
        ...
        def export_cloud_run_config(self) -> Dict[str, Any]:
            ...
            return {
                "env_vars": {
                    ...
                    "RFS_ENABLE_COLD_START_OPTIMIZATION": str(self.enable_cold_start_optimization).lower(),
    ...
    # Pydantic 없는 경우 기본 dataclass 사용
...
class ConfigManager:
    ...
    def load_config(self, force_reload: bool = False) -> RFSConfig:
        ...
        if PYDANTIC_AVAILABLE:
            # Pydantic v2를 사용한 자동 설정 로드
            match (self.config_path, self.env_file):
                case (str() as config_path, str() as env_file) if Path(config_path).exists():
                    # JSON 파일 + 환경 변수 둘 다 사용
...
spec:
  template:
    metadata:
      annotations:
        autoscaling.knative.dev/maxScale: "{cloud_config['scaling']['max_instances']}"
    ...
    spec:
      ...
      - image: gcr.io/PROJECT_ID/rfs-service:latest
        resources:
          limits:
            cpu: "{cloud_config['resource_limits']['cpu']}"
            memory: "{cloud_config['resource_limits']['memory']}"
        env:
"""
    
    for key, value in cloud_config['env_vars'].items():
...
def validate_environment() -> tuple[bool, list[str]]:
    ...
    if config.environment == Environment.PRODUCTION:
        required_vars.extend(['REDIS_URL', 'GOOGLE_APPLICATION_CREDENTIALS'])
    
    for var in required_vars:
        if not os.environ.get(var):
            errors.append(f"Required environment variable missing: {var}")
    
    # Cloud Run 환경에서 메모리 제한 확인
    if is_cloud_run_environment():
        try:
            ...
            if memory_limit.endswith('Mi') and memory_val < 256:
                errors.append("Cloud Run memory limit should be at least 256Mi")
...
def check_pydantic_compatibility() -> Dict[str, Any]:
    ...
    return {
        ...
        "pydantic_version": getattr(__import__("pydantic", fromlist=["VERSION"]), "VERSION", "unknown") if PYDANTIC_AVAILABLE else None,
