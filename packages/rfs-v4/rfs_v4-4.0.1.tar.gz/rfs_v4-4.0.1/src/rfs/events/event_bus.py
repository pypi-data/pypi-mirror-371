...
class Event:
    ...
    # 메타 정보
    event_id: str = field(default_factory=lambda: f"event_{int(datetime.now().timestamp())}")
...
class EventBus:
    ...
    async def _handle_event(self, event: Event):
        ...
        matching_subscriptions.sort(
            key=lambda sub: event.priority.value, 
            reverse=True
        )
        ...
        return [
            {
                ...
                "last_handled": sub.last_handled.isoformat() if sub.last_handled else None
            }
...
class FunctionalEventBus:
    ...
    @staticmethod
    def with_middleware(bus: EventBus, *middlewares: Callable[[Event], Event]) -> EventBus:
        """미들웨어 추가"""
    ...
    @staticmethod
    def with_handlers(bus: EventBus, handlers: List[Callable[[Event], Any]]) -> EventBus:
        """핸들러 추가"""
...
class ReactiveEventBus:
    ...
    def publish_flux(self, events: List[Event]) -> Flux[None]:
        """Flux를 통한 다중 이벤트 발행"""
        return (
            Flux.from_iterable(events)
            .map(lambda event: self.bus.publish(event))
        )
