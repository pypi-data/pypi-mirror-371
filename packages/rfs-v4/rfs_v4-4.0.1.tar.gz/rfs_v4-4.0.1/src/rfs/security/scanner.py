...
class SecurityScanner:
    ...
    def _load_security_patterns(self):
        ...
        self.dangerous_functions = {
            ...
            'pickle.load', 'pickle.loads', 'yaml.load'
        }
        ...
        self.secret_patterns = [
            ...
            (r'-----BEGIN [A-Z ]+-----', 'embedded_certificate'),
            (r'sk-[a-zA-Z0-9]{48}', 'openai_api_key'),
            (r'ghp_[a-zA-Z0-9]{36}', 'github_token'),
            (r'xoxb-[0-9]{13}-[0-9]{13}-[a-zA-Z0-9]{24}', 'slack_token')
        ]
        
        # SQL 인젝션 패턴
        self.sqli_patterns = [
            r'["\'].*\+.*["\'].*WHERE',
            r'cursor\.execute\s*\(\s*["\'][^"\']*%[sf][^"\']*["\']',
            r'query\s*=\s*["\'][^"\']*\+[^"\']*["\']',
            r'SELECT.*FROM.*WHERE.*["\'].*\+.*["\']'
        ]
        ...
        try:
            if console:
                console.print(Panel(
                    ...
                    f"🔍 스캔 유형: {', '.join(scan_types) if scan_types else '전체 스캔'}\n"
            ...
            if not scan_types or 'code' in scan_types:
                scan_tasks.append(('코드 취약점 분석', self._scan_code_vulnerabilities))
            
            if not scan_types or 'dependencies' in scan_types:
                scan_tasks.append(('의존성 보안 검사', self._scan_dependency_vulnerabilities))
            
            if not scan_types or 'config' in scan_types:
                scan_tasks.append(('설정 보안 점검', self._scan_configuration_security))
            
            if not scan_types or 'files' in scan_types:
                scan_tasks.append(('파일 권한 검사', self._scan_file_permissions))
            
            if not scan_types or 'secrets' in scan_types:
                scan_tasks.append(('시크릿 탐지', self._scan_hardcoded_secrets))
    ...
    async def _analyze_ast_vulnerabilities(self, tree: ast.AST, file_path: Path, content: str) -> List[VulnerabilityReport]:
        ...
        for node in ast.walk(tree):
            ...
            # 하드코딩된 비밀번호/키 탐지 (문자열 리터럴)
            if isinstance(node, ast.Str) and len(node.s) > 8:
    ...
    async def _scan_dependency_vulnerabilities(self) -> List[VulnerabilityReport]:
        ...
        try:
            ...
            if requirements_file.exists():
                vulnerabilities.extend(await self._check_python_dependencies(requirements_file))
            
            # pyproject.toml 검사
            pyproject_file = self.project_path / "pyproject.toml"
            if pyproject_file.exists():
                vulnerabilities.extend(await self._check_pyproject_dependencies(pyproject_file))
    ...
    async def _check_python_dependencies(self, requirements_file: Path) -> List[VulnerabilityReport]:
        ...
        try:
            ...
            try:
                    ...
                    try:
                        ...
                        for vuln in safety_data:
                            vulnerabilities.append(VulnerabilityReport(
                                ...
                                threat_level=self._map_safety_severity(vuln.get('severity', 'medium')),
                                title=f"취약한 패키지: {vuln.get('package', 'unknown')}",
                                description=vuln.get('advisory', '알려진 취약점이 있는 패키지입니다'),
                ...
                # safety 도구가 없거나 타임아웃인 경우 수동 검사
                vulnerabilities.extend(await self._manual_dependency_check(requirements_file))
    ...
    async def _scan_configuration_security(self) -> List[VulnerabilityReport]:
        ...
        try:
            ...
            for env_file in env_files:
                if env_file.is_file():
                    vulnerabilities.extend(await self._check_env_file_security(env_file))
            ...
            for config_file in config_files:
                if config_file.exists():
                    vulnerabilities.extend(await self._check_config_file_security(config_file))
    ...
    async def _display_scan_results(self):
        ...
        for level, count in severity_stats.items():
            ...
            summary_table.add_row(
                ...
                str(count),
                f"{percentage:.1f}%",
                "🚨" if level in [ThreatLevel.CRITICAL, ThreatLevel.HIGH] else "⚠️" if level == ThreatLevel.MEDIUM else "ℹ️"
            )
        ...
        if critical_high > 0:
            console.print(Panel(
                ...
                f"즉시 수정이 필요한 항목들을 우선적으로 해결하세요.\n"
                f"상세한 리포트는 보안 팀과 공유하여 검토받으시기 바랍니다.",
                title="보안 경고",
            ...
            console.print(Panel(
                ...
                f"대부분 중간 또는 낮은 위험도이므로 계획적으로 수정하세요.\n"
                f"정기적인 보안 스캔을 통해 보안 수준을 유지하시기 바랍니다.",
                title="보안 점검 완료",
                border_style="yellow"
            ))
    ...
    async def generate_security_report(self, output_path: Optional[str] = None) -> Result[str, str]:
        ...
        try:
            ...
            report = {
                ...
                'summary': {
                    level.value: sum(1 for v in self.vulnerabilities if v.threat_level == level)
                    for level in ThreatLevel
                },
                'vulnerabilities': [
                    {
    ...
    def get_vulnerability_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'critical_files': [
                v.file_path for v in self.vulnerabilities 
