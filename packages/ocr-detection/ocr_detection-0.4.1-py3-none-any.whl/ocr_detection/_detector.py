...
class PDFAnalyzer:
    ...
    def __init__(self, pdf_path: str | Path, accuracy_mode: bool = False, metadata_only: bool = False):
        ...
        Args:
            ...
            metadata_only: Use ultra-fast metadata-only mode (no text extraction)
                          Provides 5-10x speedup but reduced accuracy (85-90%)
        """
        ...
        if metadata_only:
            # Ultra-fast mode - metadata only, no text extraction
            self.performance_mode = True
            self.text_extraction_method = None  # Skip text extraction entirely
            self.page_timeout = 5.0  # Very short timeout for metadata operations
        elif accuracy_mode:
    ...
    def _render_page_to_base64_fast(
        self, page: fitz.Page, image_format: str = "jpeg", dpi: int = 72
    ) -> str:
        """Ultra-fast page rendering with minimal quality for speed.

        Optimized for serverless environments where speed > quality.
        Uses lowest DPI and JPEG compression for maximum speed.

        Args:
            page: PyMuPDF page object
            image_format: Output format (always "jpeg" for speed)
            dpi: Resolution in DPI (default 72 for speed)

        Returns:
            Base64-encoded string of the rendered page image
        """
        try:
            # Ultra-fast rendering with minimal DPI and no alpha
            # Using colorspace=fitz.csRGB for faster processing
            pix = page.get_pixmap(dpi=dpi, alpha=False)

            # Convert to JPEG bytes (faster compression than PNG)
    ...
    def _render_page_to_base64_ultra_fast(
        self, page: fitz.Page
    ) -> str:
        """Extremely fast page rendering with minimal quality.

        Uses absolute minimum settings for maximum speed.
        Quality is very poor but suitable for basic OCR detection.

        Args:
            page: PyMuPDF page object

        Returns:
            Base64-encoded string of the rendered page image or empty string
        """
        try:
            # Extremely fast rendering with very low DPI (50) and minimal options
            pix = page.get_pixmap(dpi=50, alpha=False)

            # Convert to JPEG with high compression (low quality)
            img_bytes = pix.tobytes("jpeg")
            ...
            # If rendering fails, return empty string (skip this image)
            return ""
    ...
    def _get_page_image_smart(
        ...
        """
        # Try to extract embedded images first for all page types in metadata_only mode
        # In metadata_only mode, we're more aggressive about using embedded images for speed
        if page_type in (PageType.SCANNED, PageType.EMPTY) or (self.metadata_only and page_type == PageType.TEXT):
            try:
                ...
                if image_list:
                    ...
                    for img in image_list:
                        ...
                        # Look for images that cover a significant portion of the page
                        # In metadata_only mode, be more aggressive (>30% vs >60%)
                        min_coverage = 0.3 if self.metadata_only else 0.6
                        if coverage > min_coverage and coverage > best_coverage:
        ...
        # Use ultra-fast rendering if in metadata_only mode (default for serverless)
        if self.metadata_only:
            return self._render_page_to_base64_ultra_fast(page)
        else:
    ...
    def _analyze_page_core(self, page_num: int, include_image: bool = False,
        ...
        page_image = None
        if include_image:
            # In metadata_only mode, skip image generation for high-confidence classifications
            # This reduces processing time for obvious cases
            if self.metadata_only and confidence > 0.85 and page_type in (PageType.TEXT, PageType.EMPTY):
                # Skip image generation for high-confidence text/empty pages in ultra-fast mode
                page_image = ""  # Empty string indicates skipped for performance
            else:
    ...
    def analyze_page_metadata_only(self, page_num: int) -> AnalysisResult:
        """Ultra-fast metadata-only page analysis.
        
        This method provides maximum speed by only checking PDF structure
        without any text extraction. Suitable for serverless environments.
        
        Args:
            page_num: Page number to analyze (0-indexed)
            
        Returns:
            AnalysisResult with basic classification based on metadata only
        """
        if not self.doc:
            raise RuntimeError("PDF not opened. Use within context manager.")
        ...
        try:
            ...
            # Quick image area estimation
            estimated_image_ratio = 0.0
            large_image_count = 0
            
            if images:
                for img in images[:5]:  # Check only first 5 images for speed
                    try:
                        xref = img[0]
                        ...
                        if rects:
                            for rect in rects:
                                ...
                                estimated_image_ratio += coverage
                                if coverage > 0.3:  # Large image
                                    large_image_count += 1
                    except Exception:
                        continue
                        
                estimated_image_ratio = min(estimated_image_ratio, 1.0)
            
        except Exception:
            # Fallback values if metadata access fails
            font_count = 0
            image_count = 0
            estimated_image_ratio = 0.0
            large_image_count = 0
        
        # Ultra-fast classification based on metadata only
        text_length = 0  # Unknown in metadata mode
        estimated_text_ratio = 0.0
        
        if font_count == 0 and image_count == 0:
            # No fonts, no images = empty
            page_type = PageType.EMPTY
            confidence = 0.9
        elif font_count > 0 and large_image_count == 0:
            # Has fonts, no large images = likely text
            page_type = PageType.TEXT
            confidence = 0.8
            estimated_text_ratio = 0.5  # Assume moderate text density
        elif font_count == 0 and large_image_count > 0:
            # No fonts, has large images = likely scanned
            page_type = PageType.SCANNED
            confidence = 0.8
        elif font_count > 0 and large_image_count > 0:
            # Has both fonts and large images = mixed
            page_type = PageType.MIXED
            confidence = 0.7
        else:
            # Uncertain case
            if image_count > font_count:
                page_type = PageType.SCANNED
            else:
                page_type = PageType.TEXT
            confidence = 0.6
        
        # Minimal details for metadata mode
        details = {
            ...
            "page_dimensions": {"width": page_rect.width, "height": page_rect.height},
            "estimated_image_coverage": estimated_image_ratio,
            "note": "Text length unknown in metadata mode"
        }
        ...
        return AnalysisResult(
            ...
            text_ratio=estimated_text_ratio,
            image_ratio=estimated_image_ratio,
            text_length=text_length,  # 0 in metadata mode
            image_count=image_count,
            details=details,
            page_image=None,  # No images in metadata mode
        )
    ...
    def analyze_page(self, page_num: int, include_image: bool = False,
        ...
        if self.metadata_only:
            # Ultra-fast metadata-only mode
            result = self.analyze_page_metadata_only(page_num)
            
            # Add image if requested (but this defeats the purpose of metadata-only mode)
            if include_image:
                fitz_page = self.doc[page_num]
                page_image = self._get_page_image_smart(fitz_page, result.page_type, image_format, image_dpi)
    ...
    def _analyze_pages_batch(self, page_numbers: list[int], include_images: bool = False,
        ...
        with PDFAnalyzer(self.pdf_path, accuracy_mode=(not self.performance_mode), metadata_only=self.metadata_only) as thread_analyzer:
