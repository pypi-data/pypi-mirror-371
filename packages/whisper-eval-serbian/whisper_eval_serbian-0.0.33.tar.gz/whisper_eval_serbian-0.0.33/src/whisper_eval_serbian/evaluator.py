...
class Evaluator:
    ...
    def run(self, log_to_file: bool = True) -> Tuple[List[Dict[str, str]], Dict[str, float]]:
        ...
        for item in tqdm(self.task, desc="Transcribing"):
            raw_prediction = self.model.transcribe(item["audio"]["array"], item["audio"]["sampling_rate"])
            reference = item["reference_text"]  # Already in Latin script from ASRTask

            # <-- 2. Transliterate the model's prediction to Serbian Latin
            prediction = to_latin_serbian(raw_prediction)

            # Strip leading/trailing whitespace from both
        ...
        # --- Clean text for metrics (remove newlines + strip) ---
        clean_predictions = [p.replace("\n", " ").strip() for p in predictions]
        ...
        bleu = self.bleu_metric.compute(predictions=clean_predictions, references=[[r] for r in clean_references])
        ...
        if log_to_file:
            with open("evaluation_log.txt", "a", encoding="utf-8") as f:
                ...
                for result in detailed_results:
                    ...
                    ref_log = result['reference'].replace("\n", " ").strip()
                    pred_log = result['prediction'].replace("\n", " ").strip()
