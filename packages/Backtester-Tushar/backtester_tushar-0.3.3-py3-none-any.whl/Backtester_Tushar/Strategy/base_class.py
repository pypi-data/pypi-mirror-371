...
class Strategy(ABC):
    ...
    def normalize_signals_industry_neutral(self, signals_df, window=60):
        """
        Rolling industry-neutral normalization with Numba speed.
        1. Sector-neutral rolling z-score (per sector, using t-1 info).
        2. Cross-sectional rolling z-score (per date, using t-1 info).
        """
        df = signals_df.copy()
        ...
        # --- Step 2: cross-sectional normalization (per date) ---
        df = df.sort_values(['date', 'sector']).reset_index(drop=True)
        df['signal_descriptor'] = df.groupby('date', group_keys=False)['signal_descriptor'] \
            .transform(lambda x: rolling_zscore(x.to_numpy(), window))
