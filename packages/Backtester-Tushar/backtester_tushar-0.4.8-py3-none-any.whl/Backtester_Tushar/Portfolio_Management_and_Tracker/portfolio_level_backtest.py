...
def calculate_position_metrics(positions, market_data):
    position_metrics = defaultdict(dict)
    for ticker, pos in positions.items():
        ticker_data = market_data[market_data['ticker'] == ticker]
        if not ticker_data.empty:
            close_price = ticker_data.iloc[0]['Close']
            position_metrics[ticker]['market_value'] = pos['shares'] * close_price
            position_metrics[ticker]['unrealized_pnl'] = (close_price - pos['avg_price']) * pos['shares']
            position_metrics[ticker]['direction'] = pos['direction']
...
class PortfolioManager:
    ...
    def calculate_available_deployment_capital(self, portfolio_state):
        ...
        # FIX: Use gross exposure for current_deployment instead of abs(net)
        current_deployment = sum(abs(pos.get('market_value', 0)) for pos in portfolio_state['position_details'].values())
    ...
    def execute_smart_rebalancing(self, long_portfolio, short_portfolio, current_date, signal_data, market_data):
        ...
        # FIX: Set target_tickers properly
        ...
        for ticker, pos in self.positions.items():
            ...
            if not ticker_data.empty:
                try:
                    ...
                    self.logger.warning(f"Could not extract price for {ticker}: {e}")

        # FIX: Update self.current_capital to the final working_cash
        self.current_capital = working_cash

        return total_transaction_costs, len(trades_executed)

    def calculate_comprehensive_portfolio_metrics(self, market_data, date):
        ...
        # Exposure metrics
        long_exposure = sum(pos['market_value'] for pos in position_details.values() if pos['market_value'] > 0)
        short_exposure = abs(sum(pos['market_value'] for pos in position_details.values() if pos['market_value'] < 0))
        gross_exposure = long_exposure + short_exposure
        net_exposure = long_exposure - short_exposure
        leverage_ratio = gross_exposure / portfolio_value if portfolio_value > 0 else 0

        # Deployment metrics
        deployment_ratio = gross_exposure / portfolio_value if portfolio_value > 0 else 0
        target_deployment = self.current_deployable_pct
        deployment_compliant = deployment_ratio <= target_deployment * 1.01

        # Drawdown
        self.peak_value = max(self.peak_value, portfolio_value)
        current_drawdown = max(0, (self.peak_value - portfolio_value) / self.peak_value) if self.peak_value > 0 else 0
        self.current_drawdown = current_drawdown

        # Risk metrics (simplified)
        if len(self.returns) > 1:
            daily_return = (portfolio_value - self.portfolio_values[-1]) / self.portfolio_values[-1] if self.portfolio_values else 0
            self.returns.append(daily_return)
            portfolio_vol = np.std(self.returns) * np.sqrt(252) if len(self.returns) > 1 else 0
        else:
            daily_return = 0
            portfolio_vol = 0
        ...
        metrics = {
            ...
            'target_deployment_ratio': target_deployment,
    ...
    def run_backtest(self, master_df, allocation_schedule=None):
        """
        Run comprehensive institutional-grade portfolio backtest with dynamic allocations and next-day execution.
    ...
    def _analyze_concentration_risk(self, results_df):
        ...
        for _, row in results_df.iterrows():
            ...
            if position_details:
                weights = [abs(pos['market_value']) / row['portfolio_value'] for pos in position_details.values() if row['portfolio_value'] > 0]
                if weights:
                    # Herfindahl index
