...
class PortfolioManager:
    ...
    def execute_smart_rebalancing(self, long_portfolio, short_portfolio, current_date, signal_data, market_data):
        ...
        # --- NEW: update both cash and portfolio value ---
        self.current_capital = final_portfolio_state['cash']  # keep cash logic intact
        self.last_portfolio_value = final_portfolio_state['total_portfolio_value']  # NEW: track true portfolio value
    ...
    def calculate_comprehensive_portfolio_metrics(self, market_data, current_date):
        ...
        # --- NEW: track last portfolio value for proper drawdown ---
        self.last_portfolio_value = portfolio_value

        # Update drawdown tracking using total portfolio value
        ...
        # Build dict of market values safely
        position_values = {}
        for ticker, pos in self.positions.items():
            ticker_data = market_data[market_data['ticker'] == ticker]
            if not ticker_data.empty:
                close_price = ticker_data.iloc[0]['Close']
                position_values[ticker] = {'market_value': pos['shares'] * close_price}
            else:
                self.logger.warning(f"No market data for {ticker} during leverage check, skipping...")
                position_values[ticker] = {'market_value': 0.0}
        ...
        # Apply daily costs ONLY to cash, not total portfolio
