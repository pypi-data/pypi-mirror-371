...
class Strategy(ABC):
    ...
    def normalize_signals_industry_neutral(self, signals_df, window=60):
        """
        Two-step normalization:
        1. Sector-neutral rolling z-score (time series within each sector).
        2. Cross-sectional z-score across all tickers on the same date.

        Avoids forward-looking bias by only using past data in step 1.
        """
        df = signals_df.copy()
        if 'sector' not in df.columns:
            raise ValueError("Sector column is required for industry-neutral normalization")

        df['signal_descriptor'] = pd.to_numeric(df['signal_descriptor'], errors='coerce')

        # --- Step 1: sector-neutral rolling z-score (time series) ---
        df = df.sort_values(['sector', 'date']).reset_index(drop=True)
        df['signal_descriptor'] = df.groupby('sector', group_keys=False)['signal_descriptor'] \
            .transform(lambda x: rolling_zscore(x.to_numpy(), window))

        # --- Step 2: cross-sectional normalization (per date, not rolling) ---
        cross_mean = df.groupby('date')['signal_descriptor'].transform('mean')
        cross_std = df.groupby('date')['signal_descriptor'].transform('std').replace(0, np.nan)
        df['signal_descriptor'] = (df['signal_descriptor'] - cross_mean) / cross_std
        df['signal_descriptor'] = df['signal_descriptor'].fillna(0.0)
