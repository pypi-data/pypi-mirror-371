...
class InstitutionalPortfolio:
    ...
    def execute_smart_rebalancing(self, target_long_portfolio, target_short_portfolio,
        ...
        """
        Execute rebalancing with corrected capital and slippage logic:
        - Long/short opens consume cash (shorts lock margin, no proceeds released).
        - Cash released only on reduces/closes.
        - Slippage side based on trade direction (buy vs sell).
        - Transaction costs always deducted.
        """
        ...
        for ticker in current_tickers - target_tickers:
            ...
            shares_to_trade = -pos['shares']  # full close
            execution_price = market_data.loc[market_data['ticker'] == ticker, 'Close'].iloc[0]

            # Side detection
            is_buy = (pos['direction'] == -1) or (pos['direction'] == 1 and shares_to_trade > 0)
            ...
            # Cash adjustment (closing releases capital, covering shorts consumes cash)
            cash_flow = -shares_to_trade * effective_price  # opposite of trade sign
        ...
        for ticker, target in target_positions.items():
            ...
            if abs(shares_delta) < 1:  # skip tiny
                continue
            ...
            # Side detection
            if (target['direction'] == 1 and shares_delta > 0) or (target['direction'] == -1 and shares_delta < 0):
                is_buy = True
            else:
                is_buy = False
            ...
            # Cash adjustment
            cash_flow = -shares_delta * effective_price  # buys consume, sells credit
            if target['direction'] == -1 and shares_delta > 0:
                # Opening/increasing a short → treat as margin lock (consume cash, not release)
                self.current_capital -= abs(shares_delta) * effective_price
            else:
                self.current_capital += cash_flow
            self.current_capital -= transaction_cost
    ...
    def calculate_comprehensive_portfolio_metrics(self, market_data, current_date):
        """Corrected metrics: signed market values, borrow costs once per day."""
        ...
        for ticker, pos in self.positions.items():
            ...
            if len(ticker_data) > 0:
                ...
                atr = ticker_data.iloc[0].get('ATR', 0.02)
                shares = pos['shares']
                market_value = shares * current_price * (1 if pos['direction'] == 1 else -1)
                cost_basis = shares * pos['avg_price'] * (1 if pos['direction'] == 1 else -1)
        ...
        daily_return = 0.0 if not self.portfolio_values else (portfolio_value - self.portfolio_values[-1]) / \
        ...
        metrics = {
            ...
            'gross_exposure': sum(abs(p['market_value']) for p in position_details.values()),
            'net_exposure': sum(p['market_value'] for p in position_details.values()),
            'leverage': sum(abs(p['market_value']) for p in
