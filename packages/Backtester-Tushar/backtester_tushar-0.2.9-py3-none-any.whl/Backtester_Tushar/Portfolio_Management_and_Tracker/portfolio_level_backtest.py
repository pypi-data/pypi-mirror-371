...
class InstitutionalPortfolio:
    ...
    def __init__(self,
        ...
        self.ticker_order = []  # To map covariance indices
        self.betas = {}  # To store ticker betas
        self.historical_returns = []  # For historical VaR
    ...
    def execute_smart_rebalancing(self, target_long_portfolio, target_short_portfolio,
                                  current_date, market_data, next_day_data=None):
        """
        Execute rebalancing with symmetric cash-flow logic:
        - Longs and shorts both consume capital until closed.
        - No upfront proceeds for short sales.
        - Transaction costs and borrow costs deducted immediately.
        - Cash released only when positions are closed/reduced.
        """
        ...
        # --- Construct target portfolio (desired positions) ---
        ...
        for ticker, weight in target_long_portfolio.items():
            ...
            if not ticker_data.empty:
                execution_price = ticker_data.iloc[0].get('Open', ticker_data.iloc[0]['Close'])
        ...
        # --- Execute changes vs. current positions ---
        ...
        for ticker in current_tickers - target_tickers:
            ...
            shares_to_trade = -pos['shares']  # close completely
            execution_price = market_data.loc[market_data['ticker'] == ticker, 'Close'].iloc[0]

            # Apply market impact + slippage
            volume = market_data.loc[market_data['ticker'] == ticker, 'Volume'].iloc[
                0] if 'Volume' in market_data else 1e6
            volatility = pos.get('volatility', 0.02)
            market_impact = self.market_impact_model.calculate_market_impact(shares_to_trade, volume, volatility)
            slippage_pct = self.slippage_factor * volatility
            execution_price *= (1 - slippage_pct if shares_to_trade < 0 else 1 + slippage_pct)

            # Transaction cost
            transaction_cost = abs(shares_to_trade * execution_price) * (self.transaction_cost_bps + market_impact)
            total_transaction_costs += transaction_cost

            # ✅ Release locked capital (both long and short)
            cash_release = abs(pos['shares'] * execution_price)
            self.current_capital += cash_release
            ...
            trades_executed.append({
                ...
                'slippage': execution_price - pos['avg_price']
            })
            self.trade_id += 1
            del self.positions[ticker]

        # 2. Open new / adjust existing positions
        for ticker, target in target_positions.items():
            ...
            market_impact = self.market_impact_model.calculate_market_impact(
                shares_delta,
                market_data.loc[market_data['ticker'] == ticker, 'Volume'].iloc[0] if 'Volume' in market_data else 1e6,
                volatility
            )
            slippage_pct = self.slippage_factor * volatility
            effective_price = target['price'] * (1 + slippage_pct if shares_delta > 0 else 1 - slippage_pct)

            # Transaction cost
            transaction_cost = abs(shares_delta * effective_price) * (self.transaction_cost_bps + market_impact)
            total_transaction_costs += transaction_cost
            self.current_capital -= transaction_cost

            # ✅ Cash is adjusted only if reducing/closing
            if (current_shares > 0 and shares_delta < 0) or (current_shares < 0 and shares_delta > 0):
                cash_release = abs(shares_delta * effective_price)
                self.current_capital += cash_release
        ...
        # --- Deduct borrow cost for all open shorts (daily accrual) ---
        for ticker, pos in self.positions.items():
            if pos['direction'] == -1:
                borrow_cost = (pos['volatility'] / 0.02) * (0.02 / 252) * abs(pos['shares'] * pos['avg_price'])
                total_borrow_cost += borrow_cost
        self.current_capital -= total_borrow_cost

        self.trade_log.extend(trades_executed)
        return total_transaction_costs + total_borrow_cost, len(trades_executed)
