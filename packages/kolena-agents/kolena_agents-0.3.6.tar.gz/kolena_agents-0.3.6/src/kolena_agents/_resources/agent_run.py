...
class AgentRun:
    ...
    def add_wait(
    ...
    ) -> agent_run_types.WaitRunResponse:
        ...
        """
        response = (
            self._api.add_agent_run_wait_api_v1_client_agents_agent_id_runs_wait_post(
                agent_id=agent_id,
                files=files,
                user_defined_id=user_defined_id,
                timeout=timeout,
        ...
        if response.error is not None:
            logger.error(f"Agent run failed: {response.error}")
            if response.error.status_code == 408:
                logger.debug(
