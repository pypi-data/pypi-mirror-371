...
def structure_file_with_gemini(
    ...
    try:
        ...
        if not response.text:
            print("❌ Error: The Gemini API returned an empty response. This is often due to safety filters or content restrictions.")
            # 打印 API 返回的反馈信息，这通常会告诉我们为什么响应被拦截了
            print(f"DEBUG INFO: Prompt Feedback = {response.prompt_feedback}")
            # 为了更详细的调试，可以打印整个响应对象
