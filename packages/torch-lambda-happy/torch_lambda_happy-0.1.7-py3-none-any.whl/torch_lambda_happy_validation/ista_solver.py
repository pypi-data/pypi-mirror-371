...
class IstaSolver:
    """ISTA solver for sparse linear regression."""
    ...
    def __init__(
        ...
        self.lambda_happy = LambdaHappy(X).compute_agg(nb_run=lambda_nb_run, m=lambda_m)
...
class IstaTestRunner:
    ...
    def __init__(
        self, device_type: str = "cuda", dtype: torch.dtype = torch.float32
    ) -> None:
    ...
    def run_test(self, n: int, p: int, seed: int, eps: float = 1e-6) -> Dict[str, Any]:
        ...
        y = X @ alpha_true[1:] + alpha_true[0]

        solver = IstaSolver(X, y, max_iter=1000, max_s=1000, eps=eps)
    ...
    def run_all(self, p_list: List[int], seeds: List[int]) -> None:
        ...
        filename_html = f"compute_multi_ista_solver_{self.device_type}_{self.dtype_map.get(self.dtype)}_{timestamp}.html".lower()
