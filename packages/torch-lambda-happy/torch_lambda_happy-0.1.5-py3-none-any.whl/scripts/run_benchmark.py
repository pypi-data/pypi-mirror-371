...
from torch_lambda_happy_benchmark.dependencies import (
...
from torch_lambda_happy_benchmark.benchmark import LambdaBenchmark
...
def parse_arguments():
    ...
    parser.add_argument(
        "--benchmark_partial",
        action="store_true",
        help="Benchmark each partial step of in the calculation of lambda happy",
    )
...
def main():
    ...
    choose_matplotlib_backend(args.interactive)
    ...
    if args.benchmark_2D:
        ...
        bench.show_benchmark_2D(n=args.n, p=args.p, m=args.m)
        any_ran = True

    if args.benchmark_3D:
        print("> Running 3D benchmark…")
        bench.show_benchmark_3D(n=args.n)
        any_ran = True

    if args.benchmark_float:
        print("> Running float16 vs float32 benchmark on CUDA…")
        bench.show_benchmark_float(nb_run=args.nb_run)
        any_ran = True
    ...
    if args.benchmark_partial:
        print("> Running partial benchmark…")
