...
class LambdaBenchmark:
    ...
    def _benchmark_single(
        ...
        """
        _ = self.estimator.compute(m=1_000, device_type=device_type, dtype=dtype)
        self._sync()
        start = time.perf_counter()
        _ = self.estimator.compute(m=m, device_type=device_type, dtype=dtype)
        self._sync()
    ...
    def _benchmark_many(
        ...
        """
        [
            self.estimator.compute(m=m, device_type=device_type, dtype=dtype)
            for _ in range(10)
        ]
        self._sync()
        start = time.perf_counter()
        _ = self.estimator.compute_many(
            nb_run, m=m, device_type=device_type, dtype=dtype
        )
        self._sync()
    ...
    def show_benchmark_2D(
        ...
        for m_value in m_values:
            ...
            benchmark_m_results["pytorch_cpu"].append(
                self._benchmark_many(m=m_value, device_type="cpu")
            )
            benchmark_m_results["pytorch_cuda"].append(
                self._benchmark_many(m=m_value, device_type="cuda")
            )
        ...
        for p_value in p_values:
            ...
            benchmark_nb_cols_results["pytorch_cpu"].append(
                self._benchmark_many(m=m, device_type="cpu")
            )
            benchmark_nb_cols_results["pytorch_cuda"].append(
                self._benchmark_many(m=m, device_type="cuda")
            )
    ...
    def show_benchmark_3D(
        ...
        fig.suptitle(
            ...
            f"(n={n}, dtype={self.dtype_map.get(original_estimator_dtype)})",
            fontsize=14,
        )
        # 3D plot CPU
    ...
    def show_benchmark_float(self, m_values: np.ndarray = None, nb_run=500) -> None:
        ...
        fig.suptitle(
            ...
            f"({n=}, {p=}, device={self.estimator.get_device_type()})",
            fontsize=14,
        )
    ...
    def benchmark_partial(
        ...
        print(
            f"Mean FPS over {nb_run} runs of {self.estimator.__class__.__name__:31s} : {mean_fps:.8f} FPS ({(1/mean_fps):.8f} sec)"
        )
        self.estimator = original_estimator

        fps_theorical = 1.0 / total_time if total_time > 0 else float("inf")
        print(
            f"Theorical LambdaHappy mean FPS (sum of independent steps) : {fps_theorical:.8f} FPS ({total_time:.8f} sec)"
        )
    ...
    def show_benchmark_format_conversion(
        ...
        for init_name, init_dev, init_dtype in scenarios:
            ...
            for name, device, dtype in scenarios:
                single_fps.append(
                    self._benchmark_single(m=m, device_type=device, dtype=dtype)
                )
                many_fps.append(
                    self._benchmark_many(
                        nb_run=100,
            ...
            width = 0.35
            ax.bar(x - width / 2, single_fps, width, label="single")
            ax.bar(x + width / 2, many_fps, width, label="many")
            ax.set_xticks(x)
            ax.set_xticklabels([name for name, _, _ in scenarios])
            ax.set_ylabel("FPS")
            ax.legend()
            plt.tight_layout()
            self._finalize_plot(
