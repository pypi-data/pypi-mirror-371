...
def validate_config(
    ...
    if not is_valid:
        config_path = Path(config_file).as_posix() if config_file else "<provided dict>"
        schema_path = Path(schema_file).as_posix() if schema_file else "<provided dict>"
        raise ValueError(
            f"Incorrect configurations found!\n"
