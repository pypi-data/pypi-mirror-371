...
def make_dir_ask_backup(dir_path: str):
    ...
    if Path(dir_path).is_dir():
        ans = ask_yes_no(
            f"""The directory "{dir_path}" already existed. Select an action: [yes/no/backup]?\n  * Yes: overwrite the existed directory and continue/update unfinished tasks.\n  * No: stop and exit process.\n  * Backup: backup the existed directory and create a new directory for fresh tasks."""
        )
        if ans == "yes":
            print("\tOverwrite the existed directory")
            make_dir(dir_path, backup=False)
        elif ans == "backup":
            print("\tBackup the existed directory")
...
def scan_dirs(
    ...
    without_files: list[str] = None,
) -> list[str]:
    """Return directories containing `with_files` and none of `without_files`.

    Args:
        dirs: List of directory paths to scan.
        with_files: Files that must exist in the directory. Defaults to [].
        without_files: Files that must not exist in the directory. Defaults to [].

    Returns:
        List of directory paths meeting the conditions.
    """
    if with_files is not None:
        dirs = [d for d in dirs if all(Path(f"{d}/{f}").exists() for f in with_files)]
    if without_files is not None:
        dirs = [d for d in dirs if all(not Path(f"{d}/{f}").exists() for f in without_files)]
    return dirs
