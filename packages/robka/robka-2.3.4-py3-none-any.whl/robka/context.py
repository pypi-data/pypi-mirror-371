...
class AuxData:
    def __init__(self, data: dict):
        ...
        self.query_id: str = data.get("query_id")
...
class Message:
    ...
    def edit_inline_keypad(self, inline_keypad: Dict[str, Any]) -> Dict[str, Any]:
        return self.bot.edit_message_inline_keyboard(
            chat_id=self.chat_id,
            message_id=self.message_id,
            inline_keypad=inline_keypad
        )

    def answer_callback_query(self, text: Optional[str] = None, show_alert: bool = False):
        return self.bot.answer_callback_query(
            query_id=self.raw_data.get("aux_data", {}).get("query_id"),
            text=text,
