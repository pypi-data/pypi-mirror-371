...
class Robot:
    ...
    def _process_update(self, update: dict):
        ...
        if update.get("type") in {"UpdateChatInlineButtonsCallback", "CallbackQuery", "ReceiveCallback"}:
            msg = update.get("inline_message") or (
                update.get("callback_data") and update["callback_data"].get("message")
            ) or update.get("message") or {}

            if msg and self._callback_handlers:
                aux = msg.get("aux_data") or {}
                if "callback_data" in update and isinstance(update["callback_data"], dict):
                    aux.setdefault("query_id", update["callback_data"].get("id") or update["callback_data"].get("query_id"))
                elif "query_id" in update:
                    aux.setdefault("query_id", update.get("query_id"))
                if aux:
                    msg["aux_data"] = aux
                ...
                for handler in self._callback_handlers:
                    if not handler["button_id"] or (context.aux_data and context.aux_data.button_id == handler["button_id"]):
