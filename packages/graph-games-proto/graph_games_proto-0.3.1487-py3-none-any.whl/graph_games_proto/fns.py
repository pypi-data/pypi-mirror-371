...
def getinitialstate(game_config):
    ...
    print("*************************** len(piles[0].pieces): ", len(piles[0].pieces))
    decks = []
    ...
    for deck in decks:
        for card in deck.facedown_stack:
            carduuid2deckidx[card.uuid] = deck.idx
        ...
        for path in edge.paths:
            if len(path.segments) > 0:
                ...
                if first_segment.pieces and len(first_segment.pieces) > 0:
                    first_piece = first_segment.pieces[0]
...
def append_follow_up_draw_legal_actions(game, action):
    ...
    if len(game.history) >= 2:
        ...
        if (last_action.legal_action.player_idx != legal_action.player_idx) or last_action.legal_action.name == "INITIAL-GOAL-KEEP":
            ...
            for card in game.decks[0].faceup_spots:
                if not card.is_wild:
                    game = append_to_legal_actions(
                        game,
                        LegalAction(
...
def ensure_faceup_spots_valid(game):
    for deck in game.decks:
        ...
        num_wild = sum(1 for card in non_empty_spots if card.is_wild)
        if num_wild >= 3:
            print("deck.faceup_spots before: ", [card.is_wild for card in deck.faceup_spots])
            print("num_wild:", num_wild)
            print("deck.facedown_stack[0].uuid before: ", deck.facedown_stack[0].uuid)
            print("\n")

            print("len(deck.faceup_spots) before: ", len(deck.faceup_spots))
            print("len(deck.discard) before: ", len(deck.discard))
            print("len(deck.facedown_stack) before: ", len(deck.facedown_stack))
            print("\n")
            ...
            print("len(deck.faceup_spots) mid: ", len(deck.faceup_spots))
            print("len(deck.discard) mid: ", len(deck.discard))
            print("len(deck.facedown_stack) mid: ", len(deck.facedown_stack))
            print("\n")
            ...
            print("len(deck.faceup_spots) after: ", len(deck.faceup_spots))
            print("len(deck.discard) after: ", len(deck.discard))
            print("len(deck.facedown_stack) after: ", len(deck.facedown_stack))
            print("\n")

            deck = shuffle(game.rng, deck)
            print("deck.facedown_stack[0].uuid after: ", deck.facedown_stack[0].uuid)
            game = set_deck(game, deck.idx, deck)
            game = flip_cards(game, deck.idx, num_faceup_spots)
            
            print("deck.faceup_spots after: ", [card.is_wild for card in deck.faceup_spots])
            print("\n")
...
def handle_faceup_draw_action(game, action):
    ...
    spot_idx = next((i for i, card in enumerate(deck.faceup_spots) if card.uuid == faceup_draw.card_uuid), None)
...
def get_longest_path_length(game, player_idx):
    ...
    for _ in range(500):
        ...
        # If we haven't found a longer path in 50 iterations, return
        if num_iters_since_change > 30:
    ...
    if len(game.decks) > 2:
        ...
        if longest_path_deck.facedown_stack:
            ...
            if player_with_card:
                ...
                card_idx = next((i for i, c in enumerate(player_with_card.cards) if c.deck_idx == 2), None)
...
def find_player_with_longest_path_card(game):
    for player in game.players:
        if any(card.deck_idx == 2 for card in player.cards):
            return player
    return None
...
def handle_move_pieces_to_path_action(game, action):
    ...
    for card_uuid in optional.card_uuids:
        ...
        if card_idx is not None:
            ...
            # add to discard
            game.decks[card.deck_idx].discard.append(card)
...
def match_strict_wild(game, fulfillment, cards, segments):
    ...
    for segment in segments:
        if segment.unit_uuid and segment.unit_uuid in wild_unit_uuids:
            first_matching_idx = next((i for i, card in enumerate(new_cards) if card.resource_uuid == segment.unit_uuid), None)
...
def match_non_wild_non_empty(game, fulfillment, cards, segments):
    ...
    for segment in segments:
        first_strict_matching_idx = next((i for i, card in enumerate(new_cards) if card.resource_uuid == segment.unit_uuid), None)
        first_wild_matching_idx = next((i for i, card in enumerate(new_cards) if card.resource_uuid in wild_unit_uuids), None)
