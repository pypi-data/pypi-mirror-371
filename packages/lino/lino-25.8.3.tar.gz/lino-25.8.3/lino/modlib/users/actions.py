...
def validate_sessions_limit(request):
    ...
    if qs.count() >= dd.plugins.users.active_sessions_limit:
        raise Warning(
            _(
                "There are more than {} active user sessions. Please try again later."
            ).format(dd.plugins.users.active_sessions_limit)
        )
...
class SignIn(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        if user is None:
            ...
            ar.success(
                _("Now signed in as {}").format(user), close_window=True, goto_url="/"
            )
