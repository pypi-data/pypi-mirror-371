...
from lino.utils.html import E, iselement
...
class ExtRenderer(JsCacheRenderer):
    ...
    def actions_hotkeys2js(actions_hotkeys):
        for action_hotkeys in actions_hotkeys:
            js_action = """Lino.row_action_handler('{}', 'GET', null)""".format(
                action_hotkeys.get("ba")
            )
            action_hotkeys.update({"ba": js_action})
        return actions_hotkeys

    def window_action_button(self, ar, ba, status=None, label=None, title=None, **kw):
    ...
    def quick_manage_toolbar(self, ar, obj):
        ...
        manage_btn = ar.renderer.action_button(
            obj, ar, ba, _("Manage"), icon_name="application_form"
        )
        assert manage_btn is not None
        return E.p(insert_btn, manage_btn)

    # def unused_insert_button(self,
    #                          ar,
    #                          text=None,
    #                          known_values=None,
    #                          **options):
    ...
    def action_call(self, ar, bound_action, status):
        ...
        # roger or tera1).
        return "{}()".format(self.get_panel_btn_handler(bound_action, status))

    def get_panel_btn_handler(self, ba, status={}):
        if isinstance(ba.action, WrappedAction):
            original_ba = ba.action.bound_action
            if (
                original_ba.action.parameters
                and not original_ba.action.no_params_window
            ):
                return "Lino.param_action_handler(Lino.%s)" % original_ba.full_name()
            return "Lino.wrapped_action_handler('Lino.%s')" % ba.action.action_name
        if not ba.action.action_name:
            raise Exception("Cannot make handler for nameless action %r" % ba.action)
        if ba.action.js_handler:
            if callable(ba.action.js_handler):
                h = ba.action.js_handler(ba.actor)
            else:
                h = ba.action.js_handler % ba.action.__dict__
        else:
            bp = status.get("base_params", None)
            if bp is None:
                pp = ba.action.preprocessor
            else:
                pp = "function() {return %s;}" % py2js(bp)

            if ba.action.select_rows:
                h = "Lino.row_action_handler("
            else:
                h = "Lino.list_action_handler('{}',".format(ba.actor.actor_url())
    ...
    def get_actor_url(self, actor, *args, **kw):
        return self.front_end.build_plain_url(
            "api", actor.app_label, actor.__name__, *args, **kw
        )
    ...
    def html_page_main_window(self, on_ready, request, site):
        ...
        if not on_ready:
            ar = BaseRequest(request, renderer=self)
            # html = site.get_main_html(ar, front_end=self.front_end)
            html = site.get_main_html(ar)
    ...
    def html_page_user(self, request, site):
        ...
        if request.user.is_authenticated:
            ...
            yield "Lino.user = %s;" % py2js(dict(id=user.id, name=str(user)))

            # a = rt.models.users.MySettings.default_action
            # handler = self.action_call(None, ba, dict(record_id=user.pk))
            ba = rt.models.users.Me.default_action
            handler = self.action_call(None, ba, {})
            handler = "function(){%s}" % handler
            ...
            if site.remote_user_header is None:
                ...
                a = rt.models.users.Me.get_action_by_name("sign_out")
            ...
            login_menu = dict(text=user_text, menu=dict(items=login_menu_items))

            yield "Lino.main_menu = Lino.main_menu.concat(['->',%s]);" % py2js(
                login_menu
            )

        else:
            ba = rt.models.about.About.get_action_by_name("sign_in")
            js = self.action_call(None, ba, {})
            js = "function(){%s}" % js
            login_buttons = [
                dict(xtype="button", text=ba.get_button_label(), handler=js_code(js))
            ]
            # login_buttons = [
    ...
    def write_lino_js(self, f):
        ...
        if len(settings.SITE.languages) > 1:
            for lang in settings.SITE.languages:
                ...
                menu.add_item(
                    lang.django_code,
                    lang.name,
                    javascript='Lino.set_user_language("{}")'.format(lang.django_code),
                )
        ...
        # implementation
        actors_list = [a for a in self.actors_list if a.get_handle_name is None]

        # for a in actors_list:
        #     if a.default_action is None:
        #         raise Exception("20210531 {} has no default_action".format(a))
        # generate only actors whose default_action is visible
        actors_list = [
            a
            for a in actors_list
            if a.default_action and a.default_action.get_view_permission(user_type)
        ]
        ...
        for rpt in actors_list:
            ...
            rh = rpt.get_handle()
            if isinstance(rpt, type) and issubclass(
                rpt, (tables.AbstractTable, choicelists.ChoiceList)
            ):
            ...
            window_actions = [
                ba.action for ba in rpt.get_actions() if ba.action.opens_a_window
            ]
            for ba in rpt.get_actions():
                if isinstance(ba.action, WrappedAction):
                    for ln in self.js_render_wrapped_action(rh, ba):
                        f.write(ln + "\n")
                elif ba.action.parameters and not ba.action.no_params_window:
                    pass
                elif ba.action.opens_a_window:
                    if isinstance(ba.action, (ShowDetail, ShowInsert)):
                ...
                # elif self.is_custom_action(ba.action):
                    ...
                    for parent in window_actions:
                        if parent.window_type is None:
                            raise Exception("20210514 {}.{}".format(rpt, parent))
    ...
    def a2btn(self, ba, **kw):
        a = ba.action
        selector = a
        # if isinstance(a, WrappedAction):
        #     selector = a.bound_action.action
        if selector.parameters and not selector.no_params_window:
            kw.update(
                panel_btn_handler=js_code(
                    "Lino.param_action_handler(Lino.%s)" % ba.full_name()
                )
            )
        elif isinstance(selector, SubmitInsert):
            js = "function(panel){panel.save()}"
            kw.update(panel_btn_handler=js_code(js))
        elif isinstance(selector, SubmitDetail):
            js = "function(panel){panel.save()}"
            kw.update(panel_btn_handler=js_code(js))
        elif isinstance(selector, ShowDetail):
            kw.update(panel_btn_handler=js_code("Lino.show_detail"))
        elif isinstance(selector, ShowInsert):
    ...
    def js_render_ParamsPanelSubclass(self, dh):
        ...
        if dh.main.value["layout"] == "hbox":
            ...
            yield "  layout: 'form',"
        # 20210501
        if dh.layout._datasource.params_panel_pos == "top":
            region = "north"
            yield "  autoHeight: true,"
        elif dh.layout._datasource.params_panel_pos == "bottom":
            region = "south"
            yield "  autoHeight: true,"
        elif dh.layout._datasource.params_panel_pos == "left":
            region = "west"
        elif dh.layout._datasource.params_panel_pos == "right":
            region = "east"
        else:
            region = "north"
        yield "  region: '%s'," % region
        ...
        if lc == 0:
            ...
            msg += ", datasource: %r, other datasources: %r" % (
                dh.layout._datasource,
                dh.layout._other_datasources,
            )
            msg += ", main elements: %r" % dh.main.elements
            # raise Exception(msg)
            print((20150717, msg))
        fields = [e for e in dh.main.walk() if isinstance(e, ext_elems.FieldElement)]
        yield "    this.items = %s;" % py2js(dh.main.elements)
        yield "    this.fields = %s;" % py2js(fields)
        yield (
            "    Lino.%s.superclass.initComponent.call(this);"
        ...
        if region in ("west", "east"):
            width = 20
            for f in fields:
                width = max(width, f.width or f.preferred_width)
            yield "  ,width: Lino.chars2width(%s)" % width
        yield "});"
        yield ""
    ...
    def js_render_FormPanelSubclass(self, dh):
        ...
        if not dh.main.get_view_permission(get_user_profile()):
            msg = (
                "No view permission for main panel of %s :" % dh.layout._formpanel_name
            )
        ...
        if dh.layout._formpanel_name.endswith(".DetailFormPanel"):
            yield "cls: %s," % py2js(
                "l-DetailFormPanel " + tbl.actor_id.replace(".", "-")
            )
        if dh.layout._formpanel_name.endswith(".InsertFormPanel"):
            yield "cls: %s," % py2js(
                "l-InsertFormPanel " + tbl.actor_id.replace(".", "-")
            )
        ...
        if dh.layout._formpanel_name.endswith("_ActionFormPanel"):
            yield "cls: %s," % py2js(
                "l-ActionFormPanel " + tbl.actor_id.replace(".", "-")
            )

        yield "  layout: 'fit',"
        yield "  auto_save: true,"
        hotkeys = []
        for ba in tbl._actions_list:
            if ba.action.hotkey and ba.action.hotkey.key:
        ...
        if on_render:
            ...
            for ln in on_render:
                yield "      " + ln
            yield (
                "      Lino.%s.superclass.onRender.call(this, ct, position);"
                % dh.layout._formpanel_name
            )
            yield "    }"
    ...
    def js_render_detail_action_FormPanel(self, rh, ba):
        rpt = rh.actor

        # ~ logger.info('20121005 js_render_detail_action_FormPanel(%s,%s)',rpt,ba.full_name(rpt))
        # ~ yield "// js_render_detail_action_FormPanel %s" % ba
        def doit(dtl):
            yield ""
            ...
            yield "  empty_title: %s," % py2js(ba.get_button_label())
            if rpt.hide_navigator or ba.action.hide_navigator:
                yield "  hide_navigator: true,"

            if rh.actor.params_panel_hidden:
                yield "  params_panel_hidden: true,"

            if ba.action.save_action_name is not None:
                yield "  save_action_name: %s," % py2js(ba.action.save_action_name)
            yield "  ls_bbar_actions: %s," % py2js(
                self.toolbar(rpt.get_toolbar_actions(ba.action, get_user_profile()))
            )
            yield "  ls_url: %s," % py2js(rpt.actor_url())
            if ba.action != rpt.default_action.action:
                yield "  action_name: %s," % py2js(ba.action.action_name)
            # if isinstance(ba.action, ShowInsert):
            if ba.action.default_record_id is not None:
                yield "  default_record_id: {!r},".format(ba.action.default_record_id)
            ...
            yield (
                "    Lino.%sPanel.superclass.initComponent.call(this);" % ba.full_name()
            )
            yield "  }"
            yield "});"
            yield ""

        dtl = ba.get_window_layout()
        if dtl is None:
            raise Exception("20210530")
            for dtl in rpt.extra_layouts.values():
                for ln in doit(dtl):
                    yield ln
        else:
            for ln in doit(dtl):
    ...
    def js_render_GridPanel_class(self, rh):
        ...
        kw.update(
            ls_bbar_actions=self.toolbar(
                rh.actor.get_toolbar_actions(
                    rh.actor.default_action.action, get_user_profile()
                )
            )
        )
        ...
        if rh.actor.hide_editing(get_user_profile()):
...
'</td>')""" % (rh.actor.row_height * 11)
        ...
        if rh.actor.hide_editing(get_user_profile()):
            kw.update(disabled_fields_index=rh.store.column_index("disabled_fields"))
        ...
        if on_render:
            ...
            for ln in on_render:
                yield "      " + ln
            yield (
                "      Lino.%s.GridPanel.superclass.onRender.call(this, ct, position);"
                % rh.actor
            )
            yield "    }"
    ...
    def js_render_wrapped_action(self, rh, ba):
        yield "Lino.%s = function(panel, btn, step) {" % ba.action.action_name
        ba = ba.action.bound_action
        if isinstance(ba.action, ShowDetail):
            yield "\tvar pk = panel.get_current_record().id;"
            yield "\tif (pk === -99998) pk = 0;"
            yield "\tLino.%s.run(null, {record_id: pk});" % ba.full_name()
        elif isinstance(ba.action, (ShowTable, ShowEmptyTable, ShowInsert)):
            yield "\tLino.%s.run(null);" % ba.full_name()
        else:
            if ba.action.select_rows:
                h = "\tLino.row_action_handler(%s, %s, %s)(panel, btn, step);"
            else:
                h = (
                    "\tLino.list_action_handler('{}'".format(ba.actor.actor_url())
                    + ", %s, %s, %s)(panel, btn, step);"
                )
            yield h % (
                py2js(ba.action.action_name),
                py2js(ba.action.http_method),
                ba.action.preprocessor,
            )
        yield "};"

    def js_render_custom_action(self, rh, ba):
        """Defines the non-window action handler used by
        :meth:`row_action_button`
        """
        yield "Lino.%s = function(rp, is_main, pk, params) { " % ba.full_name()
    ...
    def js_render_window_action(self, rh, ba):
        ...
        def doit(wl, ws):
            ...
            if True:
                if ws:
                    windowConfig.update(
                        maximized=False, draggable=True, maximizable=True, modal=True
                    )
            ...
            if ba.action.extjs_main_panel:
                ...
                if (
                    ba.action.hide_top_toolbar
                    or ba.actor.hide_top_toolbar
                    or ba.action.parameters
                ):
                ...
                if params_panel:
                    if ba.action.parameters:
                        ...
                        yield (
                            "  p.params_panel = new Lino.%s({});"
                            % params_panel.layout._formpanel_name
                        )
        ...
        # Note that wl may be None (if ba.action.extjs_main_panel is True)
        for ln in doit(wl, ba.get_window_size()):
            yield ln

        for xwl in ba.actor.extra_layouts.values():
            for ln in doit(xwl, xwl.window_size):
                yield ln
    ...
    def linolib_intro(self):
        ...
        def fn():
            yield "// lino.js --- generated %s by %s for %s." % (
                time.ctime(),
                escape(settings.SITE.site_version()),
                get_user_profile(),
            )
            # lino.__version__)
