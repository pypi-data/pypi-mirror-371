# Gracklepy's build system replaces __config__.py.in with __config__.py
...
def _report_error(variable_name, nominal_value, allowed_vals=None):
    ...
    raise RuntimeError(
        "There appears to have been an issue when gracklepy's build system "
...
def _is_editable_installation():
    """Returns whether gracklepy was built as an editable install"""
    state = r"wheel"
...
try:
    _grackle_build = _GrackleBuild[r"Embedded"]
