...
def configure(
    ...
    # Create config file
    config = _create_config_file(history_server_url, cost_estimator_id, live_app, connection_mode)
    ...
    if run_immediately:
        ...
        console.print("")
        _run_analyzer_after_config(connection_mode, history_server_url, cost_estimator_id, opt_out, debug, save_local)
    else:
        console.print("")
...
def _create_config_file(history_server_url, cost_estimator_id, live_app, connection_mode):
    ...
    config["connection"] = {"mode": "local" if connection_mode == "1" else "browser"}
    return config
...
def _run_analyzer_after_config(connection_mode, history_server_url, cost_estimator_id, opt_out, debug, save_local):
    """Run the analyzer after configuration is complete."""
    ...
    # Add save-local flag if provided
    if save_local:
        original_args_str = f"--save-local {original_args_str}"
    
    env = os.environ.copy()
    env['SPARK_ANALYZER_FROM_CONFIG'] = '1'
    
    if connection_mode == "1":
        # For local mode, pass the server URL
        _run_local_mode(history_server_url, cost_estimator_id, original_args_str, env)
    elif connection_mode == "2":
        # For browser mode, don't pass server URL (will be collected at runtime)
        _run_browser_mode(cost_estimator_id, original_args_str, env)

def _run_local_mode(history_server_url, cost_estimator_id, original_args_str, env):
    """Run analyzer in local mode."""
    try:
        if history_server_url != "http://localhost:18080/api/v1":
            # Custom URL
            if cost_estimator_id:
                subprocess.run(f"spark-analyzer analyze {original_args_str}--server-url '{history_server_url}' --cost-estimator-id '{cost_estimator_id}'", shell=True, env=env)
            else:
                subprocess.run(f"spark-analyzer analyze {original_args_str}--server-url '{history_server_url}'", shell=True, env=env)
        else:
            # Default URL
            if cost_estimator_id:
                subprocess.run(f"spark-analyzer analyze {original_args_str}--local --cost-estimator-id '{cost_estimator_id}'", shell=True, env=env)
            else:
                subprocess.run(f"spark-analyzer analyze {original_args_str}--local", shell=True, env=env)
...
def _run_browser_mode(cost_estimator_id, original_args_str, env):
    """Run analyzer in browser mode."""
    try:
        if cost_estimator_id:
            subprocess.run(f"spark-analyzer analyze {original_args_str}--browser --cost-estimator-id '{cost_estimator_id}'", shell=True, env=env)
        else:
            subprocess.run(f"spark-analyzer analyze {original_args_str}--browser", shell=True, env=env)
    except KeyboardInterrupt:
        console.print("\n🛑 Analysis interrupted by user.")
        console.print("💡 Tip: Your configuration is still saved and ready to use.")
        console.print("   You can run the analyzer later with: spark-analyzer")
