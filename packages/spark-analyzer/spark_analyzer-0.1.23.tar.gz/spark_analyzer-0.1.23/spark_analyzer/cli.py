...
def run_analysis(args):
    ...
    try:
        ...
        if args.save_local:
            ...
            # Always use enhanced mode (Onehouse s3 upload) when not using --save-local
    ...
    try:
        ...
        if command == 'configure':
            ...
            try:
                ...
                configure(opt_out=args.opt_out, debug=args.debug, save_local=args.save_local)
                sys.exit(0)
            except Exception as e:
                print_error_box(
                    "CONFIGURATION ERROR",
            ...
            _send_telemetry('command_execution')
            run_analysis(args)
        ...
        # Only handle KeyboardInterrupt if not running from config wizard
        if not os.environ.get('SPARK_ANALYZER_FROM_CONFIG'):
...
def _run_configuration_wizard(config, config_file_path, args=None):
    ...
    try:
        ...
        config, config_file_path = check_and_load_configuration()
        return config, config_file_path, run_immediately
    except Exception as e:
        print_error_box(
            "CONFIGURATION ERROR",
            f"Failed to run configuration wizard: {str(e)}",
        ...
        if os.getenv('SCARF_NO_ANALYTICS', '').lower() != 'true':
            def send_telemetry():
                try:
                    ...
                    requests.post(
                        'https://admin.gateway.scarf.sh/telemetry/spark-analyzer/cli-usage',
                        json=data,
                        timeout=2,
                        headers={
