...
class Saga:
    ...
    def step(self, step_id: str, action: Callable, compensation: Callable = None) -> 'Saga':
        """단계 추가 (플루언트 API)"""
        saga_step = SagaStep(
            step_id=step_id,
            action=action,
    ...
    async def execute(self, context: SagaContext) -> SagaContext:
        ...
        try:
            ...
            # 실패 - 보상 시작
            logger.error(f"Saga {self.saga_id} failed at step {context.current_step}: {e}")
    ...
    async def _execute_step(self, step: SagaStep, context: SagaContext) -> SagaContext:
        ...
        try:
            ...
            if step.attempts < step.retry_count:
                logger.warning(f"Step {step.step_id} failed, retrying ({step.attempts}/{step.retry_count})")
...
class SagaManager:
    ...
    async def start_saga(self, saga_id: str, context: SagaContext) -> SagaContext:
        ...
        self.running_sagas[context.correlation_id] = task
        
        self.total_executions += 1
        return context
    
    async def _execute_saga_with_events(self, saga: Saga, context: SagaContext) -> SagaContext:
        """이벤트와 함께 사가 실행"""
        try:
...
def build_saga_from_functions(saga_id: str, functions: List[Callable]) -> Saga:
    ...
    for func in functions:
        if hasattr(func, '_saga_step_id'):
            steps[func._saga_step_id] = func
        elif hasattr(func, '_compensation_for'):
            compensations[func._compensation_for] = func
...
class ReactiveSaga:
    ...
    def execute_with_retry(self, context: SagaContext, max_retries: int = 3) -> Mono[SagaContext]:
        """재시도와 함께 실행"""
        return (
