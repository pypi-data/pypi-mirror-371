...
class CloudRunOptimizer:
    ...
    def __init__(self, config: CloudRunConfig):
        ...
        # 전역 변수들 (Cold Start 감지용)
        self._instance_id = os.environ.get('INSTANCE_ID', f"instance_{int(time.time())}")
    ...
    def resource_monitor(self, func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            # 리소스 사용량 확인
            if (self.metrics.memory_usage > self.config.memory_threshold or 
                self.metrics.cpu_usage > self.config.cpu_threshold):
                logger.warning(
                    f"High resource usage: CPU={self.metrics.cpu_usage:.2f}, "
                    f"Memory={self.metrics.memory_usage:.2f}"
                )
...
async def optimize_cold_start(func: Callable, warm_up_data: Any = None) -> Callable:
    ...
    return Mono.from_callable(func).map(lambda f: functools.wraps(f)(
        lambda *args, **kwargs: asyncio.create_task(_warm_execute(f, warm_up_data, *args, **kwargs))
    )).await_result()
