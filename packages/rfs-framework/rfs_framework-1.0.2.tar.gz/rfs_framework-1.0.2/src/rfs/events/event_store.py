...
class EventStoreProtocol(Protocol):
    ...
    async def read_all_events(self, from_position: int = 0, max_count: int = None) -> List[Event]:
        """모든 이벤트 읽기"""
        ...
    
    async def get_stream_metadata(self, stream_id: str) -> Optional[EventStreamMetadata]:
        """스트림 메타데이터 조회"""
        ...
...
class MemoryEventStore:
    ...
    async def append_events(self, stream_id: str, events: List[Event], expected_version: int = -1) -> int:
        ...
        if stream_id not in self.streams:
            ...
            self.metadata[stream_id] = EventStreamMetadata(stream_id=stream_id)
...
class FileEventStore:
    ...
    async def _load_stream(self, stream_id: str):
        ...
        metadata = None
        try:
            async with aiofiles.open(self._get_metadata_file(stream_id), 'r', encoding='utf-8') as f:
        ...
        # 캐시에 저장
        self._cache.streams[stream_id] = events
        self._cache.metadata[stream_id] = metadata
        self._cache.all_events.extend(events)
    ...
    async def append_events(self, stream_id: str, events: List[Event], expected_version: int = -1) -> int:
        ...
        async with aiofiles.open(self._get_metadata_file(stream_id), 'w', encoding='utf-8') as f:
...
class EventStream:
    ...
    async def stream_events(self, from_version: int = 0, batch_size: int = 10) -> AsyncGenerator[Event, None]:
        """이벤트 스트리밍"""
...
class EventStore:
    ...
    async def create_snapshot(self, stream_id: str, snapshot_data: Dict[str, Any], version: int):
        """스냅샷 생성"""
...
def append_to_stream(stream: EventStream, *events: Event) -> asyncio.Task:
    """스트림에 이벤트 추가"""
    return asyncio.create_task(stream.append(*events))
