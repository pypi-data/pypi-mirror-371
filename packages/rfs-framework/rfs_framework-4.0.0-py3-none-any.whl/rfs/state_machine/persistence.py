...
class MemoryStatePersistence:
    ...
    def __init__(self):
        self.snapshots: Dict[str, StateMachineSnapshot] = {}
    
    async def save_snapshot(self, snapshot: StateMachineSnapshot) -> bool:
        """스냅샷 저장"""
        try:
            self.snapshots[snapshot.machine_name] = snapshot
            return True
...
class FileStatePersistence:
    ...
    def __init__(self, base_path: str = "./state_snapshots"):
        if not HAS_AIOFILES:
            raise ImportError("aiofiles is required for file persistence. Install with: pip install rfs-framework[eventstore]")
    ...
    async def save_snapshot(self, snapshot: StateMachineSnapshot) -> bool:
        ...
        try:
            ...
            if snapshot_dict['start_time']:
                snapshot_dict['start_time'] = snapshot_dict['start_time'].isoformat()
            snapshot_dict['snapshot_time'] = snapshot_dict['snapshot_time'].isoformat()
    ...
    async def load_snapshot(self, machine_name: str) -> Optional[StateMachineSnapshot]:
        ...
        try:
            ...
            if snapshot_dict['start_time']:
                snapshot_dict['start_time'] = datetime.fromisoformat(snapshot_dict['start_time'])
            snapshot_dict['snapshot_time'] = datetime.fromisoformat(snapshot_dict['snapshot_time'])
...
class RedisStatePersistence:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", key_prefix: str = "state_machine:"):
...
class PersistenceManager:
    ...
    def __init__(self, persistence_type: PersistenceType = PersistenceType.MEMORY, **kwargs):
    ...
    def _create_persistence(self, **kwargs) -> StatePersistence:
        ...
        elif self.persistence_type == PersistenceType.FILE:
            return FileStatePersistence(**kwargs)
        elif self.persistence_type == PersistenceType.REDIS:
    ...
    async def create_snapshot(self, state_machine) -> StateMachineSnapshot:
        ...
        return StateMachineSnapshot(
            ...
            current_state=state_machine.current_state.name if state_machine.current_state else None,
    ...
    async def restore_from_snapshot(self, state_machine, snapshot: StateMachineSnapshot):
        """스냅샷에서 상태 머신 복원"""
        # 현재 상태 설정
