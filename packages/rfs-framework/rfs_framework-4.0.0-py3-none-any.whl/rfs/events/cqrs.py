...
class Command(ABC):
    ...
    def __init__(self, command_id: str = None, correlation_id: str = None):
        ...
        self.correlation_id = correlation_id or f"corr_{int(datetime.now().timestamp())}"
...
class CommandBus:
    ...
    def __init__(self, event_bus: Optional[EventBus] = None, event_store: Optional[EventStore] = None):
    ...
    async def send(self, command: Command) -> CommandResult:
        ...
        try:
            ...
            if not handler:
                raise ValueError(f"No handler found for command: {type(processed_command).__name__}")
...
class QueryBus:
    ...
    async def send(self, query: Query) -> QueryResult:
        ...
        try:
            ...
            if not handler:
                raise ValueError(f"No handler found for query: {type(processed_query).__name__}")
...
# 함수형 헬퍼들
def create_command_result(success: bool, 
                         command_id: str, 
                         correlation_id: str,
...
def create_query_result(success: bool,
                       query_id: str,
                       data: Any = None,
