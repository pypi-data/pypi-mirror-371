...
class ActionRunner:
    ...
    timeout: int = 300  # 5분
    retry_count: int = 0
    on_failure: Optional[str] = None  # 실패 시 실행할 액션
    ...
    async def execute(self, context: Dict[str, Any]) -> Result[Dict[str, Any], str]:
        ...
        try:
            ...
            if self.action_type == ActionType.COMMAND:
            ...
            elif self.action_type == ActionType.HTTP_REQUEST:
            ...
            elif self.action_type == ActionType.GIT_OPERATION:
                return await self._execute_git_operation(context)
            elif self.action_type == ActionType.DEPLOYMENT:
    ...
    async def _execute_command(self, context: Dict[str, Any]) -> Result[Dict[str, Any], str]:
        """명령어 실행"""
        try:
            ...
            if process.returncode == 0:
                ...
                return Failure(f"명령어 실행 실패 (코드: {process.returncode}): {result['stderr']}")
                
        except Exception as e:
            return Failure(f"명령어 실행 오류: {str(e)}")
    
    async def _execute_script(self, context: Dict[str, Any]) -> Result[Dict[str, Any], str]:
        """스크립트 실행"""
        try:
            ...
            if process.returncode == 0:
                ...
                return Failure(f"스크립트 실행 실패: {result['stderr']}")
...
class AutomationEngine:
    ...
    async def _load_configuration(self) -> None:
        ...
        for workflow_name, actions_config in config.get('workflows', {}).items():
            ...
            self.workflows[workflow_name] = actions
            ...
            'triggers': [
                {
                    ...
                    'enabled': True,
                    'metadata': {
                        'description': 'main 브랜치 푸시 시 CI/CD 실행'
                    }
                },
                {
            ...
            'workflows': {
                'ci_pipeline': [
                    ...
                    },
                    {
                        ...
                        'type': 'command',
                        'config': {
                            'command': 'docker build -t rfs-app:latest .'
                        },
                        'timeout': 600
                    }
                ],
    ...
    async def _event_processor(self) -> None:
        ...
        while self.running:
            try:
                ...
                if matched_triggers:
                    if console:
                        console.print(f"🎯 이벤트 매칭: {len(matched_triggers)}개 트리거")
    ...
    async def _execute_workflow(self, workflow_name: str, event: Dict[str, Any], trigger: WorkflowTrigger) -> None:
        """워크플로우 실행"""
        try:
            ...
            for action in actions:
                ...
                action_result = {
                    ...
                    'result': result.unwrap() if result.is_success() else result.unwrap_err()
                }
                ...
                if result.is_failure():
                    ...
                    execution_result['error_message'] = result.unwrap_err()
                    ...
                    if action.on_failure:
                        if console:
                            console.print(f"🔄 실패 처리 액션 실행: {action.on_failure}")
                        # TODO: 실패 처리 액션 실행 구현
