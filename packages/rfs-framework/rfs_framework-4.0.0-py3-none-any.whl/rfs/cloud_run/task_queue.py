...
if PYDANTIC_AVAILABLE:
    class TaskDefinition(BaseModel):
        ...
        handler_path: str = Field(
            ...,
            regex=r"^/[a-zA-Z0-9\-_/]*$",
            description="작업 처리 핸들러 경로"
        )
        payload: Dict[str, Any] = Field(
            default_factory=dict,
            description="작업 페이로드 데이터"
        )
        ...
        retry_min_backoff: int = Field(
            default=1,
            ge=1,
            description="최소 재시도 백오프 (초)"
        )
        retry_max_backoff: int = Field(
            default=300,
            ge=1,
            description="최대 재시도 백오프 (초)"
        )
        ...
        def validate_payload(cls, v: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if len(payload_str.encode('utf-8')) > 100 * 1024:  # 100KB
        ...
        def to_cloud_task(self, target_url: str) -> Dict[str, Any]:
            ...
            if schedule_time > datetime.now():
                ...
                task_data['schedule_time'] = timestamp
...
class CloudTaskQueue:
    ...
    def __init__(self, 
        ...
        self.service_url = service_url or f"https://{os.environ.get('K_SERVICE', 'localhost')}-{os.environ.get('K_REVISION', 'dev')}.a.run.app"
        ...
        # 큐 설정
        self.queues: Dict[str, str] = {}  # queue_name -> queue_path
    ...
    async def create_queue(self, 
                          ...
                          max_dispatches_per_second: float = 100.0) -> Result[str, str]:
        """큐 생성"""
        ...
        try:
            ...
            self.queues[queue_name] = queue.name
    ...
    def register_handler(self, path: str, handler: Callable[[Dict[str, Any]], Any]):
        """작업 핸들러 등록"""
        self.task_handlers[path] = handler
    ...
    async def submit_task(self, task: TaskDefinition) -> Result[str, str]:
        ...
        try:
            ...
            cloud_task['name'] = f"{queue_path}/tasks/{task.task_id}"
    ...
    async def submit_batch_tasks(self, tasks: List[TaskDefinition]) -> Dict[str, Result[str, str]]:
        """배치 작업 제출"""
        results = {}
        ...
        for task in tasks:
            ...
            submit_tasks.append((task.task_id, submit_task))
        ...
        for task_id, submit_task in submit_tasks:
            try:
                ...
                results[task_id] = Failure(f"배치 제출 오류: {str(e)}")
        
        success_count = len([r for r in results.values() if isinstance(r, Success)])
    ...
    async def _publish_task_event(self, event_type: str, task: TaskDefinition, error: str = None):
        """작업 이벤트 발행"""
        try:
    ...
    def get_queue_stats(self, queue_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'pending_tasks': 0,  # Cloud Monitoring에서 조회
            'in_flight_tasks': 0,
            'completed_tasks': 0,
            'failed_tasks': 0
        }
...
class TaskScheduler:
    ...
    async def schedule_daily(self, 
                           ...
                           hour: int = 9,
                           minute: int = 0) -> Result[str, str]:
        """매일 반복 작업 스케줄링"""
        from datetime import date
        
        tomorrow = date.today() + timedelta(days=1)
        schedule_time = datetime.combine(tomorrow, datetime.min.time().replace(hour=hour, minute=minute))
    ...
    async def schedule_weekly(self,
        ...
        schedule_time = datetime.combine(next_occurrence, datetime.min.time().replace(hour=hour))
