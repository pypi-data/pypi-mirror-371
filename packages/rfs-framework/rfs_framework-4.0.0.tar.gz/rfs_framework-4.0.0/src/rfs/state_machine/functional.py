...
from typing import Dict, Any, List, Optional, Callable, Tuple, NamedTuple, Union
...
# State 관련 함수들
def create_state(name: str, 
                state_type: StateType = StateType.NORMAL,
                entry_action: Optional[Callable] = None,
                exit_action: Optional[Callable] = None) -> State:
    """순수 함수: 상태 생성"""
    return State(
        name=name,
...
def update_state_stats(state: State, 
                      enter_count: int = None,
                      exit_count: int = None,
                      total_time: float = None) -> State:
    """순수 함수: 상태 통계 업데이트"""
    return State(
        name=state.name,
...
# Transition 관련 함수들
def create_transition(from_state: str,
                     to_state: str,
                     event: str,
                     guard: Optional[Callable] = None,
                     action: Optional[Callable] = None,
                     transition_type: TransitionType = TransitionType.EXTERNAL) -> Transition:
    """순수 함수: 전이 생성"""
    return Transition(
...
def update_transition_stats(transition: Transition,
                           execution_count: int = None,
                           failure_count: int = None,
                           duration_ms: float = None) -> Transition:
    """순수 함수: 전이 통계 업데이트"""
    return Transition(
        ...
        execution_count=execution_count if execution_count is not None else transition.execution_count,
        failure_count=failure_count if failure_count is not None else transition.failure_count,
        total_duration_ms=(transition.total_duration_ms + duration_ms) if duration_ms is not None else transition.total_duration_ms
    )
...
def add_transition_to_machine(machine: StateMachineState, transition: Transition) -> StateMachineState:
    """순수 함수: 상태 머신에 전이 추가"""
    ...
    if key not in new_transitions:
        new_transitions[key] = []
    new_transitions[key].append(transition)
...
def add_event_to_queue(machine: StateMachineState, event: MachineEvent) -> StateMachineState:
    """순수 함수: 이벤트 큐에 이벤트 추가"""
    if machine.machine_state != MachineState.RUNNING:
        return machine
...
def process_all_events(machine: StateMachineState) -> StateMachineState:
    ...
    while current_machine.event_queue and current_machine.machine_state == MachineState.RUNNING:
...
def get_machine_stats(machine: StateMachineState) -> Dict[str, Any]:
    ...
    return {
        ...
        "total_transitions_defined": sum(len(trans) for trans in machine.transitions.values()),
        "total_transitions_executed": machine.total_transitions,
        "failed_transitions": machine.failed_transitions,
        "success_rate": (machine.total_transitions - machine.failed_transitions) / max(machine.total_transitions, 1),
...
# 고차 함수들
def map_states(machine: StateMachineState, mapper: Callable[[State], State]) -> StateMachineState:
    """고차 함수: 모든 상태에 함수 적용"""
...
def filter_transitions(machine: StateMachineState, predicate: Callable[[Transition], bool]) -> StateMachineState:
    """고차 함수: 전이 필터링"""
    new_transitions = {}
    for key, transitions in machine.transitions.items():
        filtered = [t for t in transitions if predicate(t)]
        if filtered:
            new_transitions[key] = filtered
...
def fold_events(machine: StateMachineState, 
               folder: Callable[[Any, MachineEvent], Any], 
               initial: Any) -> Any:
    """고차 함수: 이벤트 히스토리 폴드"""
