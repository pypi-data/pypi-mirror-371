...
class Transition:
    ...
    async def can_execute(self, context: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            print(f"Guard execution failed for transition {self.from_state.name} -> {self.to_state.name}: {e}")
            return False
    ...
    async def _execute_action(self, action, context: Dict[str, Any]):
        ...
        try:
            ...
            print(f"Action execution failed for transition {self.from_state.name} -> {self.to_state.name}: {e}")
            raise
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_duration_ms": self.total_duration_ms / max(self.execution_count, 1)
        }
...
class TransitionBuilder:
    ...
    def guard(self, guard_func: Callable[[Dict[str, Any]], bool]) -> 'TransitionBuilder':
        """가드 조건 설정"""
        self.guard = guard_func
        return self
    
    def action(self, action_func: Callable[[Dict[str, Any]], None]) -> 'TransitionBuilder':
        """액션 설정"""
