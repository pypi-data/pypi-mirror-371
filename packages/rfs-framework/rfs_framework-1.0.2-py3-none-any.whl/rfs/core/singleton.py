...
class StatelessRegistry:
    ...
    @classmethod
    def register(cls, name: str = None, dependencies: list = None):
        """
        무상태 서비스 등록 데코레이터
    ...
    @classmethod
    def _create_instance(cls, service_class: Type, service_name: str):
        """의존성 주입을 통한 인스턴스 생성"""
...
def inject(*dependency_names: str):
    """
    의존성 주입 데코레이터
...
# 미리 정의된 데코레이터들
component = lambda name=None: StatelessRegistry.register(name)
service = lambda name=None: StatelessRegistry.register(name)
repository = lambda name=None: StatelessRegistry.register(name)
...
if __name__ == "__main__":
    ...
    class DataProcessor:
        ...
        def __init__(self, calculator=None, logger=None):
            # 의존성은 생성자에서 주입받지만 상태로 저장하지 않음
            pass
        
        @inject('calculator', 'logger')
        def process_numbers(self, numbers: list, calculator, logger) -> float:
            """숫자 리스트 처리"""
