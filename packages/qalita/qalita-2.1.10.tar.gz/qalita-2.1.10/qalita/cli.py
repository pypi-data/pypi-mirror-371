...
@click.option("--ui", is_flag=True, help="Open the local web UI dashboard")
@click.option("--port", default=7070, show_default=True, type=int, help="Port for the local web UI")
@click.pass_context
def cli(ctx, ui=False, port=7070):
    """
    ...
    if ui:
        try:
            ...
            run_dashboard_ui(cfg, host="127.0.0.1", port=port)
        raise SystemExit(0)
