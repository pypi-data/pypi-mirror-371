...
def push_programmatic(config, skip_validate: bool = False, approve_public: bool = False):
    ...
    try:
        ...
        r = send_api_request(request="/api/v2/sources", mode="get")
        if r.status_code == 200:
...
def add(config):
    ...
    source["type"] = click.prompt(
        "Source type (file, folder, postgresql, mysql, oracle, mssql, sqlite, mongodb, s3, gcs, azure_blob, hdfs, sftp, http, https, ...)",
    )
