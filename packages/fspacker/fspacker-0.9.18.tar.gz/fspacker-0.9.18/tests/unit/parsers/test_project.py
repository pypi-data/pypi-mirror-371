...
class TestPoetryProject:
    ...
    def test_helloworld_embed_win64(
        ...
        monkeypatch.setattr(
            "fspacker.settings._settings.dirs.embed",
            tmp_path / "embed",
        )
        assert (
            info.embed_filepath
    ...
    def test_pygame(self, mock_pygame_normal_dir: Path) -> None:
        """Test pygame project."""
        ...
        assert (
            info.exe_file == mock_pygame_normal_dir / "dist" / "test_pygame.exe"
        )
        assert info.min_python_version == "3.12.10"

    def test_pyside2(self, mock_pyside2_normal_dir: Path) -> None:
        ...
        assert (
            info.exe_file
            == mock_pyside2_normal_dir / "dist" / "test_pyside2.exe"
        )
...
class TestProjectIsValidEntry:
    ...
    def test_file_with_ignored_folder_name(
        ...
        monkeypatch.setattr(
            "fspacker.settings._settings.ignore_folders",
            ["test_folder"],
        )
