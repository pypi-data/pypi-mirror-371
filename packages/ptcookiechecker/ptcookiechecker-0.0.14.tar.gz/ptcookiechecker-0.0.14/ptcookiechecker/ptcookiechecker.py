...
class PtCookieChecker:
    ...
    def _test_cookie_injection(self, url: str) -> None:
        ...
        test_cases = {
            "empty_value": {"test": ""},
            "invalid_chars": {"test": "aaa;bbb"}
        }

        ptprinthelper.ptprint(" ", bullet_type="TEXT", condition=not self.use_json)

        fpd_regex = re.compile(
            r"(Warning|Fatal error|Notice|in\s+[A-Z]:\\|in\s+/.*?:\d+)",
            re.IGNORECASE
        )

        for case_name, cookies in test_cases.items():
            try:
                # Create a copy of the current headers
                headers_with_cookie = dict(self.args.headers or {})

                # Convert cookies dict to header format: key=value; key2=value2
                ...
                if fpd_regex.search(response.text):
                    note = f"FPD detected with cookie injection ({case_name}): {cookies}"
                ...
                # If the server rejects the cookie request entirely, skip to next
                continue
def get_help():
    return [
