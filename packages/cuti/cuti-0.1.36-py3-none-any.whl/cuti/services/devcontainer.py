...
# Create a proper Claude wrapper to avoid stack overflow issues
# The npm-installed claude command sometimes has issues with recursive calls
RUN mv /usr/bin/claude /usr/bin/claude.orig 2>/dev/null || true && \\
    echo '#!/bin/bash' > /usr/bin/claude && \\
    echo '# TEST COMMENT TO VERIFY EDITS ARE TAKING EFFECT' >> /usr/bin/claude && \\
    echo '# Claude wrapper to ensure proper Node.js execution' >> /usr/bin/claude && \\
    echo 'export NODE_OPTIONS="--max-old-space-size=4096"' >> /usr/bin/claude && \\
    echo '# Set HOME based on current user and environment' >> /usr/bin/claude && \\
    echo 'CURRENT_USER=$(whoami)' >> /usr/bin/claude && \\
    echo 'if [ "$CURRENT_USER" = "root" ]; then' >> /usr/bin/claude && \\
    echo '    export HOME=/root' >> /usr/bin/claude && \\
    echo '    export CLAUDE_CONFIG_DIR=${CLAUDE_CONFIG_DIR:-/root/.claude}' >> /usr/bin/claude && \\
    echo 'else' >> /usr/bin/claude && \\
    echo '    export HOME=/home/$CURRENT_USER' >> /usr/bin/claude && \\
    echo '    export CLAUDE_CONFIG_DIR=${CLAUDE_CONFIG_DIR:-/home/$CURRENT_USER/.claude}' >> /usr/bin/claude && \\
    echo 'fi' >> /usr/bin/claude && \\
    echo '# Create config directory if it doesn'"'"'t exist' >> /usr/bin/claude && \\
    echo 'mkdir -p "$CLAUDE_CONFIG_DIR" 2>/dev/null || true' >> /usr/bin/claude && \\
    echo 'exec /usr/bin/node /usr/lib/node_modules/@anthropic-ai/claude-code/cli.js "$@"' >> /usr/bin/claude && \\
...
# Create a Claude wrapper script to ensure correct environment (as root)
USER root

# Claude wrapper was already created earlier, so just create aliases here
RUN ln -sf /usr/bin/claude /usr/local/bin/claude && \\
    ln -sf /usr/bin/claude /usr/local/bin/claude-safe
...
# Claude wrapper was already created, no need to recreate it
