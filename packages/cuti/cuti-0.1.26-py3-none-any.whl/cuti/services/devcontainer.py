...
'''
    ...
    def run_in_container(self, command: Optional[str] = None, rebuild: bool = False) -> int:
        ...
        # Debug: Print mount information
        current_dir = Path.cwd()
        print(f"🔍 Mounting host directory: {current_dir}")
        ...
        if command:
            # Add workspace diagnostics before running the command
            full_command = f"echo '🔍 Container workspace contents:'; ls -la /workspace; echo ''; {command}"
            docker_args.extend(["/bin/zsh", "-lc", full_command])
        else:
            # Add workspace diagnostics for interactive shell
            startup_command = "echo '🔍 Container workspace contents:'; ls -la /workspace; echo ''"
