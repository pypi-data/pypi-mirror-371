...
'''
    ...
    def run_in_container(self, command: Optional[str] = None, rebuild: bool = False) -> int:
        ...
        # Create a Docker volume for persistent Claude credentials
        volume_name = "cuti-claude-config"
        ...
        docker_args = [
            ...
            "-v", f"{volume_name}:/home/cuti/.claude",  # Use Docker volume for persistent Claude config
            "-w", "/workspace",
        ...
        # Setup initialization command for the Docker volume  
        init_script = """
# Ensure proper permissions on Claude config directory
sudo chown -R cuti:cuti /home/cuti/.claude 2>/dev/null || true

# Check if Claude config volume is already initialized
if [ ! -f /home/cuti/.claude/.initialized ]; then
    echo "🔧 Initializing Claude config volume..."
    # Create necessary directories
    mkdir -p /home/cuti/.claude/plugins/repos /home/cuti/.claude/todos /home/cuti/.claude/sessions
    mkdir -p /home/cuti/.claude/settings /home/cuti/.claude/projects /home/cuti/.claude/projects/-workspace
    
    # Copy CLAUDE.md from host if it exists  
    if [ -f /root/.cuti-global/CLAUDE.md ]; then
        cp /root/.cuti-global/CLAUDE.md /home/cuti/.claude/CLAUDE.md 2>/dev/null || true
    fi
    
    # Copy CLAUDE.md from workspace if it exists
    if [ -f /workspace/CLAUDE.md ]; then
        cp /workspace/CLAUDE.md /home/cuti/.claude/CLAUDE.md 2>/dev/null || true
    fi
    
    # Mark as initialized
    touch /home/cuti/.claude/.initialized
    echo "✅ Claude config volume initialized"
else
    echo "✅ Using existing Claude config from volume"
fi

# Check for existing credentials
if [ -f /home/cuti/.claude/.credentials.json ]; then
    echo "🔑 Found saved Claude credentials"
else
    echo "⚠️  No saved credentials. Authenticate with: claude login"
fi
"""
    ...
    def clean(self, clean_volume: bool = False) -> bool:
        ...
        # Optionally remove Claude config volume
        if clean_volume:
            self._run_command(["docker", "volume", "rm", "-f", "cuti-claude-config"])
            print("✅ Removed Claude config volume (credentials deleted)")
