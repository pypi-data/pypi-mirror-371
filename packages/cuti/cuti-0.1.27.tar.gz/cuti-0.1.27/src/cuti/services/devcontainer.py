...
# Install Claude Code CLI (claude is installed at /usr/bin/claude by npm)
...
RUN sh -c "$(wget -O- https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended \\
    ...
    && echo 'echo "🚀 Welcome to cuti dev container!"' >> ~/.zshrc \\
    && echo 'echo ""' >> ~/.zshrc \\
    && echo 'if [ "$(ls -A /workspace 2>/dev/null | wc -l)" -eq 0 ]; then' >> ~/.zshrc \\
    && echo '  echo "   📁 Workspace is empty - this is your host directory mounted at /workspace"' >> ~/.zshrc \\
    && echo '  echo "   💡 To get started:"' >> ~/.zshrc \\
    && echo '  echo "     • Create files here and they'\''ll appear on your host machine"' >> ~/.zshrc \\
    && echo '  echo "     • Or run '\''cuti container'\'' from a directory with existing files"' >> ~/.zshrc \\
    && echo '  echo ""' >> ~/.zshrc \\
    && echo 'else' >> ~/.zshrc \\
    && echo '  echo "   📁 Your host directory is mounted at /workspace"' >> ~/.zshrc \\
    && echo '  echo "   💾 Changes you make will persist on your host machine"' >> ~/.zshrc \\
    && echo '  echo ""' >> ~/.zshrc \\
    && echo 'fi' >> ~/.zshrc \\
    && echo 'echo "   🛠️  Commands available:"' >> ~/.zshrc \\
    && echo 'echo "     • cuti web        - Start web interface"' >> ~/.zshrc \\
    && echo 'echo "     • cuti cli        - Start CLI"' >> ~/.zshrc \\
    && echo 'echo "     • claude          - Claude Code CLI (auto-aliased)"' >> ~/.zshrc \\
    && echo 'echo "     • ls              - List files in your project"' >> ~/.zshrc \\
    && echo 'echo ""' >> ~/.zshrc
...
'''
    ...
    def run_in_container(self, command: Optional[str] = None, rebuild: bool = False) -> int:
        ...
        print(f"🔍 Host directory: {current_dir}")
        print(f"🔍 Host directory contents: {list(current_dir.iterdir()) if current_dir.exists() else 'Does not exist!'}")
        
        # Debug the Docker command
        mount_arg = f"{current_dir}:/workspace"
        print(f"🔍 Docker mount argument: -v {mount_arg}")
        ...
        if command:
            # Add mount verification before running the actual command
            full_command = f"echo 'Mount verification:'; ls -la /workspace; echo 'Running command...'; {command}"
            docker_args.extend(["/bin/zsh", "-lc", full_command])
        else:
            # Add mount verification for interactive shell
            startup_command = "echo 'Mount verification:'; ls -la /workspace; echo ''"
            docker_args.extend(["/bin/zsh", "-lc", f"{startup_command}; exec /bin/zsh -l"])
