...
RUN echo '#!/bin/bash' > /usr/local/bin/claude-wrapper && \\
    echo 'export HOME=/home/cuti' >> /usr/local/bin/claude-wrapper && \\
    echo 'export CLAUDE_CONFIG_DIR=/home/cuti/.claude' >> /usr/local/bin/claude-wrapper && \\
    echo 'exec /usr/bin/claude "$@"' >> /usr/local/bin/claude-wrapper && \\
...
# Create Claude wrapper as root before switching to cuti user
RUN echo '#!/bin/bash' > /usr/local/bin/claude-safe && \\
    echo 'export HOME=/home/cuti' >> /usr/local/bin/claude-safe && \\
    echo 'export CLAUDE_CONFIG_DIR=/home/cuti/.claude' >> /usr/local/bin/claude-safe && \\
    echo 'exec /usr/bin/claude "$@"' >> /usr/local/bin/claude-safe && \\
...
'''
    ...
    def run_in_container(self, command: Optional[str] = None, rebuild: bool = False) -> int:
        ...
        # Check if we're running as root or cuti user
        claude_config_mount = [
        ...
        # Wrap the command to ensure proper environment setup
        if command:
            # Ensure environment variables are set correctly for cuti user
            # Use -c instead of -lc to avoid login shell overriding HOME
            wrapped_command = f"export HOME=/home/cuti && export CLAUDE_CONFIG_DIR=/home/cuti/.claude && source /home/cuti/.zshrc && {command}"
            docker_args.extend(["/bin/zsh", "-c", wrapped_command])
        else:
