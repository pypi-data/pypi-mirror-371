...
from convert_stream.enum_libs.modules import (
    MOD_IMG_OPENCV, MOD_IMG_PIL, ModuleImage, DEFAULT_LIB_IMAGE,
    cv2, MatLike, Image, ImageOps, ImageFilter
)
...
class ImageObject(object):
    ...
    def to_opencv(self) -> MatLike:
        ...
        return cv2.cvtColor(np.asarray(bt, dtype=np.uint8), cv2.COLOR_RGB2BGR)

    def to_numpy(self) -> np.ndarray:
        bt: BytesIO = self.to_bytes()
        return np.asarray(bt, dtype=np.uint8)
