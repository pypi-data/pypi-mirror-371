...
from convert_stream.enum_libs.enums import LibImageToPdf
from convert_stream.enum_libs.modules import (
    ModImageToPdf, MOD_CANVAS, DEFAULT_LIB_IMAGE_TO_PDF, canvas, Canvas, letter, ImageReader
)
...
class ABCConvertImageToPdf(ABC):
    ...
    def __init__(self, collection_images: CollectionImages = CollectionImages()):
...
class ImplementImageToPdfCanvas(ABCConvertImageToPdf):
    ...
    def to_document(self, land_scape: bool = True, a4: bool = False) -> DocumentPdf:
        ...
        first_img_obj = self.collection_images.images[0]
...
class ImplementImageToPdfPil(ABCConvertImageToPdf):
    ...
    def to_document(self, land_scape: bool = True, a4: bool = False) -> DocumentPdf:
        ...
        if self.collection_images.images:
            raise ValueError("A lista de imagens não pode estar vazia.")
        ...
        pil_images_to_save = []
        if a4:
            # Obtém as dimensões da página A4 em pixels
            ...
            for num, img_obj in enumerate(self.collection_images.images):
                self.pbar.update(
                    ((num + 1) / max_num) * 100,
                ...
                # Cria uma nova imagem de fundo branca com o tamanho A4
            ...
            pil_images_to_save = [img.to_pil() for img in self.collection_images.images]
...
class ConvertImageToPdf(object):
    ...
    def __init__(
                self,
                collection_images: CollectionImages = CollectionImages(),
                *,
