...
class BotClient:
    ...
    async def pin_chat_message(self, chat_id: str, message_id: str, disable_notification: Optional[bool] = False):
        ...
        return await self._make_request("pinChatMessage", payload)
    ...
    async def unpin_chat_message(self, chat_id: str, message_id: str):
        ...
        return await self._make_request("unpinChatMessage", payload)
    ...
    async def unpin_all_chat_messages(self, chat_id: str, message_id: str):
        ...
        return await self._make_request("unpinAllChatMessages", payload)
    ...
    async def process_update(self, update: Union[Update, InlineMessage]):
        update_type = (
            "InlineMessage" if isinstance(update, InlineMessage) else update.type
        )
        ...
        logger.info(
            f"Processing update: {update_type}, handlers available: {len(self.handlers)}"
        )
        handled = False
        for filter_key, handler_list in self.handlers.items():
            logger.info(
                f"Checking handler list {filter_key} with {len(handler_list)} handlers"
            )
            for filters, handler in handler_list:
                logger.info(
                    f"Checking filters for handler {handler.__name__}: {filters}"
                )
                if await self._filters_pass(update, filters):
                    try:
                        logger.info(
                            f"Executing handler {handler.__name__} for update {update_type}"
                        )
                    ...
                    logger.info(
                        f"Filters {filters} failed for handler {handler.__name__} on update {update_type}"
                    )
        if not handled:
            logger.warning(
                f"No handlers matched for update {update_type} with data: {update}"
            )
    ...
    async def _filters_pass(
        ...
        for f in filters:
            ...
            logger.info(
                f"Filter {f.__class__.__name__} check for update {update_type}: {result}"
            )
            if not result:
                return False
    ...
    async def download_file(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(download_url) as response:
