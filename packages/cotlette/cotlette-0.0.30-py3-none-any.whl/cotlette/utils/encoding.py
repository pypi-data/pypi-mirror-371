...
from cotlette.utils.functional import Promise


class CotletteUnicodeDecodeError(UnicodeDecodeError):
    def __str__(self):
...
def force_str(s, encoding="utf-8", strings_only=False, errors="strict"):
    ...
    try:
        ...
        raise CotletteUnicodeDecodeError(*e.args) from None
    return s
...
def iri_to_uri(iri):
    ...
    Take an IRI (string or UTF-8 bytes, e.g. '/I ♥ Cotlette/' or
    b'/I \xe2\x99\xa5 Cotlette/') and return a string containing the encoded
    result with ASCII chars only (e.g. '/I%20%E2%99%A5%20Cotlette/').
    """
...
def uri_to_iri(uri):
    ...
    Take an URI in ASCII bytes (e.g. '/I%20%E2%99%A5%20Cotlette/') and return
    a string containing the encoded result (e.g. '/I%20♥%20Cotlette/').
    """
    if uri is None:
        return uri
    uri = force_bytes(uri)
