...
This is a simple server for use in testing or debugging Cotlette apps. It hasn't
...
from cotlette.core.handlers.wsgi import LimitedStream
...
logger = logging.getLogger("cotlette.server")
...
def get_internal_wsgi_application():
    ...
    for Cotlette's internal server (runserver); external WSGI servers should just
    be configured to point to the correct application object directly.

    If settings.WSGI_APPLICATION is not set (is ``None``), return
    whatever ``cotlette.core.wsgi.get_wsgi_application`` returns.
    """
