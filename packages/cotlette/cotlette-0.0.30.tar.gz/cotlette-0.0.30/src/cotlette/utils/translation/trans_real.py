...
class CotletteTranslation(gettext_module.GNUTranslations):
    ...
    def __init__(self, language, domain=None, localedirs=None):
        ...
        if self.domain == "fastapi":
            if localedirs is not None:
                # A module-level cache is used for caching 'fastapi' translations
                warnings.warn(
                    "localedirs is ignored when domain is 'fastapi'.", RuntimeWarning
                )
                localedirs = None
...
def translation(language):
    """
    Return a translation object in the default 'fastapi' domain.
    """
...
def check_for_language(lang_code):
    ...
    if lang_code is None or not language_code_re.search(lang_code):
        return False
    return any(
        gettext_module.find("fastapi", path, [to_locale(lang_code)]) is not None
