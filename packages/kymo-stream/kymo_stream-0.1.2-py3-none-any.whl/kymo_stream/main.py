...
@click.option(
    "--pose/--no-pose",
    default=False,
    help="Enable pose detection with YOLO (overrides --detect)",
)
@click.option(
    "--model",
    "-m",
    default=None,
    type=str,
    help="YOLO model to use (default: yolo11n.pt for detection, yolo11n-pose.pt for pose)",
)
@click.option(
    "--confidence",
    "-c",
...
def main(device, width, height, fps, detect, pose, model, confidence, skip_frames):
    ...
    if detection_enabled:
        click.echo(f"Loading YOLO {detection_mode} model: {model}")
        try:
            ...
            click.echo("Continuing without detection", err=True)
            detection_enabled = False
            yolo_model = None
            detection_mode = None
    ...
    if yolo_model is not None:
        ...
        mode_text = f"{detection_mode.capitalize()} detection" if detection_mode else "Detection"
        click.echo(f"  {mode_text}: {'ON' if detection_enabled else 'OFF'}")
        if skip_frames > 1:
        ...
        while True:
            ...
            if yolo_model is not None and detection_enabled:
                ...
                if frame_count % skip_frames == 0:
                    ...
                    # Count detections based on mode
                    if detection_mode == "pose":
                        # For pose, count number of people detected
                        num_detections = len(results[0].keypoints) if hasattr(results[0], 'keypoints') and results[0].keypoints is not None else 0
                        if num_detections > 0:
                            click.echo(f"\rPose detections: {num_detections} person(s)", nl=False)
                    else:
            ...
            if yolo_model is not None:
                mode_label = f"{detection_mode.capitalize()}" if detection_mode else "Detection"
                status_text = f"{mode_label}: {'ON' if detection_enabled else 'OFF'}"
