...
class TabDefine(QWidget):
    def __init__(self, _p):
        ...
        # BOX - Tab desc Flag
        self.tab_sd_descf_table = DefaultTableWidget(["Column_value", "Type", "Length", "Description", "CL_OID"])
        ...
        self.tab_ad_descf_table = DefaultTableWidget(["Column_value", "Type", "Length", "Description", "CL_OID"])
    ...
    def util_get_define_meta_info(folder):
        ...
        if xml_file:
            try:
                ...
                meta_info_list.append({"key": "error", "value": "XML version not found in define.xml", "ns": ""})
    ...
    def tab_button_1_f(self):
        ...
        if folder:
            ...
            flag_rows = self.tab_sd_igdef_oid_desc[self.tab_sd_igdef_oid_desc["description"].str.contains("Flag", case=False, na=False)]
            flag_last_sections = [str(row["column"]).split(".")[-1] for _, row in flag_rows.iterrows()]
            if not flag_rows.empty:
                extra_flag_rows = self.tab_sd_igdef_oid_desc[
                    self.tab_sd_igdef_oid_desc["description"].apply(
                        lambda desc: any(flag_section.lower() in str(desc).lower() for flag_section in flag_last_sections)
                    )
                ]
                if not extra_flag_rows.empty:
                    flag_rows = pd.concat([flag_rows, extra_flag_rows]).drop_duplicates()

            self.tab_sd_descf_table.setRowCount(0)
            for row_idx, row in flag_rows.iterrows():
                item1 = QTableWidgetItem(str(row["column"]))
                item2 = QTableWidgetItem(str(row["type"]))
                item3 = QTableWidgetItem(str(row["length"]))
                item4 = QTableWidgetItem(str(row["description"]))
                ...
                self.tab_sd_descf_table.setItem(row_idx, 0, item1)
                self.tab_sd_descf_table.setItem(row_idx, 1, item2)
                self.tab_sd_descf_table.setItem(row_idx, 2, item3)
                self.tab_sd_descf_table.setItem(row_idx, 3, item4)
                self.tab_sd_descf_table.setItem(row_idx, 4, item5)
    ...
    def tab_button_2_f(self):
        ...
        if folder:
            ...
            flag_rows = self.tab_ad_igdef_oid_desc[self.tab_ad_igdef_oid_desc["column"].str.endswith("FL", na=False)]
            flag_last_sections = [str(row["column"]).split(".")[-1] for _, row in flag_rows.iterrows()]
            if not flag_rows.empty:
                extra_flag_rows = self.tab_ad_igdef_oid_desc[
                    self.tab_ad_igdef_oid_desc["description"].apply(
                        lambda desc: any(flag_section.lower() in str(desc).lower() for flag_section in flag_last_sections)
                    )
                ]
                if not extra_flag_rows.empty:
            ...
            for row_idx, row in flag_rows.iterrows():
                ...
                self.tab_ad_descf_table.setItem(row_idx, 0, item1)
                self.tab_ad_descf_table.setItem(row_idx, 1, item2)
                self.tab_ad_descf_table.setItem(row_idx, 2, item3)
                self.tab_ad_descf_table.setItem(row_idx, 3, item4)
                self.tab_ad_descf_table.setItem(row_idx, 4, item5)
    ...
    def tab_button_3_f(self):
        ...
        file_path = os.path.join(file_path_base, "log_sd_igdef.csv")
        try:
        ...
        # Save tab_sd_igdef_oid (list)
        file_path = os.path.join(file_path_base, "log_sd_igdef_oid.txt")
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                for item in self.tab_sd_igdef_oid:
                    f.write(str(item) + "\n")
            mtbp3cd.gui.util_show_message(self.message_list, f"sd_igdef_oid saved: {file_path}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export sd_igdef_oid: {e}", status="f")

        # Save tab_sd_igdef_oid_desc
        file_path = os.path.join(file_path_base, "log_sd_igdef_oid_desc.csv")
        try:
        ...
        file_path = os.path.join(file_path_base, "log_ad_igdef.csv")
        try:
        ...
        # Save tab_ad_igdef_oid (list)
        file_path = os.path.join(file_path_base, "log_ad_igdef_oid.txt")
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                for item in self.tab_ad_igdef_oid:
                    f.write(str(item) + "\n")
            mtbp3cd.gui.util_show_message(self.message_list, f"ad_igdef_oid saved: {file_path}", status="s")
        except Exception as e:
            mtbp3cd.gui.util_show_message(self.message_list, f"Failed to export ad_igdef_oid: {e}", status="f")

        # Save tab_ad_igdef_oid_desc
        file_path = os.path.join(file_path_base, "log_ad_igdef_oid_desc.csv")
        try:
