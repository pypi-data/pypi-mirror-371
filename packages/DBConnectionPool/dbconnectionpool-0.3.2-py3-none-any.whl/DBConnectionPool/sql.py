                ...
                if "select" in sql:
                    with conn.select(sql) as r:
                        ...
                        for i in range(len(r.sqlres.columns)):
                            ...
                            head += (
                                column + " | "
                                if not i == len(r.sqlres.columns) - 1
                                else ""
                            )
                            print(
                                column,
                                end=" | " if not i == len(r.sqlres.columns) - 1 else "",
                            )
                        print()
...
def main():
    ...
    parser.add_argument(
        ...
        default="localhost",
        type=str,
        help="The host (ip) of the mysql server.",
    )
    parser.add_argument(
        "-p", "--port", default="3306", type=int, help="The port of the mysql server."
    )
    parser.add_argument(
        "-u", "--user", default="root", type=str, help="The user for the mysql server."
    )
    parser.add_argument(
        "-d", "--database", default=None, help="The databse to connect to."
    )
