...
class LLM:
    ...
    def call_model(self, *, model: str, input_or_contents, instructions: str | None = None,
        ...
        if fallback_plan:
            ...
            for step in fallback_plan:
                ...
                if step_provider == "openai":
                    def mk_exec(m=step_model):
                        def _exec(call_id_local, alias_local):
                            try:
                                res = self._openai_provider.call(model=m, input=input_or_contents, instructions=instructions, parameters=parameters, api_key_alias=alias_local, tags=tags, call_id=call_id_local, structured_schema=structured_schema)
                                return True, res.get("request_id"), None, res
                    ...
                    def mk_exec(m=step_model):
                        def _exec(call_id_local, alias_local):
                            try:
                                res = self._gemini_provider.call(model=m, contents=input_or_contents, system_instruction=instructions, parameters=parameters, api_key_alias=alias_local, tags=tags, call_id=call_id_local, thinking_budget=thinking_budget, include_thoughts=include_thoughts, structured_schema=structured_schema, url_context=url_context)
                                return True, res.get("request_id"), None, res
                            except Exception as e:
                                return False, None, str(e), None
                        return _exec
            ...
            ok, request_id, used_call_id, response_dict = execute_with_fallback(manager=self._manager, steps=steps, tags=tags, input_text=(input_or_contents if isinstance(input_or_contents, str) else None))
            ...
            for step in fallback_plan:
                ...
                if step_provider == "openai":
                    def mk_exec(m=step_model):
                        def _exec(call_id_local, alias_local):
                            try:
                                res = self._openai_provider.call_stream(model=m, input=input_or_contents, instructions=instructions, parameters=parameters, api_key_alias=alias_local, tags=tags, call_id=call_id_local)
                                return True, res.get("request_id"), None, res
                            except Exception as e:
                                return False, None, str(e), None
                        return _exec
                    ...
                    def mk_exec(m=step_model):
                        def _exec(call_id_local, alias_local):
                            try:
                                res = self._gemini_provider.call_stream(model=m, contents=input_or_contents, system_instruction=instructions, parameters=parameters, api_key_alias=alias_local, tags=tags, call_id=call_id_local, thinking_budget=thinking_budget, include_thoughts=include_thoughts)
                                return True, res.get("request_id"), None, res
                            except Exception as e:
                                return False, None, str(e), None
                        return _exec
