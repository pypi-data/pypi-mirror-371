...
class RabbitAdapter:
    ...
    async def connect(
        ...
        payments_q = await self.channel.declare_queue(payments_qname, durable=True)
        await payments_q.bind(self.exchange, routing_key=self.payments_qname)
        refunds_q = await self.channel.declare_queue(refunds_qname, durable=True)
        await refunds_q.bind(self.exchange, routing_key=self.refunds_qname)
        self.queues = {
            payments_qname: payments_q,
            refunds_qname: refunds_q,
        }
