...
class CrateDataPipeline:
    ...
    def _initialize_enhanced_scraper(self) -> Any:
        ...
        if not ENHANCED_SCRAPING_AVAILABLE or not self.config.enable_crawl4ai or UnifiedScraper is None:
            return None
        try:
    ...
    async def fetch_metadata_batch(self, crate_names: "List[str]") -> "List[CrateMetadata]":
        ...
        logging.info(f"Fetched metadata for {len(results)} out of " f"{len(crate_names)} requested crates.")
        return results
    ...
    async def enrich_batch(self, batch: "List[CrateMetadata]") -> "List[EnrichedCrate]":
        ...
        # Update GitHub stats
        github_repos = [c.repository for c in batch if c.repository and "github.com" in c.repository]
        if github_repos:
    ...
    async def _enhance_with_scraping(self, crate: CrateMetadata) -> None:
        ...
        try:
            ...
            if scraping_results:
                ...
                logging.info(f"Enhanced scraping for {crate.name}: " f"{len(scraping_results)} sources")
        except Exception as e:
    ...
    def _integrate_scraping_results(
        ...
        for source, result in scraping_results.items():
            ...
            if "dependencies" in structured_data and isinstance(structured_data["dependencies"], list):
                crate.enhanced_dependencies = structured_data["dependencies"]
            if "examples" in structured_data and isinstance(structured_data["examples"], list):
    ...
    def save_final_output(self, data: "List[EnrichedCrate]", dependency_data: "Dict[str, Any]") -> None:
        """Saves the final enriched data and analysis reports."""
        timestamp = time.strftime("%Y%m%d-%H%M%S")

        # Save main enriched data
        final_output_path = os.path.join(self.output_dir, f"enriched_crate_metadata_{timestamp}.jsonl")
        with open(final_output_path, "w") as f:
            for item in data:
                f.write(json.dumps(item.to_dict(), cls=CustomJSONEncoder) + "\n")

        # Save dependency analysis
        dep_file_path = os.path.join(self.output_dir, f"dependency_analysis_{timestamp}.json")
    ...
    async def run(self) -> Union["tuple[List[EnrichedCrate], Dict[str, Any]]", None]:
        ...
        crate_batches = [self.crates[i : i + batch_size] for i in range(0, len(self.crates), batch_size)]

        for i, batch_names in enumerate(crate_batches):
            logging.info(f"Processing batch {i + 1}/{len(crate_batches)} " f"({len(batch_names)} crates)")
            ...
            if not metadata_batch:
                logging.warning(
                    f"Batch {
                        i + 1} yielded no metadata. Skipping."
                )
                continue
            ...
            logging.info(
                f"Completed batch {i + 1}, " f"processed {len(all_enriched)}/{len(self.crates)} crates"
            )
