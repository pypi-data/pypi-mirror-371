...
try:
    from .utils.advanced_cache import get_cache, AdvancedCache
...
try:
    from .ml.quality_predictor import get_predictor, CrateQualityPredictor
...
class UnifiedSigilPipeline:
    ...
    def _initialize_components(self) -> None:
        try:
            ...
            # Don't initialize scraper here - create per task to avoid browser context conflicts
            self.logger.info("✅ Unified Scraper will be created per task")
    ...
    async def _gather_documentation(
        self, crate_name: str, scraper: UnifiedScraper
        ...
        try:
            results = await scraper.scrape_crate_documentation(crate_name)

            successful_sources = [
                source for source, result in results.items() if result.error is None
            ]
            failed_sources = [
                source for source, result in results.items() if result.error is not None
            ]

            self.logger.info(
                f"✅ Successfully scraped {
                    len(successful_sources)} sources: {successful_sources}"
            )
            if failed_sources:
                self.logger.warning(
                    f"⚠️  Failed to scrape {
                        len(failed_sources)} sources: {failed_sources}"
                )
    ...
    async def _download_and_extract_crate(
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(crate_url) as response:
                    if response.status != 200:
                        self.logger.error(
                            f"Failed to download {crate_url}: HTTP {
                                response.status}"
                        )
                        return None
            ...
            self.logger.info(
                f"This is optional - web scraping analysis will continue without source code"
            )
            return None
    ...
    async def _get_latest_crate_version(self, crate_name: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(
                    f"Failed to fetch crate info from {api_url}: HTTP {
    ...
    def _extract_readme_content(self, scraped_data: Dict[str, Any]) -> str:
        ...
        if combined_content.strip():
            self.logger.info(
                f"📖 Using combined README content ({
                    len(combined_content)} chars)"
            )
    ...
    async def analyze_multiple_crates(
        ...
        self.logger.info(
            f"🚀 Starting concurrent analysis of {
                len(crate_names)} crates"
        )
