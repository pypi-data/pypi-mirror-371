...
class TestIntegration:
    ...
    async def test_cache_integration(self, config, temp_cache_dir):
        ...
        with patch('rust_crate_pipeline.utils.advanced_cache.get_cache') as mock_get_cache:
            mock_cache = Mock()
    ...
    async def test_ml_predictor_integration(self, config):
        ...
        with patch('rust_crate_pipeline.ml.quality_predictor.get_predictor') as mock_get_predictor:
            mock_predictor = Mock()
    ...
    async def test_cache_and_ml_workflow(self, config, temp_cache_dir):
        ...
        with patch('rust_crate_pipeline.utils.advanced_cache.get_cache') as mock_get_cache, \
             patch('rust_crate_pipeline.ml.quality_predictor.get_predictor') as mock_get_predictor:
    ...
    async def test_component_fallback_behavior(self, config):
        ...
        with patch('rust_crate_pipeline.utils.advanced_cache.get_cache', side_effect=ImportError), \
             patch('rust_crate_pipeline.ml.quality_predictor.get_predictor', side_effect=ImportError):
    ...
    async def test_end_to_end_workflow(self, config, temp_cache_dir):
        ...
        with patch('rust_crate_pipeline.utils.advanced_cache.get_cache') as mock_get_cache, \
             patch('rust_crate_pipeline.ml.quality_predictor.get_predictor') as mock_get_predictor, \
             patch('rust_crate_pipeline.core.IRLEngine') as mock_irl_engine:
