...
class APIGateway:
    ...
    async def _authenticate_request(self, request: web.Request) -> Dict[str, Any]:
        ...
        if api_key and api_key in self.config.get("api_keys", []):
            return {"authenticated": True, "user": "api_key"}
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                return {"authenticated": True, "user": payload.get("user")}
            except jwt.InvalidTokenError:
                return {"authenticated": False, "error": "Invalid token"}

        # Public endpoints (no authentication required)
        public_paths = self.config.get("public_paths", [])
        if any(request.path.startswith(path) for path in public_paths):
            return {"authenticated": True, "user": "public"}
