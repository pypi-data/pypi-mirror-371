...
class PipelineConfig:
    ...
    crawl4ai_model: str = os.path.expanduser("~/models/deepseek/deepseek-coder-6.7b-instruct.Q4_K_M.gguf")
    ...
    def _validate_config(self) -> None:
        ...
        if self.use_azure_openai:
            if not self.azure_openai_endpoint:
                raise ConfigurationError("Azure OpenAI is enabled but AZURE_OPENAI_ENDPOINT is not set")
            if not self.azure_openai_api_key:
                raise ConfigurationError("Azure OpenAI is enabled but AZURE_OPENAI_API_KEY is not set")
