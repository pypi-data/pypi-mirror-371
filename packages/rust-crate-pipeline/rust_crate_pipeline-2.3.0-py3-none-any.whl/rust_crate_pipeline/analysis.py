...
class SourceAnalyzer:
    ...
    def _analyze_tarball_content(content: bytes) -> dict[str, Any]:
        ...
        try:
            with io.BytesIO(content) as tar_content, tarfile.open(fileobj=tar_content, mode="r:gz") as tar:
                    ...
                    if member.isfile() and member.name.endswith(".rs"):
                        ...
                        if file_content:
                            try:
                                ...
                                analysis = RustCodeAnalyzer.analyze_rust_content(content_str)
                                metrics = RustCodeAnalyzer.aggregate_metrics(metrics, analysis, structure)
                            except UnicodeDecodeError:
                                logging.warning(f"Skipping non-UTF-8 file: {member.name}")
...
class UserBehaviorAnalyzer:
        ...
        try:
            ...
            for _ in range(3):  # Retry up to 3 times
                ...
                if activity_resp.status_code == 200:
                    result["community_metrics"]["commit_activity"] = activity_resp.json()
                    break
                elif activity_resp.status_code == 202:
                    logging.info(f"GitHub is calculating stats for {owner}/{repo}, waiting...")
                    time.sleep(2)
                else:
    ...
    def _fetch_crates_io_versions(crate_name: str, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            logging.warning(f"Error fetching crates.io version data for {crate_name}: {e}")
...
class DependencyAnalyzer:
    ...
    def analyze_dependencies(crates: list[EnrichedCrate]) -> dict[str, Any]:
        ...
        most_depended = sorted(reverse_deps.items(), key=lambda item: len(item[1]), reverse=True)[:10]
