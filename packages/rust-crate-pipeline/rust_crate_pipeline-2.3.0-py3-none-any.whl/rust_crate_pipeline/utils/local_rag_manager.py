...
class LocalRAGManager:
    ...
    def _migrate_database(self) -> None:
        ...
        if missing_tables:
            ...
            self.conn.executescript(schema)
            self.conn.commit()
            self.logger.info(f"Migrated RAG database: created missing tables {missing_tables}")
        else:
    ...
    def index_codebase(self, root_path: Union[str, Path], file_patterns: Optional[List[str]] = None) -> int:
    ...
    def _identify_affected_functions(self, old_content: str, new_content: str) -> List[str]:
