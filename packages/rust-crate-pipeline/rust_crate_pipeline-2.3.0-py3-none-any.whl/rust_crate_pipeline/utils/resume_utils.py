...
def load_crate_list(
    ...
    try:
        with open(crates_file, "r", encoding="utf-8") as f:
            crates = [line.strip() for line in f if line.strip() and not line.strip().startswith("#")]
...
def get_remaining_crates(
    ...
    if skip_problematic:
        ...
        if skipped_count > 0:
            logger.info(f"⚠️  Skipped {skipped_count} known problematic crates")
    ...
    logger.info(
        f"   Progress: {len(processed)}/{len(all_crates)} " f"({len(processed) / len(all_crates) * 100:.1f}%)"
    )
...
def validate_resume_state(
    ...
    try:
        ...
        if processed_count > 0:
            ...
            logger.info(
                f"✅ Resuming from {processed_count}/{total_crates} completed " f"({progress_pct:.1f}%)"
            )
        else:
