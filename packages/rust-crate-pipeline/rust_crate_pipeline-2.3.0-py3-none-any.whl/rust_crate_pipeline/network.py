    ...
    def _fetch_metadata(self, crate_name: str) -> "dict[str, Any] | None":
        ...
        try:
            ...
            if r.ok:
                ...
                # Get dependencies
                deps_url = f"https://crates.io/api/v1/crates/{crate_name}/" f"{latest}/dependencies"
                deps_response = self.session.get(deps_url)
                deps: list[dict[str, Any]] = (
                    deps_response.json().get("dependencies", []) if deps_response.ok else []
                )
        ...
        try:
            ...
            if r.ok:
                ...
                keywords = [k.text.strip() for k in keywords_elem.find_all("a")] if keywords_elem else []
        ...
        try:
            ...
            search_url = f"https://api.github.com/search/repositories?" f"q={crate_name}+language:rust"
