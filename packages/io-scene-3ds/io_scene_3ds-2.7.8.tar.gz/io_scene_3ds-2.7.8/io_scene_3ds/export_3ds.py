...
def save_3ds(context, filepath="", collection="", items=[], scale_factor=1.0, global_matrix=None,
    ...
    if use_selection and use_invisible:
        objects = [ob for ob in items if ob.type in object_filter and ob.hide_get(view_layer=layer) or ob.select_get(view_layer=layer)]
        ...
        objects = [ob for ob in items if ob.type in object_filter and ob.hide_get(view_layer=layer) or ob.visible_get(view_layer=layer)]
    else:
