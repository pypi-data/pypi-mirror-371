...
def predict_loads_rotor_average(
    ...
    # We pass data to get a numpy array within the xarray, not a JAX one.
    sensor_names = _get_sensor_names(surrogates)
    loads = preallocate_ilktn(
        ...
        name=sensor_names,
        dtype=dtype,
        data=np.full((wt_.size, wd_.size, ws_.size, len(sensor_names)), np.nan),
    )
