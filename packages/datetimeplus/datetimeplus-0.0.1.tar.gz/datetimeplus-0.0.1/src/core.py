...
class datetime(base_datetime):
    ...
    def strftime(self, format: str) -> str:
        ...
        def _handle_padding(m: re.Match[str]) -> str:
            ...
            if specifier == "EY":
                val = to_wareki(self.date(), "EY") or ""
                return val.rjust(2)

            unpadded_val = super(datetime, self).strftime(f"%-{specifier}")
    ...
    def strptime(cls, date_string: str, format: str) -> Self:
        ...
        ------
        ValueError
            If the date_string and format string do not match.
        """
        regex_map = {
            "Y": r"\d{4}",
            "y": r"\d{2}",
        ...
        for m in re.finditer(r"%(?:(EN)|(_)?(EY)|(E)|(_)?([YymdWHIMSjUwpa%]))", format):
            ...
            if code_en:
                ...
                key = "_EY" if is_padded_ey else "EY"
            elif code_e:
                key = "E"
            elif code_other == "%":
                pattern += "%"
                ...
                key = f"_{code_other}" if is_padded_other else code_other
            else:
                continue

            group_counters[key] = group_counters.get(key, 0) + 1
            group_name = f"{key}_{group_counters[key]}"
            pattern += f"(?P<{group_name}>{regex_map[key]})"
        ...
        for i, token in enumerate(tokens):
            if token["kind"] in ("EN", "E"):
                if i + 1 < len(tokens) and tokens[i + 1]["kind"] in ("EY", "_EY"):
