...
def extract_field_universal(
    ...
    """
    Extrai um campo tanto de objeto único quanto de lista de objetos JSON.
    
    Detecta automaticamente se a coluna contém um objeto único ou uma lista
    e aplica a extração apropriada.
    ...
    Args:
        ...
        field_name: Nome do campo a ser extraído
        output_column: Nome da coluna de saída
        separator: Separador para listas (padrão: vírgula)
        
    Returns:
        DataFrame com a nova coluna contendo os valores extraídos
    """
    json_col = col(json_column)
    
    return df.withColumn(
        output_column,
        when(
            # Se for um array
            json_col.getItem(0).isNotNull(),
            when(
                size(spark_filter(json_col, lambda x: x[field_name].isNotNull())) > 0,
                concat_ws(
                    separator,
                    transform(
                        spark_filter(json_col, lambda x: x[field_name].isNotNull()),
                        lambda x: x[field_name].cast("string")
                    )
                )
            ).otherwise(None)
            ...
            when(
                json_col.isNotNull() & json_col[field_name].isNotNull(),
                json_col[field_name].cast("string")
            ).otherwise(None)
        )
    )
