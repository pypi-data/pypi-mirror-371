...
class ES2:
    ...
    def register_key(
        ...
        """
        Registers a key with the ES2 server.
        ...
        if self.index_config.key_param.check_key_dir():
            ...
            if key_list and self.index_config.key_id in key_list:
                ...
                self.indexer.register_key(self.index_config.key_id, self.index_config.eval_key_path)
            return
        else:
            raise ValueError(
    ...
    def generate_key(self, key_id: Optional[str] = None):
        ...
        if key_id is not None:
            self.index_config.key_id = key_id
        if self.index_config.key_param.check_key_dir():
            logger.info(f"Keys in {self.index_config.key_path} with key_id: {self.index_config.key_id} already exists.")
            return
        else:
            logger.info(
    ...
    def init_index_config(
        ...
        Args:
            ...
            query_encryption (bool, optional): Whether to enable query encryption. Defaults to None.
            index_encryption (bool, optional): Whether to enable database encryption. Defaults to None.
            index_type (str, optional): The type of index. Defaults to None.
    ...
    def init(
        ...
        query_encryption : bool, optional
            Whether to enable query encryption. Defaults to ``False``.
            Currently, query_encryption supports only ``False``, which means plaintext query.
        index_encryption : bool, optional
            Whether to enable database encryption. Defaults to ``True``.
            Currently, only "ip" preset supports only ``True``, which means ciphertext index.
        index_type : str, optional
            The type of index. Defaults to ``flat``.
            Currently, only ``flat`` index type is supported.
    ...
    def create_index(
        ...
        Args:
            ...
            index_encryption (bool, optional): Whether to enable database encryption.
...
def init_index_config(*args, **kwargs):
    ...
    eval_mode : str, optional
        The evaluation mode.
    query_encryption : bool, optional
        Whether to enable query encryption.
    index_encryption : bool, optional
