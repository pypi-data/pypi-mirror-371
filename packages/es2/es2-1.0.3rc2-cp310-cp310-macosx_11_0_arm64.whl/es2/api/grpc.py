...
class Indexer:
    ...
    def register_key(
        self, key_id: str, key_path: str, key_type: str = "EvalKey", preset: str = "IP", eval_mode: str = "RHOMBUS"
    ):
        """
    ...
    def create_index(
        ...
        dim: int,
        search_type: str = "ip",
        index_type: str = "cipher",
        query_type: str = "plain",
    ):
        """
        ...
        if isinstance(index_type, str) and index_type.lower() in ["plain", "cipher"]:
            index_type = index_type.lower()
        else:
            raise ValueError(f"Invalid index_type: {index_type}. Expected 'plain' or 'cipher'.")
        ...
        request.index_info.query_type = query_type
    ...
    def get_index_info(self, index_name: str):
        ...
        if response.header.return_code != es2_type_pb.ReturnCode.Success:
            ...
            return {
                ...
                "index_type": response.index_info.index_type,
                "query_type": response.index_info.query_type,
    ...
    def insert_data_bulk(
        ...
        response = self.stub.BatchInsertData(
            insert_data_request_generator(),
            metadata=self.grpc_metadata,
        )
    ...
    def search(self, index_name: str, query: List[List[float]]):
        ...
        response_stream = self.stub.Search(
            request,
            metadata=self.grpc_metadata,
        )
        ...
        outputs = [es2_type_pb.CiphertextScore(id=query_id, ctxt_score=results[query_id]) for query_id in query_ids]
