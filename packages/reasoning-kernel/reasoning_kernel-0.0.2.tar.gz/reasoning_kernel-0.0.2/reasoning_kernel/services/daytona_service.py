...
class DaytonaService:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            self.api_url = self.api_url or os.getenv("DAYTONA_API_URL", "https://api.daytona.io/v1")
            ...
            # Try SDK first if available AND explicitly enabled for this environment
            _enable_sdk = os.getenv("RK_ENABLE_DAYTONA_SDK", "0").lower() in {"1", "true", "yes"}
        ...
        try:
            if self.use_sdk and self.daytona_client:
                ...
                try:
                    ...
                    # Normalize SDK result to dict format
    ...
    async def execute_code(self, code: str, timeout: Optional[int] = None) -> ExecutionResult:
        ...
        if self.daytona_client and self.current_sandbox:
            ...
            return await self._execute_locally_with_timeout(code, execution_timeout, start_time)
    ...
    async def _execute_in_daytona_core(
        ...
        try:
            ...
            response = self.current_sandbox.process.code_run(code)
            execution_time = time.time() - start_time
    ...
    async def _validate_code_security(self, code: str) -> bool:
        ...
        try:
            ...
            # Parse and validate
            tree = ast.parse(code)
            validator = SecurityValidator(self.config.allowed_imports)
