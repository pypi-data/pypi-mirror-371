...
class ResourceManager:
    ...
    def register_directory(cls, url_path, directory):
        for up,dir in cls.paths:
            if up == url_path:
                assert dir == directory, 'Attempt to reregister {}: {}=>{}'.format(
    ...
    def register_all_resources(cls):
        for ep in importlib.metadata.entry_points(group='easy_widgets.resources'):
        ...
        def compress(it):
            for (cls, compress), rs in groupby(it, key=lambda r:(type(r), r.compress)):
                if not compress:
                    ...
                    yield from cls.compressed(self, rs)
    ...
    def serve_slim(self, file_type, href):
        ...
        content = joiner.join(getter(h) for h in href.split(';') if h)
        if self.use_cache:
    ...
    def __repr__(self):  # pragma no cover
        ...
        for u,d in self.paths:
            l.append(f'  <Path url="{u}" directory="{d}">')
        return '\n'.join(l)
...
class ResourceLink(Resource):
    ...
    def __init__(self, url, location, squash, compress):
        self._url = url
        super().__init__(location, squash, compress)
...
class JSLink(ResourceLink):
    ...
    def __init__(self, url, location='body_js', squash=True, compress=True):
        super().__init__(url, location, squash, compress)
        del self.widget
...
class CSSLink(ResourceLink):
    ...
    def __init__(self, url, squash=True, compress=True, **attrs):
        super().__init__(url, 'head_css', squash, compress)
        self.attrs = attrs
        del self.widget
...
class JSScript(ResourceScript):
    ...
    def __init__(self, text, location='body_js_tail', squash=True, compress=True):
        super().__init__(text, location, squash, compress)
        del self.widget
...
class CSSScript(ResourceScript):
    ...
    def __init__(self, text):
        super().__init__(text, 'head_css', True, True)
        del self.widget
