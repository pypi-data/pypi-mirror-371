...
from ew.tests.helpers import setup_method
...
class TestFields:
    ...
    def test_link_field(self):
        ...
        assert '<a href="/foo/5/">/foo/5/</a>' == text

        expected = '<a href="/foo">bar</a>'
        assert expected == self.ew.LinkField(href='/foo').display(value='bar')
        assert expected == self.ew.LinkField(text='bar').display(value='/foo')
        assert expected == self.ew.LinkField().display(value=dict(href='/foo', text='bar'))
        assert expected == self.ew.LinkField(label='bar').display(value=dict(href='/foo'))
        assert expected == self.ew.LinkField(href='/foo').display(value=dict(text='bar'))
        assert expected == self.ew.LinkField(attrs={'href':'/foo'}).display(value='bar')
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(value=dict(href='/foo', text='bar'))
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(href='/foo', value='bar')

    def test_link_plaintext(self):
        expected = 'bar'
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(value=dict(text='bar'))
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, label='bar').display(value=dict())
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, label='foo').display(value='bar')

    def test_link_escaping(self):
        text = '<script>alert("foo")</script>'
        href = '" onclick="alert(\'foo\')'
        expected = '<a >&lt;script&gt;alert(&#34;foo&#34;)&lt;/script&gt;</a>'
        assert expected == self.ew.LinkField().display(text=text)
        assert expected == self.ew.LinkField().display(text=text, value={})
        assert expected == self.ew.LinkField().display(label=text)
        assert expected == self.ew.LinkField().display(label=text, value={})
        assert expected == self.ew.LinkField().display(value={'text':text})
        assert expected == self.ew.LinkField(text=text).display()
        assert expected == self.ew.LinkField(text=text).display(value={})
        assert expected == self.ew.LinkField(label=text).display()
        assert expected == self.ew.LinkField(label=text).display(value={})
        assert expected == self.ew.LinkField(value={'text':text}).display()
        expected = '<a href="&lt;script&gt;alert(&#34;foo&#34;)&lt;/script&gt;">&lt;script&gt;alert(&#34;foo&#34;)&lt;/script&gt;</a>'
        assert expected == self.ew.LinkField().display(value=text)
        assert expected == self.ew.LinkField(value=text).display()
        expected = '<a href="&#34; onclick=&#34;alert(&#39;foo&#39;)">None</a>'
        assert expected == self.ew.LinkField().display(href=href)
        assert expected == self.ew.LinkField().display(href=href, value={})
        assert expected == self.ew.LinkField().display(attrs={'href':href})
        assert expected == self.ew.LinkField().display(attrs={'href':href}, value={})
        assert expected == self.ew.LinkField().display(value={'href':href})
        assert expected == self.ew.LinkField(href=href).display()
        assert expected == self.ew.LinkField(href=href).display(value={})
        assert expected == self.ew.LinkField(attrs={'href':href}).display()
        assert expected == self.ew.LinkField(attrs={'href':href}).display(value={})
        assert expected == self.ew.LinkField(value={'href':href}).display()
        expected = '&lt;script&gt;alert(&#34;foo&#34;)&lt;/script&gt;'
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(text=text)
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(label=text)
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(value=text)
        assert expected == self.ew.LinkField(plaintext_if_no_href=True).display(value={'text':text})
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, label=text).display()
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, text=text).display()
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, value=text).display()
        assert expected == self.ew.LinkField(plaintext_if_no_href=True, value={'text':text}).display()
...
class TestFieldSet:
    ...
    def test_validation(self):
        with pytest.raises(fe.Invalid):
            self.w.to_python(dict(foo='x', bar='xxy'), None)
        assert isinstance(ew.widget_context.validation_error, fe.Invalid)
        text = self.w.display()
    ...
    def test_nested_validation(self):
        with pytest.raises(fe.Invalid):
            self.nested_w.to_python(dict(fs=dict(foo='x')), None)
        assert isinstance(ew.widget_context.validation_error, fe.Invalid)
        text = self.nested_w.display()
        assert '<label for="w.fs.foo"' in text
...
class TestRowField:
    ...
    def test_validation(self):
        with pytest.raises(fe.Invalid):
            self.w.to_python(dict(a='x', b='xx'), None)
        assert isinstance(ew.widget_context.validation_error, fe.Invalid)
        text = self.w.display()
        assert '<span class="fielderror"' in text
...
class TestTableField:
    ...
    def test_validation(self):
        with pytest.raises(fe.Invalid):
            self.w.to_python([dict(a='x', b='xx'),
                              dict(a='yy', b='yy')],
                             None)
        assert isinstance(ew.widget_context.validation_error, fe.Invalid)
        text = self.w.display()
...
class TestInputFields:
    ...
    def test_email_field(self):
        ...
        with pytest.raises(fe.Invalid):
            t.to_python('asdf', None)
        with pytest.raises(fe.Invalid):
            t.to_python('asdf@bar', None)
        with pytest.raises(fe.Invalid):
            t.to_python('asdf@bar@bar.com', None)
