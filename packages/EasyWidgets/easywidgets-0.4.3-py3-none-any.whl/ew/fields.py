    ...
    def prepare_context(self, context):
        ...
        if 'value' in context:
            try:
                ...
                warnings.warn('suppressed an error that you should probably fix. ' + traceback.format_exc(),
                              EasyWidgetsWarning,
                              stacklevel=4,
                              )
        return context
...
class InputField(FieldWidget):
    ...
    def __init__(self, **kw):
        ...
        if self.validator and getattr(self.validator, 'not_empty', False) is True:
...
class CompoundField(InputField):
    ...
    def _make_schema(self):
        kwargs = {f.name: f for f in self._all_fields()
...
class RepeatedField(InputField):
    ...
    def context_for(self, repetition):
        ...
        name = context['name'] + '-' + str(repetition)
        id = context['id'] + '-' + str(repetition)
        r = Bunch(
            id=id,
            rendered_name=name,
            name=name,
...
class TableField(RepeatedField):
    ...
    def context_for(self, repetition):
        context = core.widget_context.render_context
        result = super().context_for(repetition)
        result['id'] = context['id'] + '-row-' + str(repetition)
        return result
