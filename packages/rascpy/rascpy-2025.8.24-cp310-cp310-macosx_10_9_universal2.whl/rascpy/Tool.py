...
def value_counts_weight_y(dat,y,y_label={'event':1,'unevent':0},weight=None):
    '''
...
def profit(y,pred,weight=None
           ...
           ,avg_profit_rate=0.2,avg_loss_rate=0.8,y_label={'unevent':0,'event':1}):
    '''
    依据真实的表现和预测值，给出一个大致盈利的测算
...
def predict_proba(clf,X,decimals=4):
    ...
    if isinstance(clf,GLMResultsWrapper):
        import statsmodels.api as sm
        from .Reg_Step_Wise_MP import _clf_cols
        proba_hat = pd.Series(clf.predict(sm.add_constant(X[_clf_cols(clf)])),index = X.index)
    else:
...
def mean_weight(dat,weight=None):
    if weight is not None:
        return np.around((dat * weight).sum() / weight.sum(),4)
    else:
        return np.around(dat.mean(),4)