...
import traceback
import time
from silicrop import ensure_models_available
...
class ImageProcessorApp(QWidget):
    def __init__(self):
        ...
        # Sélection du modèle selon le bouton actif
        if self.filter_150_button.isChecked():
            with pkg_resources.path(assets, 'DeepLabV3_150_flatspot_0.001_350_4_weights.pth') as model_path:
                self.model_path = str(model_path)
        elif self.filter_200_button.isChecked():
            with pkg_resources.path(assets, 'DeepLabV3_200300_notch_0.001_250_4_weights.pth') as model_path:
                self.model_path = str(model_path)
        else:
            # Aucun bouton sélectionné
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.warning(self, "Sélection requise", 
                                "Veuillez sélectionner une dimension de plaque (150 ou 200) avant de continuer.")
            return
    ...
    def run_auto_dl_process(self):
        ...
        self.ellipse_predictor = EllipsePredictor(model_path_str, self.fit_crop_widget)
