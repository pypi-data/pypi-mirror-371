...
class LOTMarker(Structure):
    _fields_ = [("name", c_char_p), ("startframe", c_size_t), ("endframe", c_size_t)]


# LOTMarkerList
class LOTMarkerList(Structure):
    _fields_ = [("ptr", POINTER(LOTMarker)), ("size", c_size_t)]
...
class LOTMask(Structure):
    _fields_ = [("mPath", mPath), ("mMode", c_int), ("mAlpha", c_ubyte)]


# mClipPath -> mMaskList
class mMaskList(Structure):
    _fields_ = [("ptr", POINTER(LOTMask)), ("size", c_size_t)]
...
class mLayerList(Structure):
    _fields_ = [("ptr", POINTER(POINTER(LOTLayerNode))), ("size", c_size_t)]
...
class mNodeList(Structure):
    _fields_ = [("ptr", POINTER(POINTER(LOTNode))), ("size", c_size_t)]
