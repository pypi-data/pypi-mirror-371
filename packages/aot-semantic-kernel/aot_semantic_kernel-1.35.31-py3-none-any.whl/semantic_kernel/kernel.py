...
class Kernel(KernelFilterExtension, KernelFunctionExtension, KernelServicesExtension, KernelReliabilityExtension):
    ...
    def _clone_plugins_safely(self) -> dict[str, Any]:
        """Clone plugins with special handling for MCP plugins to avoid serialization issues.
        
        MCP plugins contain async generator context managers that cannot be pickled/deep copied.
        These plugins maintain persistent connections and should be shared rather than duplicated.
        
        Returns:
            A dictionary of plugin names to plugin instances, where MCP plugins are shared
            and regular plugins are deep copied.
        """
        cloned_plugins = {}
        for name, plugin in self.plugins.items():
            # Import MCPPluginBase locally to avoid circular imports at runtime
            try:
                from semantic_kernel.connectors.mcp import MCPPluginBase
                
                # Check if this is a direct MCP plugin instance
                is_mcp_plugin = isinstance(plugin, MCPPluginBase)
                
                # Check if this is a KernelPlugin wrapping an MCP plugin
                if not is_mcp_plugin:
                    is_mcp_plugin = self._contains_mcp_plugin(plugin, MCPPluginBase)
                
                if is_mcp_plugin:
                    # Share MCP plugins - they manage their own persistent connections
                    # Deep copying would break the async generator context managers
                    cloned_plugins[name] = plugin
                else:
                    # Deep copy regular plugins for proper isolation
                    cloned_plugins[name] = deepcopy(plugin)
            except ImportError:
                # If MCP is not available, treat as regular plugin
                cloned_plugins[name] = deepcopy(plugin)
        return cloned_plugins

    def _contains_mcp_plugin(self, obj: Any, mcp_plugin_base_class: type) -> bool:
        """Recursively check if an object contains an MCP plugin instance.
        
        This method inspects objects to find MCP plugin references, including:
        - Direct instances
        - Instances in object attributes
        - Instances in collections (lists, dicts)
        - Instances referenced by function methods (bound methods, functools.partial)
        
        Args:
            obj: The object to inspect
            mcp_plugin_base_class: The MCPPluginBase class to check against
            
        Returns:
            True if the object contains an MCP plugin instance, False otherwise
        """
        if isinstance(obj, mcp_plugin_base_class):
            return True
        ...
        for attr_value in obj.__dict__.values():
            # Direct MCP plugin instance
            if isinstance(attr_value, mcp_plugin_base_class):
                return True
                
            # Check collections (lists, dicts) for MCP plugins
            elif isinstance(attr_value, (list, dict)):
                items_to_check = attr_value.values() if isinstance(attr_value, dict) else attr_value
                for item in items_to_check:
                    if isinstance(item, mcp_plugin_base_class):
                        return True
                        
                    # Check KernelFunctionFromMethod instances
                    elif hasattr(item, 'method'):
                        # Check bound methods
                        if hasattr(item.method, '__self__') and isinstance(item.method.__self__, mcp_plugin_base_class):
                            return True
                            
                        # Check functools.partial objects
                        elif hasattr(item.method, 'func') and hasattr(item.method.func, '__self__'):
                            if isinstance(item.method.func.__self__, mcp_plugin_base_class):
                                return True
                                
                        # Check partial arguments
                        elif hasattr(item.method, 'args') and item.method.args:
                            for arg in item.method.args:
                                if isinstance(arg, mcp_plugin_base_class):
                                    return True
        return False
    ...
    def clone(self) -> "Kernel":
        ...
        """
        return Kernel(
            plugins=self._clone_plugins_safely(),
