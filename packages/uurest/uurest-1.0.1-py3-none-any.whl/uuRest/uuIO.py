...
def load_json(filename: Path or str, encoding: str = "utf-8") -> json:
    """
...
def save_json(value, filename: Path or str, encoding: str = "utf-8", formatted: bool = True) -> None:
    """
    ...
    with open(filename, "w", encoding=encoding) as file:
        if formatted:
            json.dump(value, file, indent=4, encoding=encoding, ensure_ascii=False)
        else:
            json.dump(value, file, encoding=encoding, ensure_ascii=False)


def load_textfile(filename: Path or str, encoding: str = "utf-8") -> str:
    """
...
def save_textfile(value: str, filename: Path or str, encoding: str = "utf-8") -> None:
    """
...
def save_binary(value, filename: Path or str):
    return


def create_folder_structure(directory: Path or str) -> None:
...
def test_if_two_text_files_are_equal(filename1: Path or str, filename2: Path or str, encoding: str = "utf-8") -> bool:
    """
