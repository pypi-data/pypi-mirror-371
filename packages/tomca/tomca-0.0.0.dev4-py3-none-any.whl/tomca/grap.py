...
def voxVol(jcfg_or_vol, planes, cmap_in=None, resKey='res'):
    ...
    fig=tomca.grap.voxVol(jcfg,[5,5,5])
    ...
    if type(jcfg_or_vol) is dict:
        ...
        # Plot the cutaways
        fig = tomca.grap.voxPlanes(jcfg[resKey][plotKey_planes], detailsStr,
        ...
        # Plot the cutaways
        fig = tomca.grap.voxPlanes(plotKey_planes, detailsStr, cmap=cmap,
...
def state_xsection_CCD(jcfg, resName='res', desc=''):
    ...
    if writeFig:
        ...
        tomca.writ.fig(jcfg, fig, 'CCD_image_'+resName+desc)
    else:
        tomca.util.verb('Skipping writing of CCD image.'
...
def state_xsection_CCDx(jcfg, resName='res', desc=''):
    ...
    if writeFig:
        ...
        tomca.writ.fig(jcfg, fig_crossSections, 'CCD_XSection_'+resName+desc)
    else:
        tomca.util.verb('Skipping writing of CCD cross section.'
                       'Toggle with jcfg[\'Expt\'][\'write\']'
...
def state_xsection_flux(jcfg, plotCuts=None, desc=''):
    ...
    if 'flux' in jcfg['res']:
        ...
        if writeFig:
            tomca.writ.fig(jcfg, fig2, 'simulation_XSections'+desc)
        else:
            tomca.util.verb('Skipping writing of simulation_XSections.'
                           'Toggle with jcfg[\'Expt\'][\'write\']'
...
def state_xsection_flux_replay(jcfg, plotCuts=None, desc=''):
    ...
    if jcfg['Expt']['doReplay']:
        for replay_key in jcfg['Optode']['Detector']['configs_labels']:
            ...
            if writeFig:
                tomca.writ.fig(jcfg, fig3, 'simulation_XSections_' +
                              replay_key + '_' + desc)
            else:
                tomca.util.verb('Skipping writing of'
...
def state_xsection_property_plots(jcfg, plotCuts=None):
    ...
    for prop in propList:
        ...
        if writeFig:
            ...
            tomca.writ.fig(jcfg, figmua, titleStr)
        else:
            tomca.util.verb('Skipping writing of '+titleStr+'. Toggle with '
...
def state_xsections(jcfg):
    ...
    if jcfg['Expt']['verb'] >= 1.5:
        if 'calcFigs' in jcfg['Expt']['plot']:
            tomca.util.verb('Skipping cross-sectional plots calculation. '
...
def expt_CCD(results_list):
    ...
    tomca.writ.fig(results_list[0], fig2, 'summary_images', location='Expt')
...
def expt_middleProfiles(results_list):
    ...
    results_list[0]['Expt']['plot']['writeFigs'] = 1
    tomca.writ.fig(results_list[0], fig, 'summary_middle_profiles',
                  location='Expt')
...
def detector_angles_histogram(jcfg, inName):
    ...
    if "write" in jcfg['Expt']:
        if "fig_det_angles" in jcfg['Expt']['write']:
            ...
            tomca.writ.fig(jcfg, fig, f'angleshist_{inName}')
...
def create_curoled_image(sim, contour=False, cmap0='default', scaling=0.06):
    ...
    if contour:
        tomca.writ.fig(sim, new_fig, 'Result_img_contour')
    else:
        tomca.writ.fig(sim, new_fig, 'Result_img')
