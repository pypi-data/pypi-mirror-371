"""
functions which \b impoert data; includes functions to import jdat, json with logging in tomca.
"""
...
def jdat_json(jdatFilePath: str, verb: float):
    ...
    json_file.close()
    tomca.util.verb('Loaded analysis .json file with exptName: ' + str(param['Expt']['exptName']), verb, 2) 
    
    # Loop over all .dat files in the experiment directory, summing repeats together.
    file=[]
    dat=[]
    
    tomca.util.verb('Loading .jdat file...', verb, 2)  
    dat = jdata.load(jdatFilePath) # Notice this isnt yet implemented for mulitple jdat files
    tomca.util.verb('Loaded .jdat file: '+ jdatFilePath, verb, 1)
...
def jcfg_json(json_FilePath: str, verb: float):
    ...
    tomca.util.verb('Loading .json file...', verb, 2)  
    dat = jdata.load(json_FilePath) # Notice this isnt yet implemented for mulitple jdat files
    tomca.util.verb('Loaded .jdat file: '+ json_FilePath, verb, 1)
        ...
        if os.path.isdir(item_path):
            ...
            for file in os.listdir(item_path):
                ...
                if file.endswith('jcfg_results.json'):
                    ...
                    tomca.util.verb(f"Found file: {file_path}",verb, 2)

                    results_json = tomca.impo.jdat_json(file_path, verb=verb)
...
def svmc_vol(jcfg: dict):
    ...
    tomca.util.verb('Loaded svmc volume: '+jcfg['VolumeFile']+'(fn not finished)', jcfg["verb"], 1)
...
def blood_spectra():
    """
    Import optical properties from file embedded in tomca
...
def incident_spectrum(jcfg: dict):
    ...
    if 'determine' in source_file:
        ...
        tomca.util.verb(f"Imported incident spectrum from: {file_path}", jcfg['Expt']['verb'], 1)
        tomca.util.verb(f"Interpolating spectrum from {jcfg['Optode']['Source']['lam0']} to {jcfg['Optode']['Source']['lamN']} stepping {jcfg['Optode']['Source']['dlam']}", jcfg['Expt']['verb'], 3)
