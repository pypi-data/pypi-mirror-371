...
def json_to_mcx(jcfg):    
    ...
    def getKey(cfg_t, defaultVal, *keys):
        for key in keys:
            try:
                ...
                tomca.util.verb(warn, jcfg['Expt']['verb'], 3)
    ...
    tomca.util.verb('Converted jcfg to mcfg to simulate '+ str(mcfg['nphoton']) + ' photons', jcfg['Expt']['verb'], 2)
...
def vol_svmc(jcfg, engine = 'octave', smoothing=1):
    ...
    volHash=tomca.util.hash_vol(jcfg)
    if "Vol" not in jcfg:
        jcfg['Vol']={}
    ...
    if "MediaFormat" in jcfg['Domain']:
        if jcfg['Domain']['MediaFormat']!="svmc":
            ...
            if os.path.isfile(filepath):
                ...
                tomca.util.verb('Match found for smoothed volume, loading hash : '+ volHash, jcfg['Expt']['verb'], 1)
                jcfg['vol'] = tomca.util.pickle_in(filepath)
                if 'Vol' in jcfg:
                    pass
                else: 
                    jcfg['Vol']={}
                ...
                jcfg['Vol']['svmc'] = int(1)
                jcfg['Vol']['hash_svmc'] = tomca.util.hash_vol(jcfg)
                
            else:
                
                tomca.util.verb('No match found for smoothed volume hash : '+ volHash, jcfg['Expt']['verb'], 1)
                ...
                if engine == 'matlab':
                    ...
                    tomca.util.verb('Reshaped python volume for MATLAB', jcfg['Expt']['verb'], 3)
                elif engine == 'octave':
                    import oct2py
                    eng = oct2py.Oct2Py()
                    ...
                    tomca.util.verb('Invalid engine defined', jcfg['Expt']['verb'], 0)
                tomca.util.verb('Started '+ engine +' engine for SVMC conversion', jcfg['Expt']['verb'], 3)

                # Call SVMC
                svmcvol = np.array(eng.mcxsvmc_wrapper(vol,'smoothing',smoothing))
                tomca.util.verb('Finished importing smoothed volume into Python', jcfg['Expt']['verb'], 2)

                if engine == 'matlab':
                    # Exit MATLAB
                    jcfg['vol']=np.int8(svmcvol.reshape(svmcvol.shape,order='C')).copy()
                    tomca.util.verb('Reshaped volume for Python', jcfg['Expt']['verb'], 3)
                    eng.quit()
                else:
                    ...
                    for ij in range(10): # Try closing up to 10 times
                        try:
                            ...
                            if "Session is closed" in str(e): # This is what we want.
                                tomca.util.verb(f'Octave engine closed', jcfg['Expt']['verb'], 2)
                                break
                            else:
                                import time
                                tomca.util.verb(f'Failed to close Octave engine after {ij} try.  Retrying...', jcfg['Expt']['verb'], 1)
                                eng.exit()
                                time.sleep(1)
                        else:
                            break
                    ...
                    tomca.util.verb('Reimported volume to Python', jcfg['Expt']['verb'], 3)
                try:
                    jcfg['Domain']['MediaFormat']="svmc"
                except KeyError:
                    tomca.util.verb('Did not find MediaFormat in JSON format.  Adding in jcfg.', jcfg['Expt']['verb'], 1)
                    jcfg['mediabyte'] = "svmc"
                tomca.util.verb('Smoothed volume with svmc', jcfg['Expt']['verb'], 1)

                # Save volume to results directory and update the jcfg with the volume being used
                tomca.util.pickle_out(jcfg['vol'], filepath)
                if "Vol" not in jcfg:
                    jcfg['Vol']={}
                jcfg['Vol']['hashFile'] = filepath
                tomca.util.verb('Saving smoothed volume to volume library with hash : '+ volHash, jcfg['Expt']['verb'], 1)
            
                jcfg['Vol']['svmcDims']=str(jcfg['vol'].shape) 
                jcfg['Vol']['svmc'] = int(1)
                jcfg['Vol']['hash_svmc'] = tomca.util.hash_vol(jcfg)
    else:
        tomca.util.verb('svmc evaluation skipped, Toggle with jcfg[\'Domain\'][\'MediaFormat\']=\"svmc\"', jcfg['Expt']['verb'], 3)
        jcfg['Vol']['svmc'] = int(0)
...
def mcfg_dept_to_dict_MCX_style(res, savedetflag):
    ...
    Args:
        res (dict): Results directly from tomca.run or pmcx.run
    ...
    if 'traj' in res.keys():
        ...
        ##### extract the scatter (traj) data from res output of tomca.run or pmcx.run
...
def savedetflags_to_dict_MCX_style(jcfg, isReplay='res'):
    ...
    Args:
        res (dict): Results directly from tomca.run
    ...
    tomca.util.verb(f'Found {nMediaIndexes} media indexes in results. Parsed into'+strout, jcfg['Expt']['verb'], 2)
