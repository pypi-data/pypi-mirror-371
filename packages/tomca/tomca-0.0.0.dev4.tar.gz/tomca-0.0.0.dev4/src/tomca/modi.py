...
def vol_bcs(cfg, vol):
    ...
    tomca.util.verb('Modified volume boundary conditions', cfg['Expt']['verb'], 2)
...
def mask_detected_photons(jcfg, resName='res', mask=None):
    ...
    """
    import numpy as np
    from src.tomca.util import verb
...
def combine_sims(jcfgs):
    ...
    jcfg0=jcfgs[0]
    jcfg1=jcfgs[1]
    differences, err = tomca.util.compare_mcfg(jcfg0['mcfg'], jcfg1['mcfg'], verb=jcfg1['Expt']['verb'])
