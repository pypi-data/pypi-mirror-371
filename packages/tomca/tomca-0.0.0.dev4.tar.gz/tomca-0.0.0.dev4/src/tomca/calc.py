...
def interpolate_spectrum_generic(jcfg: dict, absorber_name: str, wls: float):
    ...
    ## @cite Nazarian2024.
        ...
        lam, mua_collagen_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "Collagen_muA_Nazarian2024_mm-1.json")

        # Boundary checking
        if lam[0] > wls[0]:
            tomca.verb(f"!! The left side of the interpolated spectral range is lower than the collagen_mua data spectral range {wls[0]}, using musp = {round(mua_collagen_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if lam[-1] < wls[-1]:
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the collagen_mua data spectral range {wls[-1]}, using musp ={round(mua_collagen_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        ...
        lam, musp_collagen_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "Collagen_muSp_Nazarian2024_mm-1.json")
        
        # Boundary checking
        if lam[0] > wls[0]:
            tomca.verb(f"!! The left side of the interpolated spectral range is lower than the collagen_musp data spectral range {wls[0]}, using musp = {round(musp_collagen_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if lam[-1] < wls[-1]:
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the collagen_musp data spectral range {wls[-1]}, using musp = {round(musp_collagen_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        ...
        lam, mua_bone_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "Bone_mua_Dolenec2019_mm-1.json")
        
        # Boundary checking
        if lam[0] > wls[0]:
            tomca.verb(f"!! The left side of the interpolated spectral range  is lower than the bone_mua data spectral range {wls[0]}, using musa = {round(mua_bone_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if lam[-1] < wls[-1]:
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the bone_mua data spectral range {wls[-1]}, using musa = {round(mua_bone_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        ...
        lam, mus_bone_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "Bone_mus_Dolenec2019_mm-1.json")
        
        # Boundary checking
        if np.floor(lam[0]) > np.round(wls[0]):
            tomca.verb(f"!! The left side of the interpolated spectral range is lower than the bone_mus data spectral range {wls[0]}, using musp = {round(mus_bone_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if np.floor(lam[-1]) < np.round(wls[-1]):
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the bone_mus data spectral range {wls[-1]}, using musp = {round(mus_bone_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        ...
        lam, eps_eumel_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "eumelanin_molar_ext_Ansari2011.json")
        eps_eumel_lam = [value / 10 for value in eps_eumel_lam] # convert to mm^-1 (current units is cm^-1/M)

        # Boundary checking
        if np.floor(lam[0]) > np.round(wls[0]):
            tomca.verb(f"!! The left side of the interpolated spectral range is lower than the eumel_eps data spectral range {wls[0]}, using left bound value = {round(eps_eumel_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if np.floor(lam[-1]) < np.round(wls[-1]):
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the eumel_eps data spectral range {wls[-1]}, using right bound value = {round(eps_eumel_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        ...
        lam, eps_pheomel_lam = tomca.impo.extracted_data(Path(RESOURCES_PATH) / "pheomelanin_molar_ext_Ansari2011.json")
        eps_pheomel_lam = [value / 10 for value in eps_pheomel_lam] # convert to mm^-1 (current units is cm^-1/M)
        
        # Boundary checking
        if np.floor(lam[0]) > np.round(wls[0]):
            tomca.verb(f"!! The left side of the interpolated spectral range is lower than the pheomel_eps data spectral range {wls[0]}, using left bound value = {round(eps_pheomel_lam[0],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
        if np.floor(lam[-1]) < np.round(wls[-1]):
            tomca.verb(f"!! The right side of the interpolated spectral range is larger than the pheomel_eps data spectral range {wls[-1]}, using right bound value = {round(eps_pheomel_lam[-1],3)}/mm for those points.", jcfg['Expt']['verb'], 0)
...
def try_evaluate_media_prop(jcfg, mediaPropName, media_idx):
    ...
    if mediaPropName in ["bf", "water", "iL_f", "fat"]:
        ...
        if propOut < 0:
            tomca.util.verb(f"!! {mediaPropName} in {layerLabel} calculated to be negative.  Truncated to 0.",jcfg['Expt']['verb'],0)
            propOut=0
        if propOut > 1:
            tomca.util.verb(f"!! {mediaPropName} in {layerLabel} calculated to be greater than 1.  Truncated to 1.",jcfg['Expt']['verb'],0)
            propOut= 1
    return propOut
...
def mu_abs_scatt(jcfg):
    ...
    for iiMedia in enumerate(jcfg["Domain"]["Media"]):
        ...
        if notFat_f + fat_f > 1:
            tomca.util.verb(f"!! fat_f + notfat_f > 1 in {layerLabel}.  fat_f adjusted from {fat_f} to {1-notFat_f}.",jcfg['Expt']['verb'],0)
        ...
        if notCollagen_f + collagen_f > 1:
            tomca.util.verb(f"!! collagen_f + notCollagen_f > 1 in {layerLabel}.  collagen_f adjusted from {collagen_f} to {1-notCollagen_f}.",jcfg['Expt']['verb'],0)
        ...
        if notWater_f + water_f > 1:
            tomca.util.verb(f"!! water_f + notWater_f > 1 in {layerLabel}.  water_f adjusted from {water_f} to {1-notWater_f}.",jcfg['Expt']['verb'],0)
        ...
        if notiL_f + iL_f > 1:
            tomca.util.verb(f"!! iL_f + notiL_f > 1 in {layerLabel}.  iL_f adjusted from {iL_f} to {1-notiL_f}.",jcfg['Expt']['verb'],0)
        ...
        if write_mua:
            ...
            tomca.util.verb(f'Absorption components: ' + str(layer_comp_labels), jcfg['Expt']['verb'], 3)
            tomca.util.verb(f'Component fractions: ' + str(np.round(layer_comp,3)), jcfg['Expt']['verb'], 3.5)
            tomca.util.verb(f'Absorption component mua\'s: ' + str(np.round(layer_comp_mua,3)), jcfg['Expt']['verb'], 3.5)
        ...
        if usp_collagen_spect is not None:
            ...
            uSProfile=1
            tomca.util.verb(f"Using collagen spectral musp rather than jcfg musp directly.",jcfg['Expt']['verb'],3)	
            
        if us_bone_spect is not None:
            mus0=us_bone_spect
            uSProfile=1
            tomca.util.verb(f"Using bone spectral mus rather than jcfg mus directly.",jcfg['Expt']['verb'],3)
        ...
        if mus0 is not None:
            ...
            tomca.util.verb(f"Using mus_factor {mus_factor} with mus0 amplitude for mus {mus}",jcfg['Expt']['verb'],3)
        ...
        # muSprime: componded scattering coefficient with the g factor
            ...
            if len(mus_spect) >1:
                ...
                muspDisp=musp
            tomca.util.verb(f"Using mus_factor {mus_factor} with musp amplitude {muspDisp} and g {g_factor} for mus {mus}",jcfg['Expt']['verb'],3)
        # Uses input mus directly from the model jcfg
        else:
            mus = jcfg["Domain"]["Media"][iiMedia[0]]["mus"]
            tomca.util.verb(f"Using mus directly from jcfg model, {mus}",jcfg['Expt']['verb'],3)
        ...
        if 'write' in jcfg['Expt']:
            if 'fig_spectra_mua' in jcfg['Expt']['write']:
                if jcfg['Expt']['write']['fig_spectra_mua']:
                    tomca.writ.fig(jcfg, plt_muASpectra, "spectra_mua")
        else:
            tomca.writ.fig(jcfg, plt_muASpectra, "spectra_mua")
        ...
        if 'write' in jcfg['Expt']:
            if 'fig_spectra_mus' in jcfg['Expt']['write']:
                if jcfg['Expt']['write']['fig_spectra_mus']:
                    tomca.writ.fig(jcfg, plt_muSSpectra, "spectra_muS")
        else:
            tomca.writ.fig(jcfg, plt_muSSpectra, "spectra_muS")
    ...
    if "imgDetectors" in jcfg['Optode']:
        if "type" in jcfg["Optode"]["imgDetectors"]:
            if jcfg["Optode"]["imgDetectors"]["type"]=='donut':
                jcfg=tomca.anly.img_detectors_donut(jcfg)
            else:
                ...
                jcfg=tomca.anly.image_detectors(jcfg, jcfg['Optode']['imgDetectors']['Pos'], jcfg['Optode']['imgDetectors']['R']/jcfg['Domain']['LengthUnit'])
            
        tomca.util.verb('Evaluated detectors in post processing', jcfg['Expt']['verb'], 1)
...
def energy_volume(jcfg):
    ...
    if jcfg['Session']['OutputType']=='flux':
        ...
        tomca.util.verb('Energy calculation requires jcfg[\'Session\'][\'OutputType\']==\'flux\'', jcfg['Expt']['verb'], 0)
...
def energy_per_media(jcfg, saveKey='res_replay'):
    ...
    tomca.util.verb('Energy per media calculated for '+ saveKey, jcfg['Expt']['verb'], 1)
