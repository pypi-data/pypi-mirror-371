...
write out jdat, json, and figures with logging in tmcx.
"""
...
def mcfg_json(jcfg):
    ...
    jdata.savet(tmp,
                ...
                indent=4)
    tmcx.util.verb('Saved ' + '_mcfg' + '.json file @: ' + outFile,
                   jcfg['Expt']['verb'], 1)
    return jcfg
...
def jcfg_json(jcfg, multi=0, desc='_jcfg'):
    ...
    if multi == 0:
        ...
        jdata.savet(tmp, outFile := os.path.join(tmp['Expt']['pathState'],
                    tmp['Expt']['time'] + desc + '.json'), indent=4)
    tmcx.util.verb('Saved ' + desc + '.json file @: ' + outFile,
                   jcfg['Expt']['verb'], 1)
    ...
    if "plot" in jcfg['Expt']:
        ...
        if jcfg['Expt']['plot']['writeFigs']:
            try:
                ...
                tmcx.util.verb('Wrote plot: ' + filename, jcfg['Expt']['verb'], 1)
            except Exception as e:
                tmcx.util.verb(f'Failed to write plot {filename}: {e}', jcfg['Expt']['verb'], 1)
        else:
            tmcx.util.verb('Did not write plot ' + filename + '. Toggle with jcfg[\'Expt\'][\'plot\'][\'writeFigs\']', jcfg['Expt']['verb'], 1.5)
    return
    ...
    if "write" in jcfg['Expt']:
        ...
        if "media_table" in jcfg['Expt']['write']:
            if jcfg['Expt']['write']['media_table']:
                ...
                tmcx.writ.fig(jcfg, outTable, 'media_table', 'state')

    if savepkl:
        jcfg['Expt']['pklData'] = jcfg['Expt']['pathState']+'_simulated_jcfg.pkl'
        tmcx.util.pickle_out(jcfg, jcfg['Expt']['pklData'])
        tmcx.util.verb('Saved .pkl file at ' + jcfg['Expt']['pklData'],
                       jcfg['Expt']['verb'], 1)
