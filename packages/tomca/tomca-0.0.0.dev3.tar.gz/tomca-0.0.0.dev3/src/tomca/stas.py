...
def bin_voxels(dat, param, acceptAngle, verb):
    ...
    tmcx.util.verb('Loaded analysis .json file with Dims: ' + str(Dim),
                   verb, 3)
    ...
    tmcx.util.verb('Binned with max: ' + str(np.max(IM)) + ' and min: '+str(
        np.min(IM)), 1, 1)
...
def virt_detector(dat, param, vDetPos, vDetRad, acceptAngle, verb):
    ...
    tmcx.util.verb('Built virtual detector at: ' + str(vDetPos) + ', R = ' +
...
def bin_voxels_tmcx(jcfg, acceptAngle=45, inName='res'):
    ...
    tmcx.util.verb('Binning photons detected', verb, 3)
    ...
    if "acceptAngle_deg" in jcfg['Optode']['Detector']:
        ...
        tmcx.util.verb('Using acceptance angle of ' + str(acceptAngle) +
                       ' degrees', verb, 2)
    ...
    tmcx.util.verb('Binned ' + str(np.squeeze(x.shape)) + ' photons with max: '
                   + str(np.max(IM)) + ' and min: '+str(np.min(IM)), verb, 1)
    try:
        # Bin detected photons into angle
        hist_values = tmcx.stas.bin_detp_angles(jcfg, weights, inName=inName)
