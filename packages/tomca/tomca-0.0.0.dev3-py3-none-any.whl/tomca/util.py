...
def prepare_state(jcfg, delta_cfg):
    ...
    if 'stateName' not in jcfg['Expt']:
        stateOut=""
        # for ii in leafs:
        #     stateOut+=" "+str(leafs[ii][-1])+"_"+str(tmcx.util.leaf_value(jcfg, leafs[ii]))
        jcfg['Expt']['stateName']=stateOut
    jcfg['Expt']['DeltaCfg'] = delta_cfg
    #tmcx.util.verb('Prepared state:'+stateOut, jcfg['Expt']['verb'], 0)

    jcfg = tmcx.make.dir_state(jcfg)
    ...
    tmcx.util.verb('Prepared state: '+json_str, jcfg['Expt']['verb'], 0)
...
def rebuild_state(jcfg, saveDir=None):
    ...
    # Save output files
    jcfg=tmcx.writ.state_outputs(jcfg)

    tmcx.util.verb('Finished state. '+ 'Save directory: ' + jcfg['Expt']['pathState'], jcfg['Expt']['verb'], 0)
...
def compare_mcfg(mcfg1, mcfg2, verb=1):
    ...
    if differences:
        ...
        tmcx.util.verb('Compared mcfgs, important differences found for combining simulations : ' + str(differences), verb, 0)
    elif unimportant_differences:
        # Log unimportant differences
        tmcx.util.verb('Compared mcfgs, unimportant differences found for combining simulations: ' + str(unimportant_differences), verb, 1)
        err = 0  # No error for unimportant differences
    else:
        # Log no differences found
        tmcx.util.verb('Compared mcfgs, no differences found', verb, 1)
