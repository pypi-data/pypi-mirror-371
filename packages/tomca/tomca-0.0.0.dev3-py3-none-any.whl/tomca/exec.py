...
def mcx_with_replays_and_cycles(jcfg: dict):
    ...
    for rep in np.arange(jcfg['Expt']['stateCycles']):
        ...
        tmcx.util.verb('Starting state cycle: ' + str(rep+1) + ' of ' +
        ...
        jcfg_tmp = tmcx.exec.mcx(copy.deepcopy(jcfg_tmp_init))

        # Parse saved photon info
        if 'SaveDataMask' in jcfg_tmp['Session']:
            jcfg_tmp = tmcx.conv.savedetflags_to_dict(jcfg_tmp)

            # Mask detected photons if a mask or detection limits
            # or acceptance angle are used
            jcfg_tmp = tmcx.modi.mask_detected_photons(jcfg_tmp)
        ...
        if 'det_angles' in jcfg['res']:
            ccd_vals, angular_dist = tmcx.stas.bin_voxels_tmcx(
                jcfg_tmp, inName='res')
            jcfg['res']['det_angles'] += angular_dist
            jcfg = tmcx.grap.detector_angles_histogram(jcfg, inName='res')
        else:
            ccd_vals, _ = tmcx.stas.bin_voxels_tmcx(
                jcfg_tmp, inName='res')
        ...
        if jcfg['Expt']['doReplay']:
            for i, det_combo in enumerate(det_combos):
                ...
                if jcfg_tmp['Expt']['doReplay']:
                    ...
                    jcfg_tmp = tmcx.modi.mask_detected_photons(
                        jcfg_tmp, resName=det_combo_label, mask=det_combo)
                    jcfg_tmp = tmcx.exec.mcx_replay(
                        jcfg_tmp, saveKey=det_combo_label)

                    # Parse photon info
                    if 'SaveDataMask' in jcfg_tmp['Session']:
                        jcfg_tmp = tmcx.conv.savedetflags_to_dict(
                        ...
                        if 'det_angles' in jcfg[det_combo_label]:
                            ...
                            jcfg = tmcx.grap.detector_angles_histogram(
    ...
    # much as possible.
    tmcx.writ.try_del(jcfg, ['mcfg', 'vol'])
    tmcx.writ.try_del(jcfg['res'], ['seeds'])
    if 'offload' in jcfg['Expt'] and jcfg['Expt']['offload']:
        tmcx.writ.try_del(jcfg['res'], ['p'])
        tmcx.writ.try_del(jcfg['res'], ['v'])
        tmcx.writ.try_del(jcfg['res'], ['x'])
        tmcx.writ.try_del(jcfg['res'], ['seeds_masked'])
        tmcx.writ.try_del(jcfg['res'], ['detp'])
        tmcx.writ.try_del(jcfg['res'], ['detp_masked'])
        tmcx.writ.try_del(jcfg['res'], ['p_masked'])
        tmcx.writ.try_del(jcfg['res'], ['v_masked'])
        tmcx.writ.try_del(jcfg['res'], ['x_masked'])

    tmcx.util.verb('Finished state cycles', jcfg['Expt']['verb'], 1)
    ...
    if jcfg['Session']['OutputType'] == 'energy':
        ...
        if det_combo_labels == []:
            if 'CalculateEnergy' in jcfg['Session']:
                if jcfg['Session']['CalculateEnergy']:
                    jcfg = tmcx.calc.energy_per_media(
                        jcfg,  saveKey='res')
        ...
        if det_combo_labels == []:
            if 'CalculateEnergy' in jcfg['Session']:
                if jcfg['Session']['CalculateEnergy']:
                    jcfg = tmcx.calc.energy_per_media(jcfg, saveKey='res')
        else:
            for det_combo_label in det_combo_labels:
                if 'CalculateEnergy' in jcfg['Session']:
                    if jcfg['Session']['CalculateEnergy']:
        ...
        if 'offload' in jcfg['Expt']:
            if jcfg['Expt']['offload']:
                if 'detp' in jcfg[det_combo_label]:
                    tmcx.writ.try_del(jcfg[det_combo_label], ['detp'])
                if 'seeds' in jcfg[det_combo_label]:
                    tmcx.writ.try_del(jcfg[det_combo_label], ['seeds'])
    tmcx.writ.try_del(jcfg['mcfg'], ['detphotons'])
    tmcx.writ.try_del(jcfg['mcfg'], ['seed'])

    return jcfg


def mcx(jcfg: dict, saveKey='res'):
    """Run the pmcx.run function, saving to the tmcx dict structures
    ...
    if saveKey != 'res':
        tmcx.util.verb('Starting replay MCX evaluation for '+saveKey,
                       jcfg['Expt']['verb'], 1)
    else:
        tmcx.util.verb('Starting primary MCX evaluation',
                       jcfg['Expt']['verb'], 1)
...
def simulate(jcfg, delta_cfg=None, offloader_cfg=0):
    ...
    tmcx.util.update_jcfg(jcfg, delta_cfg)
    jcfg = tmcx.util.prepare_state(jcfg, delta_cfg)

    # Generate volume from jcfg parameters
    if 'vol' not in jcfg:
        jcfg = tmcx.buil.vol_init(jcfg)
        jcfg = tmcx.buil.vol_layers(jcfg)
        jcfg = tmcx.buil.vol_modify_region(jcfg)
        jcfg = tmcx.buil.vol_vessel(jcfg)
        jcfg = tmcx.conv.vol_svmc(jcfg)

    # Prepare simulation optics
    jcfg = tmcx.buil.source(jcfg)
    jcfg = tmcx.buil.detectors(jcfg)

    # Import Source Spectrum
    jcfg = tmcx.buil.incident_spectrum(jcfg)

    # Evaluate optical properties
    jcfg = tmcx.calc.mu_abs_scatt(jcfg)

    # Build up a string with the interesting parameters from the simulation
    # state for figures
    jcfg = tmcx.make.state_str(jcfg)

    # Do the simulation, with any repeats of the
    # state accumulated appropriately
    jcfg = tmcx.exec.mcx_with_replays_and_cycles(jcfg)

    # Generate plots
    jcfg = tmcx.grap.state_xsections(jcfg)

    # Evaluate imaginary image detectors, if any
    jcfg = tmcx.calc.img_detectors(jcfg)

    jcfg['Expt']['tf'] = time.time()
    # Save output files
    jcfg = tmcx.writ.state_outputs(jcfg, saveDesc='jcfg_results')

    tmcx.util.verb('Finished state. ' + 'Save directory: ' +
                   jcfg['Expt']['pathState'], jcfg['Expt']['verb'], 0)
    # tmcx.util.verb(' ', jcfg['Expt']['verb'], 0)
...
def simulate_multiSource(jcfg, delta_cfg=None, offloader_cfg=0):
    ...
    if delta_cfg is None:
        delta_cfg = {}
    if isinstance(delta_cfg, tmcx.util.branch_builder) or isinstance(delta_cfg,
                                                                     dict):
    ...
    # check if it's a list of states
        ...
        for i in range(len(delta_cfg)):
            tmcx.util.verb(f'Simulating source {i+1} of {nSoures}',
                           jcfg['Expt']['verb'], 1.5)
            jcfg_combined.append(simulate(copy.deepcopy(jcfg), delta_cfg[i],
                                          offloader_cfg))
        # combine the results
        jcfg_combined = tmcx.modi.combine_n_sims(jcfg_combined)

        # Generate plots with multiple sources
        tmcx.grap.state_xsection_CCD(jcfg_combined, resName='res_replay',
                                     desc='_combinedSources')
        tmcx.grap.state_xsection_CCDx(jcfg_combined, resName='res_replay',
                                      desc='_combinedSources')
        tmcx.grap.state_xsection_flux(jcfg_combined,
                                      desc='_combinedSources')
        tmcx.grap.state_xsection_flux_replay(jcfg_combined,
                                             desc='_combinedSources')
        # Save combined sources jcfg
        tmp = tmcx.writ.reduce_jcfg(jcfg_combined)

        tmcx.writ.jcfg_json(tmp, desc='_combinedSources')
