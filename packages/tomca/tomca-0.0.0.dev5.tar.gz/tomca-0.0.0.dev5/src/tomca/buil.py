...
def vol_init(jcfg:dict):
    ...
    if "useShapes" in jcfg['Vol']:
        if jcfg['Vol']['useShapes']==0:
            tomca.util.verb("Initialized volume defined by voxels with dims " + str(jcfg['Domain']['Dim']), jcfg['Expt']['verb'], 1)
            if 'Shapes' in jcfg: 
                del jcfg['Shapes']
        else:
            jcfg['Domain']['Dim']=[xdimV, ydimV, zdimV]
            ...
            tomca.util.verb("Volume being computed as shapes with dims " + str(jcfg['Domain']['Dim']), jcfg['Expt']['verb'], 1)
    return (jcfg)
...
def vol_layers(jcfg):
    ...
    if "useShapes" in jcfg['Vol']:
        if jcfg['Vol']['useShapes']==0:
            ...
            # Verbose feedback
            tomca.util.verb('Z-axis cross section:',jcfg['Expt']['verb'], 3)
            tomca.util.verb(str(vol[1, 1, :]),jcfg['Expt']['verb'], 3)
            if jcfg['Expt']['verb'] >= 3.5:
                fig = tomca.grap.voxVol(np.sqrt(vol), [int(np.floor(jcfg['Domain']['Dim'][0]/2)), int(
    ...
    tomca.util.verb("Added " + str(nLayers) + ' layers to volume', jcfg['Expt']['verb'], 1)
...
def vol_cyl(jcfg, vol):
    ...
    if jcfg['Expt']['verb'] >= 3.5:
        fig = tomca.grap.voxVol(ang, [jcfg['Vol']['Shapes']['Cylinder']['x_pos'], int(np.floor(jcfg['Vol']['volSize'][1]/jcfg['Domain']
    ...
    # Debug tool to visualize distance in volume of every point to vessel
    if jcfg['Expt']['verb'] >= 3.5:
        fig2 = tomca.grap.voxVol(np.sqrt(dist), [jcfg['Vol']['Shapes']['Cylinder']['x_pos'], int(np.floor(jcfg['Vol']['volSize'][1] /
    ...
    # Verbose feedback
    tomca.util.verb("Built cylinder at x= "+ str(jcfg['Vol']['Shapes']['Cylinder']['x_pos']) + " dia = " + str(jcfg['Vol']['Shapes']['Cylinder']['r']) + "mm, depth= " + str(
    ...
    if jcfg['Expt']['verb'] >= 2.5:
        fig3 = tomca.grap.voxVol(vol, [jcfg['Vol']['Shapes']['Cylinder']['x_pos']-1, int(np.floor(jcfg['Vol']['volSize'][1]/jcfg['Domain']
                                ['LengthUnit']/2))-1, int(np.floor(jcfg['Vol']['Shapes']['Cylinder']['z_pos']/jcfg['Domain']['LengthUnit']))-1])
    ...
    if 'vessel' in jcfg['Vol']:
        if "useShapes" in jcfg['Vol']:
            if jcfg['Vol']['useShapes']==0:
                ...
                # Debug tool to visualize distance in volume of every point to vessel
                if jcfg['Expt']['verb'] >= 3.5:
                    fig = tomca.grap.voxVol(np.sqrt(dist), [round(jcfg['Vol']['vessel']['v_xPos']), int(np.floor(
                ...
                # Verbose feedback
                tomca.util.verb("Built blood vessel radius = " + str(
                    jcfg['Vol']['vessel']['rVess']) + "mm, depth center= " + str(v_zPos) + "mm", jcfg['Expt']['verb'], 1)
                if jcfg['Expt']['verb'] >= 3.5:
                    fig2 = tomca.grap.voxVol(vol, [round(jcfg['Vol']['vessel']['v_xPos']), int(np.floor(
                        jcfg['Domain']['Dim'][1]/2))-1, int(np.floor(v_zPos/jcfg['Domain']['LengthUnit']))-1])
                    fig2.suptitle('Cutaways of distance from blood vessel')
                jcfg['vol']=vol  
...
def source(jcfg):
    ...
    if jcfg['Optode']['Source']['Type'] == "line":
        ...
        tomca.util.verb("Built line source at "+ str(jcfg['Optode']['Source']['Pos']) + "vox, length: "+ str(jcfg['Optode']['Source']['Param1']) , jcfg['Expt']['verb'], 1)
    ...
    if jcfg['Optode']['Source']['Type'] == "fullSurface":
        ...
        tomca.util.verb('Built source of Type: ' + jcfg['Optode']['Source']['Type'] + ' at Pos: ' + str(
    ...
    if "NA" in jcfg['Optode']['Source']:
        ...
        tomca.util.verb("Set source NA to "+str(jcfg['Optode']['Source']["NA"])+" for dir vector "+str(xyz), jcfg['Expt']['verb'], 1)
    ...
    if jcfg['Optode']['Source']['Type'] == "angleinvcdf":
        ...
        if "angleinvcdf" in jcfg['Optode']['Source']:
            ...
            if jcfg['Optode']['Source']['angleinvcdf']=='calc':
                ...
                thetas, probs = tomca.impo.extracted_data(jcfg['Optode']['Source']['angleinvcdf_file'])
                import os
                file_name = os.path.basename(jcfg['Optode']['Source']['angleinvcdf_file'])
                if (('Polar' in file_name) or ('polar' in file_name)):
                    thetas, probs = tomca.calc.coord_transform_carthesian_to_polar(thetas,probs)
                    probs = np.array(probs)[np.array(thetas)>=0]
                    thetas = np.array(thetas)[np.array(thetas)>=0]
                theta_equal_probs=tomca.calc.equally_probable_theta_intervals(thetas, probs, num_data_points=3000)
                jcfg['Optode']['Source']['angleinvcdf']=theta_equal_probs
                tomca.util.verb("Calculated angles of emission with equal probability", jcfg['Expt']['verb'], 2)
            else: 
                tomca.util.verb("Relying on angleinvcdf being defined in jcfg directly", jcfg['Expt']['verb'], 1)
        else:
            pass
    ...
    if np.sqrt(np.sum(np.array(jcfg['Optode']['Source']['Dir'][:3])**2)) !=1:
        ...
        tomca.util.verb(f"Source direction not normalized. Normalized to {jcfg['Optode']['Source']['Dir']}", jcfg['Expt']['verb'], 1)
...
def detectors(jcfg):
    ...
    if jcfg['Optode']['Detector']['Type'] == "custom":
        # Leave source parameters alone, as decribed in script
        tomca.util.verb('Built detector of Type: ' + jcfg['Optode']['Detector']
...
#         jcfg['Optode']['Detector']['R']=[0,0,0]
    ...
    if jcfg['Optode']['Detector']['Type'] == "bc_planes":
        ...
        tomca.util.verb('Built detector of Type: ' + jcfg['Optode']['Detector']['Type'] +
    ...
    if jcfg['Optode']['Detector']['Type'] == "Sphere":
        ...
        tomca.util.verb('Built detector of Type: ' + jcfg['Optode']['Detector']['Type'] + ' at Pos: ' + str(
            jcfg['Optode']['Source']['Pos']), jcfg['Expt']['verb'], 1)
...
def incident_spectrum(jcfg: dict):
    ...
    if jcfg['Optode']["Source"]['spectProfile'] == "gaussian":
        ...
        tomca.util.verb(f"Building incident spectrum with gaussian profile mu={mu} sig={sig}", jcfg['Expt']['verb'], 1)
        tomca.util.verb(f"Interpolating spectrum from {jcfg['Optode']['Source']['lam0']} to {jcfg['Optode']['Source']['lamN']} stepping {jcfg['Optode']['Source']['dlam']}", jcfg['Expt']['verb'], 2)
        ...
        tomca.util.verb(f"Building incident spectrum with bandpass profile at WL={mu}nm width={sig}nm", jcfg['Expt']['verb'], 1)
        tomca.util.verb(f"Interpolating spectrum from {jcfg['Optode']['Source']['lam0']} to {jcfg['Optode']['Source']['lamN']} stepping {jcfg['Optode']['Source']['dlam']}", jcfg['Expt']['verb'], 2)
        ...
        jcfg = tomca.impo.incident_spectrum(jcfg)
...
def vol_modify_region(jcfg):
    ...
    for i, region_config in enumerate(regions):
        if not isinstance(region_config, dict):
            tomca.util.verb(f"Warning: Region config {i} is not a dict - skipping", jcfg['Expt']['verb'], 1)
            continue
            
        if 'operation' not in region_config or 'value' not in region_config:
            tomca.util.verb(f"Warning: Region config {i} missing required keys (region, operation, value) - skipping", jcfg['Expt']['verb'], 1)
            continue
        ...
        if not isinstance(region, dict):
            tomca.util.verb(f"Warning: Region {i} is not a dict - skipping", jcfg['Expt']['verb'], 1)
            continue
        ...
        if x_start_actual >= x_end_actual or y_start_actual >= y_end_actual or z_start_actual >= z_end_actual:
            tomca.util.verb(f"Warning: Invalid region {i} specified - skipping", jcfg['Expt']['verb'], 1)
            continue
        ...
        if operation == 'absolute':
            ...
            tomca.util.verb(f"Region {i+1}: Absolute assignment: Set region [{x_start_actual}:{x_end_actual}, {y_start_actual}:{y_end_actual}, {z_start_actual}:{z_end_actual}] to value {value}", 
                           jcfg['Expt']['verb'], 1)
            
        elif operation == 'increment':
            vol[x_start_actual:x_end_actual, y_start_actual:y_end_actual, z_start_actual:z_end_actual] += value
            tomca.util.verb(f"Region {i+1}: Increment operation: Added {value} to region [{x_start_actual}:{x_end_actual}, {y_start_actual}:{y_end_actual}, {z_start_actual}:{z_end_actual}]", 
                           jcfg['Expt']['verb'], 1)
            tomca.util.verb(f"Region {i+1}: Original values {original_unique_values} became {np.unique(vol[x_start_actual:x_end_actual, y_start_actual:y_end_actual, z_start_actual:z_end_actual])}", 
                           jcfg['Expt']['verb'], 2)

        else:
            tomca.util.verb(f"Warning: Invalid operation '{operation}' for region {i} - skipping", jcfg['Expt']['verb'], 1)
            continue
        ...
        if jcfg['Expt']['verb'] >= 2:
            ...
            tomca.util.verb(f"Region {i+1}: Size: {abs(dx)}x{abs(dy)}x{abs(dz)} voxels", jcfg['Expt']['verb'], 2)
            tomca.util.verb(f"Region {i+1}: Final values in region: {modified_values}", jcfg['Expt']['verb'], 2)
    ...
    tomca.util.verb(f"Successfully processed {total_regions_processed} out of {len(regions)} regions", jcfg['Expt']['verb'], 1)
