...
def dir_expt(jcfg:dict):
    ...
    tmcx.util.verb('Prepared experiment save path @ ' + jcfg['Expt']['pathExpt'], jcfg['Expt']['verb'], 0)
...
def dir_state(jcfg:dict, post_build=0, append_str=""):
    ...
    tmcx.util.verb('Prepared state save path @ ' + jcfg['Expt']['pathState'], jcfg['Expt']['verb'], 1)
...
def sim_easyHyperboloid():
    ...
    mcfg: pmcx dict
        ...
        for a pmcx.run(mcfg), or a tmcx.run(mcfg).
