...
class Interface:
    ...
    def run_simulation(self, delta_configs: list[DeltaConfig], name = "no_name", offloader_cfg = 0,  output_level: int = 0) -> list:
        ...
        if name == 'repeatKey_noNewExptFolder':
            ...
            self.config['Expt']['pathExpt']=tomca.make.dir_expt(self.config)['Expt']['pathExpt']
        ...
        if offloader_cfg == 0:
            ...
            flattened_configs=[]
            offload_results=tomca.offl.simulate(self.config, delta_configs, offloader_cfg=offloader_cfg)
    ...
    def create_flattened_configs(self, delta_configs: list[DeltaConfig]) -> list[dict]:
        ...
        for delta_config in delta_configs:
            ...
            tmp=tomca.util.leaf_finder(delta_config, delta_config, 0,[], {})
            flat=[]
            for loc in tmp:
            ...
            for config_loc in self.list_locs():
                ...
                if config_loc in delta_flattened[-1]:
                    ...
                    flattened_config[config_loc] = tomca.util.leaf_value(delta_config,config_loc)
                else:
