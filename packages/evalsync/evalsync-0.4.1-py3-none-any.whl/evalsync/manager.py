...
class ExperimentManager:
    ...
    def wait_for_all_workers(self, timeout: int | None = None):
        self.state_cond.acquire()
        self.state_cond.wait_for(lambda: len(self.connected_workers) == self.num_workers, timeout)
        self.state_cond.release()
    ...
    def wait_for_end_of_workers(self, worker_ids: Sequence[str], timeout: int | None = None):
        self.state_cond.acquire()
        self.state_cond.wait_for(
            lambda: all(
    ...
    def _send(self, client_id: str, message: Message):
        ...
        self.command_socket.send_multipart([client_id.encode(), b"", message.SerializeToString()])
