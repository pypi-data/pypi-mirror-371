...
class ExperimentWorker:
    def __init__(self, experiment_id: str | None, client_id: str | None, verbose: bool = False):
        self.context = zmq.Context()
        self.experiment_id = experiment_id if experiment_id else os.environ["EVALSYNC_EXPERIMENT_ID"]
        ...
        if self.verbose:
            logger.info(f"[evalsync-worker] command channel: {self.command_socket_path}")
    ...
    def notify_manager(self, msg: str) -> bool:
        ...
        if self.verbose:
            logger.info(f"[evalsync-worker] [+] send message: {text_format.MessageToString(message, as_one_line=True)}")
    ...
    def ready(self) -> bool:
        if self.verbose:
            logger.info(f"[evalsync-worker] API CALL: ready() - current state: {self.state}")
        if self.state == ServiceState.INIT:
            self.state = ServiceState.READY
            self.notify_manager("ready")
            if self.verbose:
                logger.info("[evalsync-worker] API CALL: ready() - SUCCESS, transitioned to READY")
            return True
        else:
            if self.verbose:
                logger.warning("[evalsync-worker] API CALL: ready() - FAILED, not in INIT state")
            return False

    def wait_for_start(self) -> bool:
        if self.verbose:
            logger.info(f"[evalsync-worker] API CALL: wait_for_start() - current state: {self.state}")
            ...
            while self.state != ServiceState.RUNNING:
                ...
                match message.command:
                    case ExperimentCommand.BEGIN:
                        ...
                        if self.verbose:
                            logger.info("[evalsync-worker] API CALL: wait_for_start() - SUCCESS, received BEGIN")
                        return True
                    case ExperimentCommand.END | ExperimentCommand.ABORT:
                        self.state = ServiceState.ERROR
                        self.notify_manager("error")
                        if self.verbose:
                            logger.warning("[evalsync-worker] API CALL: wait_for_start() - FAILED, received END/ABORT")
                        return False
        else:
            if self.verbose:
                logger.warning("[evalsync-worker] API CALL: wait_for_start() - FAILED, not in READY state")

        return False

    def end(self):
        if self.verbose:
            logger.info(f"[evalsync-worker] API CALL: end() - current state: {self.state}")
        if self.state == ServiceState.RUNNING:
            self.state = ServiceState.DONE
            self.notify_manager("done")
            if self.verbose:
                logger.info("[evalsync-worker] API CALL: end() - SUCCESS, transitioned to DONE")
            return True
        else:
            if self.verbose:
                logger.warning("[evalsync-worker] API CALL: end() - FAILED, not in RUNNING state")
            return False

    def wait_for_stop(self):
        if self.verbose:
            logger.info(f"[evalsync-worker] API CALL: wait_for_stop() - current state: {self.state}")
        if self.state == ServiceState.RUNNING:
            while self.state != ServiceState.DONE:
                ...
                match message.command:
                    case ExperimentCommand.END:
                        ...
                        self.notify_manager("done")
                        if self.verbose:
                            logger.info("[evalsync-worker] API CALL: wait_for_stop() - SUCCESS, received END")
                        return True
            ...
            if self.verbose:
                logger.warning("[evalsync-worker] API CALL: wait_for_stop() - FAILED, not in RUNNING state")
        return False
