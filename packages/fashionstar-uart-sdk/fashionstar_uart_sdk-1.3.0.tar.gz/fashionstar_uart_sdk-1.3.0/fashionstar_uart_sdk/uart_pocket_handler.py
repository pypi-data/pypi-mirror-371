...
class Monitor_data_4096:
	...
	temperature :float
	status:int
	def __init__(self, id:int, current_position:int, power:int, voltage:int, current:int, temperature:float, status:int):
		self.id = id
		self.current_position = current_position
		self.power = power
		self.voltage = voltage
...
class PortHandler(UartServoManager):
	...
	def __init__(self, port_name:str, baudrate:int):
		...
		self.read={
			...
			"Min_Position_Limit":self.ReadMinPositionLimit,
			"Max_Position_Limit":self.ReadMaxPositionLimit,

		}
		self.sync_read={
			"Monitor":self.SyncServoSonitor,
			"Monitor_4096":self.SyncServoSonitor_4096,
		}
		self.sync_write={
	...
	def ReadMinPositionLimit(self,id:int):
		return self.min_position_limit[id]
	
	def ReadMaxPositionLimit(self,id:int):
		return self.max_position_limit[id]
	...
	def SyncServoSonitor_4096(self, motors:dict[str,id])-> dict[str, Monitor_data]:
		if self.is_open == False:
			raise Exception("Please connect the device first")
		ids = [motors[id] for id in motors]
		self.send_sync_servo_monitor(servo_ids=ids)

		self.servos_4096 = copy.deepcopy(self.servos)

		for id in ids:
			if self.servos_4096[id].angle_monitor >=180:
				self.servos_4096[id].angle_monitor = 180
			elif self.servos_4096[id].angle_monitor <=-180:
				self.servos_4096[id].angle_monitor = -180
			self.servos_4096[id].angle_monitor = int(self.servos_4096[id].angle_monitor+180)/360.0*4096


		monitor_datas:dict[str, Monitor_data_4096] = {}
		for motor in motors:
			id = motors[motor]
			data = Monitor_data_4096(id,
								int(self.servos_4096[id].angle_monitor), 
								self.servos_4096[id].power, 
								self.servos_4096[id].voltage, 
								self.servos_4096[id].current, 
								self.servos_4096[id].temp, 
								self.servos_4096[id].status)
			monitor_datas[motor] = data
	...
	def SyncPositionControl_EX(self,motors:dict[str,SyncPositionControlOptions])->None:
		...
		command_data_list = [struct.pack("<BlLHHH", ids[i], targets[i], motion_times[i], powers[i], t_accs[i], t_decs[i] , powers[i])for i in ids]
