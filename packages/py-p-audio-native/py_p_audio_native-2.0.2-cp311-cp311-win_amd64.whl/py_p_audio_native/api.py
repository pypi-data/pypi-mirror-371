...
class Recorder:
    ...
    def __init__(self, device_index: Optional[int] = None, 
                 ...
                 start_channel: Optional[int] = None, end_channel: Optional[int] = None):
        """
        Initialize recorder.
        ...
        Args:
            ...
            channels: Number of channels (ignored if start_channel/end_channel specified)
            bit_depth: Bit depth (16, 24, 32)
            buffer_size: Buffer size in frames
            start_channel: Starting channel number (1-based, for channel range recording)
            end_channel: Ending channel number (1-based, for channel range recording)
        """
        ...
        self._end_channel = end_channel
        self._is_setup = False
    
    def setup(self):
        """Setup the recorder with specified parameters"""
        if self._start_channel is not None and self._end_channel is not None:
            # Use channel range setup
            success = self._recorder.setup_recording_channels(
                self._device_index, self._sample_rate, self._start_channel, 
                self._end_channel, self._bit_depth, self._buffer_size
            )
        else:
            # Use traditional channel count setup
...
def record(duration: float, output_file: Union[str, Path], 
           ...
           start_channel: Optional[int] = None, end_channel: Optional[int] = None) -> bool:
    """
    ...
    Args:
        ...
        channels: Number of channels (ignored if start_channel/end_channel specified)
        bit_depth: Bit depth
        start_channel: Starting channel number (1-based)
        end_channel: Ending channel number (1-based)
        
    Returns:
        True if recording succeeded
    """
    recorder = Recorder(device_index, sample_rate, channels, bit_depth, 1024, start_channel, end_channel)
