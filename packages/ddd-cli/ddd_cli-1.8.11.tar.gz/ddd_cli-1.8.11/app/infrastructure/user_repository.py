...
class UserRepository:
        ...
        try:
            ...
            if filters is not None:
                if not isinstance(filters, dict):
                    raise UserValueError(field="filters", detail="filters debe ser un diccionario o None")
    ...
    @staticmethod
    def get_by_id(id) -> Optional[ UserEntity ]:
        """
    ...
    def create(entity: UserEntity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> UserEntity:
        ...
        if not entity or not hasattr(entity, "to_dict"):
            raise UserValueError(field="User", detail="Entidad nula o no tiene el método 'to_dict'")
        ...
        try:
            ...
            raise UserValueError(field="data", detail=f"Error de estructura en los datos: {str(e)}") from e
        except ValidationError as e:
            raise UserValidationError(f"Error de validación: {e.message_dict}") from e
            ...
            raise UserValidationError({"integridad": "Datos duplicados o inconsistentes"})            
        except DatabaseError as e:
            raise ConnectionDataBaseError("Error al acceder a la base de datos") from e
        except Exception as e:
