...
def _init_qwen_settings():
    ...
    if current_limit != 100000:
        settings["sessionTokenLimit"] = 100000  # @@@qwen-session-limit: Global Qwen token limit
        with open(qwen_settings_path, 'w') as f:
            json.dump(settings, f, indent=2)
        # Print notification when changed
        if current_limit is None:
            print(f"✅ Initialized Qwen session token limit to 100,000 tokens")
        else:
            print(f"📝 Updated Qwen session token limit from {current_limit:,} to 100,000 tokens")
