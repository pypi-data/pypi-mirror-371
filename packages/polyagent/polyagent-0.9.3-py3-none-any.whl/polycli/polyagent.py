...
class PolyAgent:
    ...
    def run(self, 
        ...
        for attempt in range(max_retries):
            ...
            if backend == "claude-code":
                ...
                result = self._run_api(prompt, model or "glm-4.5", system_prompt, ephemeral, schema_cls, memory_serializer, **kwargs)
    ...
    def _run_api(self, prompt: str, model: str, system_prompt: Optional[str], ephemeral: bool,
                 schema_cls: Optional[Type[BaseModel]], memory_serializer: Optional[Callable], **kwargs) -> RunResult:
