...
# {
    ...
    def predict(self, X_new, avail_new=None):
        """
        ...
        if not hasattr(self, 'coeff_est'):
            raise ValueError("The model must be trained using `fit()` before calling `predict()`.")



        # Ensure the new data is preprocessed like training data
        X_new, _ = self.setup_design_matrix(X_new)

        # Reshape alternatives and ensure compatibility
        X_new = X_new.reshape(-1, len(self.alts), X_new.shape[1] // len(self.alts))

        # Default all alternatives as available if avail_new is not provided
        if avail_new is None:
            avail_new = np.ones((X_new.shape[0], X_new.shape[1]))

        # Use the model's fitted coefficients
        betas = self.coeff_est

        # Compute choice probabilities
        choice_probabilities = self.compute_probabilities(betas, X_new, avail_new)
