...
# {
    ...
    def predict_setup(self, X_new, avail_new=None, weights_new = None):
        """
        Predict choice probabilities for new data using the fitted model.
        ...
        -------
        numpy.ndarray
            Predicted choice probabilities. Shape: (n_observations, n_alternatives).
        """
        # Ensure the model is trained
        betas = self.coeff_est.copy()


        """
        Predict probabilities and outcomes for new data using the fitted model.

        Parameters:
        ----------
        X_new : array-like
            New input data for prediction (shape: [n_samples, n_features]).
        y_new : array-like, optional
            Observed outcomes for the new data (shape: [n_samples, n_alts]).
            If provided, this can be used to compute probabilities for the observed outcomes.
        alts : array-like, optional
            Alternatives for the new data.
        avail : array-like, optional
            Availability of alternatives for the new data.
        weights : array-like, optional
            Weights for the new data.

        Returns:
        -------
        Tuple[np.ndarray, np.ndarray]
            - Predicted probabilities for each alternative (shape: [n_samples, n_alts]).
            - Predicted outcomes (most likely alternative for each sample, shape: [n_samples]).
        """
        # Ensure X_new is a numpy array
        X_new = np.asarray(X_new)

        # Apply transformations if necessary
        #if hasattr(self, 'trans_func') and self.trans_func is not None:
        #    X_new = self.trans_func(X_new)

        # Adjust availability and weights if needed
        if avail_new is not None:
            avail = np.asarray(avail_new).reshape(X_new.shape[0], -1)
        else:
            avail = self.avail

        if weights_new is not None:
            weights = np.asarray(weights_new).reshape(X_new.shape[0], -1)
        else:
            weights = self.weights

        # Compute linear combination of features and coefficients
        #utility = np.dot(X_new, self.betas[:self.X.shape[1]])

        # Apply availability mask if provided
        #if avail is not None:
        #    utility = np.where(avail, utility, -np.inf)  # Set unavailable options to -∞




        # Compute choice probabilities
        choice_probabilities, logsums = self.compute_probabilities(betas, X_new, avail_new, True)
