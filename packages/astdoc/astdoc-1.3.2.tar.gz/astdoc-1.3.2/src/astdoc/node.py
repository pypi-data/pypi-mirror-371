...
def iter_child_nodes(node: AST, module: str) -> Iterator[Definition | Assign | Import]:
    ...
    instances of `Definition`, `Assign`, or `Import`. Use the `astdoc.ast` module
    ...
    """
    for child in astdoc.ast.iter_child_nodes(node):
    ...
    for child in get_child_nodes(cls.node, module):
        ...
        if isinstance(child, Definition):
            if isinstance(child.node, ast.FunctionDef | ast.AsyncFunctionDef):
                if not astdoc.ast.is_function(child.node):
                    continue
