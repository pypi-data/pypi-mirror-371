...
def _add_example_escape(text: str) -> str:
    ...
    Examples:
        ...
        ">>> print('Hello, World!')"
        >>> x[2]
        '>>> __astdoc__# This is a comment'

    """
    text = PROMPT_ONLY.sub(r"\g<pre> astdoc_BLANK_LINE", text)
    text = COMMENT_ONLY.sub(r"\g<pre>__astdoc__\g<comment>", text)
...
def _delete_example_escape(text: str) -> str:
    ...
    Examples:
        >>> text = " astdoc_BLANK_LINE\n__astdoc__# This is a comment\n"
        >>> _delete_example_escape(text)
        ' \n# This is a comment\n'

    """
    text = text.replace("astdoc_BLANK_LINE", "")
    return text.replace("__astdoc__", "")
