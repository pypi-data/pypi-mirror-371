...
from src.my_utils.configs import ColumnStrings
from src.my_utils.configs.system import System
from src.my_utils.logger import log, logerr, loginf, logsuc
from src.my_utils.pretty_print import pretty_print
from src.my_utils.reader_controller.exceptions import ContentLengthError, WrongSheetListError
from src.my_utils.reader_controller.types import DFType, IdfyGoods, XLSGood
...
class ReaderController:
    ...
    def read_data(self, sheet_name: Optional[str] = None):
        """Чтение файла"""
        self._dataframe = self._sheet_read(sheet_name=sheet_name)
    ...
    def _sheet_read(self, sheet_name: Optional[str] = None) -> DFType:
        """Считывает все данные в словарь с преобразованием всех значений в строки"""
        if sheet_name is None:
            dataframe = read_excel(self._file_path)
        else:
            try:
                dataframe = read_excel(self._file_path, sheet_name=sheet_name)
            except Exception as e:
                raise ValueError(e)
