...
def create_data_client(config: Optional[DataClientConfig] = None) -> DataClient:
    ...
    pool_size=getattr(config.postgres, "pool_size", 5),
    max_overflow=getattr(config.postgres, "max_overflow", 5),
    pool_timeout=getattr(config.postgres, "pool_timeout", 30),
    pool_recycle=getattr(config.postgres, "pool_recycle", 1800),
    pool_pre_ping=getattr(config.postgres, "pool_pre_ping", True),
    connect_args={"server_settings": {"application_name": getattr(config.postgres, "application_name", "sensory-client")}},
  )
