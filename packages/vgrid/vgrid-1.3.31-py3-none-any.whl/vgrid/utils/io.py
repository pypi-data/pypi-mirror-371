...
def process_input_data_bin(data, lat_col="lat", lon_col="lon", delimiter=None, **kwargs):
...
def validate_dggs_resolution(dggs_type: str, resolution: int) -> int:
    ...
    """        
    dggs_type_norm = dggs_type.strip().lower()
    min_res, max_res = DGGS_TYPES[dggs_type_norm]
    if resolution < min_res or resolution > max_res:
        raise ValueError(
            f"Resolution for '{dggs_type_norm}' must be in range [{min_res}..{max_res}], got {resolution}"
        )
    return resolution
...
def validate_dggal_type(dggal_type: str): 
    dggal_type = dggal_type.strip().lower()   
    available_types = list(DGGAL_TYPES.keys())
    if dggal_type not in available_types:
        raise ValueError(f"Invalid DGGA type '{dggal_type}'. Valid types are: {available_types}")
    return dggal_type

def validate_dggal_resolution(dggal_type: str, resolution: int) -> int: 
    dggal_type = validate_dggal_type(dggal_type)
    
    min_res = DGGAL_TYPES[dggal_type]["min_res"]    
    max_res = DGGAL_TYPES[dggal_type]["max_res"]
    if resolution < min_res or resolution > max_res:
        raise ValueError(
            f"Resolution for '{dggal_type}' must be in range [{min_res}..{max_res}], got {resolution}"
        )
    return resolution
...
def validate_dggrid_type(dggrid_type: str): 
    ...
    if dggrid_type not in available_types:
        raise ValueError(f"Invalid DGGRID type '{dggrid_type}'. Valid types are: {available_types}")
    return dggrid_type

def validate_dggrid_resolution(dggrid_type: str, resolution: int) -> int: 
    dggrid_type = validate_dggrid_type(dggrid_type)    
    min_res = DGGRID_TYPES[dggrid_type]["min_res"]    
    max_res = DGGRID_TYPES[dggrid_type]["max_res"]  
    if resolution < min_res or resolution > max_res:
        raise ValueError(
