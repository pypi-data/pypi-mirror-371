...
from vgrid.utils.constants import DGGS_TYPES, DGGAL_TYPES
...
def validate_dggal_resolution(dggal_type: str, resolution: int) -> int: 
    dggal_type = dggal_type.strip().lower()
    if dggal_type not in DGGAL_TYPES:
        raise ValueError(f"dggal_type must be one of {list(DGGAL_TYPES.keys())}")
