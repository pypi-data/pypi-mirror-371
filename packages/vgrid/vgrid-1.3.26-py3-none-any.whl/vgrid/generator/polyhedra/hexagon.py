"""
Hexagon Grid Generator Module.
This module provides functionality to generate hexagon faces based on icosahedron geometry.
    ...
    for vert in Vertice.vertices.values():
        ...
        for other_vert in Vertice.vertices.values():
            if vert.distance(other_vert) == min_dist:
                pairs[vert.key].add(other_vert.key)
                pairs[other_vert.key].add(vert.key)

    Face.clear()

    # Change items() to items() for Python 3 compatibility
    for vert, verts in pairs.items():
        for v1 in verts:
            if v1 == vert:
                continue
            for v2 in verts:
...
def icosahedron_dual():
    ...
    for vkey, fkeys in vertFaces.items():
...
def hexagon(level=1):
    """
    Generate hexagon faces as GeoDataFrame.
    
    Args:
        level (int): Bifurcation level for the hexagon generation (default: 1)
    
    Returns:
        geopandas.GeoDataFrame: GeoDataFrame containing hexagon face geometries
    """
    # Clear any previous state
    Vertice.clear()
    Face.clear()
    
    # Generate the hexagon geometry
    icosahedron_vertices()
    icosahedron_faces()
    icosahedron_bisect(level)
    icosahedron_dual()
    
    # Get the GeoJSON data and convert to GeoDataFrame
    geojson_data = icosahedron_geojson()
...
def hexagon_cli():
    """Command-line interface to generate and save hexagon faces as GeoJSON."""
    ...
    parser.add_option(
        "--level", dest="level", type="int", default=1, help="Bifurcation level"
    )

    options, args = parser.parse_args()

    # Generate the hexagon geometry
    gdf = hexagon(options.level)
    ...
    if options.json:
        ...
        # Save to file
        geojson_path = "hexagon.geojson"
        gdf.to_file(geojson_path, driver="GeoJSON")
        print(f"Hexagon saved as {geojson_path}")
