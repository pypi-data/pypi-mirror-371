...
def dg_grid_meta(dggs):
    ...
    for res_opt in [
        ...
        if dggs.get_par(res_opt, None) is not None:
...
class DGGRIDv7(object):
    ...
    def run(self, dggs_meta_ops):
        ...
        try:
            ...
            o = subprocess.Popen(
                [
                    os.path.join(self.working_dir, self.executable),
    ...
    def dgapi_grid_gen(self, dggs, subset_conf, output_conf):
        ...
        if subset_conf["clip_subset_type"] == "WHOLE_EARTH":
            ...
            and subset_conf["clip_region_files"] is not None
        ):
            metafile.append("clip_subset_type " + subset_conf["clip_subset_type"])
            metafile.append("clip_region_files " + subset_conf["clip_region_files"])
        elif (
            subset_conf["clip_subset_type"]
            in ["SEQNUMS", "COARSE_CELLS", "INPUT_ADDRESS_TYPE"]
            and subset_conf["clip_region_files"] is not None
        ):
            if dggs.dggs_type not in ["PLANETRISK"]:
                # metafile.append("clip_subset_type " + subset_conf['clip_subset_type'])
                # metafile.append("clip_region_files " + subset_conf['clip_region_files'])
        ...
        if (
            "input_address_type" in subset_conf.keys()
            and subset_conf.get("input_address_type", "NOPE") in input_address_types
        ):
        ...
        if "cell_output_type" in output_conf.keys():
            if (
                ...
                and output_conf["cell_output_gdal_format"] is not None
                and output_conf["cell_output_file_name"] is not None
            ):
                for elem in filter(
                    lambda x: x.startswith("cell_output_"), output_conf.keys()
        ...
        if "point_output_type" in output_conf.keys():
            if (
                ...
                in ["SHAPEFILE", "AIGEN", "GEOJSON", "TEXT"]
                and output_conf["point_output_file_name"] is not None
            ):
                for elem in filter(
                ...
                and output_conf["point_output_gdal_format"] is not None
                and output_conf["point_output_file_name"] is not None
            ):
                for elem in filter(
                    lambda x: x.startswith("point_output_"), output_conf.keys()
        ...
        if (
            "output_address_type" in output_conf.keys()
            and output_conf.get("output_address_type", "NOPE") in output_address_types
        ):
    ...
    def dgapi_grid_transform(self, dggs, subset_conf, output_conf):
        ...
        if (
            "input_file_name" in subset_conf.keys()
            and "input_address_type" in subset_conf.keys()
            and subset_conf["input_address_type"] in input_address_types
        ):
        ...
        if (
            "output_file_name" in output_conf.keys()
            and "output_address_type" in output_conf.keys()
            and output_conf["output_address_type"] in output_address_types
        ):
    ...
    def grid_stats_table(self, dggs_type, resolution, mixed_aperture_level=None):
        ...
        df.rename(
            columns={0: "Resolution", 1: "Cells", 2: "Area (km^2)", 3: "CLS (km)"},
            inplace=True,
        )
    ...
    def grid_cell_polygons_for_extent(
        ...
        if clip_geom is not None and clip_geom.area > 0:
            clip_gdf = gpd.GeoDataFrame(
                pd.DataFrame({"id": [1], "geometry": [clip_geom]}),
                geometry="geometry",
                crs=4326,
            )
            clip_gdf.to_file(
                Path(tmp_dir) / f"temp_clip_{tmp_id}.{self.tmp_geo_out['ext']}",
                driver=self.tmp_geo_out["driver"],
            )
            ...
            subset_conf.update(
                {
                    ...
                    "clip_region_files": str(
                        (
                            Path(tmp_dir)
                            / f"temp_clip_{tmp_id}.{self.tmp_geo_out['ext']}"
                        ).resolve()
                    ),
                }
            )
        ...
        output_conf = {
            ...
            "cell_output_file_name": str(
                (
                    Path(tmp_dir)
                    / f"temp_{dggs_type}_{resolution}_out_{tmp_id}.{self.tmp_geo_out['ext']}"
                ).resolve()
            ),
        }
            ...
            output_conf.update(
                {
                    ...
                    "cell_output_file_name": str(
                        (
                            Path(tmp_dir)
                            / f"temp_{dggs_type}_{resolution}_out_{tmp_id}"
                        ).resolve()
                    ),
                }
            )
        ...
        if (
            output_address_type is not None
            and output_address_type in output_address_types
        ):
        ...
        if self.debug is False:
            try:
                ...
                os.remove(
                    str(Path(tmp_dir) / f"temp_clip_{tmp_id}.{self.tmp_geo_out['ext']}")
                )
                os.remove(str(Path(tmp_dir) / "points.gen"))
            except Exception:
                pass
    ...
    def grid_cell_centroids_for_extent(
        ...
        if self.debug is False:
            try:
                ...
                os.remove(str(Path(tmp_dir) / "cells.gen"))
            except Exception:
                pass
    ...
    def grid_cell_polygons_from_cellids(
        ...
        if cell_id_list is not None and len(cell_id_list) > 0:
            ...
            subset_conf.update(
                {
                    ...
                    "clip_region_files": str(
                        (Path(tmp_dir) / f"temp_clip_{tmp_id}.txt").resolve()
                    ),
                }
            )
            ...
            if input_address_type in [
                ...
                if clip_subset_type is not None and clip_subset_type in [
                    "COARSE_CELLS"
                ]:
                    subset_conf.update(
                        {
                            ...
                            "clip_cell_addresses": " ".join(
                                [str(address) for address in cell_id_list]
                            ),
                        }
                    )
        ...
        if cell_id_list is not None and len(cell_id_list) > 0 and seq_df is not None:
            ...
            if not output_address_type == input_address_type:
                if self.debug is True:
                    print(
                        f"cannot switch address types on the fly here: {input_address_type} !=  {output_address_type}"
                    )
    ...
    def grid_cellids_for_extent(
        ...
        output_conf = {
            ...
            "point_output_file_name": str(
                (
                    Path(tmp_dir) / f"temp_{dggs_type}_{resolution}_out_{tmp_id}"
                ).resolve()
            ),
        }
        ...
        df = pd.read_csv(
            Path(tmp_dir) / f"temp_{dggs_type}_{resolution}_out_{tmp_id}.txt",
            header=None,
        )
        df = df.dropna()
        ...
        if self.debug is False:
            try:
                os.remove(
                    str(
                        Path(tmp_dir)
                        / f"temp_{dggs_type}_{resolution}_out_{tmp_id}.txt"
                    )
                )
                os.remove(
                    str(Path(tmp_dir) / f"temp_clip_{tmp_id}.{self.tmp_geo_out['ext']}")
    ...
    def cells_for_geo_points(
        ...
        geodf_points_wgs84[cols_ordered].to_csv(
            str((Path(tmp_dir) / f"geo_{tmp_id}.txt").resolve()),
            header=False,
            index=False,
        ...
        output_conf = {
            "output_file_name": str(
                (Path(tmp_dir) / f"seqnums_{tmp_id}.txt").resolve()
            ),
            "output_address_type": "SEQNUM",
            "output_delimiter": '","',
        }
    ...
    def address_transform(
        ...
        if input_address_type not in input_address_types:
            raise ValueError(f"unknown input_address_type: {input_address_type}")

        if output_address_type not in output_address_types:
            raise ValueError(f"unknown output_address_type: {output_address_type}")

        seq_df = pd.DataFrame({input_address_type: cell_id_list})
        seq_df.to_csv(
            str(
                (Path(tmp_dir) / f"temp_in_{input_address_type}_{tmp_id}.txt").resolve()
            ),
            header=False,
            index=False,
            columns=[input_address_type],
        ...
        output_conf = {
            "output_file_name": str(
                (
                    Path(tmp_dir) / f"temp_out_{output_address_type}_{tmp_id}.txt"
                ).resolve()
            ),
        ...
        df = pd.read_csv(
            dggs_ops["output_conf"]["output_file_name"],
            header=None,
            dtype={0: "str", 1: "str"},
        )
        df = df.dropna()
        ...
        if self.debug is False:
            try:
                os.remove(
                    str(Path(tmp_dir) / f"temp_in_{input_address_type}_{tmp_id}.txt")
                )
                os.remove(
                    str(Path(tmp_dir) / f"temp_out_{output_address_type}_{tmp_id}.txt")
                )
            except Exception:
                pass
    ...
    def guess_zstr_resolution(
        self, cell_id_list, dggs_type, input_address_type="Z7_STRING"
    ):
        if cell_id_list is None or len(cell_id_list) <= 0:
            raise ValueError("Expecting cell_id_list to transform.")

        if input_address_type not in ["Z3_STRING", "Z7_STRING"]:
            raise ValueError(
                f"this will likely not work for this input_address_type: {input_address_type} | only Z3 and Z7 verified"
            )

        if dggs_type not in ["ISEA3H", "ISEA7H", "IGEO7"]:
            raise ValueError(
                f"this will likely not work for this dggs_type: {dggs_type} | only Z3 and Z7 compatible"
            )
...
def specify_topo_aperture(
    ...
    aperture_type,
    aperture_res,
    dggs_num_aperture_4_res=0,
    dggs_aperture_sequence="333333333333",
):
    if topology_type not in dggs_topologies or aperture_type not in dggs_aperture_types:
        raise ValueError("topology or aperture type unknow")

    if aperture_type == "PURE":
        if topology_type == "HEXAGON":
            if aperture_res not in [3, 4, 7]:
                print(
                    f"combo not possible / {topology_type} {aperture_res} / setting 3H"
                )
                return {
                    "#short_name": "3H",
        ...
        if topology_type in ["TRIANGLE", "DIAMOND"]:
            if not aperture_res == 4:
                print(
                    f"combo not possible / {topology_type} {aperture_res} / setting 4{topology_type[0]}"
                )
                return {
                    "#short_name": f"4{topology_type[0]}",
                    "dggs_topology": topology_type,
                    "dggs_aperture_type": aperture_type,
                    "dggs_aperture": 4,
                }
        ...
        return {
            "#short_name": f"SEQ{topology_type[0]}",
            "dggs_topology": topology_type,
            "dggs_aperture_type": aperture_type,
...
def specify_resolution(
    ...
    if (
        proj_spec not in list(dggs_projections)
        or dggs_res_spec_type not in dggs_res_specify_types
    ):
        raise ValueError("base projection (ISEA or FULLER) or resolution spec unknown")

    if dggs_res_spec_type == "SPECIFIED":
        return {"dggs_proj": proj_spec, "dggs_res_specify_type": dggs_res_spec}
...
def dgconstruct(
    dggs_type: str = "CUSTOM",  # dggs_type
    ...
    if (
        not len(list(filter(lambda x: x is not None, [res, area, spacing, cls_val])))
        == 1
    ):
        raise ValueError(
            "dgconstruct(): Only one of res, area, length, or cls can have a value!"
        )
    ...
    if res is not None:
        ...
        dggs.res = dggs.dg_closest_res_to_area(
            area=area, resround=resround, metric=metric, show_info=True
        )
    elif spacing is not None:
        dggs.res = dggs.dg_closest_res_to_spacing(
            spacing=spacing, resround=resround, metric=metric, show_info=True
        )
    elif cls_val is not None:
        dggs.res = dggs.dg_closest_res_to_cls(
            cls_val=cls_val, resround=resround, metric=metric, show_info=True
        )
    else:
