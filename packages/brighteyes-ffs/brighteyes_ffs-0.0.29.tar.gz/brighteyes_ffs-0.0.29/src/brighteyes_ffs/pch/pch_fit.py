...
def fit_pch(hist, fit_info, param, psf, lBounds=[-10,-10,-10,-10], uBounds=[10,10,10,10], weights=1, n_bins=1e5, fitfun='fitfun_pch', minimize_rel_error=False):
    """
    ...
    param[0:n_comp] = np.log10(np.clip(param[0:n_comp], 1e-10, None)) # use log10 of concentration for fitting
    param[n_comp:2*n_comp] = np.log10(np.clip(param[n_comp:2*n_comp], 1e-10, None)) # use log10 of brightness for fitting
    ...
    fitresult = least_squares(fitfun, fitparam_start, args=(fixed_param, fit_info, hist/np.sum(hist), psf_compressed/np.max(psf_compressed), psf_weights, weights, minimize_rel_error), bounds=(lowerBounds, upperBounds)) #, xtol=1e-12
...
def fitfun_pch_nc(fitparam, fixedparam, fit_info, hist, psf, psf_weights=1, weights=1, minimize_rel_error=False):
    """
