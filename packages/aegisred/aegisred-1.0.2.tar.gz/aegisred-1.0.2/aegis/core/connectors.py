...
from .models import ModelResponse, AdversarialPrompt, MultiModalInput
...
class OpenAIConnector(ModelConnector):
    ...
    def send_prompt(self, prompt: AdversarialPrompt) -> ModelResponse:
        try:
            ...
            for item in prompt.multi_modal_data:
                if item.media_type.startswith("image/"):
                    base64_image = base64.b64encode(item.data).decode('utf-8')
                    content.append({
                        "type": "image_url",
                        "image_url": {"url": f"data:{item.media_type};base64,{base64_image}"}
                    })
            ...
            return ModelResponse(output_text=output_text.strip(), prompt_id=prompt.id, model_name=f"openai/{self.model_name}")
        ...
        try:
            ...
            for item in prompt.multi_modal_data:
                if item.media_type.startswith("image/"):
                    ...
                    content.insert(0, {
                        "type": "image",
                        "source": {"type": "base64", "media_type": item.media_type, "data": base64_image},
                    })
            ...
            return ModelResponse(output_text=output_text.strip(), prompt_id=prompt.id, model_name=f"anthropic/{self.model_name}")
        except Exception as e:
            return ModelResponse(output_text="", prompt_id=prompt.id, model_name=self.model_name, error=str(e))
...
class UserProvidedGeminiConnector(ModelConnector):
    ...
    def send_prompt(self, prompt: AdversarialPrompt) -> ModelResponse:
        try:
            ...
            output_text = response.text or ""
            return ModelResponse(output_text=output_text.strip(), prompt_id=prompt.id, model_name=f"gemini/{self.model_name}")
        except Exception as e:
            return ModelResponse(output_text="", prompt_id=prompt.id, model_name=self.model_name, error=str(e))
...
class OpenRouterConnector(ModelConnector):
    ...
    def send_prompt(self, prompt: AdversarialPrompt) -> ModelResponse:
        try:
            ...
            output_text = chat_completion.choices[0].message.content or ""
            return ModelResponse(output_text=output_text.strip(), prompt_id=prompt.id, model_name=f"openrouter/{self.model_name}")
        except Exception as e:
            return ModelResponse(output_text="", prompt_id=prompt.id, model_name=self.model_name, error=str(e))
