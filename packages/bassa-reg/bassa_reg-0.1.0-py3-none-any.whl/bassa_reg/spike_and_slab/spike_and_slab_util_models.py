...
class SpikeAndSlabLoader:
    ...
    def __init__(self, path: str, continue_experiment: bool = True):
        print("Loading Files")
        ...
        try:
            ...
            loaded_w = np.load(f"{samples_path}/w.npz")
            self.w = [loaded_w[f"arr_{i}"] for i in range(len(loaded_w.files))]
            ...
            # Load the log probabilities
            loaded_total_log_prob = np.load(f"{samples_path}/log_prob_total.npz")
            self.log_prob['total'] = [loaded_total_log_prob[f"arr_{i}"] for i in range(len(loaded_total_log_prob.files))]

            loaded_partial_log_prob = np.load(f"{samples_path}/log_prob_partial.npz")
            self.log_prob['partial'] = [loaded_partial_log_prob[f"arr_{i}"] for i in range(len(loaded_partial_log_prob.files))]
