...
class SpikeAndSlabRegression:
    def __init__(self, x: pd.DataFrame,
        ...
        if load_experiment is not None:
            if load_experiment.continue_experiment:
    ...
    def save(self):
        ...
        if self.config.save_samples:
            ...
            np.savez_compressed(f"{samples_path}/w.npz", *self.all_w)
    ...
    def run(self):
        if self.loaded_experiment is not None:
            if not self.loaded_experiment.continue_experiment:
                self.save()
                return
        ...
        core = SpikeAndSlabCore(self.x,
                                ...
                                total_iterations - sampling_start_iter, prediction_iterations)
        self.core = core
