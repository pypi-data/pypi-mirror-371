...
def move_front_back(xtx, xty, dest_pos):
    """
    Row/col 0 currently holds the feature we just inserted.
    Move it to `dest_pos` (0-based) and shift the others left.
    xtx : (k×k)  symmetric matrix
    xty : (k,)   vector
    """
    k = xtx.shape[0]
    if dest_pos == 0:                       # already correct
        return xtx, xty
    ...
    for i in range(dest_pos):
        perm[i] = i + 1                     # 1,2,…,dest_pos
    perm[dest_pos] = 0                      # insert former front
    for i in range(dest_pos + 1, k):
        perm[i] = i                         # dest_pos+1 … k-1
...
def create_feature_matrices(x, s0, new_feature_index):
    ...
    # Create xs_1 with new feature in front
...
class SpikeAndSlabCore:
    def __init__(self, x, y, x_test, class_priors, num_iters, internal_prediction_iters, prediction_iters):
    ...
    def init_sate(self):
        self.a = np.random.beta(self.a_alpha_prior, self.a_beta_prior, size=1)[0].astype(np.float32)
        self.s = np.random.binomial(1, self.a, size=self.x.shape[1]).astype(np.float32)
        self.tau = np.random.beta(self.tau_alpha_prior, self.tau_beta_prior, size=1)[0].astype(np.float32)
        self.sigma = np.random.beta(self.sigma_alpha_prior, self.sigma_beta_prior, size=1)[0].astype(np.float32)
        multiplier = 1.01
        i = 1
        while np.sum(self.s) == 0:
            self.s = np.random.binomial(1, self.a * multiplier ** i, size=self.x.shape[1]).astype(np.float32)
            i += 1
    ...
    def sample_geweke(self):
        """
        Geweke (1992) “getting-it-right” test, adapted for Numba jitclass.

        Steps
        -----
        1. Draw θ₀ = (σ₀, τ₀, a₀, s₀, w₀) from the prior; simulate y₀.
        2. Load that synthetic problem, run ONE Gibbs/MH sweep (`cycle()`).
        3. Record (θ₀, θ₁) pairs in the pre-allocated arrays.
        4. Restore the original sampler state.
        """

        # ---------- 1. PRIOR DRAW & SYNTHETIC DATA ----------
        ...
        # active coefficients, full-length so that x @ w works later
        w0 = (np.random.normal(0.0, tau0, size=self.x.shape[1]) * s0).astype(np.float32)


        # make x contiguous once to silence the NumbaPerformanceWarning
        ...
        # ---------- 4. ONE FULL POSTERIOR SWEEP ----------
        self.cycle(False)

        # collect post-sweep parameters
        sigma1 = self.sigma
        tau1 = self.tau
        a1 = self.a
        ...
        # element-wise assignment keeps Numba happy
        ...
        self.geweke_s_sum[i, 0] = np.sum(s0)  # s₀ total
        self.geweke_s_sum[i, 1] = s1_sum  # s₁ total
    ...
    def sample_w(self, xs, y, tau, sigma, s_size, xtx, xty):
        ...
        # Solve Q mu = X^T y -> L L.T mu = sigma^{-2} * X^T y
        # First solve L u = sigma^{-2} * xty
    ...
    def cycle(self, save=True):
        ...
        for i in range(index_order.shape[0]):
            ...
            if s_for_feature == 0:
                changed, from_0_to_1, from_1_to_0, xtx_0, xty_0, xtx_1, xty_1 = self.sample_s(s_index_to_sample, xtx_0=xtx, xty_0=xty, xtx_1=dummy, xty_1=dummy_vec, inserted_0=True)
            else:
                changed, from_0_to_1, from_1_to_0, xtx_0, xty_0, xtx_1, xty_1 = self.sample_s(s_index_to_sample, xtx_1=xtx, xty_1=xty, xtx_0=dummy, xty_0=dummy_vec, inserted_0=False)
            if changed:
                if from_0_to_1:
    ...
    def run_cycles(self, n_steps, start_collect, sample_geweke=False):
        """
        Execute `n_steps` Gibbs sweeps entirely inside Numba.

        Parameters
        ----------
        n_steps         : how many times to call `cycle()`
        start_collect   : after how many global iterations we start
                          calling `compute_internal_y_prediction()`
        progress_every  : print a one-line progress message every
                          `progress_every` iterations
        """
