...
from expreiments.utils.utils import sanitize_filenames


class CaseStudy3DataSet(Enum):
    AnatSi = 'Anat_SI.csv'
    RDKit = 'rdkit_fingerprints.csv'
    Kraken = 'yonatan_kraken.csv'
...
def data_preparation(chosen_dataset: CaseStudy3DataSet):
    ...
    y = data['measured ddG']
    data.drop(['ligand name', 'smiles', 'measured ddG'], axis=1, inplace=True)
    try:
    ...
    if chosen_dataset == CaseStudy3DataSet.RDKit:
        data = data.drop(columns=["BCUT2D_CHGHI", "BCUT2D_CHGLO"], axis=1)

    feature_names = data.columns
    feature_names = sanitize_filenames(feature_names)
    data.columns = feature_names
    x = data
    return x, y

if __name__ == '__main__':
    dataset = CaseStudy3DataSet.AnatSi
    name = dataset.value
    np.random.seed(42)
    # data preparation
