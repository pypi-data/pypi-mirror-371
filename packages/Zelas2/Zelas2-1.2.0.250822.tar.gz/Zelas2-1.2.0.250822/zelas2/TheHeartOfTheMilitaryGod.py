...
def view_pointclouds(xyz,i,i_name='intensity',colormap='gray',save_path=None):
    ...
    plotter = pv.Plotter()
    # 显示点云
    plotter.add_points(points, cmap=colormap)
    # 添加坐标轴
    plotter.add_axes()
    # 添加坐标轴的刻度
    plotter.show_grid(color='black', xlabel='X (m)', ylabel='Y (m)', zlabel='Z (m)')  # 显示网格线
    # plotter.show_grid(color='black', n_xlabels=3, n_ylabels=5, n_zlabels=3)
    # plotter.set_axis_labels(xlabel='X (m)', ylabel='Y (m)', zlabel='Z (m)')  # 单独设置坐标轴标签
    # 保存图像前设置图像的分辨率
    # plotter.screenshot(save_path, scale=3)  # scale 参数用于提高分辨
    # 显示图形
    plotter.show()
