...
class FileUtils:
    ...
    def is_empty_repository(self, repo: Repo) -> bool:
        """Check if the repository is empty."""
        try:
            if not repo.heads or not repo.iter_commits():
                return True
            return False
        except Exception as e:
            logger.warning(f"Could not determine if repository is empty: {e}")
            return False

    def create_initial_commit(self, repo: Repo):
        """Create an initial commit if repository is empty."""
        try:
            if not self.is_empty_repository(repo):
                return
            ...
            if not os.path.exists(readme_path):
                with open(readme_path, 'w', encoding='utf-8') as f:
                    f.write("# Smart Git Agent Project\n\nInitial project setup.")

            # Ensure config files are ignored before staging
            config_files = ['git-agent-config.ini', '.commit_metrics.jsonl', '.commit_analyzer_config.json',
                            '.commit_analyzer_cache/']
            for config_file in config_files:
                config_path = os.path.join(self.repo_path, config_file)
                if os.path.exists(config_path):
                    self.ensure_config_file_ignored(config_path)

            # Stage only README.md and .gitignore
            gitignore_path = os.path.join(self.repo_path, '.gitignore')
            files_to_stage = ['README.md']
    ...
    def ensure_config_file_ignored(self, config_path: str):
        """Ensure the configuration file is ignored and untracked."""
        ...
        try:
            ...
            if config_file in tracked_files or config_file in index_entries:
                logger.info(f"Removing '{config_file}' from tracking")
            ...
            self._add_config_to_gitignore(config_file)

        except Exception as e:
            logger.warning(f"Could not ensure '{config_file}' is ignored: {e}")

    def _add_config_to_gitignore(self, config_file: str):
        ...
        try:
            ...
            with open(gitignore_path, 'r', encoding='utf-8') as f:
                ...
                if config_file not in content:
                    ...
                    if self.repo.index.diff(None):
                        self.repo.index.commit("Update .gitignore to ignore configuration files")
        except Exception as e:
            logger.warning(f"Could not update .gitignore: {e}")

    def check_for_sensitive_data_in_history(self) -> bool:
        """Check if sensitive data exists in Git history."""
        try:
            cmd = [
                'git', 'log', '--all', '--pretty=format:', '--name-only', '--diff-filter=A',
                '|', 'grep', '-i', 'git-agent-config.ini'
            ]
            result = subprocess.run(' '.join(cmd), shell=True, cwd=self.repo_path,
                                    capture_output=True, text=True)

            if result.stdout.strip():
                logger.error("🚨 SENSITIVE DATA FOUND IN HISTORY! (git-agent-config.ini)")
                logger.error("Please run clean_history.py immediately!")
                return True
            else:
                logger.info("✅ No sensitive data found in history")
                return False

        except Exception as e:
            logger.warning(f"Could not check history for sensitive data: {e}")
            return False
    ...
    def _get_language_patterns(self) -> Dict[str, Set[str]]:
        ...
        patterns = {
            ...
            'flutter': {'.dart_tool', '.flutter-plugins', '.flutter-plugins-dependencies', '.packages', '.pub-cache',
                        '.pub', 'build'},
            'swift': {
    ...
    def _ensure_gitignore(self):
        ...
        gitignore_content.extend(["# === Smart Git Agent Configuration ===", *config_files, ""])
        ...
        try:
            ...
            self.repo.index.add(['.gitignore'])
            if self.repo.index.diff(None):
                self.repo.index.commit("Create .gitignore for project")
            logger.info(
                f"Created .gitignore for: {', '.join(sorted(self.detected_languages)) if self.detected_languages else 'generic'}")
        except Exception as e:
    ...
    def load_metrics_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'avg_confidence': sum(
                    m.get('confidence', 0) for m in metrics) / total_count if total_count > 0 else 0.0,
                'common_types': Counter(m.get('commit_type') for m in recent_metrics).most_common(5),
                'avg_time': sum(m.get('generation_time', 0) for m in recent_metrics) / len(
                    recent_metrics) if recent_metrics else 0.0,
        ...
        try:
            ...
            if common_types and common_types[0][1] / stats.get('recent_total', 1) > 0.6:
                recommendations.append(
                    f"Diversify commit types (currently {common_types[0][1] / stats.get('recent_total', 1):.1%} are '{common_types[0][0]}')")
            ...
            if daily_usage:
                ...
                if len(last_7_days) >= 2:
                    ...
                    if older_avg > 0:
                        ...
                        insights['trends'][
                            'usage'] = 'increasing' if trend_ratio > 1.2 else 'decreasing' if trend_ratio < 0.8 else 'stable'
