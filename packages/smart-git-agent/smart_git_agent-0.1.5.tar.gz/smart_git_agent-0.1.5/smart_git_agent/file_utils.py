...
class FileUtils:
    ...
    def _get_language_patterns(self) -> Dict[str, Set[str]]:
        ...
        common_patterns = {
            ...
            # Smart Git Agent config
            'git-agent-config.ini',  # Ensure the config file is always ignored
        }
    ...
    def _ensure_gitignore(self):
        """Ensure .gitignore exists and includes git-agent-config.ini."""
        gitignore_path = os.path.join(self.repo_path, '.gitignore')
        config_file = 'git-agent-config.ini'
        ...
        if os.path.exists(gitignore_path):
            ...
            try:
                with open(gitignore_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    # Check if git-agent-config.ini is already in .gitignore
                    if config_file not in content.splitlines():
                        # Append the entry
        ...
        try:
            ...
            logger.info(f"✅ Created .gitignore with '{config_file}' included for: {', '.join(sorted(self.detected_languages)) if self.detected_languages else 'generic'}")
        except Exception as e:
            logger.warning(f"⚠️ Could not create .gitignore: {e}")
