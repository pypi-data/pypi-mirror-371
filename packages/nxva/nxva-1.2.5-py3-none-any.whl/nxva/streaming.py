...
class MultiStreaming(object):
    ...
    def _load_and_validate_config(self, config):
        ...
        self.ffmpeg_fps = int(param.get('FFMPEG_FPS', 5))
        self.ffmpeg_hwaccel = bool(param.get('FFMPEG_HWACCEL', True))
        self.logger.info(f"Streaming global settings: width={self.w}, height={self.h}, backend={self.backend}, codec={self.codec}, "
                         f"ffmpeg_fps={self.ffmpeg_fps}, ffmpeg_hwaccel={self.ffmpeg_hwaccel}")
    ...
    def _open_camera(self, rtsp):
        ...
        if self.backend == 'ffmpeg':
            ...
            if not rtsp.startswith('rtsp://'):
                ...
                return cap
            from nxva.ffmpeg_backend import FFmpegCapture
            cap = FFmpegCapture(self.w, self.h, fps=self.ffmpeg_fps, hwaccel=self.ffmpeg_hwaccel)
            cap.open(rtsp)
            return cap
        else:
