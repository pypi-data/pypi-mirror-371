...
def get_logger(name: str, filename: str = None, level: int = INFO, file_colors: bool = False, force_write: bool = False) -> "Logger":
    """
    ...
    :param force_write: Force write to file, even if level is higher
    ...
    with _lock:
        if name not in _loggers:
            _loggers[name] = Logger(name, filename, level, file_colors, force_write)
...
class Logger:
    ...
    def __init__(self, name: str, filename: str = None, level: int = INFO, file_colors: bool = False, force_write: bool = False) -> None:
        """
    ...
    def print(self, text: str, only_file: bool = False) -> None:
        if self.filename:
    ...
    def trace(self, msg: str) -> None:
        if self.fw or self.level <= TRACE:
            log = f"\x1b[38;5;123mTRACE: {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= TRACE)

    def debug(self, msg: str) -> None:
        if self.fw or self.level <= DEBUG:
            log = f"\x1b[38;5;11mDEBUG: {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= DEBUG)

    def info(self, msg: str) -> None:
        if self.fw or self.level <= INFO:
            log = f"\x1b[38;5;251mINFO:  {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= INFO)

    def warn(self, msg: str) -> None:
        if self.fw or self.level <= WARNING:
            log = f"\x1b[38;5;208m\x1b[1mWARN:  {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= WARNING)

    def error(self, msg: str) -> None:
        if self.fw or self.level <= ERROR:
            log = f"\x1b[38;5;196m\x1b[1mERROR: {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= ERROR)

    def fatal(self, msg: str) -> None:
        if self.fw or self.level <= FATAL:
            log = f"\x1b[38;5;124m\x1b[1mFATAL: {self.dtstr}: [{self.name}]: {msg}\x1b[0m\n"
            self.print(log, not self.level <= FATAL)
