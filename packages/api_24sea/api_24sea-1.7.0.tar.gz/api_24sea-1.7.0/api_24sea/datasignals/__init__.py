...
try:
    ...
    del pd.DataFrame.datasignals  # type: ignore
...
class DataSignals:
    ...
    def __init__(self, pandasdata: pd.DataFrame):
        ...
        self.__api: C.API = C.API()
        self.__sync_with_api()

    def __sync_with_api(self):
        """Sync the local DataFrame with the API."""
        self.base_url = self.__api.base_url
        self._username = self.__api._username
        self._password = self.__api._password
        self._auth = self.__api._auth
        self._authenticated = self.__api._authenticated
        self._metrics_overview = self.__api._metrics_overview
        self._selected_metrics = self.__api._selected_metrics
