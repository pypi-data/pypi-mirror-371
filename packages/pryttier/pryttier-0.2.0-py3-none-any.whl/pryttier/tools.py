...
def if_else(cond: bool, true: Any, false: Any):
    if cond:
        return true
    else:
        return false
...
class File:
    def __init__(self, fp: str | os.PathLike[str]):
        ...
        self.size = size_format(self.stats.st_size)
        self.cdate = time_convert(self.stats.st_ctime)
        self.mdate = time_convert(self.stats.st_mtime)
        self.lastAccess = time_convert(self.stats.st_atime)
...
class Folder:
    def __init__(self, fp: str | os.PathLike[str]):
        ...
        self.stats = os.stat(self.path)
        self.name = os.path.basename(self.path)
        self.size = size_format(self.getSize())
        self.cdate = time_convert(self.stats.st_ctime)
        self.mdate = time_convert(self.stats.st_mtime)
        self.lastAccess = time_convert(self.stats.st_atime)
...
def files_from_dir(dp: os.PathLike[str]):
