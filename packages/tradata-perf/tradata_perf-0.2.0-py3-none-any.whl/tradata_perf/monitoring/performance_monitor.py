"""
Performance monitoring and metrics collection for high-performance applications.
...
class PerformanceMonitor:
    ...
    def __init__(self, max_history: int = 10000):
        ...
        self.cache_metrics: Dict[str, CacheMetrics] = defaultdict(
    ...
    def _update_response_time_stats(self, response_time: float):
        ...
        if len(response_times) >= 10:  # Need enough data for percentiles
            ...
            self.system_metrics.p95_response_time = sorted_times[
                int(len(sorted_times) * 0.95)
            ]
            self.system_metrics.p99_response_time = sorted_times[
    ...
    async def update_cache_stats(
        self, cache_name: str, size: int, memory_usage: float = 0.0
    ):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        recent_response_times = [
            r.response_time for r in recent_requests if r.status_code < 400
        ]

        recent_metrics = {
            "recent_avg_response_time": (
                statistics.mean(recent_response_times) if recent_response_times else 0.0
            ),
            "recent_p95_response_time": (
        ...
        return {
            "system": {
                ...
                "average_response_time": round(
                    self.system_metrics.average_response_time, 3
                ),
                ...
                "requests_per_minute": round(
                    self.system_metrics.requests_per_minute, 2
                ),
            },
            "caches": {
                name: {
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        health_checks = {
            ...
            "response_time_healthy": self.system_metrics.average_response_time
            < 1.0,  # Under 1 second
            "error_rate_healthy": (
                (
                    self.system_metrics.failed_requests
                    / self.system_metrics.total_requests
                    < 0.05
                )
                if self.system_metrics.total_requests > 0
