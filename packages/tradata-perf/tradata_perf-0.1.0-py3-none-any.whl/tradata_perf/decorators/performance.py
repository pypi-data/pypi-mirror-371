...
from tradata_logger import get_logger
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if log_result:
                    ...
                    # Note: logger.info is async but we can't await in sync function
                ...
                if log_result:
                    # Note: logger.error is async but we can't await in sync function
                    # This will be fixed in future versions
                    pass
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if log_result:
                    logger.error(
                        f"Async function {func.__name__} failed after {duration:.3f}s",
                        extra={
                            "function": func.__name__,
                            "duration_ms": round(duration * 1000, 2),
                            "success": False,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if threshold_ms and (duration * 1000) > threshold_ms:
                    logger.warning(
                        f"Async function {func.__name__} exceeded performance threshold",
                        extra={
                            "function": func.__name__,
                            "duration_ms": round(duration * 1000, 2),
                            "threshold_ms": threshold_ms,
...
def _log_profiling_results(
    ...
    if memory_delta is not None:
        extra_data["memory_delta_mb"] = round(memory_delta, 2)

    if cpu_delta is not None:
        extra_data["cpu_delta_percent"] = round(cpu_delta, 2)
...
async def _record_timing_metric(
    ...
    try:
        ...
        logger.error(f"Failed to record timing metric: {e}")
...
async def _record_profiling_metrics(
    ...
    try:
        ...
        logger.error(f"Failed to record profiling metrics: {e}")
...
async def _record_comprehensive_metrics(
    ...
    try:
        ...
        logger.error(f"Failed to record comprehensive metrics: {e}")
