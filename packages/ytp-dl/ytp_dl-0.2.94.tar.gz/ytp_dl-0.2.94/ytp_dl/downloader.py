...
- Desktop (prefer_avc1=False): Prefer DASH 1080p (bestvideo+bestaudio). Only if missing, fallback to progressive <=1080
- iOS (prefer_avc1=True): Same selection, then post-enforce MP4/H.264/AAC via ffmpeg if needed
- No embed-thumbnail; keep embed-metadata
...
def download_video(url, resolution=None, extension=None, prefer_avc1=False):
    """
    Desktop (prefer_avc1=False): DASH 1080p first (bestvideo+bestaudio). If truly unavailable, fallback to progressive <=1080.
    iOS (prefer_avc1=True): same selection; if output isn't MP4/H.264/AAC, transcode to MP4/H.264/AAC.
    """
    ...
    # Rotate relay each job (optional)
    ...
    try:
        ...
        if extension and extension.lower() in audio_exts:
            ...
            # ---- VIDEO selection ----
            # Attempt 1: DASH 1080p (separate A/V) – gives you 1080p VP9/AV1/H.264 as available
            fmt_1080_dash = f'bv*[height<={cap}]+ba'
            # Attempt 2: Progressive <=1080 (last resort; often lower quality)
            fmt_1080_prog = f'b[height<={cap}]'

            if prefer_avc1:
                # iOS: fetch best 1080 DASH; if unavailable, one fallback to progressive <=1080
                try:
                    ytdlp_cmd = (
                        f"{venv_path}/bin/yt-dlp -f \"{fmt_1080_dash}\" "
                        f"{common} --embed-metadata {out_tpl} {url}"
                    )
                    output = _try_run(ytdlp_cmd)
                except ValueError:
                    ytdlp_cmd = (
                        f"{venv_path}/bin/yt-dlp -f \"{fmt_1080_prog}\" "
                        f"{common} --embed-metadata {out_tpl} {url}"
                    )
                    output = _try_run(ytdlp_cmd)
            else:
                # Desktop: same, but we don't transcode afterwards
                try:
                    ytdlp_cmd = (
                        f"{venv_path}/bin/yt-dlp -f \"{fmt_1080_dash}\" "
                        f"{common} --embed-metadata {out_tpl} {url}"
