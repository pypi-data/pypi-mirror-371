...
class SyncTable():
    ...
    def get_ids2send(client):
        """
        Determine the record IDs that need to be sent based on updates and new entries in the client's records.
        
        Parameters:
            client (dict): A dictionary containing client state and records with keys including:
                - 'count' (int): The last known count of records sent.
                - 'hasindex' (bool): Indicates if the client has an index for update checks.
                ...
                - 'lastmsgtime' (float): Timestamp of the last message sent.
        
        Returns:
            tuple: Updated client dictionary and a sorted numpy array of unique record IDs that need to be sent.
        """
        ids2send = []
    ...
    def publish_table_message(table, lookbacklines, lookbackdate, snapshot, bandwidth):
        """
        Generate and return a JSON-formatted message for publishing table data with encrypted authentication.
        
        Parameters:
        - table: An object containing table metadata and records, including attributes like database, period, source, tablename, and records with count and mtime.
        - lookbacklines: Number of lines to look back in the data.
        - lookbackdate: A pandas Timestamp object representing the date to look back to; if provided, it is formatted as 'YYYY-MM-DD' in the message.
        - snapshot: Boolean indicating whether the message is for a snapshot.
        - bandwidth: Bandwidth information to include in the message.
        
        Returns:
        - A JSON string containing the encrypted token, action type, table metadata, record counts, timestamps, and additional parameters for publishing.
        """
    ...
    def subscribe_table_message(table, lookbacklines, lookbackdate, snapshot, bandwidth):
        """
        Generate a JSON-formatted subscription message for a data table with encrypted authentication.
        
        Parameters:
            table (object): An object representing the data table, expected to have attributes
                            'records', 'database', 'period', 'source', and 'tablename'.
        ...
        Returns:
            str: A JSON string containing the subscription message with encrypted token and relevant metadata.
        """
        shnumpy = table.records        
    ...
    async def websocket_subscription_loop(client):
        """
        Asynchronously manages a continuous WebSocket subscription loop for receiving, decompressing, and processing streaming data.
        Parameters:
            client (dict): A dictionary containing the WebSocket connection under the key 'conn',
                        the data table under 'table', and a 'download' counter for tracking received data size.
        """
