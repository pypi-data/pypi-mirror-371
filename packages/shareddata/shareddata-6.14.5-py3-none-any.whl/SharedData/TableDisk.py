...
class TableDisk(Table):    
    ...
    def free(self, acquire=True):        
        ...
        Optionally acquires a lock before freeing resources to ensure thread safety.
        Flushes any buffered data before freeing and removes the table's data from shared storage.
        Handles exceptions by logging errors and ensures the 'isloaded' mutex flag is reset.
    ...
    def reload(self):
        """
        Reloads the table data from disk by performing the following steps:
        - Acquires necessary locks or resources.
        - Frees current resources without acquiring new ones.
        - Downloads the latest data.
        - Allocates memory for the data.
        - If an index exists, resets its creation flag and initializes it.
        - Updates shared data references.
        - Marks the table as loaded in both header and mutex states.
        - Handles and logs any exceptions that occur during the process.
        - Ensures that resources are released after the operation.
    ...
    def delete_local(self):    
        ...
        This method attempts to remove the data associated with `self.path` from the internal `self.data` dictionary,
        frees any allocated resources, and deletes specific binary files from the local filesystem if they exist.
        If the directory becomes empty after file deletion, it removes the directory as well.
    ...
    def delete_remote(self):        
        """
        Deletes the remote folder in the S3 storage corresponding to the current database, period, source, and tablename attributes.
        
        Constructs the S3 path using the format: "{database}/{period}/{source}/{tablename}" and calls the S3DeleteFolder function to remove the folder and its contents.
        """
    ...
    def delete(self):
        """
        Deletes the table disk by performing local and remote deletions safely.
        
        This method acquires necessary resources before attempting to delete the table disk locally and remotely.
        It ensures resources are freed appropriately even if an error occurs. Any exceptions raised during the
        process are logged with the table disk's relative path and re-raised.
