...
class Metadata():
    ...
    def __init__(self, name, mode='rw', user='master'):
        ...
        """
        Initialize the object with a given name, mode, and user.
        
        Parameters:
            name (str): The name identifier for the object.
            mode (str, optional): Access mode, can be 'r' (read), 'w' (write), or 'rw' (read/write). Defaults to 'rw'.
            user (str, optional): The user role, default is 'master'.
        
        Sets up read/write permissions based on mode, determines whether to save data locally based on the environment variable 'SAVE_LOCAL',
        and initializes internal data structures for records and static data. Finally, calls the load method to populate initial data.
        """
        self.user = user
    ...
    def records(self, value):
        """
        Setter method for the 'records' property.
        Sets the internal '_records' attribute to the given value and marks 'records_chg' as True to indicate that the records have been changed.
        """
        self.records_chg = True
        self._records = value
    ...
    def __setitem__(self, tag, value):
        """
        Sets the value associated with the given tag in the static dictionary and marks the static data as changed.
        
        Parameters:
        tag (hashable): The key for the item to be set in the static dictionary.
        value (any): The value to associate with the specified tag.
        """
        self.static_chg = True
    ...
    def __getitem__(self, tag):
        """
        Retrieve the item associated with the given tag from the static dictionary.
        
        Parameters:
        tag (hashable): The key used to access the corresponding value in the static dictionary.
        
        Returns:
        object: The value associated with the specified tag in the static dictionary.
        
        Raises:
        KeyError: If the tag is not found in the static dictionary.
        """
    ...
    def save(self, save_excel=False):
        """
        Save the metadata object to local storage and optionally upload to S3.
        
        Parameters:
            save_excel (bool): If True, save the metadata as an Excel file (.xlsx) before saving the binary file.
        
        Behavior:
        - Constructs file paths for Excel (.xlsx) and binary (.bin) metadata files based on environment variable 'DATABASE_FOLDER' and user/name attributes.
        ...
        - Saves the metadata binary file locally if 'save_local' attribute is True.
        - Compresses and uploads the metadata file to S3 if 's3write' attribute is True.
    ...
    def create_metadata_io(self):
        ...
        The method constructs a header with metadata about the data's dtype description, item size, and total size,
        then writes the header, dtype description string, raw data bytes, and an MD5 hash of the dtype description
        and data into a BytesIO object.
        
        Returns:
            io.BytesIO: A BytesIO object containing the serialized header, dtype description, data, and MD5 hash.
        """
        data = self.records
    ...
    def delete(name, user='master'):
        """
        Deletes metadata files associated with the given name for a specified user.
        
        This method attempts to remove both the Excel (.xlsx) and binary (.bin) metadata files
        from the local filesystem under the user's directory within the DATABASE_FOLDER environment variable.
        It also deletes the corresponding compressed file from an S3 storage location.
        
        Parameters:
            name (str): The base name of the metadata files to delete (without extension).
            user (str, optional): The user directory under DATABASE_FOLDER. Defaults to 'master'.
        
        Returns:
            bool: True if the deletion process completes without exceptions, False otherwise.
        """
        try:            
...
def isnan(value):
    ...
    Returns:
        bool: True if the value is 'nan' (string), an empty string, or a float NaN; False otherwise.
    """
