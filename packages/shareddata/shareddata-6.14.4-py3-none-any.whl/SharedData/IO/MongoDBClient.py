    ...
    def __init__(self, user: str = 'SharedData') -> None:
        ...
        if not 'MONGODB_REPLICA_SET' in os.environ:
            ...
            self.mongodb_conn_str = (
                ...
                # f'&wtimeoutMS=5000'  # 5 second write timeout
                # f'&serverSelectionTimeoutMS=10000'  # 10 seconds for server selection
                # f'&connectTimeoutMS=5000'  # 5 seconds for individual connections
                # f'&socketTimeoutMS=15000'  # 15 seconds for operations (reduced from 30s)
                # f'&heartbeatFrequencyMS=1000'  # 1 second (matches replica set settings)
                # f'&localThresholdMS=15'  # Use nearby secondaries (15ms threshold)                
                # f'&maxStalenessSeconds=60'  # Allow 60 seconds of staleness for reads
                # f'&maxPoolSize=50'  # Connection pool size
                # f'&minPoolSize=5'  # Minimum connections ready
                # f'&maxIdleTimeMS=180000'  # 3 minutes max idle connection time
            )
