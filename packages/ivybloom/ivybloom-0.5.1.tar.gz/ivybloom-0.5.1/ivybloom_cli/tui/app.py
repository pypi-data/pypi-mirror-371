    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            ...
            with Vertical():
                ...
                try:
                    self.details_summary.styles.overflow_y = "scroll"
                    self.details_manifest.styles.overflow_y = "scroll"
                    self.details_artifacts.styles.overflow_y = "scroll"
                    self.details_structure.styles.overflow_y = "scroll"
                except Exception:
                    pass
                with TabbedContent():
                    with TabPane(title="Visualization", id="visualization"):
                        yield self.details_visualization
                    with TabPane(title="Manifest", id="manifest"):
                        yield self.details_manifest
                    with TabPane(title="Artifacts", id="artifacts"):
                        yield self.details_artifacts
                    with TabPane(title="Parameters", id="parameters"):
    ...
    def _visualize_protein_structure(self, job_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                ascii_art = pv.render_pdb_as_text(pdb_text, width=60, height=20)
                if self.details_visualization:
