...
class IvyBloomTUI(App):
    ...
    def _visualize_protein_structure(self, job_id: str) -> None:
        ...
        try:
            ...
            # Prefer simple ASCII art to avoid asciimol curses issues
    ...
    def _update_status_bar(self) -> None:
        ...
        # Build project label preferring name over id
        if self.initial_project_id:
            project_label = f"{(self._project_name or '').strip() or self.initial_project_id}"
            # If we have both and they differ, show both
            if self._project_name and self._project_name.strip() and self._project_name.strip() != self.initial_project_id:
                project_label = f"{self._project_name} ({self.initial_project_id})"
        else:
            project_label = "N/A"
        user_label = (self._user_display or "N/A")
        status = f"[dim][status: {connected}][refresh: {self.refresh_interval_secs}s][project: {project_label}][user: {user_label}] [{err}][/dim]"
