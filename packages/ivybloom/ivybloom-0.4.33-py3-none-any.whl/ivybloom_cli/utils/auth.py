...
class AuthManager:
    ...
    def get_api_key(self, username: str = "default") -> Optional[str]:
        """Retrieve stored API key from env, secure storage, or file fallback.
        
        Order of precedence:
          1) OS env var IVYBLOOM_API_KEY
          2) OS env var IVY_API_KEY (legacy)
          3) Secure storage via keyring
          4) Local file fallback in config dir
        """
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        Prefer API key for CLI endpoints. Fall back to JWT, then OAuth access token.
        This ordering ensures stable auth for both CLI commands and TUI-wrapped calls.
        """
        headers: Dict[str, str] = {}
        
        # 1) Prefer API key for stability across environments
        api_key = self.get_api_key()
        if api_key:
            headers["Authorization"] = f"Bearer {api_key}"
            return headers
        # 2) Next, dedicated stored JWT (if present)
        jwt_token = self.get_auth_token()
        if jwt_token:
            headers["Authorization"] = f"Bearer {jwt_token}"
            return headers
        # 3) Finally, OAuth access token if present and valid
