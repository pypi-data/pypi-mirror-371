...
class IvyBloomTUI(App):
    ...
    /* Ensure visualization panel has enough height and scrolls properly */
    Static.details_visualization {{
        height: 100%;
        min-height: 20;
        overflow-y: scroll;
    }}
    ...
    def __init__(self, config: Config, auth_manager: AuthManager, initial_project_id: Optional[str] = None, show_header: bool = False, show_footer: bool = False) -> None:
        ...
        # First boot toggle for optional early picker
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            ...
            with Vertical():
                ...
                self.details_visualization = Static("Press 'v' to visualize artifacts", classes="muted")
                # Enable scrolling for the visualization panel
                self.details_visualization.styles.overflow_y = "scroll"
                self.details_manifest = Static("", classes="muted")
    ...
    def _continue_boot_sequence(self) -> None:
        ...
        self._debug("_continue_boot_sequence: already authenticated -> ensuring project selection")
        self.call_later(self._ensure_project_then_load)
        # Safety net: try again shortly to ensure the picker opens if first attempt failed
        try:
            self.set_timer(2, self._ensure_project_pick)
        except Exception:
            pass
    ...
    def _ensure_project_then_load(self) -> None:
        ...
        if getattr(self, "_first_boot", True):
            try:
                ...
                self.set_timer(0.5, self._ensure_project_pick)
            except Exception:
                pass
    ...
    def _visualize_protein_structure(self, job_id: str) -> None:
        ...
        # Try to find a PDB file in artifacts
        try:
            artifact = self._artifacts.choose_artifact(job_id, "pdb")
            if not artifact:
                if self.details_visualization:
                    self.details_visualization.update("No PDB structure found for this job")
                return
            url = str(artifact.get("presigned_url") or artifact.get("url") or "")
            if not url:
                if self.details_visualization:
                    self.details_visualization.update("No download URL found for PDB structure")
                return
            pdb_content = self._artifacts.fetch_bytes(url)
            pdb_text = pdb_content.decode('utf-8', errors='ignore')
            filename = str(artifact.get("filename") or "protein.pdb")
            ...
            try:
                ...
                prefer_flatprot = bool(viz_cfg.get("prefer_flatprot", True))
                ...
                if ok and out_path:
                    ...
                    # Fallthrough to asciimol but preserve the reason, or skip if not preferred
                    if prefer_flatprot:
                        reason = f"[yellow]FlatProt not used[/yellow]: {msg}"
                        self._logger.debug(reason)
            except Exception as e:
                self._logger.debug(f"FlatProt invocation failed: {e}")
            # Use asciimol for better visualization
            try:
                visualizer = ProteinVisualizer()
                panel = visualizer.get_rich_panel(pdb_text, title=f"Protein: {filename}")
                if self.details_visualization:
                    self.details_visualization.update(panel)
                return
            except Exception as e:
                # Fallback to ASCII art if asciimol fails
                self._logger.debug(f"Asciimol visualization failed: {e}, falling back to ASCII art")
            # Fallback to simple ASCII art
            points = self._structure.parse_pdb_ca(pdb_text)
            if not points:
                if self.details_visualization:
                    self.details_visualization.update("Could not parse protein structure (no CA atoms found)")
                return
            ascii_art = self._structure.render_ascii(points, 0.0, rows=20, cols=60)
            if self.details_visualization:
                self.details_visualization.update(f"[green]Protein Structure Visualization[/green]\n\n{ascii_art}\n\n[dim]Press 'r' to refresh with a different angle[/dim]")
            # Set up rotation for next refresh
        except Exception as e:
            if self.details_visualization:
    ...
    def _visualize_best_artifact(self, job_id: str) -> None:
        ...
        try:
            # Try to find any visualizable artifact
            for artifact_type in ["pdb", "sdf", "mol", "json", "txt", "csv"]:
                artifact = self._artifacts.choose_artifact(job_id, artifact_type)
                if artifact:
                    url = str(artifact.get("presigned_url") or artifact.get("url") or "")
                    if url:
                        if artifact_type in ["pdb"]:
                            self._visualize_protein_structure(job_id)
                            return
    ...
    def _ensure_project_pick(self) -> None:
        if self._picker_open:
            return
        # If a project is already set, offer a quick switch on first boot
        if self.initial_project_id and getattr(self, "_first_boot", False):
            try:
                projects = self._projects.list_projects()
                if projects:
                    self._picker_open = True
                    self._hide_splash()
                    self._debug("_ensure_project_pick: offering switch picker on first boot")
                    self.push_screen(ProjectPicker(projects), self._on_project_picked)
                    if self.details_summary:
                        self.details_summary.update("Switch project? (Esc to keep current)")
                    return
            except Exception:
                pass
            return
