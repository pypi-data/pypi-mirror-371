...
class InstanceService(BaseService[InstanceModel]):
    ...
    async def get_instances_by_user(
        ...
        try:
            ...
            agents = await agent_service.get_agents_in_db(
                limit=limit, 
...
class Instance(InstanceModel):
    ...
    async def chat_w_fn_calls(
        ...
        print(f"Model fetched via AgentService.get_agent_in_db: {agent.model}")
        
        # Load provider if model has provider_id
        if agent.model and hasattr(agent.model, 'provider_id') and agent.model.provider_id:
            provider_service = Provider()
            agent.provider = await provider_service.get_provider_by_id(agent.model.provider_id, access_token)
