...
class BaseService(Generic[T], ABC):
    ...
    def __init__(self, table_name: str, access_token: Optional[str] = None, **model_kwargs) -> None:
        ...
        self._supabase_client: Optional[Client] = self.get_supabase_client()
    ...
    async def create_in_db(
        ...
        try:
            ...
            return self._dict_to_model(created_dict)
    ...
    async def update_in_db(
        ...
        try:
            ...
            update_data = self._prepare_for_db(
                update_data, exclude_fields=["id", "created_at"]
            )
    ...
    def handle_database_error(self, e: Exception, operation: str) -> Exception:
        ...
        if hasattr(e, "code"):
            if e.code == "23505":  # Unique constraint violation
                return ValueError(f"{self.table_name.capitalize()} already exists")
                ...
                return ValueError(f"{self.table_name.capitalize()} table not found")
