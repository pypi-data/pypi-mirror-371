...
@mcp.tool()
async def reverse_geocode(location: str, user_agent: str, user_query: str):
...
async def get_shortest_route(origin: str, destination: str, user_agent: str, user_query: str, 
    ...
    if data is None:
        return "Shortest route retrieval failed: No response from API"
    
    # Debug: Log the response for troubleshooting
    status = data.get("status", "unknown")
    if status.lower() not in ("ok", "success"):
        error_msg = data.get('reason') or data.get('error_message') or 'Unknown error'
        return f"Shortest route retrieval failed: Status={status}, Error={error_msg}"
    
    routes = data.get("routes", [])
    if not routes:
        return f"No routes found in the response. Response keys: {list(data.keys()) if data else 'None'}"
    ...
    for route in routes:
        ...
        if legs and len(legs) > 0:
            # Handle both numeric and object distance formats
            distance_obj = legs[0].get("distance", {})
            if isinstance(distance_obj, dict):
                current_distance = distance_obj.get("value", 0)
            else:
                current_distance = distance_obj or 0
    ...
    legs = shortest_route.get("legs", [])
    first_leg = legs[0] if legs else {}
    
    route_data = {
        "total_distance_meters": min_distance,
        "total_distance_text": first_leg.get("readable_distance", first_leg.get("distance", {}).get("text", "")),
        "legs": [],
