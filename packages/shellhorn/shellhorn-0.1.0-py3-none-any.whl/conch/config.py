...
class Config:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🐚 conch: Warning - Could not load config: {e}")
            return self._get_default_config()
    ...
    def get_notifiers(self, enabled_only: bool = True):
        """Get configured notifiers."""
        notifiers = []
        
        # Pushover
        pushover_config = self._config["notifications"]["pushover"]
        if pushover_config["enabled"] and pushover_config["app_token"] and pushover_config["user_key"]:
        ...
        if not notifiers and "console" in self._config.get("default_notifications", []):
            notifiers.append(ConsoleNotifier())
        
        return notifiers
    
    def set_pushover(self, app_token: str, user_key: str, device: Optional[str] = None, enabled: bool = True):
...
def get_config_from_env() -> Config:
    ...
    pushover_token = os.getenv("CONCH_PUSHOVER_TOKEN")
    pushover_user = os.getenv("CONCH_PUSHOVER_USER")
    pushover_device = os.getenv("CONCH_PUSHOVER_DEVICE")
    
    if pushover_token and pushover_user:
        config.set_pushover(pushover_token, pushover_user, pushover_device)
    
    # MQTT from environment
    mqtt_broker = os.getenv("CONCH_MQTT_BROKER")
    mqtt_port = int(os.getenv("CONCH_MQTT_PORT", "1883"))
    mqtt_topic = os.getenv("CONCH_MQTT_TOPIC", "conch")
    mqtt_username = os.getenv("CONCH_MQTT_USERNAME")
    mqtt_password = os.getenv("CONCH_MQTT_PASSWORD")
    
    if mqtt_broker:
        config.set_mqtt(mqtt_broker, mqtt_port, mqtt_topic, mqtt_username, mqtt_password)
    
    # Console notifications
    console_enabled = os.getenv("CONCH_CONSOLE_NOTIFICATIONS", "false").lower() == "true"
