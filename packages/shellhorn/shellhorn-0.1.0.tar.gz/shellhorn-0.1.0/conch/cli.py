...
def handle_config_subcommand(args: List[str]):
    ...
    if not args:
        print("Usage: conch config {show,set,test}")
        return
    ...
    if subcommand == "show":
        ...
        if len(args) < 3:
            print("Usage: conch config set KEY VALUE")
            return
        ...
        for notifier in notifiers:
            try:
                notifier.notify_success("conch config test", 1.23, 0)
...
def main():
    ...
    notifiers = config.get_notifiers()
    runner = CommandRunner(command_args, notifiers)
