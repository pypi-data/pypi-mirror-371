...
from .models import (download_model, list_installed_models, resolve_model_path,
                     set_default_model)
from .parser import parse, parse_with_probabilities, tag
from .postcode import (extract_outcode, get_county, get_post_town,
                       normalize_postcode)


def main(argv=None):
    parser = argparse.ArgumentParser(
        prog="ukaddresskit", description="UK address NER (CRFsuite)"
    )
    ...
    p_parse.add_argument(
        "--model", help=".crfsuite path (optional; will auto-resolve if omitted)"
    )
    ...
    p_set = g.add_parser(
        "set-default", help="Set default model (path or installed name)"
    )
    ...
    if args.cmd == "parse":
        model = (
            ...
            if not args.model
            else resolve_model_path(args.model)
        )
        out = (
            parse_with_probabilities(args.text, model)
            if args.probs
            else parse(args.text, model)
        )
    ...
    if args.cmd == "postcode":
        out = {
            "normalized": normalize_postcode(args.pc),
            "outcode": extract_outcode(args.pc),
        }
        if args.town:
            try:
    ...
    if args.cmd == "models":
        if args.mcmd == "list":
            items = [
                {"name": m.name, "path": str(m.path)} for m in list_installed_models()
            ]
