...
class ColorDetectionConfig(BaseConfig):
    ...
    zone: Optional[List[List[int]]] = field(default_factory=lambda: [[69, 428], [886, 390], [1048, 712], [106, 712]])
...
class ColorDetectionUseCase(BaseProcessor):
    ...
    def __init__(self):
        ...
        self.zone = self.zone = np.array([[69, 428], [886, 390], [1048, 712], [106, 712]], dtype=np.float32)
    ...
    def _is_in_zone(self, bbox: Dict[str, Any], bbox_format: str) -> bool:
        """Check if the point (x1+x2)/2, (y1+y2)/4 of a bounding box lies within the defined zone."""
        ...
        try:
            if bbox_format == "xmin_ymin_xmax_ymax":
                ...
                return False
            cx = (x1 + x2) / 2
            cy = (y1 + y2) / 4  # Modified from (y1 + y2) / 2 to (y1 + y2) / 4
            in_zone = cv2.pointPolygonTest(self.zone, (cx, cy), False) >= 0
            self.logger.debug(f"BBox point ({cx}, {cy}) in zone: {in_zone}")
            return in_zone
