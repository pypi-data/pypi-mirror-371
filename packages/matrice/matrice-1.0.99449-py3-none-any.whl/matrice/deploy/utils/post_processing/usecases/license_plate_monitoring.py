...
class LicensePlateMonitorUseCase(BaseProcessor):
    ...
    def _update_plate_texts(self, detections: List[Dict]):
        """Update plate text history, compute dominant text per track and merge duplicate tracks.

        For each ``track_id`` we keep a running list (``self.helper``) of all recognised
        plate texts *in the order they were seen*.  After every frame we:

        1.  Derive the **dominant** plate text for every track by looking only at the
            **last 50 %** of the observations for that track and selecting the most
            frequent value.  When multiple values are tied we take the one that
            appears **latest** in the observation window (i.e. closest to the
            current frame).
        2.  Merge different ``track_id``\ s that resolve to the *same* dominant
            plate text (perfect textual match).  The first track_id that produced
            the text becomes the canonical id; all other tracks are remapped to
            this id for the remainder of the processing.

        The method updates the following internal structures so that subsequent
        counting/summary steps see the correct data:
        - ``self._tracked_plate_texts``   →  mapping of canonical *track_id → text*
        - ``self._unique_plate_texts``    →  mapping of *cleaned_text → longest_raw*
        - ``detections`` list (in-place)  →  each detection now carries the
          canonical ``track_id`` and the dominant ``plate_text``.
        """
        
        def _select_dominant(text_list: List[str]) -> str:
            """Return dominant element from the last 50 % of *text_list*.

            When there is a tie we return the **latest** occurring element among
            the tied ones.
            """
            if not text_list:
                return ""
            half_idx = len(text_list) // 2  # start of the last 50 %
            window = text_list[half_idx:]
            counts = Counter(window)
            max_freq = max(counts.values())
            # All items with max frequency
            candidates = {txt for txt, cnt in counts.items() if cnt == max_freq}
            if len(candidates) == 1:
                return next(iter(candidates))
            # Tie-breaker – iterate backwards to find latest candidate
            for txt in reversed(window):
        ...
        # ------------------------------------------------------------------
        # 1)  Collect plate texts observed in this frame --------------------
        # ------------------------------------------------------------------
        for det in detections:
            raw_text = det.get("plate_text") or ""
            if not raw_text:
                continue
            cleaned = self._clean_text(raw_text)
            if len(cleaned) < self._min_plate_len:
                # Too short – ignore (length & alphanumeric checks stay intact)
                continue
            track_id = det.get("track_id")
            if track_id is None:
                continue
            # Keep full raw_text – consumers may prefer longest raw variant
            self.helper.setdefault(track_id, []).append(raw_text)

        # ------------------------------------------------------------------
        # 2)  Compute dominant text per track --------------------------------
        # ------------------------------------------------------------------
        track_to_text: Dict[Any, str] = {}
        for t_id, text_list in self.helper.items():
            dominant_raw = _select_dominant(text_list)
            if dominant_raw:
                track_to_text[t_id] = dominant_raw

        # ------------------------------------------------------------------
        # 3)  Merge tracks that share the same dominant text -----------------
        # ------------------------------------------------------------------
        text_to_canonical: Dict[str, Any] = {}
        track_id_remap: Dict[Any, Any] = {}
        for t_id, text in track_to_text.items():
            cleaned = self._clean_text(text)
            if cleaned in text_to_canonical:
                # Duplicate plate → map to existing canonical id
                canonical_id = text_to_canonical[cleaned]
                track_id_remap[t_id] = canonical_id
            else:
                text_to_canonical[cleaned] = t_id

        # Update internal dictionaries -------------------------------------
        self._tracked_plate_texts = {track_id_remap.get(t_id, t_id): txt for t_id, txt in track_to_text.items()}
        # Maintain unique cleaned → longest_raw mapping
        for raw in track_to_text.values():
            cleaned = self._clean_text(raw)
            longest = self._unique_plate_texts.get(cleaned, "")
            if len(raw) > len(longest):
                self._unique_plate_texts[cleaned] = raw

        # ------------------------------------------------------------------
        # 4)  Apply remapping & dominant text to current detections ----------
        # ------------------------------------------------------------------
        for det in detections:
            track_id = det.get("track_id")
            if track_id is None:
                continue
            canonical_id = track_id_remap.get(track_id, track_id)
            det["track_id"] = canonical_id
            dominant_text = self._tracked_plate_texts.get(canonical_id, det.get("plate_text"))
            det["plate_text"] = dominant_text

        # No need for consecutive-frame stability counters any more – remove
        self._text_history = {}
