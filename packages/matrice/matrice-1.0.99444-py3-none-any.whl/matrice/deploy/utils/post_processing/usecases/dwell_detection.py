...
class DwellConfig(BaseConfig):
    """Configuration for dwell use case in post-processing."""
...
class DwellUseCase(BaseProcessor):
    ...
    def __init__(self):
        ...
        self._zone_track_dwell_frames = {}  # zone_name -> track_id -> frame_count
        self._fps = None
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None,
        ...
        if self._fps is None and stream_info:
            self._fps = stream_info.get("input_settings", {}).get("original_fps", 30)
    ...
    def _update_zone_tracking(self, zone_analysis: Dict[str, Dict[str, int]], detections: List[Dict], config: DwellConfig) -> Dict[str, Dict[str, Any]]:
        ...
        for zone_name, zone_counts in zone_analysis.items():
            ...
            for track_id in current_tracks:
                zone_dwell = self._zone_track_dwell_frames.setdefault(zone_name, {})
                zone_dwell[track_id] = zone_dwell.setdefault(track_id, 0) + 1
    ...
    def _generate_incidents(self, counting_summary: Dict, zone_analysis: Dict, alerts: List, config: DwellConfig,
        ...
        total_long_dwell = 0
        zone_name = "Zone1"  # Assuming single zone named Zone1
        dwell_frames_dict = self._zone_track_dwell_frames.get(zone_name, {})
        long = sum(1 for f in dwell_frames_dict.values() if (f / self._fps) > config.dwell_threshold)
        total_long_dwell = long

        if total_long_dwell > 0:
            ...
            if config.alert_config and config.alert_config.count_thresholds:
                ...
                intensity = min(10.0, (total_long_dwell / threshold) * 10)
                ...
                if total_long_dwell > 30:
                    level = "critical"
                    intensity = 10.0
                    self._ascending_alert_list.append(3)
                elif total_long_dwell > 25:
                    level = "significant"
                    intensity = 9.0
                    self._ascending_alert_list.append(2)
                elif total_long_dwell > 15:
                    level = "medium"
                    intensity = 7.0
                    self._ascending_alert_list.append(1)
                else:
                    level = "low"
                    intensity = min(10.0, total_long_dwell / 3.0)
                    self._ascending_alert_list.append(0)

             # Generate human text for single zone
            human_text_lines = [f"INCIDENTS DETECTED @ {current_timestamp}:"]
            human_text_lines.append(f"\tSeverity Level: {(self.CASE_TYPE, level)}")
            human_text_lines.append(f"\tLong dwell people: {total_long_dwell}")
    ...
    def _generate_tracking_stats(
        ...
        # Generate human_text for single zone
        zone_name = "Zone1"  # Assuming single zone named Zone1
        human_text_lines = [f"Tracking Statistics:"]
        human_text_lines.append(f"CURRENT FRAME @ {current_timestamp}")
        current_count = 0
        if zone_analysis and zone_name in zone_analysis:
            zone_data = zone_analysis.get(zone_name, {})
        ...
        human_text_lines.append(f"\tCurrent count: {int(current_count)}")
        ...
        dwell_frames_dict = self._zone_track_dwell_frames.get(zone_name, {})
        dwell_times = [f / self._fps for f in dwell_frames_dict.values()]
        long_dwell = sum(1 for t in dwell_times if t > config.dwell_threshold)
        avg_dwell = sum(dwell_times) / len(dwell_times) if dwell_times else 0
        max_dwell = max(dwell_times) if dwell_times else 0
        human_text_lines.append(f"\tTotal count: {int(total_count)} (long dwell: {long_dwell}, avg: {avg_dwell:.1f}s, max: {max_dwell:.1f}s)")
    ...
    def _generate_business_analytics(self, counting_summary: Dict, zone_analysis: Dict, alerts: Any, config: DwellConfig, stream_info: Optional[Dict[str, Any]] = None, is_empty=False) -> List[Dict]:
