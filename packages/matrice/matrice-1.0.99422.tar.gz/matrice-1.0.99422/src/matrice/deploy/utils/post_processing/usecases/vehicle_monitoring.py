    ...
    def _generate_tracking_stats(self, counting_summary: Dict, zone_analysis: Dict, alerts: List, config: VehicleMonitoringConfig,
        ...
        if zone_analysis:
            ...
            for zone_name, zone_data in zone_analysis.items():
                total_count = 0
                if isinstance(zone_data, dict):
                    if "total_track_ids" in zone_data:
                        total_count = zone_data.get("total_track_ids", 0)
                    else:
                        counts_dict = zone_data.get("total_track_ids") if isinstance(zone_data.get("total_track_ids"), dict) else zone_data
                        total_count = counts_dict.get(
                            "total_track_ids",
                            sum(v for v in counts_dict.values() if isinstance(v, (int, float)))
                        )
            # zone_name = "Entrance"
            # total_count = len(self._total_count_list)
                if total_count>0:
