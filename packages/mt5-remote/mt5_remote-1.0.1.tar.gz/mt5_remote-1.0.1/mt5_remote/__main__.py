...
usage:
    ...
    rpyc_classic.py --ssl-keyfile keyfile.pem \
        --ssl-certfile certfile.pem \
        --ssl-cafile cafile.pem
"""
import sys
import os
import rpyc
...
from rpyc.utils.classic import (
    DEFAULT_SERVER_PORT,
...
class ClassicServer(cli.Application):
    mode = cli.SwitchAttr(
        ...
        default="threaded",
        help=(
            "The serving mode (threaded, forking, or 'stdio' "
            "for inetd, etc.)"
        ),
    )
    ...
    host = cli.SwitchAttr(["--host"], str, default="",
    ...
    ssl_keyfile = cli.SwitchAttr("--ssl-keyfile", cli.ExistingFile,
                                 help=("The keyfile to use for SSL. "
                                       "Required for SSL"),
                                 group="SSL", requires=["--ssl-certfile"])
    ssl_certfile = cli.SwitchAttr("--ssl-certfile", cli.ExistingFile,
                                  help=("The certificate file to use for SSL. "
                                        "Required for SSL"),
                                  group="SSL", requires=["--ssl-keyfile"])
    ssl_cafile = cli.SwitchAttr("--ssl-cafile", cli.ExistingFile,
help=(
    "The certificate authority chain file to use for SSL. "
    ...
    auto_register = cli.Flag(
        "--register",
        help=(
            "Asks the server to attempt registering with a registry server. "
    ...
    registry_host = cli.SwitchAttr(
        ...
        default=None,
        help=(
            "The registry host machine. For UDP, the default "
            "is 255.255.255.255; for TCP, a value is required"
        ),
        group="Registry",
    )
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"default = {DEFAULT_SERVER_PORT!r}, "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional command to call wine " "(omit to assume native Windows python)"
        ),
    )
    parser.add_argument(
        "-s",
