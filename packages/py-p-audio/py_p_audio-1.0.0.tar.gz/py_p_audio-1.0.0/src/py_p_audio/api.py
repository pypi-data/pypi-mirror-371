...
def list_audio_devices(device_type: Optional[DeviceType] = None,
    ...
    for device in devices:
        api_icon = "🎵" if device.api_type == APIType.ASIO else "🔊"
        type_icon = {"input": "🎤", "output": "🔈", "loopback": "🔄"}[device.device_type.value]
        default_mark = " ⭐" if device.is_default_input or device.is_default_output else ""
        loopback_mark = " 🔄" if device.is_loopback else ""
