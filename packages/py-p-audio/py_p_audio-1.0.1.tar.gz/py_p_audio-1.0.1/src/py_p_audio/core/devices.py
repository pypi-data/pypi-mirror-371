...
class DeviceManager:
        ...
        try:
            ...
            for sd_index, sd_info in enumerate(sd_devices):
                try:
                    ...
                    # Safe string handling for Japanese characters
                    try:
                        print(f"Warning: Failed to process SoundDevice device {sd_index}: {e}")
                    except UnicodeEncodeError:
                        print(f"Warning: Failed to process SoundDevice device {sd_index}: [encoding error]")
                    continue
    ...
    def _create_device_from_pyaudio(self, device_id: int, pa_index: int, 
        ...
        try:
            ...
            # Safe device name handling for Japanese characters
            device_name = pa_info['name']
            try:
                # Test if name can be safely encoded
                device_name.encode('cp932')
            except UnicodeEncodeError:
                # Replace problematic characters with safe alternatives
                device_name = device_name.encode('cp932', 'replace').decode('cp932')
