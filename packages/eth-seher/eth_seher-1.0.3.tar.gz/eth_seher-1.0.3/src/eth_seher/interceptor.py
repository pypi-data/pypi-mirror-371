...
def rpc():
    ...
    try:
        ...
        if isinstance(body, list):
            ...
            for req in body:
                ...
                if method == "eth_chainId":
                    responses.append({"jsonrpc": "2.0", "id": req.get("id"), "result": to_hex(int(CHAIN_ID))})
        ...
        # Special handling for eth_chainId
        if method == "eth_chainId":
            log_rpc(f"  → Returning configured chain ID: {CHAIN_ID}", "DEBUG")
            return jsonify({"jsonrpc": "2.0", "id": body.get("id"), "result": to_hex(int(CHAIN_ID))})
