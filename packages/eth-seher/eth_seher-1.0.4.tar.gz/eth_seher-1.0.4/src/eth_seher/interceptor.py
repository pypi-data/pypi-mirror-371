...
with open('rpc.json', 'r') as f:
    ...
    # Try to find RPC endpoint: try string key, then int key, then fallback to mainnet
    UP = rpc_config.get(str(CHAIN_ID)) or rpc_config.get(int(CHAIN_ID)) or rpc_config.get("1") or rpc_config.get(1)
    if not UP:
        print(f"WARNING: No RPC endpoint found for chain {CHAIN_ID}, using mainnet", flush=True)
        UP = rpc_config.get("1") or rpc_config.get(1)
