...
def _detect_system_theme(default_theme: str) -> str:
    ...
    if system_theme is None:
        _logger.info(f'failed to detect system theme, qthemes use "{default_theme}" theme.')
    ...
    *,
    ...
    Examples:
        ...
        1. Dark Theme ::
            ...
            app = QApplication([])
            app.setStyleSheet(qthemes.load_stylesheet())
            # or
            app.setStyleSheet(qthemes.load_stylesheet("dark"))

        2. Light Theme ::

            app = QApplication([])
            app.setStyleSheet(qthemes.load_stylesheet("light"))

        3. Automatic detection of system theme ::

            app = QApplication([])
            app.setStyleSheet(qthemes.load_stylesheet("auto"))

        4. Sharp corner ::

            # Change corner shape to sharp.
            app = QApplication([])
            app.setStyleSheet(qthemes.load_stylesheet(corner_shape="sharp"))

        5. Customize color ::

            app = QApplication([])
            app.setStyleSheet(qthemes.load_stylesheet(custom_colors={"primary": "#D0BCFF"}))
    ...
    try:
        ...
        if app is not None and not app.property("_qthemes_use_setup_style"):
    ...
    *,
    ...
    Examples:
        ...
        1. Dark Theme ::
            ...
            app = QApplication([])
            app.setPalette(qthemes.load_palette())
            # or
            app.setPalette(qthemes.load_palette("dark"))

        2. Light Theme ::

            app = QApplication([])
            app.setPalette(qthemes.load_palette("light"))

        3. Automatic detection of system theme ::

            app = QApplication([])
            app.setPalette(qthemes.load_palette("auto"))
    ...
    mk_template = partial(
        Template, filters={"color": filter.color, "palette": filter.palette_format}
    )
