...
try:
    ...
    class BasegRPCPipeline(Pipeline): # type: ignore[reportRedeclaration]
        ...
        async def _process(self, item: Dict) -> None:
            ...
            try:
                grpc_response = await self._stub.SubmitFeedMessage(feed_message)
                self.logger.info(
                    "response",
...
try:
    ...
    class BaseEnterprisegRPCPipeline(Pipeline): # type: ignore[reportRedeclaration]
        ...
        async def _process(self, item: Dict) -> None:
            ...
            await self._connected.wait()
            try:
                grpc_response = await self._stub.AddStreamPost(feed_message, metadata=[('x-api-key', self._feed_token)])
                self.logger.info(
                    "response",
                    body=MessageToDict(grpc_response),
                    id=item['_id'],
