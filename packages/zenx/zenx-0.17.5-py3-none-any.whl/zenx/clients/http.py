...
class CurlCffiClient(HttpClient):
    ...
    async def create_session(self, impersonate: str = "chrome") -> AsyncSession:
        if impersonate == "random":
            impersonate = self._get_random_fingerprint()
        session = AsyncSession(max_clients=1, impersonate=impersonate)
    ...
    async def request(
        ...
        **kwargs,
        ...
        if use_session_pool:
            if not self._session_pool.qsize():
