...
def save_tool_call(chat, tool_name, tool_args, user=None, call_id=None, reply_to_message=None):
    ...
    system_account = _get_system_account(chat.platform)
...
def save_tool_response(chat, call_id, result, user=None, reply_to_message=None):
    """
...
def save_tool_call_with_response(chat, tool_name, tool_args, result, user=None, call_id=None, reply_to_message=None):
    ...
    tool_response_msg = save_tool_response(chat, call_id, result, user, tool_call_msg)
...
def _get_system_account(platform):
    """
    Get or create a system account for tool call messages.
    
    Args:
        platform: Platform name (e.g., 'Internal', 'Telegram', etc.)
    
    Returns:
        Account instance for system messages
    """
    system_id = f"system_tool_calls"
