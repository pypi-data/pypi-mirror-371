...
def list_supported():
    ...
    console.print("Supported Database Types:", style="bold blue")
    console.print()
...
def execute_query(
    ...
    console.print(
        f"Executing query on connection '{connection_name}'...", style="bold blue"
    )
    ...
    async def _execute():
        try:
            ...
            if connection_name not in connections:
                console.print(
                    f"Connection '{connection_name}' not found", style="bold red"
                )
                return

            saved_config = connections[connection_name]
            console.print(
                f"Using saved connection: {saved_config['type']} at {saved_config['host']}:{saved_config.get('port', 'default')}",
                style="dim",
            )
            ...
            try:
                ...
                if result.success:
                    ...
                    if result.data:
                        console.print(
                            f"Results ({len(result.data)} rows):", style="bold blue"
                        )
                        table = Table(show_header=True, header_style="bold magenta")
