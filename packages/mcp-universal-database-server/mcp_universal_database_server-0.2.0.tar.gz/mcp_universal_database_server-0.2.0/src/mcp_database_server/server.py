...
from .dsn_parser import DSNParser, parse_dsn_from_env
...
class DatabaseMCPServer:
    ...
    def __init__(self):
        ...
        self._load_dsn_connections()
        self._setup_handlers()

    def _load_dsn_connections(self) -> None:
        """Load database connections from DSN environment variables."""
        # Common DSN environment variable names
        dsn_env_vars = [
            "DSN",
            "DATABASE_URL",
            "DB_URL",
        ...
        # Also check for numbered DSN variables (DSN_1, DSN_2, etc.)
        for i in range(1, 11):  # Support up to 10 DSN connections
            dsn_env_vars.extend([f"DSN_{i}", f"DATABASE_URL_{i}", f"DB_URL_{i}"])

        loaded_count = 0
        for env_var in dsn_env_vars:
            config = parse_dsn_from_env(env_var)
            if config:
                try:
                    # Use environment variable name as connection name if not already taken
                    connection_name = env_var.lower().replace("_", "-")
                    if connection_name in [
                        conn.name
                        for conn in self.connection_manager.connections.values()
                    ]:
                        connection_name = f"{connection_name}-{loaded_count + 1}"

                    config.name = connection_name
                    asyncio.create_task(self.connection_manager.add_connection(config))
                    logger.info(
                        f"Loaded DSN connection '{connection_name}' from environment variable '{env_var}'"
                    )
                    loaded_count += 1
                except Exception as e:
                    logger.warning(f"Failed to load DSN from {env_var}: {e}")

        if loaded_count > 0:
            logger.info(
                f"Successfully loaded {loaded_count} DSN connection(s) from environment variables"
            )
        else:
            logger.info("No DSN connections found in environment variables")
    ...
    def _setup_handlers(self) -> None:
        ...
        async def handle_list_tools() -> list[Tool]:
            ...
            return [
                ...
                Tool(
                    name="add_connection_from_dsn",
                    description="Add a database connection from a DSN string",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "name": {
                            ...
                            "dsn": {
                                "type": "string",
                                "description": "Database DSN (Data Source Name) string",
                            },
                        },
                        "required": ["name", "dsn"],
                        "additionalProperties": False,
                ...
                Tool(
                    name="parse_dsn",
                    description="Parse and validate a DSN string without adding connection",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "dsn": {
                                "type": "string",
                                "description": "Database DSN string to parse",
                            },
                        },
                        "required": ["dsn"],
                        "additionalProperties": False,
        ...
        async def handle_call_tool(
            ...
            try:
                ...
                elif name == "add_connection_from_dsn":
                    return await self._handle_add_connection_from_dsn(arguments)
                elif name == "parse_dsn":
                    return await self._handle_parse_dsn(arguments)
    ...
    async def _handle_add_connection_from_dsn(self, args: dict) -> list[types.TextContent]:
        """Handle add_connection_from_dsn tool."""
        name = args.get("name")
        dsn = args.get("dsn")
        
        if not name or not dsn:
            return [TextContent(type="text", text="Name and DSN are required")]
        
        try:
            # Parse DSN to create connection config
            config = DSNParser.parse_dsn(dsn, name)
            
            # Add connection
            await self.connection_manager.add_connection(config)
            
            return [
                TextContent(
                    type="text", 
                    text=f"Successfully added connection '{name}' from DSN\n"
            ...
            return [TextContent(type="text", text=f"Failed to add connection from DSN: {e}")]

    async def _handle_parse_dsn(self, args: dict) -> list[types.TextContent]:
        """Handle parse_dsn tool."""
        dsn = args.get("dsn")
        
        if not dsn:
            return [TextContent(type="text", text="DSN is required")]
        
        try:
            # Parse DSN without adding connection
            config = DSNParser.parse_dsn(dsn, "test-connection")
            
            result = f"DSN parsed successfully:\n\n"
            result += f"Database Type: {config.type.value}\n"
            result += f"Host: {config.host}\n"
            result += f"Port: {config.port}\n"
            result += f"Username: {config.username or 'None'}\n"
            result += f"Database: {config.database or 'None'}\n"
            result += f"Schema: {config.schema_name or 'None'}\n"
            result += f"SSL: {'Yes' if config.ssl else 'No'}\n"
            
            if config.options:
                result += f"Additional Options: {config.options}\n"
            
            # Show supported schemes
            result += f"\nSupported DSN schemes:\n"
            for scheme in DSNParser.get_supported_schemes():
                result += f"• {scheme}\n"
            
            return [TextContent(type="text", text=result)]
        except Exception as e:
            return [TextContent(type="text", text=f"Failed to parse DSN: {e}")]
