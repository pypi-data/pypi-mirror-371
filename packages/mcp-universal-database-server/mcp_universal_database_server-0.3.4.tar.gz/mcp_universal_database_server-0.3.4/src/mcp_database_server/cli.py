...
@app.command()
def execute_query(
    connection_name: str = typer.Argument(..., help="Connection name to use"),
    query: str = typer.Argument(..., help="SQL query to execute"),
):
    """Execute a SQL query on a saved connection."""
    console.print(f"🔍 Executing query on connection '{connection_name}'...", style="bold blue")
    ...
    async def _execute():
        try:
            ...
            if connection_name not in connections:
                console.print(f"❌ Connection '{connection_name}' not found", style="bold red")
                return
            
            saved_config = connections[connection_name]
            console.print(f"✅ Using saved connection: {saved_config['type']} at {saved_config['host']}:{saved_config.get('port', 'default')}", style="dim")
            ...
            config = ConnectionConfig(
                ...
                password=saved_config.get("password"),
                database=saved_config.get("database"),
                schema_name=saved_config.get("schema"),
                ssl=saved_config.get("ssl", False),
            )
            ...
            try:
                ...
                if result.success:
                    console.print("✅ Query executed successfully!", style="bold green")
                    if result.data:
                        console.print(f"📊 Results ({len(result.data)} rows):", style="bold blue")
                        table = Table(show_header=True, header_style="bold magenta")
                        ...
                        console.print(f"📝 Affected rows: {result.affected_rows}", style="bold green")
                else:
            ...
            console.print(f"❌ Error executing query: {e}", style="bold red")
