...
class DatabaseMCPServer:
    ...
    def _load_existing_connections(self) -> None:
        """Load existing connections from the persistent connections file."""
        try:
            ...
            for conn_name, conn_data in connections_data.items():
                try:
                    # Convert CLI format to ConnectionConfig
                    config = ConnectionConfig(
                        ...
                        schema_name=conn_data.get("schema"),
                        ssl=conn_data.get("ssl", True),
                    )

                    # Add to connection manager (synchronously for initialization)
                    try:
                        ...
                        try:
                            ...
                            asyncio.create_task(
                                self.connection_manager.add_connection(config)
                            )
                        except RuntimeError:
                        ...
                        logger.warning(f"Failed to load connection '{conn_name}': {e}")
                except Exception as e:
                    logger.warning(f"Failed to process connection '{conn_name}': {e}")

            if loaded_count > 0:
                logger.info(f"Loaded {loaded_count} existing connection(s) from file")
    ...
    def _load_dsn_connections(self) -> None:
            ...
            if config:
                try:
                    ...
                    if connection_name in [
                        conn.name
                        for conn in self.connection_manager._connection_configs.values()
                    ]:
                        connection_name = f"{connection_name}-{loaded_count + 1}"

                    config.name = connection_name
                    # Add connection synchronously during initialization
                    try:
                        # Use the current event loop if available, otherwise create a new one
                        try:
                            loop = asyncio.get_running_loop()
                        ...
                        logger.warning(
                            f"Failed to add DSN connection '{connection_name}': {e}"
                        )
                except Exception as e:
        ...
        if loaded_count > 0:
            ...
            # Save DSN connections to persistent storage (synchronously during init)
            try:
                # Use the current event loop if available, otherwise create a new one
                try:
                    loop = asyncio.get_running_loop()
                    # We're in an async context, schedule the task
                    asyncio.create_task(self._save_dsn_connections_to_file())
                except RuntimeError:
                    # No running loop, create a new one
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    loop.run_until_complete(self._save_dsn_connections_to_file())
                    loop.close()
            except Exception as e:
                logger.warning(f"Failed to save DSN connections: {e}")
        else:
            logger.info("No DSN connections found in environment variables")

    async def _save_dsn_connections_to_file(self) -> None:
        """Save DSN connections to the persistent connections file."""
        try:
            # Use the same connections file path as CLI
            connections_file = Path.home() / ".mcp_database_connections.json"
            ...
            if connections_file.exists():
                try:
                    ...
                    logger.warning(f"Failed to load existing connections: {e}")

                    # Convert DSN connections to CLI format and save
            for (
                conn_name,
                config,
            ) in self.connection_manager._connection_configs.items():
                if conn_name not in existing_connections:
                    # Convert ConnectionConfig to CLI format
                    conn_data = {
                    ...
                    existing_connections[conn_name] = conn_data

            # Save updated connections
            with open(connections_file, "w") as f:
                json.dump(existing_connections, f, indent=2)

            logger.info(
                f"Saved {len(existing_connections)} connections to {connections_file}"
            )

        except Exception as e:
            logger.error(f"Failed to save DSN connections to file: {e}")
    ...
    async def _handle_add_connection_from_dsn(
        self, args: dict
    ) -> list[types.TextContent]:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Failed to add connection from DSN: {e}")
            ]
