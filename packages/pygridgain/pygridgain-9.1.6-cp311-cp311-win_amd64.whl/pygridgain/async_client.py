...
class AsyncClient:
    ...
    def __init__(self, address: EndPoint | list[EndPoint], **kwargs):
        """
        Initialize client.

        :param address: An address or a collection of addresses to connect to. An address is a str or a tuple (str, int)
         where first value is the host and the second value is the port. If the address is a str only then default port
         is used (10800).
    ...
    @staticmethod
    def _validate_address(address: list[EndPoint]):
