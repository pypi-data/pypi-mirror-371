...
from pygridgain import IgniteError, AsyncBinaryMap
...
async def test_pop(async_map):
    ...
    assert await async_map.pop(b'1', b'5435') is b'5435'
...
async def test_put_all_type_error(async_map):
    ...
    assert err.match('Dictionary is expected in format: {Binary, Binary}')
