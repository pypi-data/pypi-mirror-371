...
from pygridgain.constants import SUPPORTED_PROTOCOLS, PROTOCOL_BYTE_ORDER
from pygridgain.end_point import EndPoint
from pygridgain.error_code import ErrorGroup, ErrorCode
from pygridgain.ignite_error import connection_errors, IgniteError
from pygridgain.bitmask_feature import BitmaskFeature
from pygridgain.protocol_context import ProtocolContext
from pygridgain.utils import create_ssl_context, check_ssl_config, extra_from_connection_config
...
class HandshakeResponse:
    version : (int, int, int)
    error: IgniteError | None
    observable_ts: int

@dataclass
class ResponseHeader:
    req_id: int
    assignment_ts: int | None
    observable_ts: int
    error: IgniteError | None
    bytes_processed: int
...
class BaseProtocol(asyncio.Protocol):
    ...
    def data_received(self, data: bytes) -> None:
        ...
        if not self._magic_received:
            if self._buffer[:4] != MAGIC_BYTES:
                raise IgniteError(ErrorCode.PROTOCOL,
    ...
    def __get_next_message(self) -> bytearray | None:
        if len(self._buffer) < 4:
            return None
...
class AsyncNodeConnection:
    ...
    * encapsulates handshake and reconnection.
    ...
    def __init__(self, cluster_connection: 'AsyncClusterConnection', end_point: EndPoint, connection_timeout: float = 10.0,
                 authenticator = None, ssl_config = None):
        """
    ...
    async def _do_connect(self, ssl_context) -> HandshakeResponse:
        ...
        self._transport, _ = await self._loop.create_connection(
            ...
            host=self._end_point.host,
            port=self._end_point.port,
            ssl=ssl_context
        )
    ...
    async def request(self, req_builder: RequestBuilder) -> bytearray | None:
        """
        Perform request.
    ...
    def _process_handshake_error(self, response: HandshakeResponse):
        ...
        if any(server_version):
            error_text += f' Server expects protocol version ' \
    ...
    def _on_handshake_fail(self, err: Exception):
        ...
        if isinstance(err, IgniteError) and err.error_group == ErrorGroup.AUTHENTICATION:
            ...
            logger.error("Failed to perform handshake, connection to node %s with protocol context %s failed: %s",
                         self._end_point, self.protocol_context, err, exc_info=True)
