...
from .ignite_error import connection_errors, IgniteError
...
class _AsyncSqlResultSet:
    ...
    async def _get_next_page(self):
        req_builder = lambda req_id: self._py_result_set.make_next_page_request(req_id)
    ...
    async def close(self):
        ...
        if not self._closed and not self._py_result_set.closed_remotely():
            self._closed = True
            try:
                req_builder = lambda req_id: self._py_result_set.make_close_request(req_id)
...
class AsyncClusterConnection:
    ...
    async def _connect(self, end_points: list[EndPoint]):
    ...
    async def request(self, req_builder: RequestBuilder, timeout: float | None) -> bytearray | None:
        """
        Perform request.
        ...
        try:
            ...
            raise IgniteError(ErrorCode.CONNECTION, f'The request has timed out')

    async def request_node(self, req_builder: RequestBuilder, timeout: float | None) -> (bytearray | None, AsyncNodeConnection):
        """
        Perform request.
    ...
    async def _do_request(self, req_builder: RequestBuilder) -> (bytearray | None, AsyncNodeConnection):
        """
        Perform request.
    ...
    async def sql(self, query: str) -> _AsyncSqlResultSet:
        ...
        :return: Result set.
        """
        req_builder = lambda req_id: _native_extension.make_sql_request(req_id, self._observable_ts, query)
