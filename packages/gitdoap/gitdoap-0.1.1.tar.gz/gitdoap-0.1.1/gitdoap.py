...
def normalize_github(
    repo_url: str, base_url: str = "https://github.com/"
) -> tuple[str, str]:
...
def github_doap(repo_url: str, base_url: str = "https://github.com/") -> Graph:
    ...
    if repo.description:
        repo_resource.add(DOAP.shortdesc, Literal(repo.description))
    ...
    repo_resource.add(DOAP.repository, URIRef(repo.svn_url + ".svn"))
    ...
    if repo.issues_url:
        repo_resource.add(DOAP["bug-database"], URIRef(repo_url + "/issues"))
    ...
    git_repo_resource = g.resource(URIRef(repo.clone_url))
    git_repo_resource.add(RDF.type, DOAP.GitRepository)
    git_repo_resource.add(DOAP.location, URIRef(repo.clone_url))
    git_repo_resource.add(DOAP.location, URIRef(repo.git_url))
    git_repo_resource.add(DOAP.location, URIRef(repo.ssh_url))
    git_repo_resource.add(DOAP.browse, URIRef(repo_url))

    svn_repo_resource = g.resource(URIRef(repo.svn_url + ".svn"))
    svn_repo_resource.add(RDF.type, DOAP.SVNRepository)
    svn_repo_resource.add(DOAP.location, URIRef(repo.svn_url))
    svn_repo_resource.add(DOAP.browse, URIRef(repo_url))
