...
class DdbAuditRepo:
    ...
    def _item_to_report(self, item: dict[str, Any]) -> UsageReport:
        """Convert a DynamoDB item to a UsageReport."""
        return UsageReport(
            api_key_id=item["api_key_id"],
            window_start=datetime.fromisoformat(item["window_start_iso"]),
            window_end=datetime.fromisoformat(item["window_end_iso"]),
            units_reported=item["units_reported"],
            ...
            stripe_idempotency_key=item["stripe_idempotency_key"],
            stripe_response=item.get("stripe_response", {}),
            stripe_usage_record_id=item.get("stripe_usage_record_id"),
            reported_at=datetime.fromisoformat(item["reported_at_iso"]),
            reported_by_request_id=item["reported_by"],
            lambda_function_version=item["lambda_function_version"],
            library_version=item["library_version"],
            source_usage_count=item.get("source_usage_count"),
            previous_window_end=(
                datetime.fromisoformat(item["previous_window_end_iso"])
