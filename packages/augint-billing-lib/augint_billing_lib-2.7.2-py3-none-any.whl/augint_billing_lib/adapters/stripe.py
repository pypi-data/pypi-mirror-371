...
class StripeAdapter:
    ...
    def _create_usage(
        ...
        return stripe.UsageRecord.create(**kwargs)  # type: ignore[no-any-return]
    ...
    def report_usage(
        ...
        # Convert stripe.UsageRecord to dict for protocol compatibility
        # Handle both real Stripe objects and mocked dicts (for tests)
        if isinstance(response, dict):
            return response
        # mypy doesn't know _create_usage can return dict in tests
        return {  # type: ignore[unreachable]
            "id": response.id,
            "object": response.object,
            "livemode": response.livemode,
            "quantity": response.quantity,
            "subscription_item": response.subscription_item,
