...
class Honeypot:
    ...
    def get_data_from_goplus(self, chain_id, token_id, security_data=None):
        ...
        while retries <= MAX_RETRIES:
            try:
                ...
                print(f"ERROR: Request to GoPlus timed out for token {token_id} using proxy {proxy}")
                retries += 1
            except ConnectionError:
                print(f"ERROR: Connection failed to GoPlus for token {token_id} using proxy {proxy}")
                retries += 1
        ...
        return {"filled_by": "no_data", "chain": chain_name, "chain_id": chain_id}
    ...
    def get_data_from_honeypot(self, chain_id, token_id, security_data=None):
        ...
        try:
            ...
            print(f"ERROR: Request to Honeypot timed out for token {token_id} using proxy {proxy}")
        except ConnectionError:
            print(f"ERROR: Connection failed to Honeypot for token {token_id} using proxy {proxy}")
        except RequestException as e:
