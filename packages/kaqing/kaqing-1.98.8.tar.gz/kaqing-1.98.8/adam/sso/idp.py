...
from adam.sso.idp_session import IdpSession
...
class CredCache:
    ...
    def __init__(self):
        if not hasattr(self, 'env_f') or not self.env_f:
    ...
    def cache(self, username: str, password: str = None):
        ...
        if updated:
            ...
            self.env_f = None
            Config.debug(f'Cached username: {username}, password: {password}')
    ...
    def login(app_host: str, username: str = None, idp_uri: str = None, forced = False, use_token_from_env = True, use_cached = True) -> IdpLogin:
        ...
        try:
            ...
            while not username or Idp.ctrl_c_entered:
                ...
                Config.debug(f'User read from cache: {default_user} with use_cached: {use_cached}')
