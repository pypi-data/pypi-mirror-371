    ...
    def load_model(self, model_id: str, **kwargs) -> bool:
        ...
        try:
            with self.model_lock:
                ...
                # Create new config with better defaults for newer models
                config = ModelConfig(
                    name=model_id,
                    model_type=kwargs.get('model_type', 'auto'),
                    precision=kwargs.get('precision', 'auto'),
                    context_length=kwargs.get('context_length', 8192),  # Increased default for newer models
...
{"tool_call": "python_eval", "args": {"code": "print(2 + 2)"}}"""
    ...
    def _generate_gguf(self, prompt: str, config: ModelConfig) -> str:
        ...
        try:
            # Use only user prompt for GGUF models to avoid showing system prompt
    ...
    def _generate_synapse(self, prompt: str, config: ModelConfig, show_thinking: bool = True) -> str:
        ...
        try:
            # Use only user prompt for synapse models to avoid showing system prompt
            ...
            gen_args = {
                ...
                "use_cache": False,  # Disable cache to avoid cache setter issues
            }
    ...
    def _generate_transformers(self, prompt: str, config: ModelConfig) -> str:
        ...
        try:
            # Use only user prompt for transformers models to avoid showing system prompt
            inputs = self.model_manager.current_tokenizer(prompt, return_tensors="pt")
            input_ids = inputs["input_ids"].to(self.model_manager.current_model.device)
            attention_mask = inputs.get("attention_mask", None)
            ...
            gen_args = {
                ...
                "eos_token_id": self.model_manager.current_tokenizer.eos_token_id,
                "pad_token_id": self.model_manager.current_tokenizer.eos_token_id,
                "use_cache": False,  # Disable cache to avoid the cache setter issue
            }
...
class CogniCLI:
    ...
    def show_logo(self):
        ...
        version_text = Text("v2.2.6 - Premium Edition", style="dim")
    ...
    def start_premium_chat_mode(self):
        ...
        while True:
            try:
                # Get user input with cognicli prompt (protected from backspace deletion)
                self.console.print(f"[bold {COGNICLI_ACCENT}]cognicli[/bold {COGNICLI_ACCENT}] [dim]>[/dim] ", end="")
                prompt = input().strip()
...
Examples:
  ...
  cognicli --ollama pull llama2:q4_K_M     # Pull specific quantization
    ...
    parser.add_argument('--version', action='version', version='CogniCLI v2.2.6 - Premium Edition')
    ...
    welcome_content = f"""
[bold {COGNICLI_ACCENT}]Welcome to CogniCLI v2.2.6 Premium![/bold {COGNICLI_ACCENT}] 🚀
...
[dim]• --model gpt2                    # Load GPT-2 → Auto-start premium chat[/dim]
[dim]• --model microsoft/DialoGPT-medium --type q4  # Load with 4-bit quantization[/dim]
[dim]• --search llama2                 # Search for Llama models[/dim]
[dim]• --ollama pull llama2:q4_K_M     # Pull quantized model from Ollama[/dim]
...
def start_interactive_mode(self):
    """Start interactive mode without a model loaded"""
    self.console.print(f"\n[dim]Type commands or load a model to start chatting. Use 'help' for available commands.[/dim]\n")
    
    while True:
        try:
            # Get user input with cognicli prompt
            prompt = Prompt.ask(
            ...
            if prompt.startswith('--'):
                self._handle_cli_argument(prompt)
                continue
            
            # If no model is loaded, suggest loading one
            if not self.model_manager.is_model_loaded():
                self.console.print(f"[yellow]No model loaded. Use '--model <id>' to load a model or 'help' for commands.[/yellow]")
                continue
...
• [cyan]status[/cyan] - Show model and system status
• [cyan]exit[/cyan] or [cyan]quit[/cyan] - Exit CogniCLI

[green]Command-Line Style Arguments:[/green]
• [cyan]--model <id>[/cyan] - Load a Hugging Face model
• [cyan]--search <query>[/cyan] - Search for models on Hugging Face
• [cyan]--info <id>[/cyan] - Show detailed model information
• [cyan]--list [filter][/cyan] - Browse available models
• [cyan]--ollama list[/cyan] - List available Ollama models
• [cyan]--ollama search <query>[/cyan] - Search Ollama models
• [cyan]--ollama pull <model>[/cyan] - Pull model from Ollama

[green]Examples:[/green]
• [dim]--model gpt2[/dim] - Load GPT-2 and start premium chat
• [dim]--model microsoft/DialoGPT-medium --type q4[/dim] - Load with quantization
• [dim]--search llama[/dim] - Search for Llama models
• [dim]--ollama pull llama2:q4_K_M[/dim] - Pull quantized model

[dim]💡 Once a model is loaded, you can chat directly by typing your message![/dim]
"""
    self.console.print(Panel(help_text, title="📚 Interactive Help", border_style=COGNICLI_ACCENT))

def _handle_cli_argument(self, prompt: str):
    ...
    try:
        ...
        if args[0] == '--model' and len(args) > 1:
            ...
            query = args[1]
            models = self.search_models(query, limit=20)
            if models:
                table = Table(title=f"🔍 Search Results for '{query}'", box=box.ROUNDED, title_style=COGNICLI_ACCENT)
                table.add_column("Model ID", style=COGNICLI_ACCENT, no_wrap=True)
                table.add_column("Downloads", style="green", justify="right")
                table.add_column("Author", style="cyan")
                
                for model in models[:10]:
                    table.add_row(model['id'], f"{model['downloads']:,}", model.get('author', 'Unknown'))
            ...
            if models:
                ...
                for model in models[:10]:
                    table.add_row(model['id'], f"{model['downloads']:,}")
                self.console.print(table)
            else:
                self.console.print("[yellow]No models found.[/yellow]")
            ...
            if args[1] == 'list':
                ...
                self._search_ollama_models(args[2])
            elif args[1] == 'pull' and len(args) > 2:
                self._pull_ollama_model(args[2])
            else:
                self.console.print(f"[red]Invalid Ollama operation: {args[1]}[/red]")
...
# Add methods to CogniCLI class
CogniCLI.start_interactive_mode = start_interactive_mode
CogniCLI.show_interactive_help = show_interactive_help
CogniCLI._handle_cli_argument = _handle_cli_argument
