...
from pyampact.alignmentUtils import orio_simmx, dp, maptimes, trim_silences, merge_grace_notes
...
def run_DTW_alignment(y, original_sr, piece, tres, width, target_sr, nharm, win_ms, hop, nmat, showSpec):
    ...
    p, q, S, D, M = align_midi_wav(
        piece, WF=y, sr=original_sr, TH=tres, width=width, tsr=target_sr, nhar=nharm, hop=hop, wms=win_ms, showSpec=showSpec)
...
def align_midi_wav(piece, WF, sr, TH, width, tsr, nhar, wms, hop, showSpec):
    """
    ...
    # Resample M along the time axis to match D.shape[1]
    M_resampled = scipy.signal.resample(M, D.shape[1], axis=1)
    M = pd.DataFrame(M_resampled, columns=range(D.shape[1]))        
    D = np.abs(D) 
