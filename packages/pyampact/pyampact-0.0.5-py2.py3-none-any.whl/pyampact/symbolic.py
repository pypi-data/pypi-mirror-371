...
class Score:
    ...
    def _assignM21Attributes(self, path=''):
        ...
        if self.path not in imported_scores:
            ...
            # read file/string as volpiano or tinyNotation if applicable
                ...
                if len(csv_df.columns) == 4: # for speech
    ...
    def midi_ticks_durations(self, i=1, df=None):
        ...
        # Enforce minimum duration and no overlaps
        grace_threshold = 0.03
        min_gap = 0.025  # Prevents overlaps when extending

        onsets = res['ONSET_SEC'].values
        offsets = res['OFFSET_SEC'].values

        for i in range(len(res) - 1):
            dur = offsets[i] - onsets[i]

            # If it's a grace note, leave it
            if dur < grace_threshold:
                continue

            next_onset = onsets[i + 1]

            # Extend to next onset
            if offsets[i] < next_onset:
                offsets[i] = next_onset

            # Clamp to avoid overlap
            if offsets[i] > next_onset - min_gap:
                offsets[i] = max(onsets[i], next_onset - min_gap)

        # Update res and DataFrame
