...
6.  [**Creating and Publishing the PyPI Package**](#creating-and-publishing-the-pypi-package)
    *   [Step 1: Project Structure](#step-1-project-structure)
    *   [Step 2: Create `setup.py`](#step-2-create-setuppy)
    *   [Step 3: Build the Package](#step-3-build-the-package)
    *   [Step 4: Upload to PyPI](#step-4-upload-to-pypi)
7.  [**Usage Guide for Jupyter/Kaggle Notebooks**](#usage-guide-for-jupyterkaggle-notebooks)
...
8.  [**Understanding the Configuration (`config.py`)**](#understanding-the-configuration-configpy)
9.  [**Pros & Cons**](#pros--cons)
10. [**Why SENTIEBL is the Best Choice**](#why-sentiebl-is-the-best-choice)
...
graph TD
    subgraph Initialization
        A["User executes main()"] --> B{"Load Config"};
        B --> C["Connect to Ollama Model"];
        C --> D["Load Prompt Library"];
    end

    subgraph Audit Loop
        D --> E{"Start Round-Robin Iteration"};
        E --> F["Select Prompt & Category"];
        F --> G["Apply Parameterization Method"];
        G --> H["Get Model Response"];
        H --> I["Analyze & Score Response"];
        I --> J{"Score > Threshold?"};
    end
    
    subgraph Findings
        J -- Yes --> K["Create & Save JSON Finding"];
        J -- No --> L["Log & Continue"];
        K --> L;
    end

    L --> E;
    E -- All prompts done or timeout --> M["End Loop"];

    subgraph Reporting
        M --> N["Aggregate All Results"];
        N --> O["Generate Markdown Dashboard"];
        N --> P["Generate Summary Report & CSVs"];
        N --> Q["Generate Visualizations"];
    end

    subgraph Output
        O --> R["Save Detailed_Findings.md"];
        P --> S["Save Detailed_Analysis.md & .csv files"];
        Q --> T["Save chart .png files"];
    end
...
```mermaid
graph TD
    A[Start] --> B{Select Prompt via Round-Robin}
    B --> C{Choose Method}
    C -- fixed --> D[Set temp=1.0, tokens=1024]
    C -- randomized --> E[Set temp=random from 0.1 to 1.0, tokens=random from 1024 to 4096]
    C -- gradual --> F[Increment temp & tokens based on index]
    D --> G[Format Prompt]
    E --> G
    F --> G
    G --> H[Send to Model]
...
```mermaid
graph TD
    A[Start: evaluate_response receives raw text] --> B(Evaluate Severity<br>Scans VULNERABILITY_PATTERNS_SEVERITY);
    A --> C(Evaluate Breadth<br>Scans VULNERABILITY_PATTERNS_BREADTH);
    
    subgraph Initial Scoring
        B --> D[Get initial severity_score & severity_rationale];
        C --> E[Get initial breadth_score & breadth_rationale];
    end

    subgraph Modifier Analysis
        A --> F{Is text a refusal?};
        A --> G{Is text length > threshold?};
    end

    subgraph Final Logic Application
        D & E & F & G --> H{Apply Logic based on Modifiers};
        H -- "Is Refusal" --> I[Adjust scores/rationales for refusal<br>e.g., 'failed safety attempt' or 'refused as expected'];
        H -- "Not Refusal" --> J{Is Verbose AND Score is 0?};
        J -- "Yes" --> K[Adjust scores/rationales for verbosity<br>e.g., 'overly verbose output'];
        J -- "No" --> L[Use initial scores & rationales];
        K --> L;
        I --> L;
    end

    L --> M[Return Final Scores & Rationales for Severity and Breadth];
    M --> N[End];
```
...
3.  **Findings Generation & Reporting**
    *   **Description:** If the scores from the analysis stage meet the `MIN_SEVERITY_TO_SAVE` or `MIN_BREADTH_TO_SAVE` thresholds, the result is flagged as a significant finding. A structured JSON object is created, and the data is logged. At the end of the run, all results and findings are aggregated by the reporting suite to create the final CSV files, Markdown reports, and visualizations.
    ```mermaid
    graph TD
        A[Start: Receive Analysis Result] --> B{Severity or Breadth >= Threshold?};
        B -- Yes --> C[Create Structured JSON Finding];
        C --> D[Save Finding to Files];
        D --> E[Add to 'all_results' list];
        B -- No --> F[Log as non-significant];
        F --> E;
        E --> G{End of Audit?};
        G -- No --> H[Continue to next prompt];
        G -- Yes --> I[Aggregate All Data];
        I --> J[Generate Markdown Dashboard];
        I --> K[Generate Summary & CSVs];
        I --> L[Generate Charts];
        L --> M[End];
    ```
...
Ensure your project is structured correctly. All the Python source code (`main.py`, `config.py`, `dictionary.py`) should reside inside a directory named `sentiebl`.
...
The `setup.py` file you provided is perfect for this structure. It tells Python's setuptools how to build and install your package, including its name, version, dependencies, and classifiers.
...
```
Now, navigate to the root of your project (`/sentiebl`) in your terminal and run the following command to create the distribution files:
```bash
python setup.py sdist bdist_wheel
```
This will create a `dist/` directory containing the source archive (`.tar.gz`) and a wheel file (`.whl`).

#### Step 4: Upload to PyPI
You can upload your package to the Python Package Index (PyPI). First, you'll need an account on [PyPI.org](https://pypi.org).

Then, run `twine` to upload the files from your `dist/` directory:
```bash
twine upload dist/*
```
You will be prompted for your PyPI username and password. After a successful upload, your package will be live and installable via `pip`.
