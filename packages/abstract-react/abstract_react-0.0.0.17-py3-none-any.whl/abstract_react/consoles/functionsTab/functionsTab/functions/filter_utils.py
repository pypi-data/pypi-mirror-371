...
def _on_map_ready(self, graph: dict, func_map: dict):
        ...
        # Build/refresh the chips
        names = sorted(self.func_map.keys())
        self._rebuild_fn_buttons(names)
        # Keep selection if possible; else pick first
        if self.current_fn and self.current_fn in self.func_map:
            self._render_fn_lists_for(self.current_fn)
        elif names:
            self._on_function_clicked(names[0])
        self.appendLog(f"[map] UI updated: {len(names)} functions\n")
