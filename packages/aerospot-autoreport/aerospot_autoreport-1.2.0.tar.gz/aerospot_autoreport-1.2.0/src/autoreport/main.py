...
class AeroSpotReportGenerator:
    ...
    def _process_uav_data(
        ...
        if len(processed_merged_data["processed_data"]) != len(ref_data):
            logger.error(
                f"无人机反演数据样本：{len(processed_merged_data['processed_data'])},光谱样本：{len(ref_data)},不匹配，请分析原因。"
            )
        else:
    ...
    def generate_indicator_maps(self):
        ...
        visualization_mode = (
            self.config.get("visualization_mode") or 
            self.config.get("company_info", {}).get("visualization_mode", "quantitative")
        )
        logger.info(f"读取到的可视化模式: {visualization_mode}")
        # 初始化建图参数
        self.maps_processor.init_maps(geo_info, satellite_img, pred_data, uav_data, kml_boundary_path, visualization_mode)
        # 遍历生成每个指标反演结果分布图
