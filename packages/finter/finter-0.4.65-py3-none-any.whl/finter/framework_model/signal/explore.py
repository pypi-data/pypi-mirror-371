...
def generate_param_combinations(
    ...
    Args:
        ...
        expert_defaults: expert point를 위한 기본값 (midpoint method에서 사용)
    ...
    if method == "full":
        ...
        # Midpoint hypercube 방법 사용
        midpoint_points = midpoint_hypercube(param_ranges, expert_defaults)
    ...
    param_combinations = [dict(zip(param_keys, combo)) for combo in params_combinations]
...
def run_param_exploration(
    ...
    if stats_results:
        ...
        if nav_list:
            nav_df = pd.concat(nav_list, axis=1, join="outer", keys=stats_df.index)
...
if __name__ == "__main__":
    from finter.framework_model.signal.main import SignalParams

    params = SignalParams(
        percentile={
            "default": 90,
            "range": list(range(80, 100, 1)),
            "description": "Top percentile",
        },
        position_rolling_window={
            "default": 252,
            "range": [21, 63, 126, 252, 504],
            "description": "Position rolling window",
        },
        momentum_window={
            "default": 300,
            "range": [20, 60, 120, 240, 300, 360, 480, 600],
            "description": "Momentum window",
        },
        reversal_effect_window={
            "default": 20,
            "range": [3, 5, 10, 20, 60],
            "description": "Reversal effect window",
        },
    )

    midpoint_points = midpoint_hypercube(params.range, params.default)
