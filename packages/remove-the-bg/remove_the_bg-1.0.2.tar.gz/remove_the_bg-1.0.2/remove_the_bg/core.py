...
def get_vendored_imports():
    ...
    try:
        ...
        # Create completely clean import environment
        import importlib.util
        import types
        
        # Import numpy using spec loader to bypass source detection
        numpy_path = vendor_path / "numpy"
        numpy_init = numpy_path / "__init__.py"
        
        if not numpy_init.exists():
            raise ImportError(f"Vendored numpy not found: {numpy_init}")
        
        # Create a new module with bypassed source detection
        spec = importlib.util.spec_from_file_location("vendored_numpy", numpy_init)
        numpy_module = importlib.util.module_from_spec(spec)
        
        # Patch the module's globals to disable source detection
        numpy_module.__file__ = str(numpy_init).replace('vendor', 'site-packages')  # Fake location
        numpy_module.__path__ = [str(numpy_path)]
        
        # Set environment to bypass numpy's source detection
        ...
        # Fallback: Try direct import with aggressive patching
        try:
            return get_vendored_imports_fallback()
        except Exception as fallback_error:
            raise ImportError(f"Failed to load vendored dependencies: {e}. Fallback also failed: {fallback_error}")


def get_vendored_imports_fallback():
    """Fallback method with aggressive numpy patching."""
    vendor_path = Path(__file__).parent / "vendor"
    
    # Completely override numpy's source detection by monkey-patching
    import builtins
    original_import = builtins.__import__
    
    def patched_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == 'numpy' or (name.startswith('numpy.') and 'vendor' in str(vendor_path)):
            # Force numpy to think it's properly installed
            if globals is None:
                globals = {}
            if '__file__' not in globals:
                globals['__file__'] = str(vendor_path / 'numpy' / '__init__.py').replace('vendor', 'site-packages')
    ...
    try:
        ...
        os.environ['NUMPY_DISABLE_SOURCE_DETECTION'] = '1'
        os.environ['PYTHONPATH'] = str(vendor_path)
        
        # Clear any existing numpy modules
        for mod_name in list(sys.modules.keys()):
