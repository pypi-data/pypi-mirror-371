...
def download_go_term_graph():
    """
    Downloads and returns the directed acyclic graph of all GO terms from a website with up-to-date
    annotations.
    ...
    ## example code for dumping the database of ID: description to a json
    # import json
    # json.dump({key: goterms[key].name for key in goterms.keys()},
              # open('../data/metadata/GO_database.json', 'w'))

    ## example code for parsing the DAG into a tabular data format
    # godb = []
    #
    # for key, g in goterms.items():
    #     godb.append({
    #         'id': g.id,
    #         'name': g.name,
...
def load_go_term_database():
    """
    Loads the GO term table stored as a file in this package.
    """
    from importlib.resources import files

    db_path = files(package="bja_utils").joinpath("resources", "GO_database.tsv")
    return pd.read_table(str(db_path), index_col=0)
