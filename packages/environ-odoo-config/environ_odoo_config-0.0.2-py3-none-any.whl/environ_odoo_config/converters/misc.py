...
from .. import utils
from ..api import Env, OdooConfig
from ..api_converter import NOT_INI_CONFIG, OdooConfigConverter, OnlyCli, RepeatableKey, SimpleKey
from ..utils import csv_set_value
...
class ConfigConverterMisc(OdooConfigConverter):
    ...
    stop_after_init: bool = SimpleKey(
        "STOP_AFTER_INIT", cli="--stop-after-init", info="stop the server after its initialization"
    )
    save_config_file: bool = SimpleKey(
        "SAVE_CONFIG_FILE", cli=["-s", "--save"], info="Save the generated config. Always True"
    )
    admin_password: str = SimpleKey("ADMIN_PASSWORD", info="The Admin password for database management")
    data_dir: Path = SimpleKey("DATA_DIR", cli=["-D", "--data-dir"], info="Directory where to store Odoo data")
    _cli_advanced: Any = OnlyCli("--dev", "--shell-interface")
