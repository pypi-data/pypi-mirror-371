...
class OdooConfigConverter:
    ...
    def __init__(self, env: Env = _DEFAULT) -> None:
    ...
    def auto_write_to_config(self, config: OdooConfig):
        for field_name, env_key in self._private_fields().items():
            ...
            value = getattr(self, field_name, ODOO_DEFAULT)
            if value:
                value = env_key.ini_converter(value)
            if value in (_DEFAULT, ODOO_DEFAULT):
                continue
            if env_key.ini_section != "options":
                config.misc.setdefault(env_key.ini_section, {})[env_key.ini_dest] = value
            else:
                config.options.setdefault(env_key.ini_section, {})[env_key.ini_dest] = value
...
class EnvKey(Generic[OUT_TYPE]):
    ...
    def propagate_field_to_other_version(self):
        dest_names = set()
    ...
    def get_by_version(self, current_version: OdooVersion) -> "EnvKey[IN_TYPE, OUT_TYPE]|None":
