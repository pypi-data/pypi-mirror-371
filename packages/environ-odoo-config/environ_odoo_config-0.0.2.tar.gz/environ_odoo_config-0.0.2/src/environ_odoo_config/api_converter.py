...
from typing_extensions import Any, Callable, Collection, Dict, Generic, LiteralString, Self, Set, Type, TypeVar, Union
...
from .utils import csv_set_value, if_not_empty, if_not_none, negate_bool, no_change
...
class OdooConfigConverter:
    ...
    @classmethod
    def new(cls, extra_env: Env | Mapping[str, Any] = _DEFAULT, *, apply_mappers: bool = True) -> Self:
        env = Env()
    ...
    def init(self, env: Env) -> Self:
        ...
        for field_name, env_key in self._private_fields().items():
            ...
            if not version_env_key:
                ...
                continue
            value = version_env_key.get_value(env)
...
class EnvKey(Generic[OUT_TYPE]):
    ...
    cli: list[str]
    """The Odoo cli argpase values can be short or long version"""
    ...
    def _post__init__(self):
        if self.other_version == _DEFAULT:
            self.other_version: list[SimpleKey[OUT_TYPE]] = []
        if self.cli != _DEFAULT and not isinstance(self.cli, list):
            self.cli = [self.cli]
        if self.odoo_version == _DEFAULT:
            self.odoo_version = OdooVersionRange(vmin=OdooVersion.V11, vmax=OdooVersion.V18)
    ...
    def propagate_field_to_other_version(self):  # noqa: C901
        ...
        for other_version in self.other_version:
            assert other_version.other_version != _DEFAULT, "Version Keys can't have sub version"
            assert type(other_version) is type(self), "Version Keys should be the same type of self"
    ...
    def set_python_info(self, field_name: str, python_type: Any):
        ...
        if python_type in (Any, "Any"):
            self.from_environ_value = no_change
            self.ini_converter = no_change
            self.cli_use_filter = if_not_none
    ...
    def cli_used(self):
        if not self.cli or self.cli == _DEFAULT:
            return None
        more_than_2lenght = list(filter(lambda it: len(it) > SHORTCUT_CLI_LENGHT, self.cli))
        if more_than_2lenght:
            return more_than_2lenght[0]
        return self.cli[0]
    ...
    def set_python_info(self, field_name: str, python_type: Any):  # noqa: C901
        ...
        if python_type in (str, "str"):
            ...
            if (
                ...
                self.cli_use_filter = negate_bool
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        if csv_converter == _DEFAULT:
            csv_converter = csv_set_value
        self.csv_converter = csv_converter
        if self.ini_converter == _DEFAULT:
            self.ini_converter = to_csv
        if self.cli_use_filter == _DEFAULT:
            self.cli_use_filter = if_not_empty
...
class OnlyCli(SimpleKey[OUT_TYPE]):
    def __init__(
        ...
        *cli: LiteralString,
        ...
        super().__init__(cli=list(cli), info=info, odoo_version=odoo_version, ini_dest=NOT_INI_CONFIG)
