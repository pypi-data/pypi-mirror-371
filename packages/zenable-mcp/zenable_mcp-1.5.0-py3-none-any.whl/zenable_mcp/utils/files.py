...
def _matches_any_pattern(
    ...
    for pattern in patterns:
        if "*" in pattern or "?" in pattern:
            if _match_glob_pattern(file_str, pattern, file_parts, file_basename):
                return True
        else:
            if _match_exact_pattern(file_str, pattern, file_basename):
                return True
    return False
...
def filter_files_by_patterns(
    ...
    for file_path in files:
        ...
        if directory_only and not file_path.is_dir():
            if handler_name:
                log.info(
                    f"{handler_name} passed in file {file_path} but directory_only filter excluded it"
                )
            continue

        # Check include patterns (if specified)
        if patterns and not _matches_any_pattern(file_path, patterns):
            if handler_name:
                log.info(
                    f"{handler_name} passed in file {file_path} but the configured pattern excluded it"
                )
            continue

        # Check exclude patterns
        if exclude_patterns and _matches_any_pattern(file_path, exclude_patterns):
            if handler_name:
                log.info(
                    f"{handler_name} passed in file {file_path} but the configured exclude pattern excluded it"
                )
            continue
...
def _expand_pattern(
    ...
    if directory_only:
        if full_path.exists() and full_path.is_dir():
            ...
            dirs = [full_path]
            dirs.extend([d for d in full_path.rglob("*") if d.is_dir()])
            return dirs
    else:
    ...
    log.warning(f"Pattern '{pattern}' did not match any {entity_type}")
    return []
