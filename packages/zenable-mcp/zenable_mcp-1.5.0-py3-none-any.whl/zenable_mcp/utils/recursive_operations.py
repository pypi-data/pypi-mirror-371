...
def find_git_repositories(
    ...
    try:
        ...
        log.info(f"Current directory is within git repository: {repo_root}")
        ...
        log.debug("Current directory is not within a git repository")
        ...
        if ".git" in dirs:
            ...
            try:
                ...
                if repo_root_str not in visited_repos:
                    ...
                    log.info(f"Found git repository: {current_path}")
...
def execute_in_git_repositories(
    ...
    for repo in git_repos:
        log.info(f"Repository found: {repo}")
    ...
    for repo in git_repos:
        ...
        try:
            ...
            log.error(f"Failed to process repository {repo}: {e}")
            ...
            log.error(f"Unexpected error in repository {repo}: {e}")
            error_result = InstallResult(
                status=InstallStatus.FAILED,
                component_name=operation_name,
                message=f"{e}",
            ...
            try:
                ...
                log.exception(f"Failed to restore working directory to {original_cwd}")
...
def execute_for_multiple_components(
    ...
    for path in paths:
        ...
        try:
            ...
            log.error(f"Failed to change to path {path}: {e}")
            ...
            if not silent:
                click.echo(
                    f"  {click.style('✗', fg='red')} Failed to access repository: {e}"
                )
...
def _display_status_line(
    ...
    if names and len(names) <= max_names:
        ...
        click.echo(f"  {click.style(icon, fg=color)} {action} {names_str}")
    else:
        # For larger lists, show count with type
        if "Would configure" in action or "Configured" == action:
            click.echo(f"  {click.style(icon, fg=color)} {action} {count} {type_str}")
        else:
            click.echo(f"  {click.style(icon, fg=color)} {count} {type_str} {action}")
...
def _display_failures(
    ...
    for i, result in enumerate(failed_results[:max_display]):
        click.echo(f"    - {result.component_name}: {result.message}")
    ...
    for result in results:
        if result.is_success and hasattr(result, "message"):
            if "saved to" in result.message:
                log.info(f"  {result.component_name}: {result.message}")
...
def _display_repository_result(result: InstallResult, dry_run: bool) -> None:
    ...
    if dry_run:
        if result.is_success:
            log.info(f"  Would install {result.component_name}")
            ...
            log.info(f"  Already installed: {result.component_name}")
    else:
        if result.is_success:
            log.info(f"  {result.component_name}: {result.message}")
        elif result.is_error:
...
def _display_failure_summary(
    ...
    click.echo(f"  {click.style('✗', fg='red')} {count} {repo_text} {verb} failures")
    ...
    for repo, components in list(failures_by_repo.items())[:max_repos]:
        ...
        click.echo(f"    - {repo.name}: {', '.join(unique_components)}")
        displayed += 1
