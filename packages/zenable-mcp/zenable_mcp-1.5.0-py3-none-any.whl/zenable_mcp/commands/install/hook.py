...
def _install_claude_hook_in_repo(repo_path: Path, dry_run: bool) -> InstallResult:
    ...
    try:
        ...
        return InstallResult(InstallStatus.FAILED, "Claude Code hook", f"Failed: {e}")
...
@click.pass_context
def hook(ctx, dry_run, recursive):
    ...
    """
    # Store dry_run and recursive in context for subcommands
    ...
    if ctx.invoked_subcommand is None:
        ...
        ctx.invoke(all_hooks, recursive=recursive)


@hook.command(name="all")
@common_hook_options
@click.pass_context
def all_hooks(ctx, is_global, dry_run, recursive):
    ...
    if recursive:
        ...
        if git_repos is None:
            ...
            if not from_parent_install:  # Only print if not called from parent
...
@common_hook_options
@click.pass_context
def claude_code_hook(ctx, is_global, dry_run, recursive):
    ...
    if recursive:
        # Run recursively - delegate to all_hooks
        return ctx.invoke(all_hooks, is_global=False, dry_run=dry_run, recursive=True)
...
@common_hook_options
@click.pass_context
def claude_hook(ctx, is_global, dry_run, recursive):
    """Install Claude Code hooks."""
    # Just invoke claude-code
    ctx.invoke(
        claude_code_hook, is_global=is_global, dry_run=dry_run, recursive=recursive
    )
