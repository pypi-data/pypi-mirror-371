...
class ClaudeContextStrategy(IDEContextStrategy):
    ...
    def detect(self) -> bool:
        ...
        try:
            ...
            if self._validated_input.is_valid_claude_hook():
                log.debug("Claude Code context detected via Pydantic validation")
                log.debug(f"Tool name: {self._validated_input.tool_name}")
                return True
        except ValidationError as e:
            log.debug(f"JSON does not match Claude Code hook structure: {e}")
    ...
    def _read_stdin(self) -> Optional[Dict]:
        ...
        if sys.stdin.isatty():
            log.debug("stdin is a TTY, not reading input")
            return None
        ...
        try:
            ...
            log.debug(f"Failed to parse stdin as JSON (may exceed 10MB limit): {e}")
