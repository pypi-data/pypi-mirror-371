...
class ClaudeCodeInputHandler(InputHandler):
    ...
    def can_handle(self) -> bool:
        ...
        if os.environ.get("CLAUDE_HOOK_EVENT_NAME"):
            logger.debug("Found CLAUDE_HOOK_EVENT_NAME environment variable")
            return True
        ...
        if has_required and isinstance(stdin_data.get("tool_input"), dict):
            logger.debug("Detected Claude Code hook format in stdin")
            return True
    ...
    def get_exit_code(self, has_findings: bool) -> int:
        ...
        Returns:
            HOOK_ADDRESS_FINDINGS if there are findings (to instruct IDE), SUCCESS otherwise
        """
        return ExitCode.HOOK_ADDRESS_FINDINGS if has_findings else ExitCode.SUCCESS
    ...
    def _read_stdin(self) -> Optional[Dict[str, Any]]:
        ...
        if sys.stdin.isatty():
            logger.debug("stdin is a tty, not reading input")
            return None
        ...
        try:
            ...
            logger.debug(f"Failed to parse stdin as JSON: {e}")
