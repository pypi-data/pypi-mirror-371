...
class InputHandlerRegistry:
    ...
    def register(self, handler: InputHandler) -> None:
        ...
        """
        logger.debug(f"Registering input handler: {handler}")
    ...
    def detect_and_parse(self) -> Optional[HookInputContext]:
        ...
        for handler in self._handlers:
            logger.debug(f"Checking handler: {handler.name}")
            try:
                if handler.can_handle():
                    capable_handlers.append(handler)
                    logger.debug(f"Handler {handler.name} can handle the input")
            except Exception as e:
                logger.warning(f"Handler {handler.name} failed during detection: {e}")
                continue
        ...
        if len(capable_handlers) == 1:
            ...
            try:
                ...
                logger.warning(f"Handler {handler.name} failed during parsing: {e}")
                return None

        logger.debug("No input handler matched")
        return None
