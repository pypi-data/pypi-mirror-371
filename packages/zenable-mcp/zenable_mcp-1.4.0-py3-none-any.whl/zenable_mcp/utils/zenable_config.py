...
from zenable_mcp.pattern_match import should_skip_file
...
def filter_files_by_zenable_config(
    ...
    try:
        ...
        if error:
            log.debug(f"Error loading config: {error}, using defaults")
        ...
        log.debug(f"Error filtering by zenable_config: {e}, returning all files")
        return files
