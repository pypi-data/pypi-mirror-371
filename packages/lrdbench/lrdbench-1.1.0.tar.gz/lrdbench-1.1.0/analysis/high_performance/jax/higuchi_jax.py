...
class HiguchiEstimatorJAX(BaseEstimator):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        valid_mask = np.isfinite(curve_lengths_np) & (curve_lengths_np > 0) & np.isfinite(k_values_np) & (k_values_np > 1)
    ...
    def _calculate_curve_lengths_batch(self, data: jnp.ndarray, k_values: List[int]) -> List[float]:
        """
    ...
    def _calculate_segment_length_jax(self, data: jnp.ndarray, m: int, k: int, n_m: int, n: int) -> jnp.ndarray:
        """
