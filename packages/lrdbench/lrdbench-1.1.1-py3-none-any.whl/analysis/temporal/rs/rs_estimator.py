...
class RSEstimator(BaseEstimator):
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.parameters['window_sizes'] is not None:
            ...
            if not all(self.parameters['window_sizes'][i] < self.parameters['window_sizes'][i+1] for i in range(len(self.parameters['window_sizes'])-1)):
            ...
            if self.parameters['max_window_size'] is not None and self.parameters['max_window_size'] <= self.parameters['min_window_size']:
    ...
    def estimate(self, data: np.ndarray) -> dict:
        ...
        if self.parameters['window_sizes'] is not None:
            ...
            if self.parameters['max_window_size'] is None:
                ...
                max_window_size = min(self.parameters['max_window_size'], len(data) // 4)
            
            if max_window_size <= self.parameters['min_window_size']:
                raise ValueError("Need at least 3 window sizes")
            
            # Generate window sizes
            window_sizes = np.logspace(np.log10(self.parameters['min_window_size']), 
                                     np.log10(max_window_size), 20, dtype=int)
            window_sizes = np.unique(window_sizes)
        
        if len(data) < min(window_sizes) * 2:
            raise ValueError(f"Data length ({len(data)}) must be at least {min(window_sizes) * 2}")
        ...
        self.results = {
            ...
            'window_sizes': self.scales.tolist() if hasattr(self.scales, 'tolist') else self.scales,
    ...
    def get_estimation_quality(self) -> Dict[str, Any]:
        ...
        if n > 2:
            ...
            f_stat = (self.results['r_squared'] / 1) / ((1 - self.results['r_squared']) / (n - 2))
    ...
    def plot_scaling(self, figsize: Tuple[int, int] = (10, 6)) -> None:
        ...
        try:
            ...
            ax.loglog(self.results['window_sizes'], self.results['rs_values'], 'bo-', markersize=6, linewidth=2)
            ...
            if 'hurst_parameter' in self.results:
                ...
                ax.loglog(self.results['window_sizes'], rs_fitted, 'r--', 
                         label=f'Fitted line (H={self.results["hurst_parameter"]:.3f})', linewidth=2)
                ax.legend()
    ...
    def plot_analysis(self, figsize: Tuple[int, int] = (12, 8)) -> None:
        ...
        ax1.loglog(self.results['scales'], self.results['rs_values'], 'bo-', 
                  label='R/S values', markersize=6)
        
        # Plot fitted line
        log_scales = self.results['log_scales']
        log_rs_fitted = self.results['intercept'] + self.results['hurst_parameter'] * log_scales
        rs_fitted = np.exp(log_rs_fitted)
        ax1.loglog(self.results['scales'], rs_fitted, 'r--', 
        ...
        ax2.plot(log_scales, self.results['log_rs'], 'bo-', 
                label='Log(R/S) values', markersize=6)
        ax2.plot(log_scales, log_rs_fitted, 'r--', 
