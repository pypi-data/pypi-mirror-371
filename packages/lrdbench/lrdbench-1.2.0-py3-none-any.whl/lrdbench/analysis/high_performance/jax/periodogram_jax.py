...
def _compute_periodogram_jax(data: jnp.ndarray, window: str = 'hann') -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def _compute_welch_psd_jax(data: jnp.ndarray, nperseg: int, window: str = 'hann') -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def _fit_power_law_jax(log_freqs: jnp.ndarray, log_psd: jnp.ndarray) -> Tuple[float, float, float]:
    """
...
class PeriodogramEstimatorJAX(BaseEstimator):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        if self.parameters['use_welch']:
            freqs, psd = _compute_welch_psd_jax(data_jax, nperseg, self.parameters['window'])
        else:
