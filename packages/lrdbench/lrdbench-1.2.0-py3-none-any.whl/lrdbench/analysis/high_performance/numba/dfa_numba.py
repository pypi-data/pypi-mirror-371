...
@jit(nopython=True, cache=True)
def _calculate_fluctuations_batch_numba(data: np.ndarray, box_sizes: np.ndarray, 
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        if self.parameters['box_sizes'] is not None:
            ...
            box_sizes = np.unique(np.logspace(
                np.log10(min_size), np.log10(max_size), 
