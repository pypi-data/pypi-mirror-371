...
class MFDFAEstimator(BaseEstimator):
    ...
    def _compute_fluctuation_function(self, data: np.ndarray, q: float, scale: int) -> float:
        """
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        multifractal_spectrum = self._compute_multifractal_spectrum(generalized_hurst, q_values)
        ...
        self.results = {
            ...
            'q_values': q_values,
            'fluctuation_functions': {q: fq.tolist() if hasattr(fq, 'tolist') else list(fq) 
                                    for q, fq in fluctuation_functions.items()}
        }
        
        return self.results
    
    def _compute_multifractal_spectrum(self, generalized_hurst: Dict[float, float], 
                                     q_values: List[float]) -> Dict[str, List[float]]:
        """
        ...
        valid_q = [q for q in q_values if not np.isnan(generalized_hurst.get(q, np.nan))]
    ...
    def get_confidence_interval(self, confidence_level: float = 0.95) -> Dict[str, Tuple[float, float]]:
        """
        ...
        return {
            'hurst_parameter': (max(0, hurst - margin), min(1, hurst + margin))
        }
    
    def plot_analysis(self, data: np.ndarray, figsize: Tuple[int, int] = (15, 10)) -> None:
        """
        ...
        for q, fq_vals in self.results['fluctuation_functions'].items():
            ...
            if np.any(valid_mask):
                axes[0, 1].loglog(np.array(scales)[valid_mask], 
        ...
        if np.any(valid_mask):
            axes[1, 0].plot(np.array(q_vals)[valid_mask], 
                           np.array(h_vals)[valid_mask], 'o-', alpha=0.7)
            axes[1, 0].axhline(y=0.5, color='r', linestyle='--', alpha=0.5, label='H=0.5')
