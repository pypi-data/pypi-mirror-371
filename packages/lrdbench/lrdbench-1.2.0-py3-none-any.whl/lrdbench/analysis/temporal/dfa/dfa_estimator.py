...
class DFAEstimator(BaseEstimator):
    ...
    def get_estimation_quality(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'goodness_of_fit': 'excellent' if self.results['r_squared'] > 0.95 else
                              'good' if self.results['r_squared'] > 0.9 else
                              'fair' if self.results['r_squared'] > 0.8 else 'poor'
        }
    ...
    def plot_scaling(self, save_path: str = None) -> None:
        ...
        # Plot fitted line
        x_fit = np.array([min(self.results['log_sizes']), max(self.results['log_sizes'])])
        ...
        # Add R² value
        ax.text(0.05, 0.95, f'R² = {self.results["r_squared"]:.3f}', 
