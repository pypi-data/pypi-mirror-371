...
class HiguchiEstimator(BaseEstimator):
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.parameters['k_values'] is not None:
            ...
            if not all(k1 < k2 for k1, k2 in zip(
                self.parameters['k_values'][:-1], 
                self.parameters['k_values'][1:]
            )):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        valid_mask = np.isfinite(lengths_arr) & (lengths_arr > 0) & np.isfinite(k_arr) & (k_arr > 1)
        valid_k = k_arr[valid_mask]
        valid_lengths = lengths_arr[valid_mask]

        if valid_k.size < 3:
            raise ValueError("Insufficient valid Higuchi points (need >=3 after filtering non-positive values)")
    ...
    def plot_scaling(self, save_path: str = None) -> None:
        ...
        ax2.scatter(k_values, curve_lengths, color='green', alpha=0.7, label='Data points')
        ...
        textstr = f'Fractal Dimension: {D:.3f}\nHurst Parameter: {H:.3f}\nR²: {r_squared:.3f}'
