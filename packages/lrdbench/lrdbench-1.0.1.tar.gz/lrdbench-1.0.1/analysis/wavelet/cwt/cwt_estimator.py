...
class CWTEstimator(BaseEstimator):
    ...
    def get_confidence_interval(self, confidence: Optional[float] = None) -> Tuple[float, float]:
        ...
        residuals = np.array(power_logs) - (
            ...
            np.mean(power_logs) - np.mean(scale_logs) * (2 * self.estimated_hurst - 1)
        )
    ...
    def plot_analysis(self, data: np.ndarray, figsize: Tuple[int, int] = (15, 10)) -> None:
        ...
        im = ax2.imshow(np.log10(self.power_spectrum), aspect='auto', 
        ...
        ax3.loglog(self.scales, list(self.scale_powers.values()), 'bo-', 
                  label='Scale Powers')
        ...
        y_line = (2 * self.estimated_hurst - 1) * x_line + np.mean(power_logs) - \
                np.mean(scale_logs) * (2 * self.estimated_hurst - 1)
        ax4.plot(x_line, y_line, 'r--', 
