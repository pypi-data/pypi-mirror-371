...
class WaveletWhittleEstimator(BaseEstimator):
    ...
    def _theoretical_spectrum_fgn(self, frequencies: np.ndarray, H: float, sigma: float = 1.0) -> np.ndarray:
        """
        ...
        spectrum[nonzero_freq] = sigma**2 * np.abs(frequencies[nonzero_freq])**(1 - 2*H)
    ...
    def _whittle_likelihood(self, H: float, wavelet_coeffs: List[np.ndarray], 
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        if len(data) < 2**max(self.scales):
            raise ValueError(f"Data length {len(data)} is too short for scale {max(self.scales)}")
        ...
        result = optimize.minimize_scalar(
            objective, 
            bounds=(0.01, 0.99),  # Avoid exact 0 and 1
            method='bounded'
        )
    ...
    def get_confidence_interval(self, confidence: Optional[float] = None) -> Tuple[float, float]:
        """
        ...
        H_range = np.linspace(self.estimated_hurst - 0.1, self.estimated_hurst + 0.1, 21)
        likelihoods = []
        ...
        for H in H_range:
            likelihoods.append(self._whittle_likelihood(H, wavelet_coeffs_list, scales_list))
    ...
    def plot_analysis(self, figsize: Tuple[int, int] = (12, 8)) -> None:
        ...
        H_range = np.linspace(max(0.01, self.estimated_hurst - 0.2), 
                            min(0.99, self.estimated_hurst + 0.2), 50)
        likelihoods = []
        ...
        ax2.axvline(self.estimated_hurst, color='r', linestyle='--', 
