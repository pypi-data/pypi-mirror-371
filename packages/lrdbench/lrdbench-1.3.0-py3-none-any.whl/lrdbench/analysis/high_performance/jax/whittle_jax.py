...
@jit
def _fgn_spectrum_jax(freqs: jnp.ndarray, hurst: float, scale: float = 1.0) -> jnp.ndarray:
    """
...
def _local_whittle_likelihood_jax(params: jnp.ndarray, freqs: jnp.ndarray, psd: jnp.ndarray) -> float:
    """
...
def _compute_welch_psd_jax(data: jnp.ndarray, window_type: int, nperseg: int = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def _compute_periodogram_jax(data: jnp.ndarray, window_type: int) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
class WhittleEstimatorJAX(BaseEstimator):
    ...
    def __init__(self, min_freq_ratio=0.01, max_freq_ratio=0.1,
                 ...
                 window='hann', nperseg=None, use_gpu=True):
        """
    ...
    def _whittle_estimate_jax(self, freqs: jnp.ndarray, psd: jnp.ndarray) -> Tuple[float, float, float]:
        """
    ...
    def _fallback_estimate_jax(self, freqs: jnp.ndarray, psd: jnp.ndarray) -> Tuple[float, float, float]:
        """
    ...
    def plot_spectrum(self, save_path: str = None, show: bool = True) -> None:
        ...
        ax.set_title(f'Whittle Spectrum (JAX Optimized)\nH = {hurst:.3f}, R² = {self.results["r_squared"]:.3f}')
        ax.legend()
