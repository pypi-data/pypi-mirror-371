...
class GPHEstimatorJAX(BaseEstimator):
    ...
    def __init__(self, min_freq_ratio=0.01, max_freq_ratio=0.1,
                 use_welch=True, window_type=1, nperseg=None,
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.window_type not in [0, 1, 2, 3]:
            raise ValueError("window_type must be 0 (boxcar), 1 (hann), 2 (hamming), or 3 (blackman)")
    ...
    def _gph_regression_jax(self, freqs, psd, min_freq, max_freq, apply_bias_correction):
    ...
    def estimate(self, data):
        ...
        # Compute periodogram
        if self.use_welch:
            freqs, psd = self._compute_welch_psd_jax(data, self.window_type, self.nperseg)
        else:
        ...
        hurst, d_parameter, intercept, r_squared, regressor, log_periodogram = self._gph_regression_jax(
