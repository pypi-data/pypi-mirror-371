...
class FractionalBrownianMotionGenerator:
    ...
    def _cholesky_method(self, n_steps: int, hurst: float, dt: float) -> np.ndarray:
        ...
        for i in range(n_steps):
            for j in range(n_steps):
                K[i, j] = 0.5 * (times[i]**(2*hurst) + times[j]**(2*hurst) - 
    ...
    def _circulant_method(self, n_steps: int, hurst: float, dt: float) -> np.ndarray:
        ...
        for i in range(n_padded):
            if i <= n_padded // 2:
                c[i] = 0.5 * (times[i]**(2*hurst) + times[0]**(2*hurst) - 
    ...
    def _jax_circulant_method(self, n_steps: int, hurst: float, dt: float) -> np.ndarray:
    ...
    def _jax_batch_generation(self, 
                              n_paths: int,
                              n_steps: int, 
                              hurst: float, 
                              dt: float) -> np.ndarray:
        ...
        def build_circulant(i):
            if i <= n_padded // 2:
                return 0.5 * (times[i]**(2*hurst) + times[0]**(2*hurst) - 
                             jnp.abs(times[i] - times[0])**(2*hurst))
            else:
                return c[n_padded - i]
        
        c = jnp.where(indices <= n_padded // 2,
                     jax.vmap(build_circulant)(indices),
                     c)
    ...
    def get_method_info(self) -> dict:
        ...
        if JAX_AVAILABLE:
            ...
            info['performance_notes']['jax'] = 'GPU-accelerated, fastest for large sequences'
...
def generate_fbm_path(n_steps: int, 
                     hurst: float, 
                     dt: float = 1.0,
                     method: str = 'auto',
...
def generate_fbm_increments(n_steps: int, 
                           hurst: float, 
                           dt: float = 1.0,
                           method: str = 'auto',
                           seed: Optional[int] = None) -> np.ndarray:
