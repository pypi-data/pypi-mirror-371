    ...
    def solve(self, 
        ...
        **kwargs
        ...
        if self.method == 'euler':
            return self._euler_maruyama(drift_func, diffusion_func, x0, t_span, hurst, **kwargs)
        elif self.method == 'milstein':
            return self._milstein(drift_func, diffusion_func, x0, t_span, hurst, **kwargs)
    ...
    def _milstein(self, 
        ...
        **kwargs
        ...
        try:
            ...
            diffusion_deriv = self._compute_diffusion_derivative(diffusion_func, x0, t_span[0])
            use_milstein = True
        except:
            warnings.warn("Cannot compute diffusion derivative, falling back to Euler-Maruyama")
            use_milstein = False
        ...
        for i in range(1, n_steps):
            ...
            if use_milstein:
                ...
                diffusion_deriv = self._compute_diffusion_derivative(diffusion_func, x_prev, t_prev)
                
                # Milstein correction term
                fbm_increment = fbm_increments[i-1]
                milstein_correction = 0.5 * diffusion * diffusion_deriv * (fbm_increment**2 - dt**(2*hurst))
                
                x[i] = x_prev + drift * dt + diffusion * fbm_increment + milstein_correction
            else:
    ...
    def _heun(self, 
        ...
        **kwargs
        ...
        for i in range(1, n_steps):
            ...
            x[i] = x_prev + 0.5 * (drift_prev + drift_curr) * dt + \
                   0.5 * (diffusion_prev + diffusion_curr) * fbm_increment
        
        return x, t_span
    
    def _compute_diffusion_derivative(self, 
                                     diffusion_func: Callable,
                                     x: np.ndarray,
                                     t: float,
...
class JAXSDESolver(SDESolver):
    ...
    def solve(self, 
        ...
        **kwargs
        ...
        # Solve using JAX
        solution = self._jit_solve(drift_func, diffusion_func, x0_jax, t_span_jax, hurst)
    ...
    def _solve_jax(self, 
                   ...
                   x0: jnp.ndarray,
                   t_span: jnp.ndarray,
                   hurst: float) -> jnp.ndarray:
        """JAX implementation of SDE solving."""
        if self.method == 'euler':
            return self._euler_maruyama_jax(drift_func, diffusion_func, x0, t_span, hurst)
        elif self.method == 'milstein':
    ...
    def _milstein_jax(self, 
        ...
        def step(x_prev, i):
            ...
            if jnp.isscalar(diffusion):
                milstein_correction = 0.5 * diffusion * diffusion * (fbm_increment**2 - dt**(2*hurst))
            else:
                # For vector case, use simplified correction
                milstein_correction = 0.5 * jnp.sum(diffusion**2) * (fbm_increment**2 - dt**(2*hurst))
            
            x_next = x_prev + drift * dt + diffusion * fbm_increment + milstein_correction
            return x_next, x_next
    ...
    def _heun_jax(self, 
        ...
        def step(x_prev, i):
            ...
            x_next = x_prev + 0.5 * (drift_prev + drift_curr) * dt + \
                     0.5 * (diffusion_prev + diffusion_curr) * fbm_increment
    ...
    def _generate_fbm_increments_jax(self, n_steps: int, hurst: float, dt: float) -> jnp.ndarray:
        """Generate fBm increments using JAX."""
...
def solve_sde(drift_func: Callable,
              diffusion_func: Callable,
              x0: np.ndarray,
              t_span: np.ndarray,
              hurst: float = 0.5,
