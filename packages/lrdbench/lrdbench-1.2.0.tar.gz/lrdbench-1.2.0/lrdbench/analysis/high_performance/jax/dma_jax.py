...
class DMAEstimatorJAX(BaseEstimator):
    ...
    def __init__(self, min_window_size: int = 4, max_window_size: int = None,
                 window_sizes: List[int] = None, overlap: bool = True, use_gpu: bool = False):
        super().__init__(
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        valid_mask = np.isfinite(fluctuation_values_np) & (fluctuation_values_np > 0) & np.isfinite(window_sizes_np) & (window_sizes_np > 1)
    ...
    def _calculate_fluctuations_batch(self, data: jnp.ndarray, window_sizes: List[int]) -> List[float]:
        """
    ...
    def _calculate_fluctuation_jax(self, data: jnp.ndarray, window_size: int) -> jnp.ndarray:
        """
        ...
        if self.parameters['overlap']:
            # Overlapping windows
            moving_avg = self._calculate_overlapping_moving_average_jax(cumsum, window_size)
        else:
            # Non-overlapping windows
            moving_avg = self._calculate_non_overlapping_moving_average_jax(cumsum, window_size)
    ...
    def _calculate_overlapping_moving_average_jax(self, cumsum: jnp.ndarray, window_size: int) -> jnp.ndarray:
        """
    ...
    def _calculate_non_overlapping_moving_average_jax(self, cumsum: jnp.ndarray, window_size: int) -> jnp.ndarray:
        """
