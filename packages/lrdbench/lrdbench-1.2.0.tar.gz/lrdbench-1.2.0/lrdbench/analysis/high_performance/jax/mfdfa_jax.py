...
class MFDFAEstimatorJAX(BaseEstimator):
    ...
    def _detrend_series_jax(self, series: jnp.ndarray, scale: int, order: int) -> jnp.ndarray:
        """
    ...
    def _compute_fluctuation_function_jax(self, data: jnp.ndarray, q: float, scale: int) -> float:
        """
        ...
        for i in range(n_segments):
            segment = segments[i]
            detrended = self._detrend_series_jax(segment, scale, self.parameters['order'])
