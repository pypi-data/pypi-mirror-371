...
def _calculate_overlapping_moving_average_numba(cumsum: np.ndarray, window_size: int) -> np.ndarray:
    """
...
@jit(nopython=True, cache=True)
def _calculate_non_overlapping_moving_average_numba(cumsum: np.ndarray, window_size: int) -> np.ndarray:
    """
...
@jit(nopython=True, cache=True)
def _calculate_fluctuation_numba(data: np.ndarray, window_size: int, overlap: bool) -> float:
    """
    ...
    if overlap:
        ...
        moving_avg = _calculate_non_overlapping_moving_average_numba(cumsum, window_size)
...
@jit(nopython=True, cache=True)
def _calculate_fluctuations_batch_numba(data: np.ndarray, window_sizes: np.ndarray, overlap: bool) -> np.ndarray:
    """
    ...
    for i in prange(n_windows):
        fluctuation_values[i] = _calculate_fluctuation_numba(data, window_sizes[i], overlap)
...
class DMAEstimatorNumba(BaseEstimator):
    ...
    def __init__(self, min_window_size: int = 4, max_window_size: int = None,
                 window_sizes: List[int] = None, overlap: bool = True):
        super().__init__(
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.parameters['window_sizes'] is not None:
            ...
            if not all(size1 < size2 for size1, size2 in zip(
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        valid_mask = np.isfinite(fluctuation_values) & (fluctuation_values > 0) & np.isfinite(window_sizes_np) & (window_sizes_np > 1)
        valid_sizes = window_sizes_np[valid_mask]
        valid_fluct = fluctuation_values[valid_mask]

        if valid_sizes.size < 3:
            raise ValueError("Insufficient valid fluctuation points for DMA (need >=3 after filtering non-positive values)")
        ...
        # Linear regression
        slope, intercept, r_value, p_value, std_err = stats.linregress(log_sizes, log_fluctuations)
    ...
    def get_confidence_intervals(self, confidence_level: float = 0.95) -> Dict[str, Tuple[float, float]]:
        ...
        return {
            'hurst_parameter': (ci_lower, ci_upper)
        }
    ...
    def plot_scaling(self, save_path: str = None) -> None:
        ...
        ax1.scatter(log_sizes, log_fluctuations, color='blue', alpha=0.7, label='Data points')
        ...
        ax2.scatter(window_sizes, fluctuation_values, color='green', alpha=0.7, label='Data points')
        
        # Plot fitted curve
        x_fit_linear = np.linspace(min(window_sizes), max(window_sizes), 100)
        y_fit_linear = np.exp(self.results['intercept']) * (x_fit_linear ** H)
        ax2.plot(x_fit_linear, y_fit_linear, 'r--', linewidth=2, 
