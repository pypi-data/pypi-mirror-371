...
class ARFIMAModel(BaseModel):
    ...
    def __init__(self, d: float, ar_params: Optional[List[float]] = None, 
        ...
        super().__init__(d=d, ar_params=ar_params, ma_params=ma_params, 
    ...
    def _validate_parameters(self) -> None:
        ...
        if not -0.5 < d < 0.5:
            raise ValueError("Fractional integration parameter d must be in (-0.5, 0.5)")
        ...
        if ar_params:
            ...
            roots = ar_poly.roots
            if np.any(np.abs(roots) >= 1 - 1e-10):  # Roots must be inside unit circle for stationarity
        ...
        if ma_params:
            ...
            roots = ma_poly.roots
            if np.any(np.abs(roots) >= 1 - 1e-10):  # Roots must be inside unit circle for invertibility
    ...
    def _spectral_method(self, n: int, d: float, ar_params: List[float], 
                        ma_params: List[float], sigma: float) -> np.ndarray:
        """
        ...
        spectral_density = self._compute_spectral_density(freqs, d, ar_params, ma_params, sigma)
    ...
    def _simulation_method(self, n: int, d: float, ar_params: List[float], 
                          ma_params: List[float], sigma: float) -> np.ndarray:
        """
    ...
    def _apply_ar_filter_efficient(self, data: np.ndarray, ar_params: List[float]) -> np.ndarray:
        """
    ...
    def _apply_ma_filter_efficient(self, data: np.ndarray, ma_params: List[float]) -> np.ndarray:
        """
    ...
    def _compute_spectral_density(self, freqs: np.ndarray, d: float, 
                                ar_params: List[float], ma_params: List[float], 
