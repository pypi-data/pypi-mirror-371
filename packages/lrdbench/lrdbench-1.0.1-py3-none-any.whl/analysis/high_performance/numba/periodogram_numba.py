...
@jit(nopython=True, cache=True)
def _compute_welch_psd_numba(data: np.ndarray, nperseg: int, window_type: int) -> Tuple[np.ndarray, np.ndarray]:
    """
...
@jit(nopython=True, cache=True)
def _fit_power_law_numba(log_freqs: np.ndarray, log_psd: np.ndarray) -> Tuple[float, float, float]:
    """
...
class PeriodogramEstimatorNumba(BaseEstimator):
    ...
    def __init__(self, min_freq_ratio: float = 0.01, max_freq_ratio: float = 0.1,
                 use_welch: bool = True, window: str = 'hann', nperseg: int = None):
        super().__init__(
    ...
    def get_estimation_quality(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'std_error': np.sqrt((1 - self.results['r_squared']) / (self.results['m'] - 2)),
