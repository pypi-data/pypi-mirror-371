...
@jit(nopython=True, cache=True)
def _calculate_rs_values_batch_numba(data: np.ndarray, window_sizes: np.ndarray) -> np.ndarray:
    """
...
class RSEstimatorNumba(BaseEstimator):
    ...
    def __init__(self, min_window_size: int = 10, max_window_size: int = None, 
                 window_sizes: List[int] = None, overlap: bool = False):
        super().__init__(
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.parameters['window_sizes'] is not None:
            ...
            if not all(self.parameters['window_sizes'][i] < self.parameters['window_sizes'][i+1] 
            ...
            if (self.parameters['max_window_size'] is not None and 
                self.parameters['max_window_size'] <= self.parameters['min_window_size']):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        if self.parameters['window_sizes'] is not None:
            ...
            window_sizes = np.logspace(
                ...
                np.log10(max_size), 20, dtype=int
            )
    ...
    def plot_scaling(self, save_path: str = None) -> None:
        ...
        ax2.scatter(window_sizes, rs_values, color='green', alpha=0.7, label='Data points')
