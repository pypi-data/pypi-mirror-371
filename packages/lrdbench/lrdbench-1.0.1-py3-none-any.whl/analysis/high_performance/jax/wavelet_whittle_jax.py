...
class WaveletWhittleEstimatorJAX(BaseEstimator):
    ...
    def _theoretical_spectrum_jax(self, frequencies: jnp.ndarray, H: float, sigma: float = 1.0) -> jnp.ndarray:
        """
        ...
        spectrum = jnp.where(
            frequencies != 0,
            sigma**2 * jnp.abs(frequencies)**(1 - 2*H),
            sigma**2
        )
        
        return spectrum
    
    def _whittle_likelihood_jax(self, H: float, coeffs: jnp.ndarray, scale: int) -> float:
        """
