...
class ContaminationModel:
    ...
    def add_trend_linear(self, data: np.ndarray, slope: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_trend_polynomial(self, data: np.ndarray, degree: Optional[int] = None) -> np.ndarray:
        """
    ...
    def add_trend_exponential(self, data: np.ndarray, rate: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_trend_seasonal(self, data: np.ndarray, period: Optional[int] = None, 
    ...
    def add_artifact_spikes(self, data: np.ndarray, probability: Optional[float] = None,
                           amplitude: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_artifact_level_shifts(self, data: np.ndarray, probability: Optional[float] = None,
                                 amplitude: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_artifact_missing_data(self, data: np.ndarray, probability: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_noise_gaussian(self, data: np.ndarray, std: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_noise_colored(self, data: np.ndarray, power: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_noise_impulsive(self, data: np.ndarray, probability: Optional[float] = None,
                           amplitude: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_sampling_irregular(self, data: np.ndarray, probability: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_sampling_aliasing(self, data: np.ndarray, frequency: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_measurement_systematic(self, data: np.ndarray, bias: Optional[float] = None) -> np.ndarray:
        """
    ...
    def add_measurement_random(self, data: np.ndarray, std: Optional[float] = None) -> np.ndarray:
        """
    ...
    def apply_contamination(self, data: np.ndarray, 
        ...
        **kwargs : dict
        ...
        for cont_type in contamination_types:
            if cont_type == ContaminationType.TREND_LINEAR:
                ...
                contaminated = self.add_trend_seasonal(contaminated, period=period, amplitude=amplitude)
            elif cont_type == ContaminationType.ARTIFACT_SPIKES:
                probability = kwargs.get('probability', None)
                amplitude = kwargs.get('amplitude', None)
                contaminated = self.add_artifact_spikes(contaminated, probability=probability, amplitude=amplitude)
            elif cont_type == ContaminationType.ARTIFACT_LEVEL_SHIFTS:
                probability = kwargs.get('probability', None)
                amplitude = kwargs.get('amplitude', None)
                contaminated = self.add_artifact_level_shifts(contaminated, probability=probability, amplitude=amplitude)
            elif cont_type == ContaminationType.ARTIFACT_MISSING_DATA:
                probability = kwargs.get('probability', None)
                contaminated = self.add_artifact_missing_data(contaminated, probability=probability)
                ...
                probability = kwargs.get('probability', None)
                amplitude = kwargs.get('amplitude', None)
                contaminated = self.add_noise_impulsive(contaminated, probability=probability, amplitude=amplitude)
            elif cont_type == ContaminationType.SAMPLING_IRREGULAR:
                probability = kwargs.get('probability', None)
                contaminated = self.add_sampling_irregular(contaminated, probability=probability)
            elif cont_type == ContaminationType.SAMPLING_ALIASING:
                frequency = kwargs.get('frequency', None)
                contaminated = self.add_sampling_aliasing(contaminated, frequency=frequency)
    ...
    def get_contamination_info(self, contamination_types: List[ContaminationType]) -> Dict[str, str]:
        """
        ...
        for cont_type in contamination_types:
            if cont_type.value.startswith('trend_'):
                info[cont_type.value] = f"Added {cont_type.value.replace('_', ' ')} trend"
            elif cont_type.value.startswith('artifact_'):
                info[cont_type.value] = f"Added {cont_type.value.replace('_', ' ')} artifacts"
            elif cont_type.value.startswith('noise_'):
                info[cont_type.value] = f"Added {cont_type.value.replace('_', ' ')} noise"
            elif cont_type.value.startswith('sampling_'):
                info[cont_type.value] = f"Applied {cont_type.value.replace('_', ' ')} effects"
            elif cont_type.value.startswith('measurement_'):
                info[cont_type.value] = f"Added {cont_type.value.replace('_', ' ')} errors"
