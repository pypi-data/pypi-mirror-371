...
class MultifractalRandomWalk(BaseModel):
    ...
    def _cascade_method(self, n: int, H: float, lambda_param: float, sigma: float) -> np.ndarray:
        """
    ...
    def _generate_fbm(self, n: int, H: float, sigma: float) -> np.ndarray:
        ...
        lags = np.arange(n)
        autocov = sigma**2 * 0.5 * (
            (lags + 1)**(2*H) - 2 * lags**(2*H) + np.maximum(0, lags - 1)**(2*H)
        )
        ...
        noise = np.random.normal(0, 1, len(eigenvalues)) + 1j * np.random.normal(0, 1, len(eigenvalues))
    ...
    def _direct_method(self, n: int, H: float, lambda_param: float, sigma: float) -> np.ndarray:
        """
