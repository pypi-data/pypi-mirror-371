...
class TorchfSDENet(BaseNeuralFSDE, nn.Module if TORCH_AVAILABLE else object):
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        super().__init__(state_dim, hidden_dim, hurst_parameter, framework='torch', **kwargs)
        ...
        self.hurst_param = nn.Parameter(torch.tensor(hurst_parameter, dtype=torch.float32))
    ...
    def set_parameters(self, params: Dict[str, Any]):
        ...
        if 'hurst_parameter' in params:
            self.hurst_param.data = torch.tensor(params['hurst_parameter'], dtype=torch.float32)
    ...
    def load_model(self, filepath: str):
        ...
        self.hurst_param.data = torch.tensor(checkpoint['hurst_parameter'], dtype=torch.float32)


class TorchLatentFractionalNet(BaseNeuralFSDE, nn.Module if TORCH_AVAILABLE else object):
    """
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        super().__init__(latent_dim, hidden_dim, hurst_parameter, framework='torch', **kwargs)
    ...
    def simulate(self, 
        ...
        **kwargs
        ...
        for i in range(n_samples):
            z_torch = torch.tensor(z_trajectory[i], dtype=torch.float32, device=self.device)
...
def create_torch_fsde_net(state_dim: int,
                          hidden_dim: int,
                          num_layers: int = 3,
                          hurst_parameter: float = 0.7,
                          dropout: float = 0.0) -> TorchfSDENet:
    """
...
def create_torch_latent_fsde_net(obs_dim: int,
                                 latent_dim: int,
                                 hidden_dim: int,
                                 num_layers: int = 3,
                                 hurst_parameter: float = 0.7,
                                 dropout: float = 0.0) -> TorchLatentFractionalNet:
    """
