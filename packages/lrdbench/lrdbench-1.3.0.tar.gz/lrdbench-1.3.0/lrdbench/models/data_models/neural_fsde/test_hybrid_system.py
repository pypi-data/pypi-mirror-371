...
def test_model_creation():
    ...
    try:
        from models.data_models.neural_fsde import create_fsde_net, create_latent_fsde_net
...
def test_numerical_solvers():
    ...
    try:
        ...
        solution, _ = solve_sde(
            drift_func, diffusion_func, x0, t_span, 
            hurst=0.5, method='euler'
        )
    ...
    try:
        ...
        for framework, result in results['frameworks'].items():
            if result['status'] == 'success':
                ...
                print(f"    {framework}: {time_info['mean']:.4f}s ± {time_info['std']:.4f}s")
            else:
                print(f"    {framework}: {result['status']} - {result.get('error', 'Unknown error')}")
