...
def _fgn_spectrum_numba(freqs: np.ndarray, hurst: float, scale: float = 1.0) -> np.ndarray:
    """
...
@jit(nopython=True, cache=True)
def _local_whittle_likelihood_numba(params: np.ndarray, freqs: np.ndarray, psd: np.ndarray) -> float:
    """
...
@jit(nopython=True, cache=True)
def _compute_welch_psd_numba(data: np.ndarray, window_type: int, nperseg: int) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if window_type == 0:  # Hann
        ...
        for i in range(nperseg):
            window[i] = 0.42 - 0.5 * np.cos(2 * np.pi * i / (nperseg - 1)) + 0.08 * np.cos(4 * np.pi * i / (nperseg - 1))
    else:  # Rectangular
        for i in range(nperseg):
            window[i] = 1.0
...
@jit(nopython=True, cache=True)
def _compute_periodogram_numba(data: np.ndarray, window_type: int) -> Tuple[np.ndarray, np.ndarray]:
    """
...
class WhittleEstimatorNumba(BaseEstimator):
    ...
    def __init__(self, min_freq_ratio=0.01, max_freq_ratio=0.1,
                 use_local_whittle=True, use_welch=True, 
    ...
    def _whittle_estimate_numba(self, freqs: np.ndarray, psd: np.ndarray) -> Tuple[float, float, float]:
        """
    ...
    def _fallback_estimate_numba(self, freqs: np.ndarray, psd: np.ndarray) -> Tuple[float, float, float]:
        """
    ...
    def get_confidence_intervals(self, confidence_level: float = 0.95) -> Dict[str, Tuple[float, float]]:
        ...
        return {
            'hurst_parameter': (float(ci_lower), float(ci_upper))
        }
    ...
    def plot_spectrum(self, save_path: str = None, show: bool = True) -> None:
        ...
        # Plot periodogram
        ax.scatter(self.results['frequency'], self.results['periodogram'], 
                  alpha=0.6, label='Periodogram', s=20)
        ...
        ax.plot(freqs, model_spectrum, 'r-', linewidth=2, 
                label=f'Model (H = {hurst:.3f})')
        
        ax.set_xlabel('Frequency')
        ax.set_ylabel('Power Spectral Density')
        ax.set_title(f'Whittle Spectrum (Numba Optimized)\nH = {hurst:.3f}, R² = {self.results["r_squared"]:.3f}')
        ax.legend()
