...
def _calculate_segment_length_numba(data: np.ndarray, m: int, k: int, n_m: int, n: int) -> float:
    """
...
@jit(nopython=True, cache=True)
def _calculate_curve_lengths_batch_numba(data: np.ndarray, k_values: np.ndarray) -> np.ndarray:
    """
...
class HiguchiEstimatorNumba(BaseEstimator):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        valid_mask = np.isfinite(curve_lengths) & (curve_lengths > 0) & np.isfinite(k_values_np) & (k_values_np > 1)
