...
class WaveletLogVarianceEstimator(BaseEstimator):
    ...
    def get_confidence_interval(self, confidence: Optional[float] = None) -> Tuple[float, float]:
        ...
        residuals = np.array(self.log_variance_values) - (
            ...
            np.mean(self.log_variance_values) - np.mean(self.scale_logs) * (2 * self.estimated_hurst - 1)
        )
    ...
    def plot_analysis(self, figsize: Tuple[int, int] = (12, 8)) -> None:
        ...
        ax1.semilogx(self.scales, list(self.log_variances.values()), 'bo-', 
                    label='Log Variances')
        ...
        ax2.scatter(self.scale_logs, self.log_variance_values, color='blue', 
                   label='Data Points')
        
        # Plot regression line
        x_line = np.array([min(self.scale_logs), max(self.scale_logs)])
        y_line = (2 * self.estimated_hurst - 1) * x_line + np.mean(self.log_variance_values) - \
                np.mean(self.scale_logs) * (2 * self.estimated_hurst - 1)
        ax2.plot(x_line, y_line, 'r--', 
                label=f'Regression (H={self.estimated_hurst:.3f})')
