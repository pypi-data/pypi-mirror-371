...
from ibm_watsonx_data_integration.common.exceptions import StageNotFoundError, UniqueStageNameNotFoundError
...
class PipelineDefinition(BaseModel):
    ...
    def __init__(self, flow: "StreamsetsFlow", **pipeline_definition_json: any) -> None:
        ...
        environment = next(
            environment
            for environment in self._flow._project.environments
            if environment.environment_id == self._flow.environment_id
        )
        engine = environment.engines[0]
        self._library_definitions: LibraryDefinitions = engine.library_definitions
...
class StreamsetsFlow(Flow):
    ...
    engine_version: str | None = Field(default="", alias="engine_version", repr=True)
    environment_id: str | None = Field(default="", alias="environment_id", repr=False)
    ...
    def _update(self, project: "Project", pipeline_definition: dict | None = None, **data: dict) -> None:
        ...
        """
        self.__init__(project=project, pipeline_definition=pipeline_definition, **data)
    ...
    def _pipeline_definition(self) -> PipelineDefinition:
        if self.__pipeline_definition_object is None:
            ...
            environment = next(
                environment
                for environment in self._project.environments
                if environment.environment_id == self.environment_id
            )
            engine = environment.engines[0]
            self.__pipeline_definition_object = PipelineDefinition(
                flow=self, **engine.api_client.get_pipeline_by_id(pipeline_id=self.get_executor_id()).json()
            )
