...
def evaluate_and_report(y_true: np.ndarray, y_pred: np.ndarray, class_names: List[str], out_dir: str) -> Dict:
    ...
    # Ensure all classes [0..K-1] are represented, even if absent in y_true/y_pred
    ...
    report = classification_report(
        y_true,
        y_pred,
        labels=labels_full,             # critical: include absent classes
        target_names=class_names,
        output_dict=True,
        zero_division=0
    )

    # A→B mapping, include zeros only for off-diagonals present
    mapping = []
    ...
    mapping.sort(key=lambda x: (-x["count"], x["true"], x["pred"]))
