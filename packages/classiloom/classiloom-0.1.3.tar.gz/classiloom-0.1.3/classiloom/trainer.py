...
def train_and_eval(dataset_dir: str, cfg: Dict[str, Any], counts: Dict[str, int], out_dir: str,
    ...
    hist = model.fit(
        ...
        verbose=verbose,              # per-epoch logs to stdout
        callbacks=callbacks,
        class_weight=cw
    )
    ...
    if fine_tune and base is not None and cfg.get("fine_tune_after", 0) > 0:
        ...
        try:
            model.compile(
                optimizer=tf.keras.optimizers.Adam(learning_rate=max(cfg["lr"]*0.1, 1e-5)),
            ...
            pass
        ft_cb = [
            tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=4, restore_best_weights=True, verbose=verbose),
            tf.keras.callbacks.ModelCheckpoint(os.path.join(out_dir, "model.ft.best.keras"), monitor="val_accuracy", save_best_only=True, verbose=verbose),
        ]
        model.fit(
            train,
            validation_data=val,
            epochs=min(10, max(2, int(cfg["epochs"]*0.3))),
            verbose=verbose,
            callbacks=ft_cb,
            class_weight=cw
        )
