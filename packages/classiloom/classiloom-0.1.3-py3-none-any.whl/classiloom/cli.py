...
def train(dataset_dir: str,
    ...
    rprint({"run_dir": run_dir, "backbone": cfg["backbone"], "img_size": cfg["img_size"], "batch": cfg["batch"], "epochs": cfg["epochs"]})

    model, history, class_names, y_true, y_pred = train_and_eval(
        dataset_dir, cfg, counts, out_dir=run_dir, mixed_precision=mixed_precision, fine_tune=fine_tune, verbose=1 if log_verbose else 0
    )
