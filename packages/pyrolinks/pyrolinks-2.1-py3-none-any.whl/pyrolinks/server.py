...
def create_app(pyro_client, route="/dl"):
    ...
    async def download_handler(request):
        ...
        start, end = byte_range if byte_range else (0, s - 1)
        ...
        if byte_range:
            headers["Content-Range"] = f"bytes {start}-{end}/{s}"

        resp = web.StreamResponse(status=206 if byte_range else 200, headers=headers)
...
async def run_server(pyro_client, host="0.0.0.0", port=8080, route="/dl"):
    app = create_app(pyro_client, route)
