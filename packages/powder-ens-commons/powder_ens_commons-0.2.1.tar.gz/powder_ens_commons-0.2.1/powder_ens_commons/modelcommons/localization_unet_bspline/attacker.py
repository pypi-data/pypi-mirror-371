...
def get_eta_vec_from_grad(x_vecs, eta, device=None):
    ...
    #cause it has this shape of (samples_in_each_batch X sensors in each sample X number_of_features)
    batch_size = eta.shape[0]
    #number of the samples in each batch
    eta_imgs = eta / abs(eta).amax(dim=(1,2,3)).view((-1,1,1,1))
    #its basically normalizing each the gradient values for each sample
    #the dimension of eta is (number_of_sample_in_a_batch X 3 channels in each samplej X height X width)
    coords = x_vecs[:,:,1:3].round().long()
    #taking all the coordinates
    #It takes the second and third columns of x_vecs (index 1 and 2), 
    #rounds them to the nearest integer, and converts them to long (integer) data type.
    #but now the confusion is : when we changed this coordinates from actual value to relative values of pixels?
    num_sensors = coords.shape[1]
    #number of sensors in each sample
    ...
    #for each sample-> invoke the image number of sensor times
    #each time invoke only the first layer
    #fetch the respective (x,y) pixel value
    #then reshape to num_sample_in_a_batch X num_sonsors array
    eta_sensors *= (x_vecs[:,:,0] != 0)
    #only keep those gradiants for those sensors that are having non-zero rss values
    if device is not None:
        return eta_sensors.to(device), eta_imgs.to(device)
    else:
        return eta_sensors, eta_imgs
    #it is returning both. the gradient with respect to x_vecs and also the normalized raw gradient

def get_eta_vec_from_grad_spec_sensor(sensor_to_perturb, x_vecs, eta, device=None):
    assert len(x_vecs.shape) == 3
    #cause it has this shape of (samples_in_each_batch X sensors in each sample X number_of_features)
    batch_size = eta.shape[0]
    #number of the samples in each batch
    eta_imgs = eta / abs(eta).amax(dim=(1,2,3)).view((-1,1,1,1))
    #its basically normalizing each the gradient values for each sample
    #the dimension of eta is (number_of_sample_in_a_batch X 3 channels in each sample X height X width)
    coords = x_vecs[:,:,1:3].round().long()
    #taking all the coordinates
    #It takes the second and third columns of x_vecs (index 1 and 2), 
    #rounds them to the nearest integer, and converts them to long (integer) data type.
    ...
    # Set eta_sensors to zero for the sensors in sensors_to_perturb
    mask = torch.isin(x_vecs[:, :, 3], sensor_to_perturb)  # Create a mask for the sensor ids to perturb
    #this is creating a true-false array of shape batch_size X sensors per sample
    eta_sensors[~mask] = 0.0000000001 #not making it exact zero to avoid some error

    # Set eta_imgs values (in the first layer) to zero for the corresponding sensor positions
    eta_imgs[torch.arange(batch_size).repeat_interleave(num_sensors),
             1,  # Second layer (channel 1)
             coords[:, :, 1].flatten(),
             coords[:, :, 0].flatten()][~mask.flatten()] = 0.0000000001  
...
def get_random_attack_vec_spec_sensor(sensors_to_perturb, x_vecs, grad, rss_low, rss_high, epsilon=0.5):
    #the grad function actually comes with gradient value for each pixels of the input images. 
    #it comes with x_vecs containing [rss, x_coord, y_coord, sensor_number, sensor_category]
    #grad: containing gradient value for each pixel of all the images for all three layers
    #rss_low is the 10th percent of all rss values and rss_high is the 90th percent of rss values
    #epsilon says the max tolerance level
    eta_vecs, eta_imgs = get_eta_vec_from_grad_spec_sensor(sensors_to_perturb, x_vecs, grad)
    #this function is responsible for coming up with gradient for each element of the given vector from 
    #the gradiants of the images of all the pixels belong to each channel
    ##it is returning both. the gradient with respect to x_vecs and also the normalized raw gradient
    rand_select = np.random.randint(9)
    #use any of these 10 methods
    rand_epsilon = np.random.uniform(0,epsilon)
    #taking a random tolerance level withing the max range
    hilo_epsilon = np.random.uniform(0.3,0.7)
    #used for mostly high_low attacks
    if rand_select == 0:
...
# to conclude, the function basically selects a random vector and then utilizes it for doint random perturbation
#from the gradient of the values

def get_random_attack_vec_spec_sensor_all_epsilon(sensors_to_perturb, x_vecs, grad, rss_low, rss_high, epsilon=0.5):
    #the grad function actually comes with gradient value for each pixels of the input images. 
    #it comes with x_vecs containing [rss, x_coord, y_coord, sensor_number, sensor_category]
    #grad: containing gradient value for each pixel of all the images for all three layers
    #rss_low is the 10th percent of all rss values and rss_high is the 90th percent of rss values
    ...
    #the gradiants of the images of all the pixels belong to each channel
    ##it is returning both. the gradient with respect to x_vecs and also the normalized raw gradient
    rand_select = np.random.randint(2)
    #use any of these 10 methods
    # rand_epsilon = np.random.uniform(0,epsilon)
    rand_epsilon = epsilon
    #taking a random tolerance level withing the max range
    hilo_epsilon = np.random.uniform(0.3,0.7)
    #used for mostly high_low attacks
    if rand_select == 0:
...
def worst_case_attack_spec_sensor(x_vecs, y_vecs, dlloc, img_shape, return_many_preds=False, num_return=20):
    num_images = len(x_vecs)
    preds = torch.zeros((2, len(x_vecs), 2)).to(dlloc.device)
    many_preds = [[], []]
    for ind, x_vec in enumerate(x_vecs):
...
def top_percent_attack_vec(x_vecs, eta_vecs, epsilon, percent, sign=True):
    #comes with parameters: x_vecs, eta_vecs, rand_epsilon, 1
    # or, x_vecs, eta_vecs, rand_epsilon, np.random.random()/2
    adv_x = x_vecs.clone()
    ...
    return adv_x
#top_percent_attack_vec() simply select to k-th percentile value
#then any eta_value means gradient more than that are kept for perturbation.
...
def high_low_eta_attack_vec(eta_imgs, x_vecs, epsilon, num_high=1, num_low=1, sign=False):
    #comes with parameters such as: eta_imgs, x_vecs, hilo_epsilon, np.random.randint(1,6), np.random.randint(1,6)
    #eta_imgs actually carries normalized (probably absolute) grad values for each pixel of the image
    assert len(eta_imgs.shape) == 4
    assert len(x_vecs.shape) == 3
    eta_imgs = eta_imgs[:,1].clone()
    #extract second channel gradiants only
    batch_size = eta_imgs.shape[0]
    #number of samples
    new_eta = torch.zeros_like(x_vecs)
    #empty tensor of same shape of the input array
    high_values, high_inds = eta_imgs.view(batch_size, -1).topk(num_high)
    #convert the gradient array to (batch size, channels*height*width)
    #take top k values and respective indices
    ...
    #convert the high and low indices back to an image
    for ind in range(batch_size):
...
def get_random_attack_vec(x_vecs, grad, rss_low, rss_high, epsilon=0.5):
    ...
    # copied_grad = eta_vecs.detach().clone()
    # print("x_vecs shape = ", x_vecs.shape)
    # print("eta_vecs shape = ", eta_vecs.shape)
    # print("Shape of X vecs = ", x_vecs.shape)
    # print("Sample X value = ",x_vecs[0][10])
    # print("Type = ", type(x_vecs))
    # print("Shape of Gradient Vecs = ", eta_vecs.shape)
    # print("Sample grad value = ", eta_vecs[0])
    # print("Type = ", type(eta_vecs))
    # print("Shape of eta_imgs = ", eta_imgs.shape)
    # print("Type = ", type(eta_imgs))
    #this function is responsible for coming up with gradient for each element of the given vector from 
    #the gradiants of the images of all the pixels belong to each channel
    ##it is returning both. the gradient with respect to x_vecs and also the normalized raw gradient
    rand_select = np.random.randint(9)
...
def get_random_attack_vec_spec_attack(x_vecs, grad, rss_low, rss_high, attack_id, epsilon=0.5):
    #the grad function actually comes with gradient value for each pixels of the input images. 
    #it comes with x_vecs containing [rss, x_coord, y_coord, sensor_number, sensor_category]
    #grad: containing gradient value for each pixel of all the images for all three layers
    #rss_low is the 10th percent of all rss values and rss_high is the 90th percent of rss values
