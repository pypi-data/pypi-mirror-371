...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupSSE() {
    if (retryCount >= MAX_RETRY_ATTEMPTS) {
      console.log('Maximum retry attempts reached. Hot reload disabled.');
      return;
    }
    ...
    eventSource.onerror = (event) => {
      retryCount++;
      console.log(`SSE connection failed (attempt ${retryCount}/${MAX_RETRY_ATTEMPTS})`);
      eventSource.close();
      ...
      if (retryCount < MAX_RETRY_ATTEMPTS) {
        ...
        console.log('Hot reload connection failed permanently. Please restart the server.');
      }
    };
  }
