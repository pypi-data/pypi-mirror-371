...
from .pool_pomes import pool_acquire, pool_release
...
def db_assert_access(errors: list[str] | None,
    ...
    engine = _assert_engine(errors=op_errors,
                            engine=engine)
    if engine:
...
def db_get_param(key: DbParam,
    ...
    """
    # assert the database engine
    engine = _DB_ENGINES[0] if not engine and _DB_ENGINES else engine
...
def db_connect(errors: list[str] | None,
    ...
    if engine:
        result: Any = pool_acquire(engine=engine,
                                   logger=logger)
    if not result:
...
def db_close(errors: list[str] | None,
             connection: Any,
             logger: Logger = None) -> None:
    """
    ...
    if not pool_release(conn=connection,
                        logger=logger):
        ...
        try:
            ...
            err_msg: str = (f"Error closing the connection '{connection}': "
                            f"{str_sanitize(source=f'{e}')}")
            if logger:
                logger.error(msg=err_msg)
            if isinstance(errors, list):
...
def db_select(errors: list[str] | None,
    ...
    if engine:
        connection = connection or db_connect(errors=errors,
                                              engine=engine,
                                              logger=logger)
    if engine == DbEngine.MYSQL:
        pass
