...
for _db_engine in _DB_ENGINES:
    ...
    _POOL_DATA[_db_engine] = {
        PoolParam.SIZE: env_get_int(key=f"{APP_PREFIX}_{_prefix}_POOL_SIZE",
                                    def_value=10),
        PoolParam.OVERFLOW: env_get_int(key=f"{APP_PREFIX}_{_prefix}_POOL_OVERFLOW",
                                        def_value=5),
        PoolParam.TIMEOUT: env_get_int(key=f"{APP_PREFIX}_{_prefix}_TIMEOUT",
                                       def_value=60),
        PoolParam.RECYCLE: env_get_int(key=f"{APP_PREFIX}_{_prefix}_RECYCLE",
                                       def_value=3600)
    }
...
class DbConnectionPool:
    ...
    such as the *SQLAlchemy* pool package. These are the configuration parameters:
      ...
      - *pool_overflow*: maximum number of additional connections allowed, for pool exaustion handling
    ...
    *db_pomes.py* will attempt to obtain a connection from this pool on *db_connect()*, and to return it
    ...
    def __init__(self,
        ...
        uppercase names (nsmely, *POOL_SIZE*, *POOL_OVERFLOW*, *POOL_TIMEOUT*, and *POOL_RECYCLE*) are used.
        If still not specified, these are the default values used:
          - *pool_size*: 10 connections
          - *pool_overflow*: 5 connections
        ...
        :param pool_overflow: number of additional connections beyond *pool_size*, when demand spikes
        ...
        if pool_data:
            ...
            self.pool_overflow: int = pool_overflow or pool_data[PoolParam.OVERFLOW]
            ...
            if logger:
                logger.debug(msg=f"{self.db_engine} pool created: "
                                 f"size {self.pool_size}, overflow {self.pool_overflow}, "
            ...
            with self.stage_lock:
                ...
                for conn_item in self.conn_data:
                    if conn_item[ConnStage.AVAILABLE]:
                        ...
                        break
                if not result and len(self.conn_data) < self.pool_size + self.pool_overflow:
