...
def parallel_generations(
    ...
    if n_tasks <= len(intermediate_generations):
        print("Intermediate generations covered everything, no further generations are needed.")
        return intermediate_generations[:n_tasks]
    
    n_tasks -= len(intermediate_generations)
    ...
    if args.instruction_tokens:
        instruction_tokens = args.instruction_tokens.replace('\\n', '\n').split(",")
    ...
    elif args.apply_default_chat_template:
        instruction_tokens = None, tokenizer.eos_token, None
    else:
        instruction_tokens = None

    if accelerator.is_main_process:
        print(
            f"Number of problems for this task is {n_tasks + curr_sample_idx}. Starting at task {curr_sample_idx}"
        )
    ...
    ds_tokenized = TokenizedDataset(
        ...
        max_length=args.max_length_generation if args.max_new_tokens is None else args.max_length_generation - args.max_new_tokens ,
        ...
        apply_default_chat_template = args.apply_default_chat_template
    )
