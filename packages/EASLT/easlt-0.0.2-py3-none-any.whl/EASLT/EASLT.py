...
class AutomaticShiftedLogTransformer(BaseEstimator, TransformerMixin):
    ...
    def _reverse_transform(self, x_transformed, beta, global_shift, max_val=None):
        """Enhanced inverse transformation using base-10 exponential"""
        try:
            if abs(beta) < self.epsilon:
                # For beta ≈ 0: log10(x_shifted + 1) -> 10^x - 1
                result = 10**x_transformed - 1.0
            elif beta > 0:
                # For beta > 0: log10(x_shifted + beta) -> 10^x - beta
                result = 10**x_transformed - beta
            else:
                # For beta < 0: log10(max_val - x_shifted + |beta| + 1) -> max_val - 10^x + |beta| + 1
                if max_val is None:
                    # Fallback when max_val is not available
                    result = 10**x_transformed - abs(beta) - 1.0
                else:
                    result = max_val - 10**x_transformed + abs(beta) + 1.0
            
            # Remove the global shift that was added during forward transformation
