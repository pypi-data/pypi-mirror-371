...
class TimetableQuery:
    ...
    def check_response(self, response: Union[str, bytes], em: ErrorMessenger, label=""):
        if self.saveraw is not None and os.path.isdir(self.saveraw) and len(label) > 0:
            try:
                with open(
                    os.path.join(self.saveraw, str.lower("{}.html".format(label))),
                    "w",
                    encoding="utf-8",
                ) as f:
                    f.write(response)
...
def search_tag(
    tag, tag_name, attribute, string, matcher=(lambda x, y: re.search(x, y) is not None)
):
    try:
        if re.compile(tag_name, re.I).match(tag.name):
            return tag.has_attr(attribute) and matcher(string, tag[attribute])
    except Exception:
        return False
...
def course_tag_is_section(x):
    return search_tag(
        x, "div", "id", "win0divSSR_CLSRSLT_WRK_GROUPBOX", lambda x, y: y.startswith(x)
    )


def section_tag_is_classname(x):
    return search_tag(x, "a", "id", "MTG_CLASSNAME")
...
def extract_section(section, descr, log=False, err_msg_prefix=""):
    ...
    instrs = [
        x.strip()
        for x in section(lambda x: search_tag(x, "span", "id", "MTG_INSTR"))[0].contents
        if isinstance(x, str)
    ]
    topic = [
        x.strip()
        for x in section(lambda x: search_tag(x, "span", "id", "MTG_TOPIC"))[0].contents
        if isinstance(x, str)
    ]
...
def extract_course(course, year, term, log=False):
    ...
    for section in course(course_tag_is_section):
        ...
        if subsection is None:
            continue
        descr = section.find(
            lambda x: search_tag(x, "div", "id", "win0divDERIVED_CLSRCH_DESCRLONG")
        )
...
def distribute_shared_sections(
    ...
    for section in sections:
        ...
        for bad_section in comp_secs[1]:
            ...
            #
            if (
                arr_lookup(section) == 1
                and sec_comps[bad_section["label"]][0] == sec_comps[section["label"]][0]
            ):
                continue
