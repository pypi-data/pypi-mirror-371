...
def hot_add(pipeline: Gst.Pipeline, *elements: Gst.Element) -> bool:
    ...
    for element in elements:
        if element.get_parent():
            raise RuntimeError(f"Element {element.get_name() or element} already has a parent")
        pipeline.add(element)
        if element.get_parent() is not pipeline:
            raise RuntimeError(f"Failed to add {element.get_name() or element} to pipeline")
...
def reconnect(src_element: Gst.Element, sink_element: Gst.Element, 
              src_pad: str = "src", sink_pad: str = "sink") -> bool:
    ...
    if not src_pad_obj:
        raise RuntimeError(f"Source pad '{src_pad}' not found on {src_element.get_name() or src_element}")
    
    sink_pad_obj = sink_element.get_static_pad(sink_pad)
    if not sink_pad_obj:
        raise RuntimeError(f"Sink pad '{sink_pad}' not found on {sink_element.get_name() or sink_element}")
...
def replace_element(pipeline: Gst.Pipeline, old_element: Gst.Element, 
                   new_element: Gst.Element) -> bool:
    ...
    while True:
        ...
        if result == Gst.IteratorResult.OK:
            peer = pad.get_peer()
            if peer:
                src_connections.append((pad.get_name(), peer.get_parent_element(), peer.get_name()))
        elif result == Gst.IteratorResult.DONE:
            break
        elif result == Gst.IteratorResult.ERROR:
            return False
    ...
    while True:
        ...
        if result == Gst.IteratorResult.OK:
            peer = pad.get_peer()
            if peer:
                sink_connections.append((peer.get_parent_element(), peer.get_name(), pad.get_name()))
        elif result == Gst.IteratorResult.DONE:
            break
        elif result == Gst.IteratorResult.ERROR:
            return False
...
def insert_element(pipeline: Gst.Pipeline, element: Gst.Element,
                  after: Gst.Element, before: Gst.Element,
                  after_pad: str = "src", before_pad: str = "sink",
                  element_src: str = "src", element_sink: str = "sink") -> bool:
...
def branch_insert(tee: Gst.Element, *branch_elements: Gst.Element) -> Gst.Element | None:
