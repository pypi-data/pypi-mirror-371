...
class HTTP:
    ...
    def SetHeaders(headers: dict = None) -> None:
        ...
        '''
    @staticmethod
    def Get(url: str, form: str | dict | None = None, delayTime: int = 60000, maxRetries: int = 3, backoffFactor: int | float = 0.5, raiseForStatus: bool = True) -> str:
        '''
        Get获取数据

        text = HTTP.Get("", form=None, delayTime=60000, maxRetries=3, backoffFactor=0.5, raiseForStatus=True)
        ...
        :param backoffFactor:[可选参数] 重试退避因子，使用指数退避算法避免请求风暴。默认0.5秒
        :param raiseForStatus:[可选参数] 是否抛出HTTP错误，状态码不是200就抛出异常。默认True
        :return:获取的网络数据的结果
        '''
    @staticmethod
    def Post(url: str, form: dict | list[tuple] | str | bytes | None = None, delayTime: int = 60000, maxRetries: int = 3, backoffFactor: int | float = 0.5, raiseForStatus: bool = True) -> str:
        '''
        Post提交表单

        text = HTTP.Post("", form=None, delayTime=60000, maxRetries=3, backoffFactor=0.5, raiseForStatus=True)
        ...
        '''
    @staticmethod
    def PostJson(url: str, form: dict | None = None, delayTime: int = 60000, maxRetries: int = 3, backoffFactor: int | float = 0.5, raiseForStatus: bool = True) -> str:
        '''
        Post提交JSON表单

        text = HTTP.PostJson("", form=None, delayTime=60000, maxRetries=3, backoffFactor=0.5, raiseForStatus=True)

        :param url:[必选参数] Post链接地址
        :param form:[可选参数] Post时传递的表单数据，字典类型。默认None
        :param delayTime:[可选参数] 超时时间，单位毫秒，默认60000毫秒
        :param maxRetries:[可选参数] 最大重试次数，默认3次
        :param backoffFactor:[可选参数] 重试退避因子，使用指数退避算法避免请求风暴。默认0.5秒
        :param raiseForStatus:[可选参数] 是否抛出HTTP错误，状态码不是200就抛出异常。默认True
        ...
        '''
    @staticmethod
    def PostFile(url: str, files: dict[str, str | list[str]], form: dict | list[tuple] | str | bytes | None = None, delayTime: int = 60000, maxRetries: int = 3, backoffFactor: int | float = 0.5, raiseForStatus: bool = True) -> str:
        '''
        Post提交文件

        text = HTTP.PostFile("", files=None, form=None, delayTime=60000, maxRetries=3, backoffFactor=0.5, raiseForStatus=True)

        :param url:[必选参数] Post链接地址
        :param files:[必选参数] 上传的文件字典，key为文件字段名，value为文件路径或文件路径列表，例如：{\'files\':\'文件路径\'} 或 {\'files\':[\'文件1路径\', \'文件2路径\']} 或 {\'file1\':[\'文件1路径\', \'文件2路径\'], \'file2\':[\'文件3路径\', \'文件4路径\']}
        :param form:[可选参数] Post时传递的表单数据，可以是字典、元祖组成的列表、字符串或字节，默认None
        :param delayTime:[可选参数] 超时时间，单位毫秒，默认60000毫秒
        :param maxRetries:[可选参数] 最大重试次数，默认3次
        :param backoffFactor:[可选参数] 重试退避因子，使用指数退避算法避免请求风暴。默认0.5秒
...
class Log:
    @staticmethod
    def ModifyConf(level: int = 2, maxsize: int = 10, path: str = None) -> None:
        """
        修改日志配置

        Log.ModifyConf(level=2, maxsize=10, path=None)

        :param level:[可选参数] 整数型(0~3)，日志记录级别。0：只可记录错误信息；1：只可记录警告信息、错误信息；2：只可记录一般信息、警告信息、错误信息；3：可记录调试信息、一般信息、警告信息、错误信息。默认2
        :param maxsize:[可选参数] 整数型，日志文件超过该数值MB就建立新的日志，日志的文件名会增加“_序号”。默认10
...
class File:
    ...
    def Compress(srcfiles: str | list[str], dstpath: str, pwd: str = '', level: str = 'standard') -> str:
        ...
        sRet = File.Compress(srcfiles, dstpath, pwd=\'\', level=\'standard\')
...
def retry(num: int | None = 2, delay: int | float = None, wait: int | float = None):
    """
    重试装饰器

    @retry(num=2, delay=None, wait=None)
