...
def toMaxAge(expires):
    """Convert expires (datetime|int|str|None) to max_age (seconds or None)."""
    if expires is None:
        return None
    if isinstance(expires, numbers.Integral):
        return int(expires)
    if isinstance(expires, datetime.timedelta):
        return max(0, int(expires.total_seconds()))
    if isinstance(expires, datetime.datetime):
        now = datetime.datetime.now(expires.tzinfo) if expires.tzinfo else datetime.datetime.utcnow()
        delta = (expires - now).total_seconds()
        return max(0, int(delta))
    try:
        from p01.testbrowser import utils
        exp_ts = utils.http2time(expires)
        return max(0, int(exp_ts - time.time()))
    except Exception:
        return None


def setCookie(resp, name, value, expires=None, max_age=None, **kw):
    if max_age is None and expires is not None:
        max_age = toMaxAge(expires)
    return resp.set_cookie(name, value, max_age=max_age, **kw)
...
class Cookies(MutableMapping):
    ...
    def create(self, name, value, domain=None, expires=None, max_age=None,
        path=None, secure=None, comment=None, commenturl=None, port=None):
        if value is None:
        ...
        self._setCookie(name, value, domain, expires, path, secure, comment,
            commenturl, port, now=now, max_age=max_age)

    def change(self, name, value=None, domain=None, expires=None, max_age=None,
        path=None, secure=None, comment=None, commenturl=None, port=None):
        now = int(time.time())
        if expires is not None and self._is_expired(expires, now):
            # shortcut
            ...
            self._change(self._get(name), value=value, domain=domain,
                expires=expires, max_age=max_age, path=path, secure=secure,
                comment=comment, commenturl=commenturl, port=port, now=now)

    def _change(self, ck, value=None, domain=None, expires=None, max_age=None,
        path=None, secure=None, comment=None, commenturl=None, port=None,
        now=None):
        if value is None:
            value = ck.value
        ...
        if expires is None:
            expires = ck.expires
        if max_age is None:
            # CookieJar speichert kein max_age; aus expires ableiten (TTL in Sek.)
            if ck.expires is not None:
                now = now if now is not None else int(time.time())
                max_age = max(0, int(ck.expires - now))
            else:
                max_age = None
        if path is None:
            path = ck.path
        ...
        self._setCookie(ck.name, value, domain, expires, path, secure, comment,
            commenturl, port, ck.version, ck=ck, now=now, max_age=max_age)
    ...
    def _setCookie(self, name, value, domain, expires, path, secure, comment,
        commenturl, port, version=None, ck=None, now=None, max_age=None):
