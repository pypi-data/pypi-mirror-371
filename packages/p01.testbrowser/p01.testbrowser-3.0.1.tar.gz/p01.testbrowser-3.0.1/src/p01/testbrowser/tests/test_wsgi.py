...
from p01.testbrowser.tests.helper import demo_app
from p01.testbrowser._compat import urlencode
from p01.testbrowser._compat import url_quote


class SimpleLayer(p01.testbrowser.testing.Layer):
...
class TestBrowser(unittest.TestCase):
    ...
    def test_redirect_and_raiseHttpErrors(self):
        ...
        browser = p01.testbrowser.testing.Browser(wsgi_app=app)
    ...
    def test_redirect(self):
        ...
        self.assertRaises(p01.testbrowser.exceptions.HostNotAllowed,
                          browser.open,
...
##      self.assertEquals(browser.url, url)
    ...
    def test_allowed_domains(self):
        browser = p01.testbrowser.testing.Browser(wsgi_app=demo_app)
        ...
        self.assertTrue(browser.body.startswith('Hello world!\n'))
    ...
    def test_non_ascii_urls(self):
        teststr = u'ひらがな'
        # Encode the string and manually ensure ~ is encoded as %7E
        encoded = teststr.encode('utf-8')
        # encoded = encoded.replace(b'~', b'%7E')  # Force ~ encoding
        url = "http://localhost/%s" % url_quote(encoded)
        ...
        # Compare normalized URLs (handle both ~ and %7E)
        self.assertEqual(req.url.replace('~', '%7E'), url.replace('~', '%7E'))
...
class TestWSGILayer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        SIMPLE_LAYER.testSetUp()

    def tearDown(self):
        SIMPLE_LAYER.testTearDown()

    def test_layer(self):
        """When the layer is setup, the wsgi_app argument is unnecessary"""
        browser = p01.testbrowser.testing.Browser()
        browser.open('http://localhost')
        self.assertTrue(browser.body.startswith('Hello world!\n'))
...
class TestAuthorizationMiddleware(unittest.TestCase):
    ...
    def setUp(self):
        app = WSGITestApplication()
        self.unwrapped_browser = p01.testbrowser.testing.Browser(wsgi_app=app)
        app = p01.testbrowser.testing.AuthorizationMiddleware(app)
        self.browser = p01.testbrowser.testing.Browser(wsgi_app=app)
