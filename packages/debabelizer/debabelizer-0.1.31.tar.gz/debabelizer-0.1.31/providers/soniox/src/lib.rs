...
use tokio_tungstenite::{connect_async, tungstenite::Message, WebSocketStream, MaybeTlsStream, tungstenite};
use url::Url;
use uuid::Uuid;
...
impl SonioxStream {
    async fn new(
        ...
        // Create connection request with Bearer authorization header like the Python implementation
        ...
        // Send configuration matching the legacy Python implementation
        ...
        eprintln!("DEBUG: sample_rate = {}", sample_rate);
        eprintln!("DEBUG: num_channels = {}", num_channels);
        eprintln!("DEBUG: format = {:?}", config.format);
        
        let mut init_message = json!({
            "api_key": api_key,
            "audio_format": "pcm_s16le",  // Default PCM format
            "sample_rate": sample_rate,
            "num_channels": num_channels,
            "model": "stt-rt-preview-v2",  // Use the correct Soniox model name
            "enable_language_identification": auto_detect_language || config.language.is_none(),
            "include_nonfinal": config.interim_results,
        });
        
        // Debug log the exact JSON being created
        eprintln!("DEBUG: init_message = {}", serde_json::to_string_pretty(&init_message).unwrap_or_default());
        ...
        // Log the configuration being sent (but mask the API key)
        ...
        // The Python implementation doesn't wait for a response, it just sends the config
        // and continues. Let's do the same to match the behavior.
        tracing::info!("Soniox WebSocket connection established, configuration sent");
...
impl SttStream for SonioxStream {
    async fn send_audio(&mut self, chunk: &[u8]) -> Result<()> {
        tracing::info!("Sending audio chunk to Soniox: {} bytes", chunk.len());
        let result = self.websocket
            .lock()
            .await
            .send(Message::Binary(chunk.to_vec()))
            .await
            .map_err(|e| {
                tracing::error!("Failed to send audio to Soniox: {}", e);
                DebabelizerError::Provider(ProviderError::Network(e.to_string()))
            });
        
        if result.is_ok() {
            tracing::info!("Successfully sent audio chunk to Soniox");
        }
    ...
    async fn receive_transcript(&mut self) -> Result<Option<StreamingResult>> {
        tracing::debug!("Waiting for Soniox transcript...");
        let mut ws = self.websocket.lock().await;
        
        match ws.next().await {
            Some(Ok(Message::Text(text))) => {
                tracing::info!("Received Soniox text message: {}", text);
                
                let response: SonioxResponse = serde_json::from_str(&text)
                    .map_err(|e| {
                        tracing::error!("Failed to parse Soniox response: {} - raw text: {}", e, text);
                ...
                if let Some(tokens) = response.tokens {
                    if tokens.is_empty() {
                        // Empty tokens array - this is the initial confirmation message
                        // Instead of returning None (which ends the Python iterator),
                        // create a dummy "keep-alive" result to keep the stream active
                        tracing::debug!("Received empty tokens array (confirmation message), sending keep-alive");
                    ...
                    // Process tokens - for now, combine them into a single result
                    let mut full_text = String::new();
                    let mut words = Vec::new();
                    let mut confidence_sum = 0.0;
                    let mut confidence_count = 0;
                    ...
                    // No tokens field - just a status message
                    tracing::debug!("Received Soniox status message without tokens");
                    Ok(None)
                }
            }
            Some(Ok(Message::Binary(data))) => {
                tracing::warn!("Received unexpected binary message from Soniox: {} bytes", data.len());
                Ok(None)
            }
            Some(Ok(Message::Close(close_frame))) => {
                let reason = close_frame.as_ref()
                    ...
                    .unwrap_or_default();
                tracing::error!("Soniox WebSocket closed{}", reason);
                Ok(None)
            }
            Some(Ok(Message::Ping(_))) => {
                tracing::debug!("Received ping from Soniox");
                Ok(None)
            }
            Some(Ok(Message::Pong(_))) => {
                tracing::debug!("Received pong from Soniox");
                Ok(None)
            }
            Some(Err(e)) => {
                tracing::error!("Soniox WebSocket error: {}", e);
                Err(DebabelizerError::Provider(ProviderError::Network(
                    e.to_string(),
                )))
            }
            None => {
                tracing::warn!("Soniox WebSocket stream ended (connection closed)");
                Ok(None)
            }
            _ => {
                tracing::warn!("Received unknown message type from Soniox");
                Ok(None)
            }
        }
    }
