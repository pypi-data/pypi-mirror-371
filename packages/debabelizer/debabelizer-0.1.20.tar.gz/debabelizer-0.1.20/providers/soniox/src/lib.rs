...
impl SonioxStream {
    async fn new(
        ...
        // Send initial configuration - start with minimal fields
        let mut init_message = json!({
            "api_key": api_key,
            "include_nonfinal": config.interim_results,
        });
        
        // Only add punctuation and diarization if they are explicitly enabled
        // Skip them for now to isolate the field name issues
        // if config.punctuate {
        //     init_message["enable_automatic_punctuation"] = json!(true);
        // }
        // if config.diarization {
        //     init_message["enable_speaker_identification"] = json!(true);
        // }
        ...
        // Wait for initial response/confirmation from Soniox
        {
            let mut ws = websocket.lock().await;
            match ws.next().await {
                Some(Ok(Message::Text(response))) => {
                    tracing::info!("Soniox initial response: {}", response);
                    
                    // Parse the response to check for errors
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&response) {
                        if let Some(error) = parsed.get("error") {
                            return Err(DebabelizerError::Provider(ProviderError::Authentication(
                        ...
                        if let Some(status) = parsed.get("status") {
                            if status == "error" {
                                ...
                                return Err(DebabelizerError::Provider(ProviderError::Authentication(
                                    format!("Soniox status error: {}", msg)
                                )));
                            }
                        }
                    }
                    tracing::info!("Soniox WebSocket initialized successfully");
                }
                Some(Ok(Message::Close(close_frame))) => {
                    ...
                    return Err(DebabelizerError::Provider(ProviderError::Network(
                        format!("Soniox WebSocket closed during initialization{}", reason)
                    )));
                }
                Some(Ok(_)) => {
                    tracing::warn!("Received non-text message during Soniox initialization");
                }
                Some(Err(e)) => {
                    return Err(DebabelizerError::Provider(ProviderError::Network(format!("WebSocket error during initialization: {}", e))));
                }
                None => {
                    return Err(DebabelizerError::Provider(ProviderError::Network("WebSocket closed during initialization".to_string())));
                }
            }
        }
