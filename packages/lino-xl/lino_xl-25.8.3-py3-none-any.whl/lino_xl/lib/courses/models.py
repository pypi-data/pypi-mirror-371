...
# from lino_xl.lib.accounting.utils import ZERO, ONE
...
from lino.mixins import Referrable, Sequenced, BabelNamed
...
from .actions import PrintPresenceSheet, PrintDescription
from .ui import *

cal = dd.resolve_app('cal')  # returns None if cal is not installed

# if is_installed('courses'):
...
class Slot(Sequenced, StartEndTime):
...
class Topic(BabelNamed, Publishable, Clonable):
...
class Line(Referrable, Clonable, ExcerptTitle, ContactRelated, Publishable):
    ...
    every_unit = Recurrences.field(_("Recurrency"),
                                   default='weekly',
                                   blank=True)
    ...
    options_cat = dd.ForeignKey('products.Category',
                                verbose_name=_("Options category"),
                                related_name="courses_lines_by_options_cat",
                                blank=True,
                                null=True)

    fees_cat = dd.ForeignKey('products.Category',
...
class Course(Reservation, Clonable, Publishable, MemoReferrable):
    ...
    teacher = dd.ForeignKey(dd.plugins.courses.teacher_model,
                            verbose_name=dd.plugins.courses.teacher_label,
                            blank=True,
                            null=True)
    ...
    print_description = PrintDescription(show_in_toolbar=False)
    print_presence_sheet = PrintPresenceSheet(show_in_toolbar=False)
    print_presence_sheet_html = PrintPresenceSheet(show_in_toolbar=False,
    ...
    def print_actions(self, ar):
        ...
        elems.append(ar.instance_action_button(self.print_description))
    ...
    def get_detail_action(self, ar):
        # if ar.actor.detail_action is None:
        #     raise Exception("20240422 {} has no detail action!?".format(ar.actor))
    ...
    def update_cal_from(self, ar):
        ...
        #     ar.debug("No start date because state is %s", self.state)
        #     return None
        return self.start_date

    def update_cal_event_type(self):
        if self.line_id and self.line.event_type_id:
            return self.line.event_type
        return settings.SITE.get_config_value('default_event_type')
    ...
    def events_by_course(self, **kwargs):
        ...
        return cal.Event.objects.filter(**kwargs)
    ...
    def requested(self, ar):
        ...
        # return rt.models.courses.EnrolmentsByCourse.create_request(
...
class Enrolment(UserAuthored, Certifiable, DateRange):
    ...
    quick_search_fields = dd.plugins.courses.pupil_name_fields
    ...
    def pupil_choices(cls, course):
        ...
        return dd.plugins.courses.pupil_model.objects.all()

    def create_pupil_choice(self, text, ar=None):
        """
        Called when an unknown pupil name was given.
        Try to auto-create it.
        """

    def create_pupil_choice(self, *args, **kwargs):
        # Pupil = dd.resolve_model(pupil_model)
        return dd.plugins.courses.pupil_model.create_from_choice(
            *args, **kwargs)
    ...
    def get_overview_elems(self, ar):
        if self.course_id:
            return [self.course.as_summary_item(ar)]
    ...
    def get_guest_role(self):
        if self.course.line:
            return self.course.line.guest_role or settings.SITE.get_config_value('pupil_guestrole')
        return settings.SITE.get_config_value('pupil_guestrole')

    def make_guest_for(self, event):
        if not self.state.uses_a_place:
            # print("20210707 not self.state.uses_a_place", event)
            return
        gr = self.get_guest_role()
        if gr is not None:
            return cal.Guest(event=event, partner=self.pupil, role=gr)
    ...
    def full_clean(self, *args, **kwargs):
        ...
        # if self.remark.startswith("a"):
        #     raise ValidationError("20230909 sandbox!")
...
dd.update_field(Enrolment, 'overview', verbose_name=Course._meta.verbose_name)

dd.update_field(Enrolment, 'course', blank=True)

dd.inject_field(
    'system.SiteConfig', 'pupil_guestrole',
    dd.ForeignKey('cal.GuestRole',
                  verbose_name=_("Default guest role for participants"),
...
#
#     def cmd(parser, s, cmdname, usages):
#
#         pk = s
#         txt = None
#
