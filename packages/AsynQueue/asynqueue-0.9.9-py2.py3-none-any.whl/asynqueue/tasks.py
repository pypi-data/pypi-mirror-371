...
class TaskHandler(object):
    ...
    @defer.inlineCallbacks
    def update(self, task, ephemeral=False, sequential=False):
        """
        ...
        run. If the task failed, the result will be a string
        containing information about the failure.

        Note that there is no mechanism for obtaining such
        ...
        @keyword sequential: Set C{True} to have update tasks run
        ...
        for worker in self.roster(task.series):
            ...
            newTask = task.copy()
            worker.run(newTask)
            d = newTask.d.addErrback(newTask.info.aboutFailure)
            if sequential:
                result = yield d
