...
from asynqueue.test.testbase import deferToDelay, blockingTask, Tasks, \
...
class TestProcessQueue(TestCase):
    ...
    def test_error_warn(self):
        with self.stderr() as stderr:
            ...
            self.assertEqual(
                ...
                "STDERR not blank: '{}'".format(stderr.getvalue()))
    ...
    def test_error_returnFailure(self):
        ...
        with self.stderr() as stderr:
            ...
            self.assertEqual(
                len(stderr.getvalue()), 0,
                "STDERR not blank: '{}'".format(stderr.getvalue()))

    @defer.inlineCallbacks
    def test_crashWorkers(self):
        with self.stderr() as stderr:
            self.q = process.ProcessQueue(2, warn=True)
            d1 = self.q.call(self.t._blockingTask, 1, minTime=5.0)
            yield deferToDelay(0.1)
            d2 = self.q.call(self.t._blockingTask, 2, minTime=6.0)
            yield deferToDelay(0.1)
            tasks = yield self.q.crashWorkers()
            # I would expect two unfinished tasks to be returned, but this
            # isn't worth pursuing
            self.assertEqual(len(tasks), 2)
            for d in d1, d2:
                result = yield d
                self.assertEqual(result, 'Pipe disconnect')
