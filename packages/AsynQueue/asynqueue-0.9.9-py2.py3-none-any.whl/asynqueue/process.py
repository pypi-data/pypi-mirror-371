...
class ProcessQueue(TaskQueue):
    ...
    def crashWorkers(self):
        """
        Call this when my workers are too dedicated to their long-running
        tasks and you don't feel like waiting forever for them to
        finish. Detaches and terminates them all.

        Returns a C{Deferred} that fires with a list of the unfinished
        tasks.
        """
        tasks = []; dList = []
        workers = self.th.roster('process')
        # The list will shrink as workers get detached, so iterate
        # over a static copy of it
        for worker in list(workers):
            d = self.detachWorker(worker, crash=True)
            d.addCallback(tasks.append)
            dList.append(d)
        return defer.DeferredList(dList).addCallback(lambda _: tasks)
...
class ProcessWorker(object):
    ...
    The author did some testing of backoff coefficients with unit
    tests, where the reactor wasn't doing much other than running the
    asynqueue and Twisted's trial test runner.
    ...
    def do_next(self, ID):
        ...
        """
        def gotLock(null):
            if self.cMain is None:
                return errors.ProcessError("Process dead")
            self.cMain.send(ID)
    ...
    def run(self, task):
        ...
        if task is None:
            ...
            if self.cMain is not None:
                # Process needs to go away
            ...
            if self.cMain is not None:
                ...
                # "Wait" here (in Twisted-friendly fashion) for a
    ...
    def stop(self):
        ...
        def handle_error(failure):
            if failure.check(BrokenPipeError):
                # This error is fairly common on shutdown and is
                # squashed
                return
            return failure
        
        yield self.dt.deferToAll()
        yield self.run(None).addErrback(handle_error)
        yield self.dLock.stop()
        self._killProcess()

    def crash(self):
        """
        Rudely terminates the worker by killing its process. Any
        C{Deferred} issued from calls to L{DeferredTracker.deferToAll}
        of my I{dt} will fire after the next iteration delay.

        Returns my list of outstanding I{tasks}.
        """
        self._killProcess()
        self.dt.quitWaiting()
        return self.tasks
...
class ProcessBase(object):
    ...
    def pid(self):
        """
        B{Property}: A PID for my current process. Used to uniquely
        identify an instance of me running on a particular Python
        interpreter vs others on other interpreters.
        """
    ...
    def data(self):
        """
        B{Property}: A dict whose attributes are not shared by any other
        instances of me that may be running on other Python
        interpreters.
        """
        return self._pb_data.setdefault(self.pid, {})
