...
class Delay(object):
    ...
    def untilEvent(self, eventChecker, *args, **kw):
        ...
        if the event actually happened, C{False} if a timeout
        occurred, or C{None} if there was an error calling
        I{eventChecker}.
        ...
        @param eventChecker: A callable that returns an immediate
            boolean value indicating if an event occurred. You can
            supply any args/kw that the callable will take.
        ...
        if self.triggerID is None or eventChecker(*args, **kw):
            ...
            while self.triggerID is not None:
                try: yes = eventChecker(*args, **kw)
                except: yes = result = None
                if yes:
                    result = True
                    break
                if result is None:
