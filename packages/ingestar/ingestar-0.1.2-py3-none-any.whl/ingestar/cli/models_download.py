...
from ingestar.utils.enum_class import ModelPath
from ingestar.utils.models_download_utils import auto_download_and_get_model_root_path
...
def download_and_modify_json(json_str, local_filename, modifications):
    """使用JSON字符串并修改内容"""
...
"""
    config_file_name = os.getenv('INGESTAR_TOOLS_CONFIG_JSON', 'ingestar.json')
    ...
    download_and_modify_json(json_str, config_file, json_mods)
...
def download_models(model_source, model_type):
    ...
    logger.info(f"Downloading {model_type} model from {os.getenv('INGESTAR_MODEL_SOURCE', None)}...")
