...
from ingestar.utils.cli_parser import arg_parse
from ingestar.utils.config_reader import get_device
from ingestar.utils.model_utils import get_vram
...
@click.option(
    ...
    type=click.Path(),
    required=False,
    help='The root directory of the local model files',
)
...
def main(
        ...
        device_mode, virtual_vram, model_source, model_path, ** kwargs
):
    ...
    if not backend.endswith('-client'):
        def get_device_mode() -> str:
            if device_mode is not None:
                ...
                return get_device()
        if os.getenv('INGESTAR_DEVICE_MODE', None) is None:
            os.environ['INGESTAR_DEVICE_MODE'] = get_device_mode()
        ...
        def get_virtual_vram_size() -> int:
            ...
            return 1
        if os.getenv('INGESTAR_VIRTUAL_VRAM_SIZE', None) is None:
            os.environ['INGESTAR_VIRTUAL_VRAM_SIZE']= str(get_virtual_vram_size())

        if os.getenv('INGESTAR_MODEL_SOURCE', None) is None:
            os.environ['INGESTAR_MODEL_SOURCE'] = model_source
        if os.getenv('INGESTAR_MODEL_PATH', None) is None:
            if model_path:
                os.environ['INGESTAR_MODEL_PATH'] = model_path
