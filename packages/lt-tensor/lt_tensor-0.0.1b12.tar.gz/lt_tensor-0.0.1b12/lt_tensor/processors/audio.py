__all__ = ["AudioProcessor", "AudioProcessorConfig", "convert_to_float16"]
...
class AudioProcessor(Model):
    def __init__(
        ...
        self.window = nn.Parameter(
            torch.hann_window(self.cfg.win_length), requires_grad=False
        )
    ...
    def compute_mel(
        ...
        wave: Tensor,
        norm: bool = False,
        norm_fn: Optional[Callable[[Tensor], Tensor]] = log_norm,
        eps: float = 1e-5,
    ) -> Tensor:
        if wave.ndim == 1:
        ...
        if norm:
            return norm_fn(results, eps=eps)
        return results
    ...
    def loss_fn(self, inputs: Tensor, target: Tensor):
        mel_loss = F.l1_loss(self.compute_mel(inputs), self.compute_mel(target))
        with torch.no_grad():
            target = self.normal_minmax(target, -2.0, 2.0)
        inputs = self.normal_minmax(inputs, -2.0, 2.0)
        half = self.cfg.n_stft // 2
        comp_inp = self.stft(inputs, return_complex=False)
        inp_spec = comp_inp[..., :half, :, 0].sin()
        inp_phase = comp_inp[..., -half:, :, 0].exp()

        with torch.no_grad():
            comp_tgt = self.stft(target, return_complex=False)
            tgt_spec = comp_tgt[..., :half, :, 0].sin()
            tgt_phase = comp_tgt[..., -half:, :, 0].exp()

        spec_loss = F.l1_loss(inp_spec, tgt_spec)
        phase_loss = F.l1_loss(inp_phase, tgt_phase)
        angle_diff_loss = F.l1_loss(comp_inp.angle(), comp_tgt.angle())
        try:
            pitch_loss = F.huber_loss(
                self.compute_pitch(inputs), self.compute_pitch(target)
            )
        except:
            pitch_loss = torch.tensor([0.0], device=inputs.device)
        return spec_loss, phase_loss, angle_diff_loss, mel_loss, pitch_loss
    ...
    def collate_wave(self, waves: List[Tensor], same_size: bool = False):
        ...
        if same_size:
            ...
            return torch.stack(waves, dim=0).view(B, 1, waves[0].shape[-1])
