...
class EventPool(object):
    ...
    def _start_offline_worker_processor_with_restart(self, queue: str, event_queue: asyncio.Queue):
    ...
    async def _process_offline_workers(self, queue: str, event_queue: asyncio.Queue):
        """定期检测离线worker并使用XCLAIM转移其pending消息"""
        prefixed_queue = self.get_prefixed_queue_name(queue)
        prefixed_queue_bytes = prefixed_queue.encode() if isinstance(prefixed_queue, str) else prefixed_queue
        logger.info(f"Started offline worker processor for queue {queue}")
        
        # 智能等待consumer manager初始化
        # 使用指数退避策略，最多等待约15秒 (0.1 + 0.2 + 0.4 + 0.8 + 1.6 + 3.2 + 6.4 ≈ 12.7秒)
        wait_times = [0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.4]
        ...
        if not current_consumer:
            ...
            try:
                ...
                logger.error(f"Failed to initialize consumer manager for queue {queue}: {e}")
                return
        ...
        while not self._stop_reading:
            try:
                ...
                total_workers = 0
                alive_workers = 0
                offline_with_transferred = 0
                
                # 批量处理配置（优化后的值）
                batch_size = 500  # 增大批量，提高吞吐量
                max_batch_workers = 20  # 增加并行处理数量
                scan_complete = False
                current_cursor = last_scan_cursor
                
                # 继续从上次的位置扫描
                while not scan_complete and len(offline_workers) < max_batch_workers:
                    ...
                    for worker_key in keys:
                        ...
                        if ':HISTORY:' in worker_key or ':REUSE:LOCK' in worker_key or ':REUSING' in worker_key:
                            continue
                        ...
                        try:
                            ...
                            messages_transferred = worker_data.get('messages_transferred', 'false').lower() == 'true'
                            
                            if is_alive:
                                alive_workers += 1
                            elif messages_transferred:
                                offline_with_transferred += 1
                            
                            # 找到离线且消息未转移的worker
                            if not is_alive and not messages_transferred:
                                queues_str = worker_data.get('queues', '')
                                worker_queues = queues_str.split(',') if queues_str else []
                                
                                # 检查这个worker是否负责当前队列
                                if queue in worker_queues:
                                    offline_workers.append((worker_key, worker_data))
                                    
                                    # 如果这批已经收集了足够的离线worker，停止收集
                                    if len(offline_workers) >= max_batch_workers:
                                        # 保存当前游标位置，下次从这里继续
                                        last_scan_cursor = current_cursor
                                        break
                        except Exception as e:
                ...
                # 如果收集到了离线worker，处理它们
                if offline_workers:
                    logger.info(f"Processing batch of {len(offline_workers)} offline workers for queue {queue} (scanned {total_workers} total workers)")
                    for _, wd in offline_workers:
                        logger.debug(f"  - Worker: {wd.get('consumer_id')}, is_alive={wd.get('is_alive')}, messages_transferred={wd.get('messages_transferred')}")
                    ...
                    for worker_key, worker_data in offline_workers:
                        ...
                        # 使用分布式锁防止并发处理
                        lock_key = f"{self.redis_prefix}:CLAIM:LOCK:{consumer_name}"
                        ...
                        if not await lock.acquire():
                            logger.debug(f"Another process is claiming messages for {consumer_name}")
                            continue
                        
                        try:
                            # 检查是否正在关闭
                            if self._stop_reading:
                                logger.info("Stopping message recovery due to shutdown signal")
                                break
                            
                            # 获取当前的consumer名称（用于claim操作）
                            try:
                                current_consumer = self.consumer_manager.get_consumer_name(queue)
                                if not current_consumer:
                                    logger.error(f"Failed to get consumer name for queue {queue}")
                                    continue
                                logger.info(f"Current consumer for claiming: {current_consumer}")
                                
                                # 检查是否是同一个consumer（worker重启后复用了自己的ID）
                                is_same_consumer = (current_consumer == consumer_name)
                                if is_same_consumer:
                                    # 同一个consumer不需要处理，listening_event会自动读取pending消息
                                    logger.info(f"Skipping {consumer_name} - same consumer will handle its own pending messages via XREADGROUP")
                                    continue
                            except Exception as e:
                                logger.error(f"Error getting consumer name for queue {queue}: {e}")
                                continue
                            
                            # 获取该离线consumer的所有pending消息
                            pending_batch_size = 500  # 优化：增大批量
                            last_id = '-'
                            total_claimed = 0
                            
                            # 先检查是否有pending消息
                            try:
                                pending_info = await self.async_binary_redis_client.xpending(prefixed_queue_bytes, prefixed_queue_bytes)
                                logger.info(f"Total pending messages in queue {queue}: {pending_info}")
                            except Exception as e:
                                logger.error(f"Error checking pending messages for queue {queue}: {e}")
                                continue
                            
                            while True:
                                # 获取pending消息
                                pending_batch = await self.async_binary_redis_client.xpending_range(
                                    prefixed_queue_bytes, prefixed_queue_bytes,
                                    min=last_id, max='+',
                                    count=pending_batch_size
                                )
                                
                                logger.debug(f"Got {len(pending_batch) if pending_batch else 0} pending messages in batch")
                                ...
                                for msg in pending_batch:
                                    ...
                                    logger.debug(f"Checking message {msg['message_id']} from consumer {msg_consumer} (looking for {consumer_name})")
                                    if msg_consumer == consumer_name:
                                        message_ids.append(msg['message_id'])
                                
                                logger.info(f"Found {len(message_ids)} messages belonging to offline consumer {consumer_name}")
                                
                                if message_ids:
                                    # 使用XCLAIM转移消息到当前consumer
                                    logger.info(f"Attempting to XCLAIM {len(message_ids)} messages from {consumer_name} to {current_consumer}")
                                    try:
                                        claimed = await self.async_binary_redis_client.xclaim(
                                            prefixed_queue_bytes, prefixed_queue_bytes,
                                            current_consumer,
                                            min_idle_time=0,
                                        ...
                                        logger.info(f"XCLAIM result: successfully claimed {len(claimed) if claimed else 0} messages")
                                    except Exception as e:
                                        logger.error(f"XCLAIM failed for {len(message_ids)} messages: {e}")
                                        continue
                                    
                                    if claimed:
                                        # 将claimed的消息直接加入内存队列执行
                                        # 注意：不要ACK消息，让它们保持在pending状态，由实际处理它们的代码去ACK
                                        added_count = 0
                                        for msg_id, msg_data in claimed:
                                            # 将bytes类型的msg_id转换为字符串
                                            if isinstance(msg_id, bytes):
                                                msg_id = msg_id.decode('utf-8')
                                            
                                            # 添加恢复标记到消息数据中
                                            msg_data['_recovered'] = 'true'
                                            msg_data['_recovery_time'] = str(time.time())
                                            msg_data['_original_consumer'] = consumer_name
                                            msg_data['_original_worker'] = consumer_id
                                            
                                            # 处理消息格式，确保与正常消息兼容
                                            # 恢复的消息应该直接使用，通过_process_message_common处理
                                            # 使用相同的处理逻辑
                                            ...
                                            # 检查是否正在关闭
                                            if self._stop_reading:
                                                logger.info(f"Stopping message recovery due to shutdown signal, {len(claimed) - added_count} messages not added")
                                                break
                                            ...
                                            if isinstance(result, tuple) and result[0] == 'async_put':
                                                ...
                                                try:
                                                    await asyncio.wait_for(
                                                        self._async_put_task(event_queue, result[1]),
                                                        timeout=1.0
                                                    )
                                                    added_count += 1
                                                    logger.debug(f"Added recovered message {msg_id} to event queue")
                                                except asyncio.TimeoutError:
                                                    logger.warning(f"Timeout adding recovered message {msg_id} to event queue (queue might be full)")
                                                    break
                                                except Exception as e:
                                                    logger.error(f"Failed to add recovered message {msg_id} to event queue: {e}")
                                            else:
                                                # 消息已经被处理（可能是routing消息）
                                                added_count += 1
                                        
                                        # 不要在这里ACK消息！让实际处理消息的代码去ACK
                                        # 这样如果程序崩溃，消息不会丢失，会在下次启动时重新处理
                                        
                                        total_claimed += len(claimed)
                                        logger.info(f"Claimed {len(claimed)} messages from {consumer_name}, {added_count} added to event queue")
                                
                                if len(pending_batch) < pending_batch_size:
                                    break
                                
                                last_id = pending_batch[-1]['message_id']
                            
                            if total_claimed > 0:
                                logger.info(f"Total claimed {total_claimed} messages from offline worker {consumer_id}")
                            else:
                                logger.info(f"No pending messages found for offline worker {consumer_id}")
                            
                            # 标记该worker的消息已转移（但如果是自己且正在关闭，不标记）
                            if not (is_same_consumer and self._stop_reading):
                                await self.async_redis_client.hset(worker_key, mapping={
                                    'messages_transferred': 'true',
                                    'transferred_count': str(total_claimed),
                                    'transferred_time': str(time.time()),
                                    'transferred_by': current_consumer
                                })
                                logger.info(f"Marked worker {consumer_id} as messages_transferred=true")
                            else:
                                logger.info(f"Not marking messages_transferred for {consumer_id} during shutdown")
                            
                            # 删除原consumer（已经转移了所有消息）
                            try:
                                consumer_name_bytes = consumer_name.encode() if isinstance(consumer_name, str) else consumer_name
                                await self.async_binary_redis_client.execute_command(b'XGROUP', b'DELCONSUMER', 
                                                                             prefixed_queue_bytes, prefixed_queue_bytes, consumer_name_bytes)
                                logger.info(f"Deleted consumer {consumer_name} from queue {queue}")
                            except Exception as e:
                                logger.warning(f"Failed to delete consumer {consumer_name}: {e}")
                                
                        except Exception as e:
                            logger.error(f"Error claiming messages from {consumer_name}: {e}")
                        finally:
                            await lock.release()
                
                # 动态调整扫描间隔
                if offline_workers:
                    # 如果发现了离线worker，更频繁地扫描
                    await asyncio.sleep(5)
                elif scan_complete:
                    # 完成了一轮完整扫描且没发现离线worker，等待较长时间
        ...
        async def listen_event_by_task(queue, task_name):
            ...
            while not self._stop_reading:
                ...
                try:
                    ...
                    # 否则使用短阻塞时间以平衡响应速度和CPU使用
                    block_time = 0 if (delayed_list and len(delayed_list) > 0) else 50
                    
                    messages = await self.async_binary_redis_client.xreadgroup(
                        groupname=group_name_bytes,
                        consumername=consumer_name_bytes,
                    ...
                    #     logger.info(f'先消费之前的消息 {messages=} {group_name_bytes=} ')
                    if not messages:
                        check_backlog = False
                        # 如果没有消息且没有延迟任务，稍微等待以避免CPU空转
                        if not (delayed_list and len(delayed_list) > 0):
    ...
    async def _scan_and_load_delayed_tasks_to_list(self, queue: str, task_list: list, task_lock: asyncio.Lock):
        ...
        try:
            ...
            # 批量添加到list（使用锁保证线程安全）
            if tasks_to_add:
                logger.info(f'我拿到消息了：{task_list=}')
