...
class HeartbeatConsumerStrategy:
    ...
    async def _transfer_pending_to_temp_list(self, worker_key: str, worker_data: dict, queues: list):
        """将离线worker的pending消息转移到临时list"""
        consumer_id = worker_data.get('consumer_id')
        ...
        try:
            ...
            for queue in queues:
                ...
                temp_list_key = f"{self.redis_prefix}:PENDING_RECOVERY:{queue}"
                
                # 使用分布式锁防止并发处理
                recovery_lock_key = f"{self.redis_prefix}:RECOVERY:LOCK:{queue}:{consumer_name}"
                ...
                try:
                    ...
                    while True:
                        ...
                        for msg in pending_batch:
                            ...
                            if msg_consumer == consumer_name:
                                consumer_messages.append(msg['message_id'])
                        
                        if consumer_messages:
                            # 使用XCLAIM将消息claim到临时consumer
                            temp_consumer = f"temp-recovery-{uuid.uuid4().hex[:8]}"
                            ...
                            claimed = await self.async_redis.xclaim(
                                ...
                                temp_consumer,
                                min_idle_time=0,
                                message_ids=consumer_messages,
                                force=True
                            )
                            
                            if claimed:
                                # 将claimed的消息信息转存到临时list
                                pipeline = self.async_redis.pipeline()
                                for msg_id, msg_data in claimed:
                                    recovery_data = {
                                        ...
                                        'recovery_time': str(current_time),
                                        'data': msg_data  # 保持原始数据，只序列化一次
                                    }
                                    # 将恢复信息推入临时list，只序列化一次
                                    pipeline.rpush(temp_list_key, dumps_str(recovery_data))
                                    # ACK原始消息
                                    pipeline.xack(prefixed_queue, prefixed_queue, msg_id)
                                
                                await pipeline.execute()
                                total_transferred += len(claimed)
                                logger.info(f"Transferred {len(claimed)} messages from {consumer_name} to temp list")
                    ...
                    if total_transferred > 0:
                        # 记录恢复数量
                        await self.async_redis.hset(worker_key, 'recovered_count', str(total_transferred))
                        logger.info(f"Total transferred {total_transferred} messages for worker {consumer_id} queue {queue}")
                    
                    # 删除原consumer
                    try:
                        await self.async_redis.execute_command('XGROUP', 'DELCONSUMER', 
                                                              prefixed_queue, prefixed_queue, consumer_name)
                        logger.info(f"Deleted consumer {consumer_name} from queue {queue}")
                    except Exception as e:
                        logger.warning(f"Failed to delete consumer {consumer_name}: {e}")
                        
                except Exception as e:
                    logger.error(f"Error transferring pending messages for {consumer_name}: {e}")
                finally:
                    await recovery_lock.release()
            
            # 处理完所有队列后的日志
            logger.info(f"Completed pending message transfer for worker {consumer_id}")
                    
        except Exception as e:
            logger.error(f"Error in _transfer_pending_to_temp_list for worker {consumer_id}: {e}")
    ...
    async def _mark_worker_offline_and_cleanup_v2(self, worker_key: str, worker_data: dict, queues: list):
        ...
        try:
            ...
            pipeline.hset(worker_key, mapping={
                ...
                'offline_time': str(current_time),
                'shutdown_reason': 'heartbeat_timeout',
                'messages_transferred': 'false'  # 标记消息尚未转移
            })
            ...
            if messages_transferred != 'true':
                ...
                # 标记消息已转移
                await self.async_redis.hset(worker_key, 'messages_transferred', 'true')
                logger.info(f"Marked messages as transferred for worker {consumer_id}")
