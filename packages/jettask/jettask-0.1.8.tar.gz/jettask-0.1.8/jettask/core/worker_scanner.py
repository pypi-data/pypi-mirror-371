...
class WorkerScanner:
    ...
    async def scan_timeout_workers(self) -> List[Dict]:
        ...
        # O(log N) 获取超时 workers
        timeout_worker_ids = await self.async_redis.zrangebyscore(
            self.active_workers_key, 
            min=0, 
            max=cutoff_time
        )
        
        if not timeout_worker_ids:
            return []
        
        # 批量获取 worker 详细信息
        pipeline = self.async_redis.pipeline()
        for worker_id in timeout_worker_ids:
        ...
        for worker_id, worker_data in zip(timeout_worker_ids, workers_data):
            if not worker_data:
                # Hash 不存在，清理 ZSet
