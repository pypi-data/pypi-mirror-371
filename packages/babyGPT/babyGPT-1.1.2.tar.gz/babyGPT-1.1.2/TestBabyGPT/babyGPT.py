...
__date__      = '2025-August-23'   
__url__       = 'https://engineering.purdue.edu/kak/distGPT/babyGPT-1.1.2.html'
...
Version 1.1.2:
    ...
    There was an error in the code that creates a new vocab dictionary for the
    cleaned up tokens. I have fixed the error in this version and also provided the
    tokenizers trained on the athlete-news dataset with 1.1.1 and 1.1.2 training
    code, the former for the base tokenizer and the latter for the cleaned-up
    version. The two tokenizers are in the Examples directory and their names begin
    with the prefixes 111 and 112, respectively.  You'll notice that getting rid of
    superfluous tokens reduced the size of the tokenizer vocab from 50002 to 35035.
    The base tokenizer with the vocab size of 50002 was trained with a target vocab
    size of 50000.
...
How to inform each process as the CUDA type of the tensor being created as shown
above. As can see, it is done with the simple expedient of declaring the "device" to
be the same as for the "input_sequence".  That makes completely unambiguous the
...
You'll obviously need to make sure that the numbers "111", "20025", and "30000"  
...
The examples I have shown below are based on the following assumptions:
