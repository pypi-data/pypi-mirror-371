...
class MemoryManager:
    ...
    def __init__(self, max_memory_gb: Optional[float] = None, 
        ...
        config = get_config()
        self.max_memory_bytes = (max_memory_gb or config.performance.max_memory_gb) * 1024**3
    ...
    def optimize_dataframe(self, df: pd.DataFrame, 
        ...
        for col in optimized_df.select_dtypes(include=['object']).columns:
            if optimized_df[col].nunique() / len(optimized_df) < 0.5:  # 50%'den az unique değer
    ...
    def chunk_processing(self, df: pd.DataFrame, 
                        operation: Callable,
    ...
    def memory_efficient_apply(self, df: pd.DataFrame, 
                              func: Callable,
                              axis: int = 0,
                              **kwargs) -> pd.Series:
        """
        ...
        **kwargs : dict
        ...
        if axis == 0:  # Satır bazında
            return self.chunk_processing(df, lambda chunk: chunk.apply(func, axis=axis, **kwargs))
        else:  # Sütun bazında
            return df.apply(func, axis=axis, **kwargs)
    
    def smart_groupby(self, df: pd.DataFrame, 
    ...
    def _get_optimization_recommendations(self) -> List[str]:
        ...
        if current_memory['usage_percent'] > 80:
            recommendations.append("High memory usage detected. Consider optimizing DataFrames or using chunk processing.")
        
        if current_memory['swap_percent'] > 20:
            recommendations.append("High swap usage detected. Consider reducing memory footprint.")
        
        if self.cache:
            cache_stats = self.cache.get_stats()
            if cache_stats['hit_rate'] < 0.3:
                recommendations.append("Low cache hit rate. Consider adjusting cache size or eviction strategy.")
        
        if len(self.memory_history) > 10:
            recent_usage = [h['usage_percent'] for h in self.memory_history[-10:]]
            if max(recent_usage) - min(recent_usage) > 30:
                recommendations.append("High memory usage variability. Consider implementing memory pooling.")
...
def chunk_process_dataframe(df: pd.DataFrame, operation: Callable, **kwargs) -> pd.DataFrame:
