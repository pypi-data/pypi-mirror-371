...
class SmartCleaner:
    ...
    def _is_likely_datetime(self, series: pd.Series, sample_size: int = 100) -> bool:
        ...
        sample = series.dropna().sample(min(sample_size, len(series.dropna())), random_state=42)
        ...
        for col in self.potential_datetime_cols:
            try:
                ...
                if new_nulls / len(self.df) < 0.1:  # Less than 10% failed conversions
                    ...
                    errors.append(f"❌ Failed to convert '{col}' - too many parsing errors")
    ...
    def _handle_missing_data(self, aggressive: bool = False) -> Dict[str, Any]:
        ...
        for col in missing_columns.index:
            ...
            if missing_pct > 0.8 and aggressive:
                ...
                self.cleaning_log.append(f"🗑️ Dropped column '{col}' ({missing_pct:.1%} missing)")
                
            elif missing_pct > 0.5:
                # High missing rate - just flag for manual review
                self.suggestions.append(f"⚠️ Column '{col}' has {missing_pct:.1%} missing - consider dropping or manual imputation")
                ...
                if strategy == 'median':
                    ...
                    self.cleaning_log.append(f"📊 Filled '{col}' missing values with mean ({fill_value:.2f})")
                    ...
                    self.cleaning_log.append(f"📊 Filled '{col}' missing values with mode ({fill_value})")
                ...
                if missing_pct < 0.2:  # Less than 20% missing
                    ...
                    self.cleaning_log.append(f"📝 Created 'Missing_Data' category for '{col}'")
    ...
    def _handle_duplicates(self, aggressive: bool = False) -> Dict[str, Any]:
        ...
        if duplicate_pct > 0.05 or aggressive:  # >5% duplicates or aggressive mode
            ...
            self.cleaning_log.append(f"🔄 Removed {removed_count} duplicate rows ({duplicate_pct:.1%})")
            ...
            self.suggestions.append(f"💡 {duplicate_count} duplicate rows detected ({duplicate_pct:.1%}) - consider manual review")
            return {'step': 'duplicate_handling', 'removed_count': 0}
        ...
        for col in self.numeric_cols:
            ...
            if outlier_pct > 0.1:  # More than 10% outliers
                if aggressive and outlier_pct < 0.3:  # Remove if <30% and aggressive
                    ...
                    self.cleaning_log.append(f"🎯 Removed {outlier_count} outliers from '{col}' ({outlier_pct:.1%})")
                    ...
                    self.suggestions.append(f"⚠️ Column '{col}' has {outlier_pct:.1%} outliers - manual review recommended")
    ...
    def _optimize_data_types(self) -> Dict[str, Any]:
        ...
        if optimized_columns:
            self.cleaning_log.append(f"💾 Optimized data types - saved {total_memory_saved:.2f} MB")
    ...
    def _clean_categorical_data(self) -> Dict[str, Any]:
        ...
        for col in self.categorical_cols:
            if col in self.df.columns:  # Column might have been dropped
                ...
                if self.df[col].dtype == 'object':
                    ...
                    if len(rare_categories) > 0 and len(rare_categories) < original_unique * 0.3:  # Don't group if >30% are rare
                        self.df[col] = self.df[col].replace(rare_categories, 'Other_Category')
                        actions_taken.append({
                            'column': col,
                        ...
                        self.cleaning_log.append(f"🏷️ Grouped {len(rare_categories)} rare categories in '{col}' as 'Other_Category'")
        ...
        return {
            'step': 'categorical_data_cleaning',
            'actions_taken': actions_taken
        }
    ...
    def _calculate_quality_improvement(self) -> Dict[str, float]:
        ...
        original_missing_pct = (self.original_df.isnull().sum().sum() / original_size) * 100
        original_duplicates_pct = (self.original_df.duplicated().sum() / original_len) * 100
        ...
        memory_reduction_pct = 0.0 if original_memory == 0 else ((original_memory - current_memory) / original_memory) * 100
        
        row_reduction_pct = 0.0 if original_len == 0 else ((len(self.original_df) - len(self.df)) / original_len) * 100
    ...
    def clean(self, df: Optional[pd.DataFrame] = None, aggressive: bool = False) -> Dict[str, Any]:
    ...
    def handle_duplicates(self, df: pd.DataFrame, strategy: str = 'remove') -> pd.DataFrame:
    ...
    def handle_outliers(self, df: pd.DataFrame, columns: Optional[List[str]] = None, method: str = 'iqr', aggressive: bool = False) -> pd.DataFrame:
        ...
        result = df.copy()
        target_columns = columns if columns is not None else result.select_dtypes(include=[np.number]).columns.tolist()
    ...
    def _generate_post_cleaning_recommendations(self) -> List[str]:
        ...
        if current_shape[0] < original_shape[0]:
            row_loss_pct = ((original_shape[0] - current_shape[0]) / original_shape[0]) * 100
            if row_loss_pct > 10:
                recommendations.append(f"⚠️ Lost {row_loss_pct:.1f}% of rows during cleaning - verify this is acceptable")
        
        if current_shape[1] < original_shape[1]:
            col_loss = original_shape[1] - current_shape[1]
            recommendations.append(f"📝 Dropped {col_loss} columns - document reasoning for stakeholders")
        
        # Missing data
        remaining_missing = self.df.isnull().sum().sum()
        if remaining_missing > 0:
            recommendations.append("🔍 Review remaining missing values for domain-specific imputation")
        
        # Data types
        if len(self.df.select_dtypes(include=['object']).columns) > 0:
            recommendations.append("🏷️ Consider encoding categorical variables for machine learning")
...
def analyze_data_quality(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in categorical_cols:
        if df[col].dtype == 'object':
            ...
            has_leading_trailing_spaces = df[col].astype(str).str.strip().ne(df[col].astype(str)).any()
            ...
            if len(unique_values) > 1:
                ...
                for lower_val, variants in case_variants.items():
                    if len(variants) > 1:
                        consistency_issues.append(f"'{col}' has case variants: {variants}")
    ...
    if quality_report['missing_data']['missing_percentage'] > 20:
        recommendations.append("🔴 High missing data rate (>20%) - investigate data collection process")
    ...
    if len(consistency_issues) > 0:
        recommendations.append("🟡 Data consistency issues detected - standardize formatting")
