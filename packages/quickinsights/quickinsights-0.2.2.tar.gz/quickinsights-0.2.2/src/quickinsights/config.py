...
class QuickInsightsConfig:
    ...
    def _load_from_environment(self):
        ...
        env_mappings = {
            ...
            'QI_GPU_ENABLED': ('performance', 'gpu_enabled', lambda x: x.lower() == 'true'),
            'QI_CACHE_ENABLED': ('performance', 'cache_enabled', lambda x: x.lower() == 'true'),
    ...
    def _validate_system_resources(self):
        ...
        if self.performance.max_memory_gb > available_memory_gb * 0.8:
            ...
            logger.info(f"Memory limit adjusted to {self.performance.max_memory_gb:.1f} GB")
        ...
        if self.performance.gpu_enabled:
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    if self.performance.gpu_memory_fraction * gpu_memory_gb > available_memory_gb * 0.5:
                        self.performance.gpu_memory_fraction = (available_memory_gb * 0.5) / gpu_memory_gb
                        logger.info(f"GPU memory fraction adjusted to {self.performance.gpu_memory_fraction:.2f}")
                else:
                    self.performance.gpu_enabled = False
                    logger.warning("GPU requested but not available, disabling GPU support")
            except ImportError:
                self.performance.gpu_enabled = False
    ...
    def save_config(self, file_path: Optional[Union[str, Path]] = None) -> None:
        ...
        if file_path is None:
            file_path = self.config_file or Path.home() / '.quickinsights' / 'config.json'
