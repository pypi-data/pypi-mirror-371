...
class CacheItem:
    ...
    def __init__(self, key: str, value: Any, size_bytes: int, 
                 access_count: int = 0, last_access: float = None):
        self.key = key
        self.value = value
...
class SmartCache:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Cache configuration loaded: max_size={self.max_size_bytes / (1024*1024):.1f} MB")
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        # Sıkıştırma uygula
        for key, item in self.cache.items():
            if (item.size_bytes > self.compression_threshold_bytes and 
                not hasattr(item.value, '_compressed')):
                try:
...
def cache_function(max_age_seconds: Optional[float] = None, 
                  key_prefix: str = ""):
    """
