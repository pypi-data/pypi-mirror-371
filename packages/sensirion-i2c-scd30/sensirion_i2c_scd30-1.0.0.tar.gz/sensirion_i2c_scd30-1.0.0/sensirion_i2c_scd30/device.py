...
#
"""
The class Scd30DeviceBase implements the low level interface of the sensor.
The class Scd30Device extends the Scd30DeviceBase. It provides additional functions to ease the use of the
sensor.
"""
...
class Scd30DeviceBase:
    ...
    def activate_auto_calibration(self, do_activate):
        ...
        .. note::
            Note that the most recently found self-calibration parameters will be actively used for self-calibration disregarding the status of this feature.
            Finding a new parameter set by the here described method will always overwrite the settings from external recalibration and vice-versa.
            The feature is switched off by default. To work properly SCD30 has to see fresh air on a regular basis.
            Optimal working conditions are given when the sensor sees fresh air for one hour every day so that ASC can constantly re-calibrate.
            ASC only works in continuous measurement mode. ASC status is saved in non-volatile memory.
            When the sensor is powered down while ASC is activated SCD30 will continue with automatic self-calibration after repowering without sending the command.
...
class Scd30Device(Scd30DeviceBase):
    ...
    def await_data_ready(self):
        """
        Repeatedly call the get_data_ready() until the ready flag is set to 1. If the minimal measurement interval is 2s we iterate at most 200 times.
