...
@zope.interface.implementer(interfaces.IMethodPublisher)
...
@zope.interface.implementer(interfaces.IJSONRPCPublisher)
...
@zope.interface.implementer(interfaces.IJSONRPCRequest,
...
class JSONRPCRequest(HTTPRequest):
    ...
    The p01.jsonrpcclient JavaScript method JSONRPCProxy converts a
    ...
    def _decode(self, data):
        """Decode bytes to text with charsets; py2/3 safe"""
    ...
    _typeFormat = re.compile(r'([a-zA-Z][a-zA-Z0-9_]+|\\.[xy])$')
    ...
    #     try:
    #         #data = json.read(res)
    #         data = p01.json.api.jsonReader(res)
    #     except:
    #         # catch any error since we don't know which library is used as
    #         # parser
    #         raise p01.jsonrpc.exception.ParseError
    #     # get the params
    ...
    def processInputs(self):
        """Read body (bytes), decode to text, parse JSON-RPC, add _args/form."""
        stream = self._body_instream
        chunks = []
        incoming = stream.read(1000)
        while incoming:
            chunks.append(incoming)
            incoming = stream.read(1000)
        if chunks:
        ...
        try:
            data = p01.json.api.jsonReader(text)
        except Exception:
            if not raw:
                raise p01.jsonrpc.exception.ParseError
        params = data.get('params', [])
                    ...
                    if isinstance(arg, dict):
                        for key, d in list(arg.items()):
                            ...
                            if pos > 0:
                                ...
                                if match is not None:
                                    key, type_name = key[:pos], key[pos+1:]
                                    if type_name == 'list' and not isinstance(
                                        d, list):
                                        d = [d]
                                    if type_name == 'tuple' and not isinstance(
                                        d, tuple):
                                        d = tuple(d)
                            self.form[key] = d
            elif isinstance(params, dict):
                args = params
                for key, d in list(args.items()):
                    key = str(key)
                    pos = key.rfind(":")
                    if pos > 0:
                        match = self._typeFormat.match(key, pos + 1)
                        if match is not None:
                            ...
                            if type_name == 'list' and not isinstance(d, list):
                                d = [d]
                            if type_name == 'tuple' and not isinstance(d,
                                tuple):
                                d = tuple(d)
                    self.form[key] = d
                args = []
    ...
    def setResult(self, result):
        ...
        if interfaces.IJSONRPCErrorView.providedBy(result):
            ...
            # from z3c.json.interfaces import IJSONWriter
                ...
                result = p01.json.api.jsonWriter(wrapper)
