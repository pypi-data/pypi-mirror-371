...
def test_image_is_visible(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    app.save_screenshot()
...
def test_image_wait(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path, timeout=3.0)
    assert image.wait() is True


def test_image_wait_not(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path, timeout=1.0)
    assert not image.wait_not()


def test_image_tap(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path)
...
def test_image_tap_duration(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path)
...
def test_image_scroll_down(app: Shadowstep, android_settings_open_close, system_image_path):
...
def test_image_zoom_unzoom(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path)
    assert isinstance(image.zoom(), ShadowstepImage)
    assert isinstance(image.unzoom(), ShadowstepImage)


def test_image_drag_to_coordinates(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
    image = app.get_image(image=connected_devices_image_path)
    result = image.drag(to=(100, 100))
    assert isinstance(result, ShadowstepImage)


def test_image_is_contains(app: Shadowstep, android_settings_open_close, connected_devices_image_path):
