...
class SharepointClient(O365Client):
    ...
    def _start_(self, **kwargs):
        ...
        if self._srcfiles:
            for file in self._srcfiles:
                if isinstance(file, dict) and 'directory' in file:
    ...
    async def verify_sharepoint_access(self):
        ...
        try:
            ...
            self._logger.info(f"SharePoint site accessible: {self._site_info.display_name}")
            ...
            self._logger.error(f"SharePoint access verification failed: {e}")
            raise RuntimeError(f"SharePoint access verification failed: {e}") from e

    async def _resolve_site(self) -> DriveItem:
        """Resolve SharePoint site using Graph API."""
        if self._site_info:
            return self._site_info

        try:
            site_path = f"/sites/{self.site}" if self.site else ""
            ...
            if site and site.id:
                ...
                raise RuntimeError(f"Could not resolve SharePoint site: {site_identifier}")

        except Exception as e:
            raise RuntimeError(f"Failed to resolve SharePoint site: {e}") from e

    async def _resolve_drive(self, library_name: str = "Documents") -> DriveItem:
        """Resolve document library drive using Graph API."""
        if self._drive_info:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to resolve document library: {e}"
            ) from e

    async def _ensure_folder(self, folder_path: str, create: bool = True) -> DriveItem:
        """Ensure folder exists using Graph API."""
        ...
        try:
            # Use the correct path syntax: /drives/{drive-id}/root:/{path}
            folder_item = await self.graph_client.drives.by_drive_id(drive_id).items.by_drive_item_id(f"root:/{folder_path}:").get()  # noqa
            if folder_item:
                return folder_item
        except Exception:
            if not create:
        ...
        for segment in [s for s in folder_path.split("/") if s]:
            ...
            children = await self.graph_client.drives.by_drive_id(drive_id).items.by_drive_item_id(parent_id).children.get()  # noqa
            ...
            created = await self.graph_client.drives.by_drive_id(drive_id).items.by_drive_item_id(parent_id).children.post(new_folder)  # noqa
    ...
    def _normalize_directory(self, directory: str, drive_info) -> str:
        """
        Normalize a caller-provided directory to be relative to the drive root.

        Accepts any of:
        - "Stores/Sub"
        - "Shared Documents/Stores/Sub"
        - "Documents/Stores/Sub"
        - "sites/<site>/Shared Documents/Stores/Sub"  (site prefix will be stripped)

        Returns: e.g. "Stores/Sub" or "" for the root.
        """
        dir_norm = (directory or "").replace("\\", "/").strip().strip("/")
        if not dir_norm:
            return ""

        # If it starts with "sites/<site>/...", strip that prefix safely
        if dir_norm.lower().startswith("sites/"):
            parts = dir_norm.split("/")
            # Find the first occurrence of the library name and keep what's after
            for i, p in enumerate(parts):
                if p.lower() in ("shared documents", "documents"):
                    dir_norm = "/".join(parts[i + 1:])
                    break
            else:
                # No library segment found; leave as-is (caller may have passed a drive-root path already)
                dir_norm = "/".join(parts[1:]) if len(parts) > 1 else ""

        # Now strip a leading library or drive name if present
        first = dir_norm.split("/", 1)[0]
        root_names = {"shared documents", "documents"}
        if getattr(drive_info, "name", None):
            root_names.add(str(drive_info.name).lower())

        if first.lower() in root_names:
            dir_norm = dir_norm[len(first):].lstrip("/")
    ...
    async def upload_files(
        ...
        drive_info = await self._resolve_drive()
        normalized_folder = self._normalize_directory(target_folder, drive_info)
        folder_info = await self._ensure_folder(normalized_folder, create=True)

        results: List[Dict[str, Any]] = []

        target_folder = normalized_folder or '/'
    ...
    async def file_search(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for spec in getattr(self, "_srcfiles", []):
                ...
                # Normalize directory to be drive-root relative (e.g., "Stores/Sub")
                directory = self._normalize_directory(directory_raw, drive_info)

                self._logger.info(
                    f"=== Searching '{directory or '/'}' for '{pattern or '<empty>'}' "
                    f"(ext={wanted_ext or 'any'}) ==="
                )
    ...
    async def file_lookup(
        ...
        """
        Build a 'destinations' list (same shape as file_search) for files with known names.
        Each entry may contain:
        - directory (str): starting folder under the drive (e.g., "Shared Documents/Stores")
        - filename  (str): exact filename; may optionally include subfolders ("sub/dir/mkdocs.yml")
        """
        ...
        for spec in specs:
            ...
            # Split filename into possible subpath + basename
            fn_path = Path(filename_raw)
            fn_name = fn_path.name
            fn_subdir = fn_path.parent.as_posix().strip("/")

            # Normalize base directory to drive-root relative
            base_dir = self._normalize_directory(directory_raw, drive_info) if directory_raw else ""

            # If filename included subfolders, append them to the base directory
            effective_dir = f"{base_dir}/{fn_subdir}".strip("/") if fn_subdir else base_dir

            # Resolve item via colon-path id: /drives/{id}/items/root:/effective_dir/fn_name:/
            colon_id = f"root:/{effective_dir}/{fn_name}:/".replace("//", "/") if effective_dir else f"root:/{fn_name}:/"  # noqa

            self._logger.debug(
                f"🎯 Resolving exact file: {effective_dir or '/'} → {fn_name}"
            )
            ...
            try:
                ...
                self._logger.error(f"❌ Not found: {effective_dir}/{fn_name} ({e})")
                continue

            # Build drive-root-relative path for return/logging
            full_path = await self._get_item_full_path(drive_id, item.id)
            destinations.append({
                "item": item,
                "path": full_path,
                "server_relative_url": full_path
