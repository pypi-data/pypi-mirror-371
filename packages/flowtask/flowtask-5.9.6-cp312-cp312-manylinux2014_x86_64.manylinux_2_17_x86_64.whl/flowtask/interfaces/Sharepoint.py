...
class SharepointClient(O365Client):
    ...
    def connection(self):
        """Enhanced connection method with better error handling for app-only auth."""
        # calling before run:
        self._start_()
        ...
        try:
            if username is not None and password is not None:
                ...
                try:
                    ...
                    self._logger.warning(f"Office365: Authentication Error: {err}")
            else:
                # App-only authentication (client credentials)
                self._logger.info(
                    "Using app-only authentication with client credentials"
                )

                # Create client credential object
                self.auth_context = ClientCredential(client_id, client_secret)

                # Create context with app-only authentication
                self.context = ClientContext(self.url).with_credentials(self.auth_context)
                #  Sharepoint Scope
                scopes = [
                    f"https://{self.tenant}.sharepoint.com/.default",
                ...
                try:
                    # Test access to the web
                    web = self.context.web
                    self.context.load(web)
                    self.context.execute_query()
                    self._logger.info(f"Successfully connected to SharePoint site: {web.title}")
                except Exception as test_err:
                    self._logger.error(f"App-only authentication failed during test: {test_err}")
                    raise RuntimeError(
                        f"App-only authentication test failed. "
                        f"Please ensure the app has proper permissions and site access: {test_err}"
                    ) from test_err

            # Create Graph client
            from office365.graph_client import GraphClient
            self._graph_client = GraphClient(
                acquire_token_callback=lambda: self._access_token
            )
            self._logger.debug("Office365: Authentication success")
    ...
    def acquire_token(self, scopes: list = None) -> dict:
        """Enhanced token acquisition with better error handling."""
        client_id = self.credentials.get("client_id") if hasattr(self, "credentials") else None
        if not client_id:
            client_id = self._default_client_id
        client_secret = self.credentials.get("client_secret") if hasattr(self, "credentials") else None
        if not client_secret:
            client_secret = self._default_client_secret
        tenant_id = self.credentials.get(
            'tenant_id', self._default_tenant_id
        ) if hasattr(self, "credentials") else self._default_tenant_id

        if not scopes:
            # For SharePoint, we need both SharePoint and Graph scopes
            scopes = [
                "https://graph.microsoft.com/.default",
                f"https://{self.tenant}.sharepoint.com/.default"
            ]
        ...
        # Try to acquire token for each scope
        result = None
        for scope in scopes:
            try:
                result = app.acquire_token_for_client(scopes=[scope])
                if "access_token" in result:
                    self._logger.debug(f"Successfully acquired token for scope: {scope}")
                    break
            except Exception as e:
                self._logger.warning(f"Failed to acquire token for scope {scope}: {e}")
                continue

        if not result or "access_token" not in result:
            error_message = result.get('error_description', 'Unknown error') if result else 'No token result'
            error_code = result.get('error', 'Unknown error code') if result else 'No error code'
            raise RuntimeError(
    ...
    async def test_permissions(self) -> dict:
        """Test method to verify app permissions and access."""
        results = {
            "site_access": False,
            "folder_access": False,
            "upload_access": False,
        ...
        try:
            ...
            self._logger.info(f"✅ Site access successful: {web.title}")

        except Exception as e:
            results["errors"].append(f"Site access failed: {e}")
            self._logger.error(f"❌ Site access failed: {e}")
            return results

        try:
            # Test 2: Folder access
            test_folder = "/sites/Navigator-Navigator-dev/Shared Documents"
            folder = self.context.web.get_folder_by_server_relative_url(test_folder)
            self.context.load(folder)
            self.context.execute_query()
            results["folder_access"] = True
            self._logger.info(f"✅ Folder access successful: {test_folder}")

        except Exception as e:
            results["errors"].append(f"Folder access failed: {e}")
            self._logger.error(f"❌ Folder access failed: {e}")

        try:
            # Test 3: Upload permissions (try to get folder for upload)
            upload_folder = "/sites/Navigator-Navigator-dev/Shared Documents/Stores"
            folder = self.context.web.get_folder_by_server_relative_url(upload_folder)
            self.context.load(folder)
            self.context.execute_query()
            results["upload_access"] = True
            self._logger.info(f"✅ Upload folder access successful: {upload_folder}")

        except Exception as e:
            results["errors"].append(f"Upload folder access failed: {e}")
            self._logger.error(f"❌ Upload folder access failed: {e}")
