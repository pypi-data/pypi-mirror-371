...
def _validate_output_read_line(
    ...
    if section in ("Command", "Group"):
        ...
        if line.startswith(command_input):
           assert len(line) > len(command_input), "Validating help output failed on line: " + line
    ...
    if section == "Arguments":
        ...
        if line.startswith("--") or line.startswith("-"):
            ...
            # first_index = line.find(": ")
            assert -1 != first_index, "Validating help output failed on line: " + line
            assert len(line) > first_index + 1, "Validating help output failed on line: " + line
    ...
    if section in ("Commands",):
        ...
        assert len(subgroups) + len(commands) > subgroups_index + commands_index, "None or missing expected commands provided in test for " + command_input
        ...
        if commands_index < len(commands) and first_word_of_line == commands[commands_index]:
            ...
            assert len(line.replace(first_word_of_line, "", 1).lstrip()) > len(first_word_of_line), 'Missing help text in "Commands" on line: ' + line
            ...
            assert len(help_text) > 0, 'Missing help text in "Commands" section on line: ' + line
...
def _validate_output_read_section_name(line):
    ...
    if line.strip() and not line[0].isspace():
        ...
        if line == "Commands":
            return "Commands"
        if line in ("Options", "Common Options"):
            return "Arguments"
        if line == "Group":
            return "Group"
        if line == "Subgroups":
...
def validate_output(
    command_input, subgroups=(), commands=()
    ...
    try:
        ...
        # fix for python casing conversions with environment names
        ...
        for line in lines:
            ...
            read_section_output = _validate_output_read_section_name(line)
            ...
            updated_indices = _validate_output_read_line(
                command_input,
                line,
                section,
                subgroups,
        ...
        assert "Start" != section, f"Command {help_command}: incomplete help text: {returned_string}"

        # Check that we have traversed completely through both
        # subgroups and commands
        assert len(commands) == commands_index, (
        ...
        assert len(subgroups) == subgroups_index, (
        ...
        if not err:
            ...
            raise AssertionError(f"ERROR: Command {help_command} returned error at execution. Output: {returned_string} Error: {err}") from exception
    ...
    for command in commands:
        validate_output(command_input + " " + command)
...
def test_help_documentation():
    ...
    validate_output(
        "osdu config",
        commands=("default", "info", "list", "update"),
    )

    validate_output(
        "osdu crs",
        commands=("areas", "info", "list", "summary", "transforms"),
    )

    validate_output(
        "osdu dataload",
        commands=("checkrefs", "ingest", "status", "verify"),
    )

    validate_output(
        "osdu entitlements",
        subgroups=("groups", "members"),
        commands=("info", "mygroups"),
    )

    validate_output(
        "osdu file",
        commands=("download", "info"),
    )
    ...
    validate_output(
        ...
        commands=("add", "groups", "list", "remove"),
    )

    validate_output(
        "osdu legal",
        commands=("add", "delete", "info", "listtags"),
    )
    ...
    validate_output(
        "osdu schema",
        commands=("add", "get", "info", "list"),
    )

    validate_output(
        "osdu search",
        commands=("id", "info", "kind", "query"),
    )

    validate_output(
        "osdu storage",
        commands=("add", "delete", "get", "info", "list", "versions"),
    )
    ...
    validate_output(
        "osdu workflow",
        commands=("get", "info", "list", "register", "runs", "status", "unregister"),
    )
