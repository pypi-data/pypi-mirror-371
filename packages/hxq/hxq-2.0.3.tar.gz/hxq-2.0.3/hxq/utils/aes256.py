...
if __name__ == '__main__':
    ...
    #     key = sys.argv[1]
    #     sourceStr = sys.argv[2]
    # else:
    key = input('Please enter key:')
    sourceStr = input('Please enter str:')
    argus_iv = 'NjtPECu47eSOm36s'
    argus_AES = AES256Crypto(key, Mode.CBC, Padding.SPACE, argus_iv)
    print('Plaintext:', sourceStr)
    encryptedpassword = argus_AES.encrypt(sourceStr)
    print('Encrypted string:', encryptedpassword)
    password = argus_AES.decrypt(encryptedpassword)
    print('Decrypted string:', password)
    decryptStr = input('Please enter Encrypted string:')
    print('Decrypted string:', argus_AES.decrypt(decryptStr))
