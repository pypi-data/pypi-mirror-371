...
class Transaction:
    ...
    def replace(self, filter: str = "", vars: Union[Dict, List, None] = None, **kwargs) -> Iterator[Dict]:
        ...
        for row in self._table._query_cursor(type="replace", filter=filter, vars=vars, tx_id=self._id).rows():
            yield row
