...
def create_subprocess(args: list[str], stdout: int, stderr: int, stdin: int | None = None, cwd=None, env=None) -> subprocess.Popen:
    """
    Create subprocess for deterministic-zip with the specified arguments

    :param args: Arguments to pass to deterministic-zip
    ...
    """
    return subprocess.Popen([os.path.join(os.path.dirname(__file__), "deterministic-zip"), *args], stdout=stdout, stderr=stderr, stdin=stdin, cwd=cwd, text=True)


def exec_silently(args: list[str], timeout: int = -1, cwd=None) -> subprocess.Popen:
    """
    Execute deterministic-zip silently with given arguments
...
def exec_with_templated_output(args: list[str],
    ...
    """
    Run deterministic-zip using the specified args with templated stdout and stderr.
