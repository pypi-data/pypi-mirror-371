...
from .errors import KidAIError, KidAISafetyError
...
class CLHKidAI:
    ...
    def __init__(self, api_key: Optional[str] = None, config: Optional[CLHKidAIConfig] = None, mock: Optional[bool] = None):
        ...
        self._client = None if self.config.mock else _get_openai_client(api_key=api_key)
    ...
    def ask(self, prompt: str, *, system: Optional[str] = None) -> str:
        ...
        if self.config.safe_mode and not is_allowed(prompt, self.config.extra_blocklist):
            raise KidAISafetyError("That topic isn’t allowed in kid-safe mode. Try another question 🙂")
    ...
    def starter(self, name: str) -> str:
        ...
        if key not in STARTERS:
            raise KidAIError(f"Unknown starter '{name}'. Try one of: {', '.join(STARTERS)}")
