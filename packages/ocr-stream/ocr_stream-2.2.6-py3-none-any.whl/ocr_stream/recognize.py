...
from convert_stream import ImageObject, LibImage, DocumentPdf, PageDocumentPdf, ConvertPdfToImages, PdfStream
from ocr_stream.bin_tess import BinTesseract
from ocr_stream.modules import LibOcr, DEFAULT_LIB_OCR, ModuleOcr
...
class RecognizeImage(object):
    ...
    def __init__(self, bin_tess: BinTesseract, *, lib_ocr: LibOcr = DEFAULT_LIB_OCR):
        super().__init__()
...
class RecognizePdf(object):
    ...
    def recognize_page_pdf(self, page: PageDocumentPdf) -> DocumentPdf:
        """
        ...
        img: ImageObject = _conv.to_images()[0]
    ...
    def recognize_document(self, doc: DocumentPdf) -> DocumentPdf:
        ...
        images: list[ImageObject] = _conv.to_images()
    ...
    def recognize_file_pdf(self, file: File) -> DocumentPdf:
        file_doc = DocumentPdf.create_from_file(file)
        recognized_doc = self.recognize_document(file_doc)
        del file_doc
