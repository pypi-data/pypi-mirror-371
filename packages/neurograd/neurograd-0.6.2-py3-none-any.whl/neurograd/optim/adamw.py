...
@conditional_fuse
def fused_adamw_step(param, grad, m, v, lr, beta1, beta2, epsilon, t, decay):
    ...
    m_hat = m / (1 - beta1 ** t)
    v_hat = v / (1 - beta2 ** t)
    # Decoupled weight decay applied before gradient step (PyTorch-style)
    ...
    def step(self) -> None:
        ...
        for i, (name, param) in enumerate(self.params):
            if param.requires_grad and param.grad is not None:
                ...
                # Update moments and parameters, weight decay at end
                param.data = fused_adamw_step(param.data, param.grad, m, v, self.lr,
                                              self.beta1, self.beta2, self.epsilon, self.t, self.weight_decay)
