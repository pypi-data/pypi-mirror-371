...
import threading, queue  # <-- added for prefetching
...
class DataLoader:
    def __init__(self, dataset: Dataset, batch_size: int = 32, shuffle: bool = True,
                 ...
                 prefetch_batches: int = 8):  # <-- added param
        ...
        self.prefetch_batches = int(prefetch_batches)  # <-- store it
    ...
    def __iter__(self):
        ...
        def producer():
            try:
                for idx in range(len(self)):
                    q.put(self[idx], block=True)  # uses __getitem__ (and its per-sample threadpool)
            finally:
                q.put(STOP)
...
class ImageFolder(Dataset):
    ...
    # --------- CHANGED: faster JPEG path via imdecode + reduced decode ----------
    def _load_image(self, path: str) -> np.ndarray:
        mode = (self.img_mode or "RGB").upper()

        # Heuristic for reduced decode (use only when DOWNSCALING a lot)
        reduce = 0
        if self.img_shape is not None:
            h, w = self.img_shape
            max_side = max(int(h), int(w))
            # only apply reduced decode for small targets; disable for 227+
            if max_side <= 128:
                reduce = 4
            elif max_side <= 160:
                reduce = 2
            else:
                reduce = 0   # <-- ensures no reduction for 227
        ...
        def _reduced_flag():
            if mode in ("L", "GRAY", "GREY", "GRAYSCALE"):
                ...
                # No reduced RGBA; decode unchanged then convert
                return cv2.IMREAD_UNCHANGED
            else:  # "RGB" default -> use COLOR flags (BGR decode)
        ...
        # Read raw bytes so we can use imdecode + reduced flags
        ...
        # Convert to requested color layout
        if mode == "RGB" and arr.ndim == 3 and arr.shape[2] == 3:
            # imdecode COLOR gives BGR
