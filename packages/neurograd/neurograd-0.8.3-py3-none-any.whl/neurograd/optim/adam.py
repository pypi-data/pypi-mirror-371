...
class Adam(Optimizer):
    ...
    def __init__(self, model_parameters: Generator[Tuple[str, Tensor], None, None], lr: float = 0.01,
        ...
        self.first_momentum = [(name, xp.zeros_like(param.data)) for name, param in self.params if param.requires_grad]
        self.second_momentum = [(name, xp.zeros_like(param.data)) for name, param in self.params if param.requires_grad]
        self.beta1 = beta1
        self.beta2 = beta2
