...
class MCPClient:
    ...
    async def _connect_pyodide(self):
        ...
        def handle_endpoint(event):
            data = event.data
            print(f"Received 'endpoint' event: {data}")
            if data.startswith('/'):
        ...
        self.event_source.addEventListener('endpoint', handle_endpoint)
        ...
        await asyncio.wait_for(message_url_future, timeout=10)
