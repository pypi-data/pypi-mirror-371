...
docker_tag = "ubuntu-24.04-x86_64:v1.7.0"
...
def generate_temp_name():
    ...
    temp_filename = f"{username}-{timestamp}_{random_part}"
...
class DockerContainer:
    def __init__(self, opt):
        ...
        self.image = os.getenv("DOCKER_IMAGE") or docker_tag
    ...
    @property
    def image_url(self):
        return os.getenv("DOCKER_URL") or "http://8.149.140.24/artifactory/generic-local/docker/ubuntu-24.04.v1.7.0.tar.xz"
    ...
    def image_build(self):
        docker_file = textwrap.dedent("""
            ...
            RUN curl -L "https://occ-oss-prod.oss-cn-hangzhou.aliyuncs.com/resource//1721095219235/Xuantie-900-gcc-linux-6.6.0-glibc-x86_64-V2.10.1-20240712.tar.gz" | \
            ...
            RUN curl -L "https://github.com/neovim/neovim/releases/download/v0.11.3/nvim-linux-x86_64.tar.gz" | gunzip | tar -x --strip-components=1 -C /usr/local
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            docker_filename = os.path.join(temp_dir, "Dockerfile")
            util.save(docker_filename, docker_file)
            build_cmd = f"docker build -t {self.image} ."
            if os.system(build_cmd) != 0:
                raise Exception(build_cmd + " failed")

    def image_load(self):
        data_queue = Queue(maxsize=1024)  # 控制内存使用
        ...
        def image_load_thread():
            ...
            while True:
                ...
                docker_process.stdin.write(decompressed_data)

            docker_process.stdin.close()
            returncode = docker_process.wait()
            if returncode != 0:
                raise RuntimeError("Docker加载失败")

        downloader = threading.Thread(target=download_thread)
        extractor = threading.Thread(target=image_load_thread)
        downloader.start()
        extractor.start()
        downloader.join()
        extractor.join()
...
class DockerRunner(object):
    def __init__(self, opt):
        self.opt = opt
        self.USER = os.getenv("DOCKER_USER") or getpass.getuser()
        self.UID = os.getuid()
        self.GID = os.getgid()
    ...
    def local_path(self):
        ...
        util.save_if_not_exists(nvim_init, textwrap.dedent("""\
        ...
        util.save_if_not_exists(nvim_plugins_init, "return {}\n")
