...
class CellValueValidationValidator(BaseValidator):
    ...
    def _parse_cell_reference(self, cell_ref: str) -> tuple[int, int]:
        ...
        Args:
            cell_ref: Cell reference in A1 notation (e.g., "A1", "B2", "Sheet1!A1")
        ...
        # Handle sheet-qualified references (e.g., "Sheet1!A1" or "Mar 2025'!A2")
        if "!" in cell_ref:
            # Split on the last occurrence of "!" to handle sheet names with "!" in them
            sheet_part, cell_part = cell_ref.rsplit("!", 1)
            cell_ref = cell_part.strip()
