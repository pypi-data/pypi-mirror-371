...
class ValidateA1RangesUtility(SingleBatchUtility):
    ...
    def execute_single(self, **kwargs) -> UtilityResult:
        ...
        try:
            ...
            if is_google_sheets:
                print(f"📊 Accessing Google Sheets: {detection_result.get('file_name', 'Unknown')}")
            else:
                print(f"📁 Accessing local Excel file: {detection_result.get('file_name', 'Unknown')}")
            ...
            if tab_name:
                ...
                if tab_name not in tabs_result.get("tabs", []):
                    return UtilityResult(
                        ...
                        error="tab_not_found",
                        data={"available_tabs": tabs_result.get("tabs", []), "requested_tab": tab_name}
                    )
                ...
                print(f"📋 Processing all {len(sheets_to_process)} tabs: {', '.join(sheets_to_process)}")
                ...
                try:
                    if is_google_sheets:
                        ...
                        if spreadsheet_id_match:
                            ...
                            values = sheet_data_result.get("values", []) if sheet_data_result.get("success", False) else []
                    else:
    ...
    def execute_batch(self, **kwargs) -> UtilityResult:
        ...
        try:
            ...
            if tab_name:
                ...
                print(f"📋 Reading metadata from default sheet")
            
            metadata_result = get_spreadsheet_data(metadata_file, range_spec, auth_credentials=kwargs.get("auth_credentials", {}))
