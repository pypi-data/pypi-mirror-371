...
def get_spreadsheet_tab_analysis(
    spreadsheet_source: Union[str, Path], auth_credentials: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not tabs_result["accessible"]:
            raise ValidationError(
                f"Unable to access spreadsheet: {tabs_result['error']}"
            )
        ...
        return {"success": False, "error": str(e), "tabs_found": 0, "analysis": None}
...
def sanitize_spreadsheet_tab_names(
    ...
    try:
        # Get analysis first
        analysis_result = get_spreadsheet_tab_analysis(
        ...
        try:
            ...
            if isinstance(spreadsheet_source, str) and spreadsheet_source.startswith(
                ...
                extract_sheet_id(spreadsheet_source)
                ...
                for original_name, new_name in rename_mapping.items():
                    try:
                        ...
                        spreadsheet.update_sheet_properties(
                            sheet_name=original_name, new_name=new_name
                        )
                        changes_applied += 1
                    except Exception as e:
                        # Continue with other renames even if one fails
                        print(
                            f"Warning: Failed to rename '{original_name}' to '{new_name}': {e}"
                        )
        ...
        return {"success": False, "error": str(e), "changes_made": False}
