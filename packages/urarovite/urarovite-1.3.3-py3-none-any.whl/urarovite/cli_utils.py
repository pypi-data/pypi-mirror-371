...
from urarovite.utils.open_ended_ranges_util import detect_open_ended_ranges_in_spreadsheet
...
class OpenEndedRangesUtility(SingleBatchUtility):
    ...
    def _add_utility_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "spreadsheet_source",
            help="Google Sheets URL or Excel file path to analyze"
        )
        parser.add_argument(
            "--operation",
            choices=["flag", "fix"],
    ...
    def _extract_utility_args(self, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        if args.mode == "batch":
            base_args.update({
                "url_columns": args.url_columns.split(",") if args.url_columns else [],
            })
...
class UtilityRegistry:
    ...
    def __init__(self):
        ...
        self.register("cell-value-validation", CellValueValidationUtility(
            "cell-value-validation",
            "Validate cell values against expected values from metadata sheets"
        ))
        self.register("detect-open-ended-ranges", OpenEndedRangesUtility(
