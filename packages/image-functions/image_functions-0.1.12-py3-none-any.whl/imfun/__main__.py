...
def crop_image(image, **kwargs):
    ...
    [mask, points] = crop_image(image, **show, **cmap, **window_name)
    ...
    mask: output mask, an image with the cropping rectangle drown in it.
    ...
    if points3[2] - points3[0] > 0:         # if x2 > x1 in (x1, y1),(x2,y2)
        if points3[3] - points3[1] > 0:     # if y2 > y1 in (x1, y1),(x2,y2)
            mask = image[points3[1]:points3[3], points3[0]:points3[2]]
        else:                               # if y1 > y2 in (x1, y1),(x2,y2)
            mask = image[points3[3]:points3[1], points3[0]:points3[2]]
    else:
        if points3[3] - points3[1] > 0:
            mask = image[points3[1]:points3[3], points3[2]:points3[0]]
        else:
            mask = image[points3[3]:points3[1], points3[2]:points3[0]]
...
class crop_multiple1(object):
    ...
    def run(self, image, width, height, img_type, cmap):
        ...
        while (not self.done):
            ...
            if width % 2 == 0:
                if height % 2 == 0:
                    difference1 = tuple((int(int(width/2)), int(int(height/2))))
                    difference2 = tuple((int(-int(width/2)), int(-int(height/2))))
                else:
                    difference1 = tuple((int(int(width/2)), int(int(height/2)+1)))
                    difference2 = tuple((int(-int(width/2)), int(-int(height/2))))
            else:
                if height % 2 == 0:
                    difference1 = tuple((int(int(width/2)+1), int(int(height/2))))
                    difference2 = tuple((int(-int(width/2)), int(-int(height/2))))
                else:
                    difference1 = tuple((int(int(width/2)+1), int(int(height/2)+1)))
                    difference2 = tuple((int(-int(width/2)), int(-int(height/2))))
...
def crop_multiple(images, cmap):
    ...
    [masks, points] = crop_multiple(images, cmap=cv2.COLORMAP_PINK)
    ...
    cmap: Chose the prefered colormap. If 'None', image in grayscale.
          Some colormap exemples: cv2.COLORMAP_PINK, cv2.COLORMAP_HSV,
          cv2.COLORMAP_PARULA, cv2.COLORMAP_JET, cv2.COLORMAP_RAINBOW, etc.
    masks: output cropped images/masks (a list of 'numpy.ndarray' variable)
    points: a variable of type 'list' with the 2 main points to draw the crop-
            ping rectangle (upper-left and lower-right).
    ...
    if img_type == 3:       # Grayscale images
        masks = np.zeros((len(I), I00.shape[0], I00.shape[1]), np.uint8)
        masks[0,...] = I00
    elif img_type == 4:     # Color images
        masks = np.zeros((len(I), I00.shape[0], I00.shape[1], I00.shape[2]), np.uint8)
        masks[0,...] = I00
    ...
    for n in range(1,len(I)):
        ...
        [masks[n,...], point1, point2] = crop_class.run(I[n], width, height,
                                                     img_type, cmap)
...
class crop_poly_multiple1(object):
    ...
    def run(self, image, points, equalize, img_type, cmap, show, window_name):
        ...
        points1 = np.array(points,dtype='int')
        while (not self.done):
            # THIS COPY OF ORIGINAL IMAGE FORCE THAT ONLY ONE RECTANGLE BE
            # DRAWN IN EACH IMAGE PLOT. Try to comment and see what happens.
            image2 = image_c.copy()
            ...
            current1 = np.array(self.current,dtype='int')
            ...
            # To use 'polylines', we have to enter with a 'int' array with
        ...
        cv2.fillConvexPoly(image3, np.array(points1,dtype='int'),
                           (255,255,255))
        # Closing this window we prevent that this window remain after running
...
def crop_poly_multiple(images, **kwargs):
    ...
    [masks, points] = crop_poly_multiple(images, **cmap, **show, ...)
    ...
    # First saved point.
    pointA = []
    pointA.append(np.asarray(points, dtype=int))
...
def label_image_segments(root, classes, **kwargs):
    ...
    Example:
    --------
    masks = label_image_segments(root, classes, show = True)
    ...
    **kwargs : 
    ...
    ------
    masks : list
        A list with the label images (masks), all of `numpy.ndarray` type.
...
# TO DO: implement colormap when labeling grayscale images
def label_sequence_from_image(image_dir, numb_points, numb_seq, **kwargs):
    '''
    Interact with the mouse to label a sequence of pixels (a profile), acquired
    from images on a given folder.
    ...
    Example of use:
        ...
        data = imfun.label_sequence_from_image(image_dir, numb_points, numb_seq, save_dir=r'D:\Data\Sequences\OCT')
...
def select_points_from_sequence(sequence, numb_points):
    """
    Interactively select a given number of points from a 1D sequence using mouse clicks.
    
    This function displays a 1D numerical sequence (e.g., a pixel intensity profile)
    as a line plot. The user selects `numb_points` points on this profile by clicking
    with the mouse. The function returns the positions (indices) of the selected points.
    
    Example of use:
    ---------------
    >>> seq = [10, 15, 22, 18, 12, 30, 28, 25]
    >>> points = select_points_from_sequence(seq, numb_points=3)
    (User clicks 3 points on the interactive plot)
    >>> print(points)
    [2, 5, 6]   # example output (positions chosen by the user)
    
    Parameters:
    -----------
    sequence : list, tuple, or 1D numpy.ndarray
        The numerical sequence from which points will be selected.
        Typically, this is a profile of pixel intensities or any 1D data.
        
    numb_points : int
        Number of points the user must select from the sequence.
        The function will not return until this number of points is chosen.
    
    Returns:
    --------
    selected_indices : list of int
        A list of indices (positions in the sequence) chosen by the user.
        Indices correspond to the x-axis positions in the plotted data.
    
    Workflow:
    ---------
    1. The sequence is displayed as a line plot with markers.
    2. The user clicks `numb_points` times on the plot.
       - Each click selects the closest index on the sequence.
       - Selected points are highlighted in red.
    3. Once `numb_points` points are chosen, the window closes automatically.
    4. The indices of the selected points are returned.
    
    Key Features:
    -------------
    - **Interactive selection**: User visually chooses points of interest.
    - **Index return**: Only indices are returned, not values (suitable for ML/AI).
    - **Duplicate protection**: A point cannot be selected more than once.
    
    Notes:
    ------
    - To select points, click near the desired x-axis position (index).
    - The function will keep the plot open until the required number of points
      is selected.
    
    See Also:
    ---------
    - `matplotlib.pyplot` for interactive plotting.
    - Can be combined with ML/AI pipelines to create labeled training datasets.
    ...
    class InteractivePointSelector:
        def __init__(self, data, numb_points):
            self.data = data
            self.numb_points = numb_points
            self.selected_points = []  # list of indices
            
            # Initialize the figure and axis
            self.fig, self.ax = plt.subplots()
            self.ax.plot(self.data, marker='o', linestyle='-', label='Sequence')
            self.ax.set_title(f"Select {self.numb_points} points")
            self.ax.set_xlabel("Index")
            self.ax.set_ylabel("Value")
            self.ax.legend()
        ...
        def on_click(self, event):
            """Handle mouse click events to select nearest index."""
            if event.xdata is not None:  # inside plot area
                idx = int(round(event.xdata))
                if 0 <= idx < len(self.data):
                    if idx not in self.selected_points:  # avoid duplicates
                        self.selected_points.append(idx)
                        self.ax.scatter(idx, self.data[idx],
                                        color="red", s=100, zorder=3)
                        self.fig.canvas.draw()
                    
                    # If enough points selected, close plot
                    if len(self.selected_points) >= self.numb_points:
                        plt.close(self.fig)
        
        def get_points(self):
            """Start the GUI loop until numb_points are chosen."""
            self.fig.show()
            plt.pause(0.1)
            while len(self.selected_points) < self.numb_points:
                plt.pause(0.1)
    ...
    selector = InteractivePointSelector(sequence, numb_points)
    selected_indices = selector.get_points()
