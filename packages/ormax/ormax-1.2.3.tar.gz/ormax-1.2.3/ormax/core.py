...
class Model(metaclass=ModelMeta):
    ...
    async def _fast_insert(self):
        ...
        pk_field = next((f for f in self._fields if self._fields[f].primary_key), None)
        if pk_field is not None and self._fields[pk_field].auto_increment and lastrowid:
    ...
    async def _fast_update(self):
        ...
        If no primary key is defined or value is None, updates all rows.
        """
        table_name = self.get_table_name()
        set_clauses = []
        values = []
        ...
        # Build set clauses and values for changed fields
        for field_name, field in self._fields.items():
            if field.primary_key or field.auto_increment:
                continue
            if self._data.get(field_name) != self._original_data.get(field_name) and field_name not in auto_now_fields:
        ...
        if pk_field and pk_value is not None:
            ...
            # No primary key: update all rows
            where_clause = ""

        # Build final query
        query = f"UPDATE {table_name} SET {', '.join(set_clauses)}" + (f" {where_clause}" if where_clause else "")
...
class Database:
    ...
    async def transaction(self):
        ...
        try:
            ...
            elif not is_nested and not db_type == "sqlite":
