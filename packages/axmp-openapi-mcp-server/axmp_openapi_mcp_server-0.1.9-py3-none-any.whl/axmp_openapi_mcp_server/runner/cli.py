...
def main(
    ...
    """Run the MCP server."""
    # NOTE: if all the backend servers are None, then backend-server-auth-configs is not required
    if transport == TransportType.STDIO:
        if backend_server_auth_configs == "":
            backend_server_auth_configs = None
