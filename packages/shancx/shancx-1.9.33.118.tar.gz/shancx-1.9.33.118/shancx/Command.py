...
def runcommand(cmd, timeout= 300) -> bool:
    try:
        ...
        if result.stderr:
            logging.info(f"Command error output:\n{result.stderr}")        
        logging.info("Command succeeded!")
        return True
    except subprocess.TimeoutExpired:
        logging.error(f"Timeout after {timeout} seconds! File may be too large or network is slow.")
        return False
    except subprocess.CalledProcessError as e:
        error_msg = e.stderr.decode().strip() if e.stderr else "Unknown error"
        logging.error(f"Download failed! Return code: {e.returncode}, Error: {error_msg}")
        return False
