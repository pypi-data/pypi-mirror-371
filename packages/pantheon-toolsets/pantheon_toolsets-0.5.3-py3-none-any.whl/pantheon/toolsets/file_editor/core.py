...
class FileEditorToolSet(ToolSet):
    ...
    def _print_diff_display(self, original_content: str, new_content: str, file_path: str):
        """
        Print a visual diff display with Rich colors directly to console.
        """
        ...
        header_width = 75
        logger.info("")
        logger.info(f"╭─ [bold]File changes:[/bold] {display_path} " + "─" * (header_width - 16 - len(display_path)) + "╮")
        
        # Print diff with colors - show only meaningful changes
        ...
        for line in diff:
            ...
            if line.startswith('-'):
                # Red for deletions
                padding = max(0, header_width - len(display_line) - 2)
                logger.info(f"   [red]{display_line}[/red]" + " ")
                showed_content = True
            elif line.startswith('+'):
                # Green for additions  
                padding = max(0, header_width - len(display_line) - 2)
                logger.info(f"   [green]{display_line}[/green]" + " ")
                showed_content = True
            elif line.startswith('@@'):
                # Skip line number headers for cleaner display
                continue
            elif line.startswith(' '):
                # Context lines - only show a few for reference
                padding = max(0, header_width - len(display_line) - 2)
                logger.info(f"   [dim]{display_line}[/dim]" + " ")
                showed_content = True
        
        if not showed_content:
            logger.info(f"   [dim]No visible changes[/dim]" + " " * (header_width - 20))
    ...
    async def read_file(
        ...
        try:
            ...
            if show_line_numbers:
                ...
                # Return empty content since we've already printed
                content = ""
            else:
