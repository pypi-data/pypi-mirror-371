...
from pantheon.toolsets.python.python_interpreter import (
...
async def test_python_interpreter_toolset():
    ...
    with Engine() as engine:
        ...
        s = await connect_remote(
            toolset.service_id, server_urls=SERVER_URLS + [not_exist_server_url]
        )
        ...
        assert resp["result"] == 2
        s = await connect_remote(toolset.service_id, server_urls=SERVER_URLS)
        resp = await s.invoke(
            "run_python_code", {"code": "res = 1 + 1", "result_var_name": "res"}
        )
        assert resp["result"] == 2
