...
def generate_book(input_dir: Union[str, Path],
    ...
    if input_path.exists():
        ...
        if force_typst:
            logging.info("🎯 FORCE_TYPST detected - forcing Typst engine for book")
            ...
            logging.info(f"📊 Book analysis: avg complexity={avg_complexity:.1f}, max={max_complexity:.1f}")
            
            if avg_complexity <= book_typst_threshold and max_complexity <= book_max_threshold:
                logging.info(f"🎯 Book Router: Recommending Typst for book (avg: {avg_complexity:.1f}) - superior typography")
                kwargs['force_engine'] = 'typst'
            else:
                logging.info(f"🎯 Book Router: Recommending ReportLab for very complex book (avg: {avg_complexity:.1f})")
