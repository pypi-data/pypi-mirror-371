...
Screenshots are automatically compressed if needed to prevent transmission issues."""
)
...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    Images are always compressed to JPEG and capped to ≤500KB including base64 overhead.
    """
    ...
    try:
        ...
        # Always compress to JPEG and ensure final base64 payload ≤ 500KB
        base64_max_bytes = 500 * 1024
        # Base64 increases size by ~4/3; target raw size accordingly
        ...
        # Try a few progressively stricter targets to ensure we stay ≤ 500KB base64
        for factor in (1.0, 0.9, 0.8):
            candidate_bytes, candidate_format = compress_image(
                tmp_path, int(target_raw_size * factor)
            )
            candidate_b64 = b64encode(candidate_bytes).decode("utf-8")
            if len(candidate_b64.encode("utf-8")) <= base64_max_bytes:
                image_data = candidate_bytes
                base64_data = candidate_b64
                format_used = candidate_format
                break
            # keep best attempt for fallback
            if image_data is None or len(candidate_bytes) < len(image_data):
                image_data = candidate_bytes
                base64_data = candidate_b64
                format_used = candidate_format
