...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    Returns:
    - ImageContent: Base64-encoded screenshot (PNG or JPEG)
    - TextContent: Capture metadata (window info, timestamp)

    Note: Can capture windows in background but not minimized windows.
    Images are automatically compressed if needed to stay under 1MB to prevent transmission issues.
    """
    ...
    try:
        ...
        # Always compress to stay under 1MB MCP limit after base64 encoding to prevent stack overflow
        # Target raw size = 1MB / 1.45 (45% overhead for base64 encoding and safety margin)
        mcp_limit_in_bytes = 1024 * 1024
        target_raw_size = int(mcp_limit_in_bytes / 1.45)

        # Check if image needs compression
        if len(image_data) > target_raw_size:
