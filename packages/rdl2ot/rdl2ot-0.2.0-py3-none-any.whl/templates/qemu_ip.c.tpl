...
#define {{name|upper}}_IRQ_NUM      {{interrupts|length}}u

#define R32_OFF(_r_) ((_r_) / sizeof(uint32_t))

#define R_LAST_REG (R_{{ interfaces[-1].regs[-1].name|upper }})
...
struct {{name|upper}}State {
    ...
    MemoryRegion mmio;
    IbexIRQ irqs[{{name|upper}}_IRQ_NUM];
...
static void {{name|lower}}_update_irqs({{name|upper}}State *s)
{
    uint32_t state_masked = s->regs[R_INTR_STATE] & s->regs[R_INTR_ENABLE];

    trace_{{name|lower}}_irqs(s->ot_id, s->regs[R_INTR_STATE], s->regs[R_INTR_ENABLE],
                       state_masked);

    for (int index = 0; index < {{name|upper}}_IRQ_NUM; index++) {
...
static void {{name|lower}}_receive(void *opaque, const uint8_t *buf, int size)
{
    {{name|upper}}State *s = opaque;
}
