...
def main():
    ...
    if (command == REAL_FILE and not REAL_FILE in all_cmd
        ...
        if '--version' in flags and not '--help' in flags:
            print (f'{command} {version} (flipcase)')
            exit (0)
    ...
    if '--reset' in flags and is_active_styles(string):
    ...
    if command in flip(option='tuple'):
        ...
        print (flip(string, command), end=_end, ansi=is_ansi)
    else:
        print (string, end=_end, ansi=is_ansi)
