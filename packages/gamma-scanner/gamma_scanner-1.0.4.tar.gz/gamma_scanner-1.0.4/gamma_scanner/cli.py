...
def scan_command(scanner: GammaScanner, args):
    ...
    for file_path in scan_files:
        try:
            ...
            # Use the actual pattern matching engine
            file_matches = []
            matches = scanner.match(content)
            for match in matches:
                file_matches.append({
                    'rule': match['rule_name'],
                    'pattern': str(match.get('match', {}).get('condition', {}).get('expression', 'unknown')),
                    'type': match.get('match', {}).get('type', 'pattern'),
                    'file': file_path,
                    'details': match.get('match', {}).get('details', {})
                })
