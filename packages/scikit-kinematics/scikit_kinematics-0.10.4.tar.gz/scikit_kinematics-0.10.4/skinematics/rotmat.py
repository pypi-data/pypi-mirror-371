...
def R_2d(angle=90):
    """2-dimensional rotation matrix The argument is entered in degree.
    ...
    R = np.array(
        [[np.cos(a_rad), -np.sin(a_rad)], [np.sin(a_rad), np.cos(a_rad)]]
    )
...
def R_2d_s(angle='alpha'):
    """
    Symbolic 2D rotation matrix, by an angle with the given name
